...
readonly HISTORY_VOLUME="openwepp-quality-history"
...
setup_runner() {
  ...
  remote docker run --rm --user root --entrypoint /bin/bash \
    --mount "type=volume,src=${HISTORY_VOLUME},dst=/quality-history" \
    "${IMAGE}" -c 'chown 10001:10001 /quality-history && chmod 0700 /quality-history'
