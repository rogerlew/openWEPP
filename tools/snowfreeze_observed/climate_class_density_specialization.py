#!/usr/bin/env python3
"""Run the SNOWDENSITY-10.3.22 climate-class density specialization gate."""
...
SCHEMA = "snowdensity10-3-22-climate-class-density-specialization-rerun-v1"
CONTRACT = "SC-SNOWFREEZE-001 INV-SNOWFREEZE-077 INV-SNOWFREEZE-050"
PACKAGE_DIR = (
    REPO_ROOT
...
DEFAULT_OUTPUT = REPO_ROOT / "target/snowdensity10_3_22_climate_class_density_specialization"
DEFAULT_HILL_BINARY = REPO_ROOT / "target/debug/openwepp-cli-hill"
ARTIFACT_STEM = "climate-class-density-specialization-rubric"
ACTIVATED = "activated_bundle"
CANDIDATE = "climate_class_density"
...
def diagnose(
    ...
    gates = evaluate_gates(site_reports, model_summaries, comparison, conservation_proof)
    report = {
        "schema": SCHEMA,
        "contract": CONTRACT,
        "evidence_class": (
        ...
        "authority": {
            "sturm1995_thresholds_source_verified": True,
            "sturm2021_used_as_cross_check_only": True,
            "fixture_fitting_used": False,
            "nsidc0768_runtime_lookup_allowed": False,
        },
        ...
        "summary": summarize(model_summaries, comparison, gates),
        "gates": {
...
def model_specs() -> list[cross.ModelSpec]:
    return [
        cross.ModelSpec(
            ...
            note="current no-env default",
            lever_rank_eligible=False,
        ),
        cross.ModelSpec(
            model_id=CANDIDATE,
            mechanism="Sturm 1995 forcing-derived climate-class density candidate",
            availability="current_direct_runtime",
            env={
                cross.MELT_ENV: None,
                cross.DENSITY_ENV: "physics_bulk_climate_class_density_v1",
                cross.PHASE_ENV: None,
            },
            source="OPENWEPP_SNOWDENSITY09_DENSITY_MODEL opt-in",
            note="SNOWDENSITY-10.3.22 opt-in candidate; default unchanged",
...
def evaluate_gates(
    ...
    if availability_failures:
        ...
        return {
            ...
            "bidirectional_densification_flip": GateResult(False, reason),
            "persistence_guardrail": GateResult(False, reason),
    ...
    bidirectional = bidirectional_densification_gate(site_reports)
    persistence = persistence_guardrail_gate(candidate_vs_default)
    conservation = conservation_gate(conservation_proof)
    return {
        "cross_snotel_primary": GateResult(primary_pass, primary_reason),
        "bidirectional_densification_flip": bidirectional,
        "persistence_guardrail": persistence,
...
def bidirectional_densification_gate(site_reports: list[dict[str, Any]]) -> GateResult:
    improvements = []
    regressions = []
    for site in site_reports:
        ...
        for cell in candidate_profile["cells"]:
            if not cell["forcing_robust"] or "densification" not in cell["signature"]:
                continue
    ...
    if passed:
        return GateResult(True, f"densification robust cells improved without regressions: {improvements}")
    return GateResult(
        False,
        f"bidirectional densification evidence incomplete; improvements={improvements}; regressions={regressions}",
    )


def persistence_guardrail_gate(candidate_vs_default: dict[str, Any]) -> GateResult:
    worse = candidate_vs_default["worse_robust_cells_vs_activated"]
    passed = worse == 0
    return GateResult(
        passed,
        f"candidate worse robust cells vs activated default: {worse}",
    )
