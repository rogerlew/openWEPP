...
def metadata(text: str, path: Path) -> dict[str, str]:
    ...
    format_lines = [i for i, line in enumerate(unfenced(text)) if re.match(r'^\s*contract_format\b', line)]
    ...
    def __post_init__(self):
        ...
        for n, line in enumerate(self.lines, 1):
            if re.match(r'^#{1,6} ', line):
                ...
                if slug not in self.explicit_anchors:
...
class ContractSet:
    ...
    def resolve(self, source: Document, ref: str, normative=True) -> tuple[Document, str]:
        ...
        if path not in self.documents:
            ...
            owner_entry = path.parent.with_suffix('.md')
            if owner_entry.name.startswith('SC-') and owner_entry.exists():
                owner_entry = safe_path(self.root, source.path, str(owner_entry.relative_to(source.path.parent)) if owner_entry.is_relative_to(source.path.parent) else os.path.relpath(owner_entry, source.path.parent))
            ...
            target = Document(path, read_file(path), 'external', False)
            # External historical sidecars cannot act as normative authority.
            if normative and ('provenance' in path.parts or 'work-packages' in path.parts):
    ...
    def validate_provenance(self, bei_doc, bei):
        ...
        for doc in historical.values():
            ...
            for ix, (start, title) in enumerate(sections):
                ...
                if not re.fullmatch(r'\d{4}-\d{2}-\d{2}', fields['effective_date']):
