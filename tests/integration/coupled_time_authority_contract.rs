...
fn canonical_contract_carries_complete_time_authority() {
    ...
    for required in [
        ...
        "ScheduledOnce",
        "DiagnosticReduction",
        "one narrow receipt-custody successor",
        "every ordinary empty no-op fails `ERR-CT-013`",
...
fn frozen_vectors_have_separating_event_constraint_and_duration_cases() {
    ...
    require_case_ids(
        ...
        &[
            ...
            "receipt_custody_noop",
            "receipt_custody_noop_zero_lineage",
            "receipt_custody_noop_wrong_class",
            "receipt_custody_noop_empty_ledger",
...
fn validate_case_expectations(cases: &[Value]) {
    for case in cases {
        assert_ne!(
            ...
            "{} may not bypass executable semantics",
            case["id"]
        );
        let expected = case
            .get("expected")
            .and_then(Value::as_object)
            .expect("every case has expected object");
        assert!(expected.contains_key("status"));
        if expected["status"] == "rejected" {
            assert!(expected.contains_key("error"));
            assert_eq!(
                expected["before_sha256"], expected["after_sha256"],
                "{} must prove atomic no-op",
                case["id"]
            );
        }
    }
...
fn assert_ambiguous_lengths_separate(cases: &[Value]) {
    assert_ne!(
        cases
            .iter()
            .find(|c| c["id"] == "ambiguous_length_left")
            .unwrap()["expected"]["sha256"],
        cases
            .iter()
            .find(|c| c["id"] == "ambiguous_length_right")
            .unwrap()["expected"]["sha256"],
...
fn validate_identity_domains(cases: &[Value], ids: &[&str]) {
    ...
    for domain in model["identity_domain_fields"]
        .as_object()
        .expect("closed identity domains")
        .keys()
    {
        let kat_id = format!("{}_identity_kat", domain.replace('-', "_"));
        assert!(
            ids.contains(&kat_id.as_str()),
            "missing exact framed KAT for {domain}"
        );
        let case = cases.iter().find(|c| c["id"] == kat_id).unwrap();
        let declared = model["identity_domain_fields"][domain].as_array().unwrap();
        assert_eq!(
            case["fields"].as_array().unwrap().len(),
            declared.len(),
            "{domain} field cardinality"
        );
...
fn independent_reference_model_passes_without_rust_expected_value_calls() {
    ...
    assert_eq!(
        actual["schema"],
        "OPENWEPP_COUPLED_TIME_REFERENCE_RESULTS_V1"
    );
    ...
    assert_eq!(
        ...
        vectors["expected_reference_results_sha256"]
            .as_str()
            .unwrap()
    );
    ...
    for (index, (case, result)) in cases.iter().zip(results).enumerate() {
        ...
        assert_eq!(
            ...
            &expected,
            "structural reference mismatch at case {index}: {}",
            case["id"]
        );
    }
}
...
fn restart_schema_is_additive_and_direct_v10_is_protected() {
    ...
    let package =
        read("docs/work-packages/20260820-coupled-time-authority-implementation-001/package.md");
    assert!(
        package.contains("DirectV10 restart V1 schema, vectors, manifest, and bytes are protected")
    );
...
fn independent_semantic_schema_validator_rejects_poison_population() {
    ...
    assert_eq!(
        report["schema"],
        "OPENWEPP_COUPLED_TIME_SEMANTIC_VALIDATION_RESULTS_V1"
    );
    ...
    for id in [
        ...
        "leading_zero",
        "equal_support",
        "owner_digest_corrupt",
        "owner_set_digest_corrupt",
        "participant_not_owner",
        "controller_bytes_corrupt",
        "missing_policy",
        "candidate_omitted",
        "candidate_wrong_owner",
        "candidate_wrong_disposition",
        "candidate_bad_base64",
        ...
        "field_order_poison",
        "whitespace_poison",
        "duplicate_field_poison",
        "future_scheduled_receipt",
        "invalid_scheduled_receipt_id",
        "outbox_records_digest_corrupt",
        "outbox_state_invalid",
        "outbox_sequence_overflow",
    ] {
        let result = results
            .iter()
            .find(|result| result["id"] == id)
            .unwrap_or_else(|| panic!("missing poison {id}"));
