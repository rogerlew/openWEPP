...
fn carnivorous_adobo_fixture_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/watershed/carnivorous-adobo")
}

fn assert_repo_local_fixture_path(path: &Path) {
    let path_text = path.to_string_lossy();
    assert!(
        path_text.contains("tests/fixtures/watershed/carnivorous-adobo"),
