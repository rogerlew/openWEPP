...
const PRODUCTION_CARGO_MANIFESTS: &[&str] = &[
...
#[test]
fn meteorology_crate_is_workspace_member_but_not_production_dependency() {
    ...
    for manifest in PRODUCTION_CARGO_MANIFESTS {
        let text = repo_text(manifest);
        assert!(
            !text.contains("openwepp-meteorology"),
            "{manifest} must not depend on openwepp-meteorology in 10.3.5a"
        );
    }
}
