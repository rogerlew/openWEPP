...
fn seeded_wb20_surface() -> HillslopeWritebackSurface {
...
#[test]
fn wb20_contract_conformance_forward_lane_excludes_observed_targets() {
    ...
    let mut surface = seeded_wb20_surface();
    ...
    surface
        .state_surface
        .remove(&BoundarySymbol::from("wb12_runoff_observed"));
    surface
        .state_surface
        .remove(&BoundarySymbol::from("wb12_storage_observed"));

    let report = scheduler
        .execute_with_kernel(&topology_report, &mut kernel, surface)
        .expect("wb20 execution should return typed report");
    ...
    let runoff_delta = report
        .writeback_surface
        .flux_surface
        .get(&BoundarySymbol::from("wb12_runoff_closure_delta"))
        .expect("runoff closure delta should be present")
        .as_f64();
    assert!(runoff_delta.abs() <= TOL);

    let storage_delta = report
        .writeback_surface
        .flux_surface
        .get(&BoundarySymbol::from("wb12_storage_closure_delta"))
        .expect("storage closure delta should be present")
        .as_f64();
    assert!(storage_delta.abs() <= TOL);
}

#[test]
fn wb20_contract_conformance_rejects_invalid_lane_selector_domain() {
    let graph = parse_topology_fixture_str(VALID_TOPOLOGY).expect("fixture should parse");
    let topology_report =
        validate_pre_execution_topology(&graph).expect("topology report should build");
    let scheduler = HillslopePhaseScheduler::canonical();
    ...
    let report = scheduler
        .execute_with_kernel(&topology_report, &mut kernel, surface)
        .expect("wb20 failure should return typed report");
...
#[test]
fn wb20_contract_conformance_compatibility_lane_keeps_observed_closure_path() {
    let graph = parse_topology_fixture_str(VALID_TOPOLOGY).expect("fixture should parse");
    let topology_report =
        validate_pre_execution_topology(&graph).expect("topology report should build");
    let scheduler = HillslopePhaseScheduler::canonical();
