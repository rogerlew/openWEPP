    ...
    pub(crate) fn try_complete(
        ...
        for lane_index in 0..frame.identity.lane_count {
            ...
            let profile_authority = Stage3PublicationProfileAuthorityV1::try_from_day_input(
                ...
                lane_index,
                lane,
                publication_inputs.get(lane_index).ok_or(
            ...
            .filter(|parcel| &parcel.destination_ofe_id == ofe_id)
        {
            ...
            match parcel.parcel_kind {
                ...
                openwepp_land_surface_energy::LiquidParcelKind::RoutedRunon => {
                    add_nonnegative(&mut out.runon_m, amount_m)?;
                }
...
mod tests {
    ...
    fn parsed_soil_profile_authority_installs_exact_values_and_rejects_poisons() {
        ...
        assert!(
            Stage3PublicationProfileAuthorityV1::try_from_day_input(0, 0, lane, &omitted).is_err()
        );
        ...
        assert!(
            Stage3PublicationProfileAuthorityV1::try_from_day_input(0, 0, lane, &partial).is_err()
        );
        ...
        assert!(Stage3PublicationProfileAuthorityV1::try_from_day_input(
            0,
            0,
            lane,
            &invalid_tolerance,
        ...
        let mut tolerance_overwrite =
            DirectDayFrame::seed(frame.identity, 0, 0).expect("profile day");
        tolerance_overwrite
            .hydrology_projection_inputs
    ...
    fn adaptive_day_support_gate_traverses_authenticated_event_owner_bridges() {
        ...
        assert!(
            validate_complete_support_headers(
                0,
                0,
                DAY_NS,
                identity,
        ...
        reordered.swap(0, 1);
        assert!(
            validate_complete_support_headers(0, 0, DAY_NS, identity, &headers, &reordered)
                .is_err()
        );
        ...
        assert!(validate_complete_support_headers(
            0,
            0,
            DAY_NS,
            identity,
    ...
    fn adaptive_day_support_gate_rejects_omission_duplicate_and_reorder() {
        ...
        assert!(
            validate_complete_support_headers(0, 0, DAY_NS, identity, &duplicated, &[]).is_err()
        );

        let mut reordered = headers;
        reordered.swap(42, 43);
        assert!(
            validate_complete_support_headers(0, 0, DAY_NS, identity, &reordered, &[]).is_err()
        );
    }
    ...
    fn adaptive_day_support_gate_rejects_parent_and_interval_substitution() {
        ...
        assert!(
            validate_complete_support_headers(0, 0, DAY_NS, identity, &cross_parent, &[]).is_err()
        );
    ...
    fn accepted_terminal_liquid_requires_exact_sealed_receiver_custody() {
        ...
        assert!(
            validate_accepted_terminal_liquid_receiver_custody(f64::NAN, terminal_liquid_m)
                .is_err(),
    ...
    fn committed_producer_is_the_real_row_consumer_and_never_overwritten() {
        ...
        assert!(
            !executor
                .contains("day_frame.normalization.precipitation_m = day_input.precipitation_m")
        );
    }
