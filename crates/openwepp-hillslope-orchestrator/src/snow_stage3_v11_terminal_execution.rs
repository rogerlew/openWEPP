...
> {
    ...
    let mut provider = |request: crate::hydrology::CoveredTerminalTrialRequestV1| {
        ...
        #[cfg(test)]
        let discrete_complete_endpoint =
            mode == CoveredTerminalExecutionMode::DiscreteCompleteEndpoint;
        #[cfg(not(test))]
        let discrete_complete_endpoint = false;
        let stack = DirectV11SnowCoveredRealConsumerStack::new(
            ...
            DirectV11SnowCoveredStackInputs {
                ...
                day_index,
                interval_index,
                finalize_wb14_parent_interval: discrete_complete_endpoint
                    && request.support.end_ns() == beginning_clock.parent_support().end_ns(),
                wb14_coupled_child_binding:
                    crate::direct_runtime::DirectWb14CoupledChildBindingV1 {
                        proposed_upper_bound_s_bits: if discrete_complete_endpoint {
                            let duration_s = f64::from_bits(request.support.duration_s_bits());
                            if duration_s <= 60.0 {
                                60.0_f64.to_bits()
                            } else if duration_s <= 900.0 {
                                900.0_f64.to_bits()
                            } else {
                                1_800.0_f64.to_bits()
                            }
...
pub(crate) fn evaluate_real_discrete_complete_endpoint_v1(
    ...
    let support_duration_s = f64::from_bits(support.duration_s_bits());
    let selected_upper_bound_s_bits = if support_duration_s <= 60.0 {
        60.0_f64.to_bits()
    } else if support_duration_s <= 900.0 {
        900.0_f64.to_bits()
    } else {
        1_800.0_f64.to_bits()
    };
    ...
    let evidence = RealDiscreteCompleteEndpointEvidenceV1 {
        ...
        end_liquid_bits: result.end_retained_liquid_kg_m2.to_bits(),
