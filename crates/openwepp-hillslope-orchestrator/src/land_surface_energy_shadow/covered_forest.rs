...
use super::covered_derived_ingress::{
    CoveredIngressSchedule, derive_fixed_cap_canopy_ingress, execute_unified_with_derived_ingress,
};
...
/// Execute the covered forest transaction while deriving all canopy-liquid
/// ingress from the accepted fixed-authorization E04 ledgers.
///
/// Unlike [`execute_covered_forest_shadow`], this boundary accepts cadence and
/// noncovered ingress only. It is therefore the required boundary for the V8
...
pub(crate) fn execute_covered_forest_shadow_derived_ingress(
    ...
    let mut retained_final = None;
    let hydrology_candidate = execute_unified_with_derived_ingress(
        soil_adapter,
        surface_configuration,
        receiver_expectations,
        &submitted_request_batch,
        ...
        |authorizations| {
            ...
            let ingress = derive_fixed_cap_canopy_ingress(
                surface_configuration,
                std::slice::from_ref(&final_tile),
                ingress_schedule,
            )?;
            let mut ending_lse_tiles = vec![final_tile.ending_tile_state_pre_ingress.clone()];
            ending_lse_tiles.extend_from_slice(companion_ending_lse_tiles);
