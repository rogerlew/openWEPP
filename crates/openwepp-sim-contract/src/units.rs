...
pub fn canonical_boundary_unit_entries() -> Vec<BoundaryUnitEntry> {
    use DimensionClass::{
        Area, Count, Density, Depth, Fraction, HydraulicConductivity, RadiationDaily,
        RadiationHourly, Rate, Temperature, Time, Unitless, VolumetricWaterContent, WindSpeed,
    };
    use DomainClass::{
        AnyFinite, CountNonNegative, NonNegativeFinite, PositiveFinite, SignedFinite, UnitInterval,
    };
    use TypedBoundaryRequirement::{FollowUpRequired, ScalarException};
    ...
    vec![
        ...
        BoundaryUnitEntry::new(
            "timem",
            &["timem_{idx4}", "hs{ofe}_timem_{idx4}"],
            "s",
            Time,
            NonNegativeFinite,
            "openwepp-climate-runtime-adapter",
        ...
        BoundaryUnitEntry::new(
            ...
            ScalarException,
            Some(
                "fractional hourly cloud-cover diagnostic uses BoundaryValue::scalar until HPHYS0275 typed variants are expanded",
            ),
            &[],
        ),
        BoundaryUnitEntry::new(
        ...
        BoundaryUnitEntry::new(
            "snow_hourly_melt",
            &["snow.hourly.melt_m_{idx4}", "snow.hourly.melt_raw_m_{idx4}"],
            "m",
            Depth,
            SignedFinite,
            "openwepp-hillslope-orchestrator",
