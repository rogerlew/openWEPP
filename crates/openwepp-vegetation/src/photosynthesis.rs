...
pub fn peaked_response(t: f64, ha: f64, hd: f64, entropy: f64) -> Result<f64, VegetationError> {
    let a = arrhenius(t, ha)?;
    let numerator = 1.0 + (298.15 * entropy - hd).exp();
    let denominator = 1.0 + (t * entropy - hd).exp();
    let v = a * numerator / denominator;
    if v.is_finite() {
        Ok(v)
    } else {
...
pub fn fvcb(i: FvcbInput) -> Result<FvcbResult, VegetationError> {
    if i.ci_pa < i.gamma_pa || i.kc_pa <= 0.0 || i.ko_pa <= 0.0 || i.vcmax < 0.0 || i.jmax <= 0.0 {
        return Err(VegetationError::Domain("FvCB"));
    }
    let j = electron_transport(i.par_abs, i.jmax)?;
    let ac = i.vcmax * (i.ci_pa - i.gamma_pa) / (i.ci_pa + i.kc_pa * (1.0 + i.oi_pa / i.ko_pa));
    let aj = j * (i.ci_pa - i.gamma_pa) / (4.0 * i.ci_pa + 8.0 * i.gamma_pa);
    let ap = 3.0 * i.tp;
