...
pub const MODEL_VERSION: &str = "OPENWEPP_C3_WOODY_V4";
pub const MODEL_SHA256: &str = "8ace38d1148f95261306cd6b0bf6f22e23ac8ead4cb6897dbdb53061b78ee437";
...
mod tests {
    ...
    const RELEASED_V4_BYTES: &[u8] = include_bytes!(
    ...
    #[test]
    fn embedded_definition_has_exact_released_v4_identity() {
        let model = load_model_definition().expect("admitted model bytes");
        assert_eq!(model.version, "OPENWEPP_C3_WOODY_V4");
        assert_eq!(model.sha256, MODEL_SHA256);
        assert_eq!(model.bytes, RELEASED_V4_BYTES);
        assert_eq!(MODEL_BYTES, RELEASED_V4_BYTES);
    }
    ...
    fn historical_v1_definition_is_preserved_but_not_executable() {
        ...
        let error = validate_model_definition(HISTORICAL_V1_BYTES, MODEL_VERSION, MODEL_SHA256)
            .expect_err("historical V1 bytes must not pass the V4 executable identity gate");
        assert_eq!(
            error,
    ...
    fn historical_v3_definition_is_preserved_but_not_executable() {
        ...
        let error = validate_model_definition(HISTORICAL_V3_BYTES, MODEL_VERSION, MODEL_SHA256)
            .expect_err("historical V3 bytes must not pass the V4 executable identity gate");
        assert_eq!(
            error,
            VegetationError::ModelDigestMismatch {
                expected: MODEL_SHA256.into(),
