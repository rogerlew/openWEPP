...
mod tests {
    ...
    fn historical_v1_definition_is_preserved_but_not_executable() {
        ...
        let error = validate_model_definition(HISTORICAL_V1_BYTES, MODEL_VERSION, MODEL_SHA256)
            .expect_err("historical V1 bytes must not pass the V6 executable identity gate");
        assert_eq!(
            error,
    ...
    fn historical_v3_definition_is_preserved_but_not_executable() {
        ...
        let error = validate_model_definition(HISTORICAL_V3_BYTES, MODEL_VERSION, MODEL_SHA256)
            .expect_err("historical V3 bytes must not pass the V6 executable identity gate");
        assert_eq!(
            error,
            VegetationError::ModelDigestMismatch {
                expected: MODEL_SHA256.into(),
    ...
    fn historical_v4_definition_is_preserved_but_not_executable() {
        ...
        let error = validate_model_definition(HISTORICAL_V4_BYTES, MODEL_VERSION, MODEL_SHA256)
            .expect_err("historical V4 bytes must not pass the V6 executable identity gate");
        assert_eq!(
            error,
            VegetationError::ModelDigestMismatch {
                expected: MODEL_SHA256.into(),
    ...
    #[test]
    fn historical_v5_definition_is_preserved_but_not_executable() {
        assert_eq!(
            format!("{:x}", Sha256::digest(HISTORICAL_V5_BYTES)),
            HISTORICAL_V5_SHA256
        );
        assert_ne!(HISTORICAL_V5_BYTES, MODEL_BYTES);

        let error = validate_model_definition(HISTORICAL_V5_BYTES, MODEL_VERSION, MODEL_SHA256)
            .expect_err("historical V5 bytes must not pass the V6 executable identity gate");
        assert_eq!(
            error,
            VegetationError::ModelDigestMismatch {
                expected: MODEL_SHA256.into(),
