...
fn canopy_phenology_02_real_consumers_share_the_typed_native_state() {
    ...
    assert!(
        ...
        "native canopy runtime authority must reject schedule gaps"
    );
}

#[test]
fn canopy_phenology_02_negative_vpd_fails_closed_without_tolerance() {
    let mut negative = r7g_snow_forcing(0.0, 0.0);
    negative.tdpt_c = 10.0;
    assert!(direct_native_forest_vpd_pa(&negative).is_err());

    let zero = r7g_snow_forcing(5.0, 5.0);
    let zero_vpd = direct_native_forest_vpd_pa(&HillslopeDirectClimateDayForcing {
        tdpt_c: 5.0,
        ..zero
    })
    .expect("exactly zero VPD is in domain");
    assert_eq!(zero_vpd.to_bits(), 0.0_f64.to_bits());
}
...
fn direct_publication_day_input_and_helpers_source() -> String {
    [
        ...
        include_str!("../direct_publication/day_input_and_helpers/00b_ksatadj_authority_impl.rs",),
    ]
    .join("\n")
}
