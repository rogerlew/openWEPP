    ...
    I: Iterator<Item = OsString>,
{
    ...
    while let Some(argument) = args.next() {
        let argument = argument.into_string().map_err(|_| {
            AssuranceError::Usage("adopt-report-source argument must be UTF-8".to_owned())
        })?;
        match argument.as_str() {
            ...
            _ => {
                return Err(AssuranceError::Usage(format!(
                    "unknown or duplicate adopt-report-source argument '{argument}'\n{USAGE}"
                )));
            }
        }
    }
...
mod tests {
    ...
    fn adopt_report_source_parses_complete_requests_before_backend_entry() {
        ...
        assert!(
            ...
            .unwrap_err()
            .to_string()
            .contains("duplicate adopt-report-source")
        );
        assert!(
            amend(
                missing,
                &["adopt-report-source", "--report", "report", "--check"],
            )
            .unwrap_err()
            .to_string()
            .contains("--path")
