...
fn constraint(clock: &CoupledClockStateV1, end: u128) -> StepConstraintV1 {
    StepConstraintV1::new(
        clock_parent(clock),
...
fn accepted_receipt_authenticates_reduction_and_atomic_parent_outbox() {
    ...
    let commit = commit_parent(&mut c, candidate).unwrap();
    assert!(ParentCommitCandidateV1::new(&c, vec![]).is_err());
    let _ = commit;
}
#[test]
