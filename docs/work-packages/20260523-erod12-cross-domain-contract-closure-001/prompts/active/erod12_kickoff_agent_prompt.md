...
- /workdir/openWEPP/docs/work-packages/20260523-erod12-cross-domain-contract-closure-001/package.md


You are executing
`20260523-erod12-cross-domain-contract-closure-001` for the monolithic
openWEPP scientific hydrology/erosion model.

Objectives:
1. Close Wave-0 cross-domain contract ownership/guard blockers identified by
   EROD10 after EROD11 closure.
2. Publish canonical cross-domain ownership/guard authority across required
   companion erosion contracts.
3. Publish explicit EROD13 entry verdict (`GO` or `HOLD`) with rationale.
...
- This package is governance/contracts scoped; do not implement production
  erosion kernel physics in EROD12.
...
- For legacy migration packages, physics authority updates must be authored in
  canonical `SC-*` contracts before any production behavior claim.
...
- Correctness over completion: unresolved high-severity cross-domain ambiguity
  remains `HOLD`.
