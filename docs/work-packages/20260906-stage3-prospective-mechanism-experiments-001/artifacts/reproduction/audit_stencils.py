...
def proof_closed_component(coordinates,index,n):
    if index >= 10*n or index%10 < 6:
        return False
    local=index%10
    if local in (8,9):
        return True
    beta=coordinates[10*(index//10)+4+(local-6)]
    return bits(beta)==bits(1.0)
...
def enumerate_sweep(base, replay):
    ...
    for index, current in enumerate(coordinates):
        ...
        if base['represented_snow'] and index >= 10*n+2:
            kind = 'IdentityAnchor'
        elif replay and base['represented_snow'] and proof_closed_component(coordinates,index,n):
...
def audit(data, replay):
    ...
    return {'status':'PASS', 'oracle':'independent coordinate-domain binary64 enumeration',
            'replay_expected':replay,'qualification':'EXP-STAGE3-20260906-R-PC1','maps':data['maps'],'solves':data['solves'],
