...
def main():
    ...
    parser.add_argument('--compiler-doc-list', type=Path)
    args = parser.parse_args()
    compiler_docs = json.loads(args.compiler_doc_list.read_text()) if args.compiler_doc_list else []
    if (compiler_docs != sorted(set(compiler_docs)) or
            any(not p.startswith('docs/') or '..' in Path(p).parts or str(Path(p)) != p
                for p in compiler_docs)):
        raise ValueError('compiler document catalogue must be sorted unique canonical docs paths')
    if any(not (args.root/p).is_file() for p in compiler_docs):
        raise ValueError('missing required compiler document')
    ...
    patch = git(args.root, 'diff', '--binary', args.base, '--', 'crates', 'src', 'tests',
                ...
                'flake.lock', 'rust-toolchain', 'rust-toolchain.toml', 'build.rs', *compiler_docs)
    ...
    untracked = [p for p in git(args.root, 'ls-files', '--others', '--exclude-standard', '-z').decode().split('\0')
                 if included(p) and (args.root/p).is_file()]
    ...
    with tarfile.open(args.out / 'changed-source-snapshot.tar.gz', 'w:gz') as archive:
        for path in sorted(set(changed+untracked)):
            if included(path) and (args.root/path).is_file():
                archive.add(args.root/path, arcname=path, recursive=False)
    manifest = {'name':args.name, 'base':args.base,
                ...
                'compiler_document_inputs':compiler_docs,
