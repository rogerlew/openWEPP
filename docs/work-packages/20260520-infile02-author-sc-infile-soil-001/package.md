...
## Objective
Author and disposition `SC-INFILE-SOIL-001` and its paired openWEPP-owned input
specification for surface `infile-soil-sol` (.sol).

## Why This Package Exists
The parser input-surface registry marks `infile-soil-sol` as `planned`.
...
- Update parser input-surface registry disposition for `infile-soil-sol`.
...
- Draft/refresh `docs/specifications/wepp-input-files/specs/soil-file.spec.md`.
- Draft/refresh `SC-INFILE-SOIL-001` with required section set, propagation map,
...
## Exit Criteria
- `docs/specifications/wepp-input-files/specs/soil-file.spec.md` exists and covers this surface.
- `SC-INFILE-SOIL-001` includes all required parser-contract sections.
...
- Dual verification artifacts are complete with no unresolved high-severity
  findings.
- Registry entry for `infile-soil-sol` is updated consistently with disposition.
