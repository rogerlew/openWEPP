# CQR21 - Climate Runtime Adapter Complexity Refactor
...
Decompose the current CQR21 target in
`crates/openwepp-climate-runtime-adapter/src/lib.rs`, originally tracked as
rank 15 with snapshot CRAP `380`, CC `19`, and coverage `0%`, so the live
target function and any newly extracted helpers have CRAP `<= 30`.

## Rationale

The shared climate runtime adapter feeds both hillslope and watershed runtime
publication. CQR21 must reduce local complexity without changing public runtime
request/error APIs, typed guards, stable error IDs, parser compatibility,
runtime formulas, units, float expression order, or science-contract behavior.
...
- Private behavior-preserving helper extraction in `lib.rs`.
...
- No public API, error ID, parser compatibility, runtime publication, unit,
  formula, float expression order, or science-contract behavior changes.
- No climate disaggregation math changes.
- No branch switching.
...
- Record protected runtime request/error API, typed guard, unit, formula,
  compatibility, and science-contract behavior.
...
- Preserve public surface, stable errors, units, formulas, float expression
  order, and runtime projection behavior.
...
- Run the required closure gates:
  ...
  4. `cargo deny check`
  5. `markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260615-cqr21-climate-runtime-adapter-complexity-001 --format json`
  6. `git diff --check`
...
- Current CQR21 target function and any newly extracted helpers have CRAP
  `<= 30`.
...
- No public API, error ID, parser compatibility, formula, float expression
  order, unit, or science-contract behavior changes.
