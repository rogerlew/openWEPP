...
`crates/openwepp-watershed-orchestrator/src/runtime_inputs_mod/chaninp.rs` with
CRAP above `30` to `<= 30`, or record an ADR-0021-style disposition when a row
is not safely reducible as behavior-preserving CQR. Preserve WS12 impoundment
runtime projection behavior, output coefficients, typed fail-closed behavior,
and all existing valid-input output identity.
...
`SC-IMPOUND-001` is required because this target projects parser active-branch
payloads into WS12 runtime coefficient families and guards Chapter-14
stage-discharge behavior. This CQR package must not amend contract authority.
...
`crates/openwepp-watershed-orchestrator/src/runtime_inputs_mod/chaninp.rs`,
`tests/integration/infile_watershed_impoundment_parser_contract.rs`,
`tests/integration/wshedw5_typed_watershed_runtime_contract.rs`, or
...
- characterization tests required to make WS12 runtime coefficient projection
  safe to refactor;
...
- focused tests that prove existing parser/runtime impoundment behavior.

Out of scope:

- science-formula changes;
- Chapter-14/WS12 threshold, tolerance, or contract-authority changes;
...
3. If characterization tests are added or materially changed, record ADR-0021
   ...
   - applicable obligation-to-test binding, including `SC-IMPOUND-001` WS12
     active-projection obligations when contract-derived behavior is covered.
