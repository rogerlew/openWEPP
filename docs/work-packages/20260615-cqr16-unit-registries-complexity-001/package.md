# CQR16 - Unit Registry Complexity Refactor
...
Decompose the current CQR16 target in
`crates/openwepp-sim-contract/src/units_mod/registries.rs`, originally tracked
as rank 10 with snapshot CRAP `506`, CC `22`, and coverage `0%`, so the live
target function and any newly extracted helpers have CRAP `<= 30`.

## Rationale

The sim-contract unit registry maps canonical symbols, aliases, units,
publication units, and scalar exceptions used by contract checks. CQR16 must
reduce complexity without changing registry rows, alias coverage, unit meanings,
status taxonomy, contract behavior, or public API.
...
- Private behavior-preserving helper extraction in `registries.rs`.
...
- No public API, module visibility, dependency, or caller behavior change.
- No symbol, alias, unit, publication-unit, scalar-exception, error/status, or
  contract-semantic change.
- No unrelated unit-registry cleanup, schema redesign, or broad sim-contract
  refactor.
...
- `crates/openwepp-sim-contract/src/units_mod/registries.rs`
- Focused tests under existing sim-contract test paths if characterization is
...
- Record registry rows, aliases, units, scalar exceptions, and public API
  protected boundaries.

### Phase B - Precondition and Focused Characterization

- Run existing focused sim-contract tests before production edits.
...
- Preserve row order where observable, public constructors, aliases, status
  behavior, and exact registry values.
...
- Run the required closure gates:
  ...
  4. `cargo deny check`
  5. `markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260615-cqr16-unit-registries-complexity-001 --format json`
  6. `git diff --check`
...
- Current CQR16 target function and any newly extracted helpers have CRAP
  `<= 30`.
...
- No public API, unit-registry row, alias, unit, publication unit,
  scalar-exception, stable status/error, or contract behavior change is
  introduced.
