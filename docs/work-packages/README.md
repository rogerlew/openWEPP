...
- [20260911 Bounded execution and external Pro review](20260911-bounded-execution-external-review-001/package.md): model routing, checkpoint budgets and owner-mediated review exchange.
...
workspace eleven. Dual terminal verification passed after truthful closure
record reconciliation. Status: `COMPLETE`.
...
Historical Child-1 successor that executed the actual Stage-3/V11 terminal-handoff
...
closure. Status: `SUPERSEDED / HISTORICAL`; Child 3 is blocked; production Stage 3 remains
...
- `20260820-c3-woody-v11-segmented-support-001/` — COMPLETE / CHILD 2B /
  TERMINAL A+B PASS: released V11 segmented support with an explicit
...
- `20260804-snow-warm-mixed-prepeak-loss-energy-attribution-001/` — COMPLETE /
  ...
  corrected loss is concentrated on warm/mixed days at all four canonical
  sites, `cmelt` is the largest annual-first positive empirical term, and the
  scaled Snowbird lane adds more storage plus smaller state-exposed loss. All
  claims remain diagnostic and noncausal; no correction is authorized.
...
- `20260804-snow-wet-compaction-operand-authority-duplicate-alias-closure-001/`
  ...
  snow-contact-rain authority, one private typed real-consumer handoff, and an
  aligned fail-closed offline replay. The corrected accumulated driver is
  `55.7-61.9%` of the retired alias across four sites; upstream mass and all
  density/layer closures pass. The verified Snowbird `1.2155576` derivative is
  development-only. 21L is admitted after corrected-state rebaselining.
...
- `20260724-gate-planner-quality-deferral-hold-lift-001/` and
  ...
  and isolated the public-audit consumer; attempt 13 then passed 2,279 full and
  36 science-manual tests, published 11 compact files with zero actionable
  CRAP rows, and passed two independent terminal verifications.
...
- `20260808-vegetation-radiation-interception-conductance-slice-001/`
  - EXECUTED-HOLD / SUPERSEDED: independent literature and rights intake
    completed, but no constitutive chain, contract, suite, or Rust implementation
    was admitted. The user selected the broader pinned MIT-licensed
...
- `20260807-snow-stage3-paradise-wy2015-support-resolution-001/`
  - COMPLETE / SUPPORT CAUSE LOCALIZED / NON-BLOCKING DIAGNOSTIC: 19 partial
    ...
    followed by 183 same-state-only hours with sequential thin-pack status.
    The frozen failure remains; no physical or persistence claim changed.
...
- `20260804-snow-prepeak-mass-transition-physics-adjudication-001/` - COMPLETE /
  ...
  material at only two sites, post-CoE cold opportunity is small, and Stage 3
  is outside the authoritative SWE/runoff consumer path. No correction.
...
- `20260729-assure-maint03-tracked-human-review-rendering-001/` -
  ...
  and materialized all three complete reports for human review without creating
  approval or public-catalog state. The exact 92-file lane, Markdown, links,
  21 SVG consumers, anchored generation, 2,163-test full workspace, dual
  review, and dual terminal verification pass.
- `20260729-assure-maint02-v2-report-admission-and-full-catalog-rendering-001/` -
  ...
  rendering. It admitted the completed CAL-09 draft, produced byte-identical
  91-file builds in two unrelated roots, and checked the complete stable preview
  at `target/assurance-preview/`. CAL-09 remains draft; no human approval,
  release transfer, or tracked-public publication was created.
- `20260728-gate-planner-agent-friction-qualification-001/` -
  ...
  command/source/tests. Dual review and verification passed; no repair
  prerequisite or CI role exists, and the manual route remains prospective.
- `20260727-gate-planner-legacy-execution-ci-retirement-001/` -
  ...
  a 31-row consumer inventory, while preserving direct authority, history,
  optional quality observation, and protected-data ownership without a linter
  CI role.
- `20260727-gate-planner-advisory-linter-thin-slice-001/` -
  ...
  proof with dual review and verification. It has no CI or lifecycle role.
...
- `20260723-cqr-nightly-b03r-aggregate-001/` and its main-module scaffold are
...
- `20260722-testgate-coverage-include-identity-recovery-001/` -
  ...
  coverage-test module to its natural source path and proving exact full-source
  LLVM/cargo-crap attribution through dual review and verification.
...
- `20260722-testgate-executor-sentinel-recovery-001/` -
  ...
  spawn ordering. Focused gates, package admission, and dual review pass;
...
- `20260721-cqr-testgate-recovery-07-planner-001/` -
  ...
  helper are below CRAP 30, module line/region coverage and all function floors
  pass, and renewed dual review and dual terminal verification are complete.
...
- `20260712-dc-cqr-preint-hb02-texture-mass-fraction-domain-001/` -
  ...
  decomposed five-class producer reaches 98.020% lines / 97.318% regions and
  maximum CRAP 14.042 with real Wave-1/Yalin consumers.
- `20260712-dc-cqr-preint-hb01-strict-positive-threshold-001/` -
  ...
  all 15 strict-positive inputs; exact-zero authoritative runoff suppresses
  only the pointwise solve while positive-upstream continuity remains active.
  Expanded-slice coverage is 95.673% lines / 94.649% regions, maximum CRAP is
  15.136, and the 344-test erosion profile passes.
...
- `20260711-dc-watershed-channel-extra-rating-001/` - EXECUTED-COMPLETE
  ...
  lines / 99.373% regions, the function floor is 96.970%, maximum CRAP is
  21.0005, focused suites pass 38/38 and 20/20, and the workspace passes 1,765
  tests with 3 configured skips. Dual review/verification pass.
...
- `20260711-dc-fdir-finite-value-guard-001/` - EXECUTED-COMPLETE (2026-07-11):
  ...
  production coverage is 97.397% lines / 98.065% regions, minimum function
  coverage is 85.366%, maximum CRAP is 17, and the exact-final workspace passes
  1,730 tests with 3 configured skips. Dual review/verification pass. Closure
  is parser-boundary only; no runtime consumer/readiness claim is made.
...
- `20260710-wshedw11a-channel-hourly-sediment-authority-001/` -
  ...
  cycle-2 re-confirmation RATIFIED, `codex_reconfirmation2.md`):
  ratified per-`dtchr`-interval quasi-steady channel sediment sequencing in
  `SC-ROUTE-001` v51-v53 (`INV-ROUTE-015..020`: biconditional lane
  activation, water-grid quantum, the unique hydraulic-profile operand map
  — `qe(it) := q1(it)`, `qt(it) := qin(it)`, the published wave total
  `qlat(it)` partition-only, and the derived per-unit-length
  `qlat_eff(it) := qe/leff` as the solve operand — monotonic geometry carry
  with tillage-only reseed, pinned-`dcap.for` widening realization with
  ...
  rationale), dual verification (both PASS-WITH-NOTES), the Codex reopen
  cycle (6 findings accepted+fixed in v52; `GAP-ROUTE-014` records the two
  migrated-lane detachment terminals W11 must correct before interval
  reuse), and the re-confirmation residuals (v53 total-vs-per-length
  lateral fix) are dispositioned. `WSHED-W11-HOLD-001` stands lifted (Codex
  RATIFIED); W11 resumes at Phase B per `artifacts/w11-handoff.md`.
...
- `20260711-cqr-nightly-04-kernel-writeback-001/` -
  EXECUTED-COMPLETE-CQR-NIGHTLY (2026-07-11): fresh nightly batch 01 target #4
  completed characterization-only closure of kernel writeback. Production
  `writeback.rs` is unchanged; nine tests bind logical/indexed acceptance,
  ordered typed violations, exact status IDs, sorted state/flux mutation, and
...
- `20260706-mofefid-d15-blocker-resolution-001/` —
  ...
  shadow path now passes the ignored H2637 fixture and release endpoint timing
  completes. Current timing is `91.59 s` user / `1:31.67` wall for
...
- `20260706-mofefid-d15-opt-in-production-activation-rerun-001/` —
  ...
  D13. No partial activation flip. Follow-on blocker-resolution package closed
  the terminal-bin failure; active-owner implementation and timing-budget
  adjudication remain. D16 remains blocked.
...
- `20260705-mofefid-d15-opt-in-production-activation-001/` —
  ...
  the follow-on D15 rerun held on timing/active-path preconditions, and
  `20260706-mofefid-d15-blocker-resolution-001` has now closed the
  terminal-bin failure. Remaining blockers are active production ownership and
  the adjudicated timing-regression hold.
...
- `20260731-snow-surface-eb-04c-thin-pack-thermal-domain-closure-001/` —
  ...
  fixed `1 kg m^-2` terminal branches while retaining CoE snow mass, cold
  content, and typed guards: total mass `<=1` suspends before partition, while
  a resolved pack's lower volume `<1` collapses to a continuing one-volume
  solve and lower equality remains two-volume. All 22 captured failures pass
  their formerly rejected processing day; focused, quick, frost, and Critical
  full gates plus dual review and verification pass. No clamp, epsilon pressure,
  forced melt, coefficient tuning, or geometry correction was introduced.
