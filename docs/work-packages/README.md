...
- `20260602-hphys0245-wb11-wb18-layer-telemetry-and-storage-continuity-001/`
  - Purpose: add and execute gated diagnostics-only telemetry for `H1`, `H7`,
    and `H39` day `1..30`, tracing WB11/WB18 layer/storage/percolation symbols
    across scheduler boundaries before authorizing production physics changes.
  - Status: completed; diagnostics identify WB18 aggregate storage writeback
    and WB19 day-1 lateral transfer as the next focused remediation surfaces.
- `20260602-hphys0246-wb18-aggregate-storage-writeback-closure-001/`
  - Purpose: execute contract-first WB18 aggregate storage writeback closure so
    `wb11_soil_water` preserves baseline-authoritative total soil-water
    semantics across percolation, then rerun H1/H7/H39 telemetry to reassess
    residuals.
- `20260602-hphys0247-h39-single-ofe-hourly-waterbalance-closure-001/`
  - Purpose: execute a larger autonomous ExecPlan-style HPHYS package to close
    complete sensible water balance for H39 single-OFE hourly hillslope output,
    using hourly manifest proof, internal residual-ledger closure, and H39 WAT
    semantic parity against the pinned legacy baseline as acceptance gates.
- `20260602-hphys0248-wb18-h39-dp-pe-lineage-closure-001/`
  - Purpose: diagnose, correct, and validate WB18 H39 early-season
    percolation/deep-seepage lineage (`Dp`/`Pe`) using contract-first
    authoring, pinned baseline `watbal_hourly`/`purk`/`perc` authority, dual
    agent review, targeted H39 validation, and full `H1..H39` hillslope
    semantic metrics for continuation.
  - Status: hold; corrected H39 early-season WB18 `Dp`/`Pe` scale and produced
    full `H1..H39` metrics, but full semantic parity remains open.
- `20260602-hphys0249-wb17-et-snow-runoff-storage-closure-001/`
  - Purpose: diagnose, correct, and validate WB17 `Ep`/`Es`, snow/runoff
    timing, and aggregate storage lineage using contract-first authority,
    dual review, and the full `H1..H39` hillslope suite for continuation
    metrics.
...
- `20260602-hphys0251-swu-root-uptake-stress-closure-001/`
  - Purpose: execute the HPHYS0250 continuation recommendation by migrating
    baseline-authoritative `swu.for` actual root uptake and water-stress
    magnitude details, using H1/H13/H39 diagnostics and full `H1..H39`
    semantic metrics as closure evidence.
  - Status: HOLD; `pltol` projection/normalization and layer `UPi`/`Ui`
    publication are implemented and gated, but full-suite parity remains
    `0/39` and metrics point upstream to storage availability before root
    uptake.
- `20260602-hphys0252-wb19-lateral-storage-availability-closure-001/`
  - Purpose: execute the HPHYS0251 continuation recommendation by diagnosing,
    correcting, and validating WB19 lateral-transfer storage availability
    feeding post-WB19 WB17 root uptake, anchored to H1/H13/H39 diagnostics and
    full `H1..H39` semantic metrics.
  - Status: `HOLD`; baseline-authoritative `fzdrfc` lateral capacity and
    withdrawal semantics are corrected and gated, but full-suite semantic pass
    remains `0/39` and apples-to-apples HPHYS0251 deltas are zero for selected
    water-balance residual families.
- `20260602-hphys0253-h1-day1-storage-localization-diagnostics-001/`
  - Purpose: execute the HPHYS0252 continuation recommendation with a
    diagnostic-only H1 t=0/day-1 storage and conservation localization audit
    plus a fresh full `H1..H39` semantic metric snapshot before authorizing the
    next process-surface correction.
  - Status: completed/HOLD; H1 day-1 accounting closes internally, but the
    candidate starts `20.153260 mm` below the baseline WAT-derived t=0 proxy;
    next focus is contract-first WB11 initial/runtime storage projection.
- `20260602-hphys0254-wb11-initial-storage-projection-closure-001/`
  - Purpose: execute the HPHYS0253 continuation recommendation by diagnosing,
    correcting, and validating baseline-authoritative WB11 initial/runtime
    storage projection for layer `st(i)`/`theta`/`watcon` and aggregate
    `soil_water` seeding.
  - Status: completed/HOLD; WB11 normalized hydrology seed projection is
...
- `20260607-refactor006-openwepp-runner-hillslope-mod-mechanical-modularization-001/`
  - Purpose: execute REFACTOR006 by mechanically modularizing
    `openwepp-runner/src/hillslope/mod.rs`, preserving public API/runtime
    behavior, and updating layout-coupled tests that currently assert
    monolithic source-file residency.
- `20260607-refactor007-openwepp-runner-hillslope-01-scheduler-and-trace-mechanical-modularization-001/`
  - Purpose: execute REFACTOR007 by mechanically modularizing
...
- `20260607-refactor009-openwepp-runner-hillslope-00-runner-intake-and-lane-setup-mechanical-modularization-001/`
  - Purpose: execute REFACTOR009 by mechanically modularizing
    `openwepp-runner/src/hillslope/00_runner_intake_and_lane_setup.rs`,
    splitting intake/lane-setup concerns into cohesive modules while
    preserving runtime/API behavior, typed guards, and `.rs` line-count
    governance.
- `20260607-refactor010-openwepp-hillslope-orchestrator-tests-mechanical-modularization-001/`
  - Purpose: execute REFACTOR010 by mechanically modularizing
    `openwepp-hillslope-orchestrator/src/tests.rs`, splitting the
