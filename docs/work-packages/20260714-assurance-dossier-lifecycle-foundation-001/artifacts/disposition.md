...
The package deliberately did **not** publish a favorable validation or fitness
claim. Its generated pilot retained candidate, aggregate empirical, and
aggregate software-verification labels at closure. Subsequent
scientific-audience review rejected those aggregate labels as a valid
assessment of the snow/frost model or its evidence because the publication
architecture omitted most of the relevant scientific method and results. The
historical generated bytes remain v1 tooling evidence, not an accepted
snow/frost conclusion.
...
Post-closure user direction on `2026-07-15 UTC` supersedes the original
follow-on sequence: vendoring is not the next operational package. The handoff
remains dormant while WEPPcloud and the dossier set are immature. A dedicated
wepppy package becomes mandatory during the openWEPP beta release campaign and
must close before that beta is issued in WEPPcloud; it must refresh rather than
reuse the foundation's historical compatibility identities.

The binding next package is the documentation-only `ASSURE-02` gate in
`docs/ROADMAP.md`. It must define and review the manuscript-first v2 public
product, machine/audit boundary, pilot-selection evidence, real nonpublic
process-kernel prototype, retirement plan, and implementation decomposition.
No compiler, schema, generated-content, v1 retirement, or new scientific
campaign begins before that acceptance. This post-closure correction does not
reopen the lifecycle foundation's engineering gates.
