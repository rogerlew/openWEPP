...
- Decision: Keep downstream wepppy mutation out of scope.
  ...
  Date/author: 2026-07-14, Codex.
- Decision: Defer downstream vendoring until the openWEPP beta release campaign
  in WEPPcloud.
  Rationale: neither the WEPPcloud documentation surface nor the dossier set is
  ready for integration. The handoff remains a dormant compatibility contract,
  not authorization to vendor. A refreshed, separately reviewed wepppy package
  is mandatory before the beta release is issued.
  Date/author: 2026-07-15, user direction recorded by Codex.
- Decision: Treat the v1 SNOTEL public output as a rejected scientific-
  communication prototype and require a documentation-first v2 reset.
  Rationale: subsequent scientific-audience review found that the generated
  page led with internal lifecycle and aggregate evidence labels while omitting
  most of the existing scientific method, results, figures, and discussion. The
  labels are not accepted as an adjudication of the snow/frost model or its
  evidence. The engineering package remains complete; no v2 code, retirement,
  or new public report is authorized before the `ASSURE-02` documentation gate.
  Date/author: 2026-07-15, user direction recorded by Codex.
...
At closure, the package characterized its generated pilot as a candidate with
insufficient aggregate empirical evidence and blocked aggregate verification.
Subsequent scientific-audience review rejected that reduction as a valid
characterization of the openWEPP snow/frost model or its evidence base. The
historical labels and generated bytes remain evidence of what the v1 compiler
produced, not an accepted scientific conclusion. The underlying equations,
contracts, datasets, validation campaigns, and results retain their own
authority and evidence.

Post-closure sequencing direction: downstream vendoring is deliberately
deferred. It is not the next package and the retained handoff is not a readiness
claim. Vendoring must instead execute as a refreshed, dedicated gate before the
openWEPP beta release in WEPPcloud.

The binding next step is the documentation-only `ASSURE-02` gate in
`docs/ROADMAP.md`. It must establish and review the manuscript-first v2
architecture and a real nonpublic process-kernel prototype before any compiler,
schema, generated-content, retirement, or downstream implementation begins.
