...
def collect(binary, output, policy, case, ofes=1, physical=True, timeout=180,
    ...
    cases = PACKAGE / "artifacts/cases.json"
    ...
    argv = ["taskset", "-c", "0", str(binary), test,
    ...
    try:
        ...
        observation = json.loads(observation_path.read_text()) if observation_path.exists() else {}
        ...
        receipt["observation_complete"] = (
            ...
            and isinstance(observation.get("counts"), dict)
            and (not physical or bool(rows)))
        ...
        if inputs_path.exists() and outputs_path.exists():
            ...
            receipt["files_complete"] = bool(inputs["files"]) and bool(outputs["files"]) and all(
                (base / name).is_file() and digest(base / name) == expected
                for base, files in ((root, inputs["files"]), (root / "output", outputs["files"]))
...
def main():
    ...
    result = collect(args.binary, args.output, args.policy, args.case, args.ofes,
                     not args.minimal, args.timeout, args.test)
    print(json.dumps({k: result.get(k) for k in ("case", "policy", "ofes", "exit_code", "timeout", "runner_wall_s", "execution_valid", "completed_days")}))
