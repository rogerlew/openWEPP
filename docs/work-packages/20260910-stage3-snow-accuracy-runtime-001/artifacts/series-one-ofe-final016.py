...
def main():
    p=argparse.ArgumentParser();p.add_argument('binary',type=Path);p.add_argument('directory',type=Path);p.add_argument('--candidate',action='append',required=True);p.add_argument('--case',default='continuity');p.add_argument('--source',type=Path,required=True);a=p.parse_args()
    ...
    for job in jobs:
        ...
        r=collect(a.binary,a.directory/job['name'],job['policy'],a.case,physical=False)
