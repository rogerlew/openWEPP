...
```text
cargo test -p openwepp-hillslope-orchestrator cqr20_project_annual_extension_controls  PASS before production refactor
cargo test -p openwepp-hillslope-orchestrator cqr20_project_annual_extension_controls  PASS after production refactor
...
markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260615-cqr20-projection-helpers-complexity-001 --format json  PASS
git diff --check                                                                  PASS
```

Ran: first `cargo clippy --workspace --all-targets -- -D warnings` attempt
failed on a test-only `needless_pass_by_value` warning in the new
characterization helper. The helper was changed to borrow
`YearlyAnnualExtension`; final clippy pass exited `0`.
