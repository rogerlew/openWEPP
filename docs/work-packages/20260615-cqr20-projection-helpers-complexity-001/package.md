# CQR20 - Projection Helpers Complexity Refactor
...
Decompose the current CQR20 target in
`crates/openwepp-hillslope-orchestrator/src/runtime_inputs/05_projection_helpers.rs`,
originally tracked as rank 14 with snapshot CRAP `384`, CC `29`, and coverage
`25%`, so the live target function and any newly extracted helpers have CRAP
`<= 30`.

## Rationale

The projection helpers module participates in parser-to-runtime surface
adaptation for hillslope execution. CQR20 must reduce local complexity without
changing public runtime projection behavior, typed guards, stable error IDs,
aliases, parser compatibility, symbols, units, formulas, float expression order,
...
- Private behavior-preserving helper extraction in `05_projection_helpers.rs`.
...
- No public API, error ID, alias, symbol, unit, parser compatibility, runtime
  projection, float expression order, output formula, or science-contract
  behavior changes.
- No unrelated hillslope runtime input cleanup.
- No branch switching.
...
- `crates/openwepp-hillslope-orchestrator/src/runtime_inputs/05_projection_helpers.rs`
- Focused tests under existing hillslope runtime input test paths if
...
- Record protected runtime projection, typed guard, symbol, unit, formula, and
...
- Preserve public surface, stable errors, aliases, symbols, units, formulas,
  float expression order, and runtime projection behavior.
...
- Run the required closure gates:
  ...
  4. `cargo deny check`
  5. `markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260615-cqr20-projection-helpers-complexity-001 --format json`
  6. `git diff --check`
...
- Current CQR20 target function and any newly extracted helpers have CRAP
  `<= 30`.
...
- No public API, error ID, alias, symbol, unit, parser compatibility, formula,
  float expression order, or science-contract behavior changes.
