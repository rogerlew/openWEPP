# CQR24 - Scheduler Seed Runtime Complexity Refactor
...
Decompose the current CQR24 target in
`crates/openwepp-runner/src/hillslope/scheduler_trace/scheduler_seed_and_runtime.rs`,
originally tracked as rank 18 with snapshot CRAP `317`, CC `58`, and coverage
`57%`, so the live target function and any newly extracted helpers have CRAP
`<= 30`.

## Rationale

The scheduler seed/runtime path feeds kernel-facing runtime state and output
publication. CQR24 must reduce local complexity without changing public API,
runtime symbols, units, formulas, parser compatibility, typed guard behavior,
publication behavior, or science-contract behavior.
...
- No public API, module visibility, dependency, caller behavior, parser
...
- No unrelated scheduler, runner, hillslope output, or kernel cleanup.
...
- Preserve formula operand order, public signatures, typed status behavior,
  publication shape, runtime symbols, and science-contract behavior.
...
- Run the required closure gates:
  ...
  4. `cargo deny check`
  5. `markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260615-cqr24-scheduler-seed-runtime-complexity-001 --format json`
  6. `git diff --check`
...
- Current CQR24 target function and any newly extracted helpers have CRAP
  `<= 30`.
...
- No public API, formula, runtime symbol, alias, unit, stable status/error,
  parser compatibility, publication schema, or science-contract behavior change
  is introduced.
