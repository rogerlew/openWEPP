# CQR13 - Runtime Core Types Complexity Refactor
...
Decompose the current CQR13 target in
`crates/openwepp-hillslope-orchestrator/src/runtime_inputs/00_core_types.rs`,
originally tracked as rank 7 with snapshot CRAP `964`, CC `65`, and coverage
`40%`, so the live target function and any newly extracted helpers have CRAP
`<= 30`.

## Rationale

`00_core_types.rs` owns shared runtime-input error and parser projection types
for hillslope runtime publication. CQR08 closed the earlier display/error-code
target in this file; CQR13 must re-run live metrics and close the next ranked
complexity row without changing public API, error IDs, variant semantics, or
runtime/kernel-facing projection behavior.
...
- Supporting metrics: before/after LCOV, target-function identity, line counts,
  suppression census, public API parity, guard/error parity, and line-count
  governance.
...
- No stable error-code, display text, typed variant, field, alias, symbol, unit,
  parser-compatibility, scheduler behavior, or kernel projection behavior
  change.
...
This package is kernel-affecting because runtime-input core types define
fail-closed projection errors and request surfaces consumed by kernel-facing
hillslope runtime publication. The current-scope acceptance gate is public API,
error/guard, and projection-surface parity:

- preserve every public type, variant, constructor, and function signature;
- preserve stable error IDs, display strings, fields, and allowed strings;
- preserve parser compatibility and runtime/kernel projection meaning;
...
4. Package artifacts:
   ...
   - `artifacts/cqr13-quality-plan-report.md`
   - `artifacts/cqr13-public-api-surface-parity-report.md`
   - `artifacts/cqr13-crap-before.md`
   - `artifacts/cqr13-crap-after.md`
   - `artifacts/cqr13-coverage-closure.md`
   - `artifacts/cqr13-numeric-equivalence.md`
   - `artifacts/cqr13-kernel-profile-compliance.md`
   - `artifacts/cqr13-implementation-and-test-evidence.md`
   - `artifacts/cqr13-line-count-governance-checklist.md`
   ...
   - `artifacts/cqr13_disposition.md`
...
- Record public API, error-code/display, guard, symbol, alias, parser, and
...
- Preserve exact statement order, float expression order/grouping,
  short-circuiting, typed errors, symbols, fields, display strings, and public
  behavior.
...
- Run the required closure gates:
  ...
  4. `cargo deny check`
  5. `markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260615-cqr13-runtime-core-types-complexity-001 --format json`
  6. `git diff --check`
...
- Current CQR13 target function and any newly extracted helpers have CRAP
  `<= 30`.
...
- No public API, error-code, display text, typed variant, formula, threshold,
  typed guard, symbol, alias, unit, parser-compatibility, scheduler behavior, or
  kernel projection behavior change is introduced.
