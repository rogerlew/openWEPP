...
def _raw_residual(case: dict[str, Any], ground_config: dict[str, Any],
    ...
    canopy_capacity = 0.08
    preliminary_canopy_liquid = min(
        forcing["canopy_liquid_kg_m2_tile"], canopy_capacity)
    wet_fraction = (preliminary_canopy_liquid / canopy_capacity) ** (2.0 / 3.0)
    ...
    sun_maximum = _class_state(case, "sun", tsun, qcan, 1.0)
    shade_maximum = _class_state(case, "shade", tshade, qcan, 1.0)
    rb = 1.0 / forcing["gb_leaf_m_s"]
    transpiration = {
    ...
    emax = {
        "sun": rho * (sun_maximum["surface_q_kg_kg"] - qcan)
            / (rb + sun_maximum["rs_s_m"]) * dry_sun,
        "shade": rho * (shade_maximum["surface_q_kg_kg"] - qcan)
            / (rb + shade_maximum["rs_s_m"]) * dry_shade,
    }
    qsat_wet = qsat(twet, pressure)
    wet_potential = rho * forcing["gb_wet_m_s"] * (qsat_wet - qcan) * wet_area
    wet_cap = preliminary_canopy_liquid / case["dt_s"]
    ...
    hydraulic = [transpiration["sun"] - q1_sun, transpiration["shade"] - q1_shade,
        transpiration["sun"] - emax["sun"] * V3.vulnerability(psi_sun, p["p50_leaf"], p["ck"]),
        transpiration["shade"] - emax["shade"] * V3.vulnerability(psi_shade, p["p50_leaf"], p["ck"]),
        q1_sun + q1_shade - q2, q2 - math.fsum(row["q_final_kg_m2_tile_s"] for row in q3)]
    water_scale = max(WATER_ATOL, emax["sun"], emax["shade"],
    ...
    for iteration in range(max_iterations + 1):
        ...
        if norm <= 1.0:
            ...
            if valid_trial(prospective):
                try:
                    prospective_detail = evaluator(prospective)
                ...
                if prospective_detail is not None:
                    ...
                    prospective_ok = (prospective_steps is not None
                        ...
                        and all(prospective_steps[key] <= tolerance
                                for key, tolerance in step_tolerances.items())) \
                        if step_measure is not None else max(abs(value) for value in delta) <= 1.0e-8
                    if prospective_ok:
                        return {"accepted": True, "iterations": iteration, "solution": x,
                            "detail": detail, "residual_norm_history": history,
...
def solve_joint(case: dict[str, Any], ground_config: dict[str, Any],
    ...
    for iteration in range(max_iterations + 1):
        ...
        prospective_steps = {"hydraulic_mm": max(abs(value) for value in delta[:4]),
            ...
            "humidity_kg_kg": abs(delta[11]), "ci_pa": 0.0}
        if norm <= 1.0:
            prospective = [value + change for value, change in zip(x, delta, strict=True)]
            _, prospective_detail = _raw_residual(
                case, ground_config, ground_state, prospective, caps)
            prospective_steps["ci_pa"] = max(
                abs(prospective_detail[name]["ci_pa"] - detail[name]["ci_pa"])
        ...
        if norm <= 1.0 and (prospective_steps["temperature_k"] <= 1.0e-8
            ...
            candidate = {"vegetation": {
                "component_temperatures_k": detail["component_temperatures_k"],
                "canopy_air_temperature_k": detail["canopy_air_temperature_k"],
                ...
                "hydrology_mass_mutation": None}
            return {"accepted": True, "pass": "capped" if caps else "potential",
                "iterations": iteration, "solution": x,
                "residual_norm_history": history,
                "backtracking_count": backtracking, "step_norms": prospective_steps,
                "pivot_magnitude": pivot, "matrix_norm": matrix_norm,
                "components": detail, "candidate": candidate,
                "beginning_sha256": beginning_hash, "rollback_sha256": beginning_hash}
        accepted = False
        for exponent in range(21):
            ...
            if trial_norm < norm:
                ...
                last_steps = {"hydraulic_mm": max(abs(value) for value in applied[:4]),
                    ...
                    "ci_pa": max(abs(trial_detail[name]["ci_pa"] - detail[name]["ci_pa"])
                        for name in ("sun", "shade"))}
                x = trial
                backtracking += exponent
                accepted = True
...
def build_joint_vectors() -> dict[str, Any]:
    ...
    singular_start = [-1686.2904133837442, -1076.3752175138482,
        -549.0225635913952, -16498.141817800282, 0.766937989633371,
        0.9109488176438152, 311.86772781064366, 303.10905982004255,
        310.8624155950483, 302.1303879493163, 296.17326995392574,
        0.010745754067385132, 287.007378728809, 303.21171850315795,
        300.8843611870778]
    singular = solve_joint(bundle["case"], bundle["ground_config"],
        bundle["ground_state"], start=singular_start)
    # Positive, valid parameters and warm start selected by a bounded
    # deterministic search for the canonical Rust iteration guard.
    limited_case = copy.deepcopy(bundle["case"])
    limited_case["parameters"].update({
        "k1_max": 0.0001444299653183656,
        "k2_max": 1.5389243831286362e-06,
        "k3_max_m_s": 1.96921997755462e-07,
    })
    limited_start = [-5706.990986525235, -3953.815285369903,
        -8895.703204372228, -5772.288321118055, 0.6656073679507759,
        0.6884113518642979, 294.4120192978403, 294.26327754903633,
        295.16563313461495, 285.26320181951417, 290.3389563894221,
        0.010992020878824095, 302.3849691953893, 300.9120115026359,
        296.6326168436724]
    ...
    backtracking_case = copy.deepcopy(bundle["case"])
    backtracking_case["parameters"].update({"k1_max": 8.08843246855763e-06,
        "k2_max": 1.0989772741113142e-05, "k3_max_m_s": 4.518272802798021e-05})
    backtracking_start = [-10505.902951449543, -727.5877852992373,
        -3826.381288830032, -3930.3127202055744, 0.23806946975643398,
        0.7032291199036342, 287.4611652856252, 285.3369474561113,
        309.2473441391362, 302.5239146010649, 293.62424380358834,
        0.003123110787329362, 289.6124780725629, 297.27429717267876,
        302.8901285969406]
    backtracking = solve_joint(backtracking_case, bundle["ground_config"],
    ...
    # Preserve the runtime divergence explicitly: Python reaches its
    # backtracking guard, while Rust admits 50 strict-decrease updates.
    limited["rust_expected_failure"] = "iteration_limit"
    limited["rust_expected_iterations"] = 50
    backtracking_bundle = {**bundle, "case": backtracking_case}
    backtracking = _typed_numerical_failure(
        backtracking, backtracking_bundle, "outer_gas_energy_hydraulic_coupling")
...
def build_mandatory_scenario_vectors() -> dict[str, Any]:
    ...
    def covered_record(column: dict[str, Any], identity: str,
        ...
        result = solve_covered_column(column, caps=caps, start=start)
        if expected_failure is None:
            result = require(result, identity)
        elif (result.get("accepted") or result.get("candidate") is not None
                or result.get("failure") != expected_failure):
            raise AssertionError(
                f"mandatory scenario {identity} expected {expected_failure}: {result}")
    ...
    dry_litter["start"] = [-8442.933385788541, -5060.8111475066235,
        ...
        295.8150648904064]
    dry_litter_record = covered_record(
        dry_litter, "dry_litter_covered", expected_failure="backtracking_limit")
    ...
    sensible_vapor_feedback = {
        "dry_cool_ground": covered_record(
            ...
            expected_failure="backtracking_limit"),
    ...
    wet_evaporation_record = covered_record(
        wet_evaporation, "wet_canopy_evaporation", expected_failure="singular")
    ...
    def capped_covered_record(identity: str, surface_factor: float,
        ...
        final = solve_covered_column(column, caps=caps)
        if expected_failure is None:
            final = require(final, identity + ":final")
        elif (final.get("accepted") or final.get("candidate") is not None
                or final.get("failure") != expected_failure):
            raise AssertionError(
                f"mandatory scenario {identity} expected {expected_failure}: {final}")
    ...
    partial_surface_cap = capped_covered_record(
        "partial_surface_cap", 0.55, 1.0, expected_failure="singular")
    partial_top_layer_cap = capped_covered_record(
        "partial_top_layer_cap", 1.0, 0.55, expected_failure="singular")
