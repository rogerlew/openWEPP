# CQR33 - Watershed Structure Parser Complexity Closure
...
Close the current CQR33 target in
`crates/openwepp-input-contract/src/parsers/watershed_structure.rs`,
originally tracked as rank 27 with snapshot CRAP `240`, CC `15`, and coverage
`0%`, so the live target function and any newly extracted helpers have CRAP
`<= 30`.

## Rationale

The watershed-structure parser preserves legacy `.str` compatibility and emits
typed parser structures consumed by watershed runtime projection. This package
is limited to characterization and private behavior-preserving decomposition of
the scoped high-CRAP parser target.
...
- No grammar, token order, compatibility mode, typed error, error ID, error
  variant, field name, alias, unit, parser-output shape, runtime-projection,
  or kernel-facing semantic changes.
...
- preserve parser token order, branch selection, count/cardinality guards,
  field names, error IDs, error variants, and emitted structure shape;
- preserve downstream runtime/kernel-facing meanings of parsed watershed
  structure fields;
...
4. Package artifacts:
   ...
   - `artifacts/cqr33-quality-plan-report.md`
   - `artifacts/cqr33-crap-before.md`
   - `artifacts/cqr33-crap-after.md`
   - `artifacts/cqr33-coverage-closure.md`
   - `artifacts/cqr33-parser-equivalence.md`
   - `artifacts/cqr33-implementation-and-test-evidence.md`
   - `artifacts/cqr33-line-count-governance-checklist.md`
   ...
   - `artifacts/cqr33_disposition.md`
...
- `crates/openwepp-input-contract/src/parsers/watershed_structure.rs` only if
  live metrics require behavior-preserving refactor.
...
- Record protected parser APIs, compatibility controls, typed errors, error
  IDs, cardinality guards, field names, and output structure.
...
- Preserve public parser APIs, parse order, branch behavior, typed errors,
  error IDs, compatibility controls, count/cardinality validation, and output
  structure.
...
- Run the required closure gates:
  ...
  4. `cargo deny check`
  5. `markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260615-cqr33-watershed-structure-parser-complexity-001 --format json`
  6. `git diff --check`
...
- Current CQR33 target function and any newly extracted helpers have CRAP
  `<= 30`.
...
- Public parser APIs, grammar, token order, compatibility controls, typed
  error behavior, error IDs, error variants, field names, units, parser-output
  shape, and runtime/kernel-facing meanings are unchanged.
