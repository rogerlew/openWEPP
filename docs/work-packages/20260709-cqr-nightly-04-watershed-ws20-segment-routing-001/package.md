...
`crates/openwepp-watershed-orchestrator/src/lib_mod/kernel/routing/02_ws20_segment_routing.rs`
with CRAP above `30` to `<= 30`, or record an ADR-0021-style
disposition when a row is not safely reducible as behavior-preserving CQR.
Preserve WS20/WS21 channel sediment routing behavior, typed fail-closed guards,
floating-point statement order, accumulation order, diagnostics semantics, and
all existing valid-input output identity.
...
`SC-ROUTE-001` and `SC-SED-001` are required because this target is
science-tier channel sediment routing code. This package must not amend
...
- characterization tests required to make WS20/WS21 segment-routing helpers
  safe to refactor;
...
- focused tests proving routing guard behavior and numeric/API identity.
...
2. Add module-local characterization tests before refactoring when existing
   coverage is insufficient.
3. Because this is science-tier channel sediment routing, record ADR-0021
   ...
   - applicable obligation-to-test binding against `SC-ROUTE-001` /
     `SC-SED-001` without creating new authority.
4. Record the behavior oracle and command evidence in
   `artifacts/characterization.md`.

### Phase C - Refactor

1. Extract cohesive guard clusters, routing case branches, or loop bodies one at
   a time.
