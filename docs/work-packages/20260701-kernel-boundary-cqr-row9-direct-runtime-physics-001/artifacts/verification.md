...
- Full workspace LCOV tests passed and wrote
  `/tmp/openwepp-row9-full-after.lcov`.
- Full workspace CRAP JSON written to
  `/tmp/openwepp-crap-row9-full-after.json`.
- Row #9 owned functions above CRAP 30: `0`.

See `crap-after.md` for per-function after values from the row #9 before-list.
...
- `cargo nextest run --workspace --profile full`: `1246` tests run, `1246`
...
```text
markdown-doc lint --path docs/work-packages/20260701-kernel-boundary-cqr-row9-direct-runtime-physics-001 --path docs/work-packages/README.md
markdown-doc validate --path docs/work-packages/20260701-kernel-boundary-cqr-row9-direct-runtime-physics-001 --path docs/work-packages/README.md
```
...
/usr/bin/time -v target/release/openwepp-cli-hill \
  ...
  --output-dir /tmp/kernel-boundary-cqr-row9-h2637/output \
  --manifest-path /tmp/kernel-boundary-cqr-row9-h2637/output/manifest.json
```

Results:

- Release build exited 0.
- H2637 exited 0; wall `1:07.27`; max RSS `79736 KiB`.
...
- CLI output files were emitted through the runfile output path
  `/tmp/stage1-seed-authority/after-1b/output`; the explicit row #9 output
