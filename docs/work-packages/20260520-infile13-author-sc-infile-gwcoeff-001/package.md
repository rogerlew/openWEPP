...
## Objective
Author and disposition `SC-INFILE-GWCOEFF-001` and its paired openWEPP-owned input
specification for surface `infile-gwcoeff` (gwcoeff.txt).

## Why This Package Exists
The parser input-surface registry marks `infile-gwcoeff` as `planned`.
...
- Update parser input-surface registry disposition for `infile-gwcoeff`.
...
- Draft/refresh `docs/specifications/wepp-input-files/specs/gwcoeff.spec.md`.
- Draft/refresh `SC-INFILE-GWCOEFF-001` with required section set, propagation map,
...
## Exit Criteria
- `docs/specifications/wepp-input-files/specs/gwcoeff.spec.md` exists and covers this surface.
- `SC-INFILE-GWCOEFF-001` includes all required parser-contract sections.
...
- Dual verification artifacts are complete with no unresolved high-severity
  findings.
- Registry entry for `infile-gwcoeff` is updated consistently with disposition.
