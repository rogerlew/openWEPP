...
type AuthorityContext = (String, Option<FieldDisposition>);

fn recursive_mapping_sources(repo: &Path) -> Vec<(String, String, Vec<AuthorityContext>)> {
    ...
    while let Some((name, path, inherited, authority_path, ancestry)) = queue.pop_front() {
        ...
        let entry = found.entry(name.clone()).or_insert((path, Vec::new()));
        let context = (authority_path.clone(), inherited);
        ...
        for (field, ty) in members {
            ...
            for token in ty.split(|c: char| !c.is_ascii_alphanumeric() && c != '_') {
                if token.chars().next().is_some_and(char::is_uppercase)
                    ...
                    && !ancestry.iter().any(|ancestor| ancestor == token)
                {
                    let mut child_ancestry = ancestry.clone();
                    child_ancestry.push(token.to_owned());
                    queue.push_back((
                        token.to_owned(),
                        path.clone(),
                        child_inherited,
                        format!("{authority_path}.{field}"),
                        child_ancestry,
                    ));
                }
            }
    ...
    found
        .into_iter()
        .map(|(name, (path, contexts))| (name, path, contexts))
        .collect()
}
...
fn disposition(source_type: &str, field: &str) -> FieldDisposition {
    ...
    if source_type.ends_with("Inputs")
        || matches!(
            source_type,
            "DirectDayForcing"
                | "DirectWinterFrostPartitionOutcome"
...
fn generate_metadata_and_ledger(root: &Path) {
    ...
    for (name, path, contexts) in recursive_mapping_sources(&repo) {
        ledger.push_str(&format!("## `{name}`\n\n| Authority path | Source field | Rust type | Classification | Source operands | Reconstruction operation | Exact comparison | Mismatch poison | Omission consequence |\n|---|---|---|---|---|---|---|---|---|\n"));
        let source = fs::read_to_string(repo.join(path)).unwrap();
        for (field, ty) in fields(&source, &name) {
            let intrinsic = disposition(&name, &field);
            for (authority_path, inherited) in &contexts {
                ...
                let field_path = format!("{authority_path}.{field}");
                ledger.push_str(&format!("| `{field_path}` | `{field}` | `{ty}` | {class} | {operands} | {operation} | {comparison} | {poison} | {omission} |\n"));
                rows.push(serde_json::json!({"authority_path":field_path,"source_type":name,"source_field":field,"rust_type":ty,"classification":class,"source_operands":operands,"reconstruction_operation":operation,"exact_comparison_rule":comparison,"mismatch_poison":poison,"omission_consequence":omission}))
            }
        }
        ledger.push('\n')
    }
