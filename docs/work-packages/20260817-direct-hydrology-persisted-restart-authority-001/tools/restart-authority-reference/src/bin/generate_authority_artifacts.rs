...
fn recursive_mapping_sources(repo: &Path) -> Vec<(String, String, Vec<Option<FieldDisposition>>)> {
    ...
    while let Some((name, path, inherited)) = queue.pop_front() {
        let context = inherited.map_or("persisted", |value| value.0);
        if !seen.insert((name.clone(), context)) {
            continue;
        }
        ...
        if !entry.1.contains(&inherited) {
            entry.1.push(inherited);
        }
...
fn generate_metadata_and_ledger(root: &Path) {
    ...
    for (name, path, contexts) in recursive_mapping_sources(&repo) {
        ...
        for (field, ty) in fields(&source, &name) {
            let intrinsic = disposition(&name, &field);
            let effective: Vec<_> = if intrinsic.0.starts_with("persisted") {
                contexts
                    .iter()
                    .map(|context| context.unwrap_or(intrinsic))
                    .collect()
            } else {
                vec![intrinsic]
            };
            let join = |index: usize| {
                let mut values = effective.iter().map(|value| match index {
                    0 => value.0,
                    1 => value.1,
                    2 => value.2,
                    _ => value.3,
            ...
            let class = join(0);
            let operands = join(1);
            let operation = join(2);
            let poison = join(3);
            let has_persisted = effective.iter().any(|value| value.0.starts_with("persisted"));
            let has_reconstructed = effective
                .iter()
                .any(|value| !value.0.starts_with("persisted"));
            let comparison = if has_persisted && has_reconstructed {
                "exact persisted projection and canonical reconstruction re-projection equality"
            } else if has_persisted {
