...
with CRAP above `30` to `<= 30`, or record an ADR-0021-style disposition when a
row is not safely reducible as behavior-preserving CQR. Preserve typed boundary
symbol strings and runtime behavior for all existing valid inputs.
...
Read the nearest relevant `SC-*` contract only if implementation work changes
contract authority, symbol meaning, output semantics, or runtime guard behavior.
This CQR package is expected to preserve existing strings exactly and should not
amend contracts.
...
- characterization tests required to make typed symbol projection safe to
  refactor;
...
- focused tests that prove existing symbol strings.
...
- `tests/integration/arch22_typed_state_surface_contract.rs`
- `tests/integration/erod11_alias_boundary_ownership_contract.rs`
- `docs/work-packages/20260709-cqr-nightly-01-kernel-contract-typed-symbol-surfaces-001/**`
...
1. Extract cohesive enum/string mapping blocks one at a time.
2. Preserve exact symbol string values, formatting, dynamic index formatting,
