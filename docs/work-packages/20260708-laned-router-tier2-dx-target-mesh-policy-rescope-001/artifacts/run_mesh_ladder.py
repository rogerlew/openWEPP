#!/usr/bin/env python3
"""Run the active-plain target-dx mesh ladder for the selected cohort."""
...
RUN_ROOT = ARTIFACTS / "mesh-ladder-runs"
SUMMARY_JSON = ARTIFACTS / "mesh-ladder-summary.json"
SUMMARY_MD = ARTIFACTS / "mesh-ladder-summary.md"
...
def write_markdown(summary: dict[str, Any]) -> None:
    ...
    for rec in summary["runs"]:
        ...
        lines.append(
            "| {member} | {rung} | {status} | {wall} | {user} | {steps} | {trace_rows} | {outlet} | {storage} | {tail} | {tdet} |".format(
