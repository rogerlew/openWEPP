...
| `direct_runtime/surface_liquid_ingress.rs` | 1,952 | PASS: ingress construction and complete frozen partition operands; crate-local tests are separate. |
| `direct_runtime/surface_liquid_ingress_tests.rs` | 1,983 | PASS; focused independent partition, coordinated identity and routing tests. |
...
| `direct_runtime/surface_liquid_closure.rs` | 2,300 | WARN: independent chronological WB14 partition and routed projection; future campaign split will separate frozen operand projection from comparison/diagnostics. |
