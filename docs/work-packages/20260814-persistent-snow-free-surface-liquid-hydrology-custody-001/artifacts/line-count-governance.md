...
| `land_surface_energy_shadow/mod.rs` | 2,896 | WARN: cohesive default-off arbitration with receiver-failure, unified-entry preflight and standalone sealing extracted; public callback errors are canonically enveloped through the shared taxonomy, and no further additions are allowed without cohesive extraction. |
...
| `land_surface_energy_shadow/receiver_validation.rs` | 2,604 | WARN: self-contained expected topology, centralized boundary-aware error taxonomy, canonical identity/domain/cardinality/bound preflight, frozen rollback-row validation and complete framed digest encoding. |
...
| `land_surface_energy_shadow/unified_entry_preflight.rs` | 646 | PASS; complete input-only configuration/state/request/ingress/WB14/soil-source/receiver-expectation preflight, raw-attempt plus v3 framing and computed-snapshot provenance before authorization or callback execution. |
...
| `tests/integration/land_surface_energy_real_hydrology_shadow_contract/unified_boundary_tests.rs` | 687 | PASS; receiver-expectation layer/lineage callback exclusion, field-sensitive attempted hashing and exhaustive raw/canonical callback-error taxonomy matrix. |
...
| `tests/integration/land_surface_energy_real_hydrology_shadow_contract/precedence_tests.rs` | 712 | PASS; extracted mixed public E002 through E008, complete configured-source/ingress-identity-before-E003/callback, attempted-hash alias resistance, cross-input envelope, nonfinite lane and condensation-context precedence matrix. |
