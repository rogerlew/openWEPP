...
| `direct_runtime/surface_liquid_owner.rs` | 2,638 | WARN: cohesive owner schema, canonical persistence, checked proportional D/A/F, contextual failure payloads, and candidate validation; crate-local tests are separate, and a future campaign split will separate persistence/schema from arbitration validation. |
| `direct_runtime/surface_liquid_owner_tests.rs` | 1,095 | PASS; crate-local owner and checked-arithmetic tests include canonical-last tiny-positive and contextual serialized-byte failures. |
...
| `land_surface_energy_shadow/mod.rs` | 2,811 | WARN: cohesive default-off arbitration and contextual receiver construction; independent validation and framed hashing are extracted. |
| `land_surface_energy_shadow/receiver_validation.rs` | 998 | PASS; self-contained expected-topology, arithmetic-preflight, closure, canonical-error validation, and framed digest encoding. |
