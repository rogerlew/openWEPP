...
| `land_surface_energy_shadow/mod.rs` | 2,985 | WARN: cohesive default-off arbitration with receiver-failure and unified-entry preflight extracted; standalone finalization now seals the exact three-owner rollback set, and no further additions are allowed without cohesive extraction. |
...
| `land_surface_energy_shadow/unified_entry_preflight.rs` | 413 | PASS; complete input-only configuration/state/request/ingress/WB14/soil-source E001/E002 preflight, attempted hashing and computed-snapshot provenance before E003/domain checks or callback execution. |
...
| `tests/integration/land_surface_energy_real_hydrology_shadow_contract/precedence_tests.rs` | 656 | PASS; extracted mixed public E002 through E008, complete configured-source/ingress-identity-before-E003/callback, attempted-hash alias resistance, cross-input envelope, nonfinite lane and condensation-context precedence matrix. |
| `tests/integration/land_surface_energy_real_hydrology_shadow_contract/sealed_receiver_context_tests.rs` | 349 | PASS; extracted sealed LSE/thermal exact-owner, exact rollback-set, configured-context, topology and rollback-hash poisons. |
| `tests/integration/land_surface_energy_real_hydrology_shadow_contract/snow_preflight_tests.rs` | 566 | PASS; complete snow/albedo/layer/frost/fine-layer count, ordering, uniqueness, reciprocal membership and runtime-carry E003-before-E004 public-domain matrix. |
