...
| `direct_runtime/surface_liquid_ingress_context_tests.rs` | 512 | PASS; extracted cadence, public exact-context/precedence, full-infiltration, late-arithmetic and exact mass/Q round-trip poison tests. |
...
| `land_surface_energy_shadow/mod.rs` | 2,980 | WARN: cohesive default-off arbitration, snow-free domain preflight and category-wide cross-input public precedence; independent validation, checked receiver aggregation, error completion and framed hashing are extracted. No further additions are allowed without cohesive extraction. |
| `land_surface_energy_shadow/receiver_validation.rs` | 2,482 | WARN: self-contained expected topology, canonical identity/domain/cardinality/bound preflight, exact configured context and owner-specific rollback hashes, E010/E011 closure taxonomy and complete framed digest encoding; derived arithmetic preflight is extracted. |
...
| `tests/integration/land_surface_energy_real_hydrology_shadow_contract.rs` | 2,906 | WARN: complete unified public-boundary acceptance and receiver taxonomy poison matrix; raw-attempt hash, precedence, sealed-receiver context and snow-domain cases are extracted, and no further cases may be added without another cohesive split. |
...
| `tests/integration/land_surface_energy_real_hydrology_shadow_contract/snow_preflight_tests.rs` | 153 | PASS; exhaustive snow-lane/runtime-carry scalar E003-before-E004 public-domain matrix. |
| `direct_runtime/surface_liquid_closure.rs` | 2,955 | WARN: independent chronological WB14 partition, raw source/OFE mass and exact mass/Q join, routed projection and persistent endpoint join; exact-Q, frozen raw-parent mass and arithmetic-preflight helpers are extracted, and no further additions are allowed without cohesive split. |
