# CQR Nightly Batch 02 Target 02 Kickoff
...
`docs/work-packages/20260709-cqr-nightly-b02-02-direct-runtime-audit-error-helpers-001/package.md`
through disposition.
...
the mechanical/CQR guides, ADR-0021, `SC-WATBAL-001`, `SC-OFEROUTE-001`, and the
target module.

Conditional: `docs/standards/local-ci-gate-selection.md` when a focused gate
needs tier selection.

On-demand: adjacent callers and
...
contracts, serialization, fail-closed behavior, public output, typed API, or
exact diagnostic strings. Preserve arithmetic and short-circuit order.
...
subagents for CQR metric checks, focused/full gates, direct-runtime error
identity, review, and verification. Outputs: package-local compact metrics and
review / verification artifacts. Write access: read-only unless explicitly given
a bounded target-module or package-artifact fix.
