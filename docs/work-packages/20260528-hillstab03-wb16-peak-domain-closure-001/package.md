...
## Rationale
HILLSTAB02 removed the dominant parser failure families (`SOL-E-006`,
`MAN-E-009`), exposing `HKERNEL-WB16-PEAK-E-003` as a top residual blocker to
stability hold-lift progress.

## Scope
### Included
- Contract authority updates for WB16 peak-runoff domain behavior where needed.
- Contract-derived tests that reproduce and pin `HKERNEL-WB16-PEAK-E-003`.
- Pre-implementation contract gate evidence.
- Production runtime/kernel/orchestrator updates required to close this failure
  family.
...
### Excluded
- `HKERNEL-EROD14-WAVE2-E-003` closure (handled in follow-on package).
...
1. Remediation analysis:
   - `artifacts/hillstab03-wb16-remediation-report.md`
2. Rerun outcomes:
   - `artifacts/hillstab03-rerun-results.json`
   - `artifacts/hillstab03-rerun-delta-report.md`
3. Required package artifacts:
   - `artifacts/hillstab03-contract-implementation-evidence.md`
   - `artifacts/hillstab03-contract-test-implementation-evidence.md`
   - `artifacts/hillstab03-preimplementation-contract-gate.md`
   - `artifacts/hillstab03-implementation-and-test-evidence.md`
   - `artifacts/hillstab03-kernel-profile-compliance-checklist.md`
   - `artifacts/owned-file-manifest.md`
   - `artifacts/gate-results.md`
   - `artifacts/hillstab03_disposition.md`
...
## Intended Write Set
- `docs/work-packages/20260528-hillstab03-wb16-peak-domain-closure-001/**`
...
- Derive the `HKERNEL-WB16-PEAK-E-003` failing case set from HILLSTAB02 logs
  and classify dominant domain triggers.

### Phase B - Contract authority updates
- Amend canonical `SC-*` contract text for any missing WB16 domain/guard
  authority required by observed residuals.

### Phase C - Contract-derived tests + pre-implementation gate
- Add failing vectors/tests covering each dominant trigger branch.
- Record pre-implementation gate failure evidence before code edits.

### Phase D - Production closure
- Implement runtime/kernel/orchestrator closure for `WB16` failure branches.
...
## Exit Criteria
- `HKERNEL-WB16-PEAK-E-003` is eliminated from rerun cohorts or residuals are
  explicitly bounded with authoritative rationale.
