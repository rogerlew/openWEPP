#!/usr/bin/env python3
"""Run HPHYS0254 diagnostic-only H1 localization and full 39 metrics."""
...
def summarize_h1(run_root: Path) -> None:
    ...
    trace_path = run_root / "hillslope_output/H1.hphys0254.trace.jsonl"
...
def summarize_targeted_storage(run_root: Path, hillslope_ids: list[int]) -> None:
    reports = run_root / "reports"
    rows = []
    for hillslope_id in hillslope_ids:
        ...
        post_seed_wb11 = trace_value(post_seed, "wb11_soil_water_mm")
        post_seed_theta = trace_value(post_seed, "wb18_theta_sum_m", 1000.0)
        post_seed_gap = trace_value(post_seed, "wb11_minus_theta_sum_m", 1000.0)
        post_wb13_total = trace_value(post_wb13, "wb13_total_soil_mm")
        rows.append(
            [
                f"H{hillslope_id}",
                ...
                baseline_terms["Total-Soil"],
                post_wb13_total - baseline_terms["Total-Soil"],
                candidate_terms["Dp"] - baseline_terms["Dp"],
                candidate_terms["latqcc"] - baseline_terms["latqcc"],
                candidate_terms["Ep"] - baseline_terms["Ep"],
            ]
        )

    md = "# HPHYS0254 Targeted H1/H7/H39 Storage Summary\n\n"
    md += "Status: complete\n\nEvidence mode: ran\n\nRan:\n\n"
    for hillslope_id in hillslope_ids:
        md += f"- H{hillslope_id} trace: `{run_root / f'hillslope_output/H{hillslope_id}.hphys0254.trace.jsonl'}`.\n"
    md += "\n"
    md += markdown_table(
        [
            "Hillslope",
            ...
            "post_seed theta mm",
            "wb11-theta mm",
            "baseline inferred t0 mm",
            "post_seed - baseline t0 mm",
            "candidate day1 Total-Soil mm",
            "baseline day1 Total-Soil mm",
            "Total-Soil diff mm",
            "Dp diff mm",
            "latqcc diff mm",
            "Ep diff mm",
    ...
    md += "\nInterpretation:\n\n"
    md += "- `post_seed - baseline t0` is the pre-scheduler storage projection signal.\n"
    md += "- Day-1 process diffs remain diagnostic continuation signals, not closure claims.\n"
    reports.joinpath("targeted_h1_h7_h39_storage_summary.md").write_text(md, encoding="utf-8")
...
def run_semantics(run_root: Path) -> None:
    ...
    md = "# HPHYS0254 Full 39 Semantic Summary\n\n"
    md += f"- Root: `{run_root}`\n"
...
def main() -> int:
    ...
    for hillslope_id in targeted_ids:
        trace_path = output / f"H{hillslope_id}.hphys0254.trace.jsonl"
        ...
        trace_rows.append(
            {
                "hillslope_id": hillslope_id,
                "rc": trace_result.rc,
                "seconds": f"{trace_result.seconds:.3f}",
        ...
        if trace_result.rc != 0:
            ...
            return trace_result.rc
    write_status(reports / "targeted_trace_status.tsv", trace_rows)
    summarize_h1(run_root)
    summarize_targeted_storage(run_root, targeted_ids)
