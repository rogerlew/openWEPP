#!/usr/bin/env python3
"""Run the frozen CAL-07 executor and focused evidence gates."""
...
def main() -> None:
    ...
    for gate, command in commands:
        run(command)
        gate_rows.append(
            {"gate": gate, "status": "PASS", "command": " ".join(command)}
        )
    with (ART / "gate-results.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(stream, fieldnames=("gate", "status", "command"))
        writer.writeheader()
