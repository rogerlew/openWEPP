# CQR Nightly 03 Runner Snowbench Kickoff
...
- `docs/specifications/science-contracts/AGENTS.md` and
  `docs/specifications/science-contracts/contracts/SC-SNOWFREEZE-001.md` only if
  implementation touches contract authority, snow/frost science behavior,
  default activation posture, or contract-derived integration tests.
...
- adjacent snowbench modules imported by `openwepp-snowbench`

Required-reading budget: `103090` bytes, `OK`; map:
...
`crates/openwepp-runner/src/bin/openwepp-snowbench.rs` end-to-end. Reduce every
...
- No science-formula, threshold, tolerance, contract-authority, serialization,
  fail-closed, default-activation, or public-output semantic changes.
- Preserve argument parse order, error precedence, help behavior, request field
  values, stdout/stderr text, and command dispatch behavior.
- Preserve floating-point statement order, expression grouping, accumulation
  order, and short-circuit behavior in any touched diagnostic call path.
...
`crates/openwepp-runner/src/bin/openwepp-snowbench.rs` or package-local
artifacts.
