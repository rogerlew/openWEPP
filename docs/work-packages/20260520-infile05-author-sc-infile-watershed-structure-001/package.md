...
## Objective
Author and disposition `SC-INFILE-WATERSHED-STRUCTURE-001` and its paired openWEPP-owned input
specification for surface `infile-watershed-structure-str` (.str).

## Why This Package Exists
The parser input-surface registry marks `infile-watershed-structure-str` as `planned`.
...
- Update parser input-surface registry disposition for `infile-watershed-structure-str`.
...
- Draft/refresh `docs/specifications/wepp-input-files/specs/watershed-structure-file.spec.md`.
- Draft/refresh `SC-INFILE-WATERSHED-STRUCTURE-001` with required section set, propagation map,
...
## Exit Criteria
- `docs/specifications/wepp-input-files/specs/watershed-structure-file.spec.md` exists and covers this surface.
- `SC-INFILE-WATERSHED-STRUCTURE-001` includes all required parser-contract sections.
...
- Dual verification artifacts are complete with no unresolved high-severity
  findings.
- Registry entry for `infile-watershed-structure-str` is updated consistently with disposition.
