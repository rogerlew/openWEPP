...
| --- | --- | --- |
| scaffold documentation lint | SUPERSEDED | final package Markdown lint is the current evidence |
...
| first extraction scoped Clippy | SUPERSEDED | final exact-source scoped Clippy PASS is the current evidence |
...
| affected-surface CRAP/coverage | SUPERSEDED | corrected final LLVM-region and CRAP evidence at `68e9b747` is authoritative |
| campaign-global TESTGATE | DEFERRED-MASTER | not a package-local selected gate; the master seven-package ExecPlan owns one changed-head qualification only after all seven module packages close |
...
| final package Markdown lint | PASS | 28 files, zero errors and warnings at `80f63776` |
