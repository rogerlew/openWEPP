...
fn read_forcing(path: &Path) -> Result<Vec<Forcing>, Box<dyn Error>> {
    ...
    input
        ...
        .map(|line| {
            ...
            let vpd = parse_f64(fields[8])?;
            if !vpd.is_finite() || vpd < 0.0 {
                return Err(format!("invalid admitted VPD in row: {line}").into());
            }
            Ok(Forcing {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 4 {
        return Err("usage: cal07c-executor ENSEMBLE FORCING OUTPUT".into());
    }
    ...
    let mut output = String::from(
        "site_id,candidate_id,date,year,doy,vpd_pa,vpd_source,gsi,foliar_activity_fraction,live_foliar_biomass_kg_m2,leaf_on_allocation_kg_m2,leaf_off_litter_kg_m2,mass_closure_residual_kg_m2\n",
    );
            ...
            for day in forcing.iter().filter(|day| day.site == site) {
                let result = state.advance(
                    ...
                    GsiDailyForcing {
                        ...
                        vapor_pressure_deficit_pa: day.vpd,
