# CQR Nightly 07 - Input Management Parser

Package: `20260709-cqr-nightly-07-input-management-parser-001`
Status: `QUEUED`
...
`crates/openwepp-input-contract/src/parsers/management.rs` with CRAP above `30`
to `<= 30`, or record an ADR-0021-style disposition when a row is not safely
reducible as behavior-preserving CQR. Preserve management parser behavior,
typed fail-closed errors, datver and `lanuse` authority gates, public parser
APIs, input-file semantics, and output model identity for all existing valid
inputs.
...
This package is glue-tier parser CQR. It must not amend parser contracts,
management/plant-file authority, datver gates, `lanuse` rules, or error
taxonomy. If a row cannot be closed without changing those semantics, hold
rather than changing authority.
...
`crates/openwepp-input-contract/src/parsers/management.rs`,
`tests/integration/infile_management_parser_contract.rs`,
`tests/integration/infile_management_yaml_contract.rs`, or package-local
artifacts.
...
- characterization tests required to make the management parser safe to
  refactor;
- behavior-preserving helper extraction or control-flow simplification inside
  the target module;
- local private helpers needed by extraction;
- focused parser integration tests that prove existing behavior;
- package artifacts and prompt material.

Out of scope:

- parser contract, plant-file spec, or `lanuse` authority changes;
- datver, `lanuse`, option-domain, record-arity, count, or reference guard
  behavior changes;
- public parser API changes;
- serialization or parsed output model semantics changes;
- broad fixture rewrites or unrelated parser cleanup;
...
Line-count note: the target starts at `2851` lines, above the 2000-line WARN
threshold and below the 3000-line blocker. Characterization tests should stay in
integration tests or a dedicated test include; package closure is blocked if the
production target crosses 3000 lines without a valid refactor.
...
3. If characterization tests are added or materially changed, record ADR-0021
   ...
   - applicable obligation-to-test binding for parser behavior and authority
     guards without creating new authority.
...
2. Preserve statement order, parsed field order, typed error behavior, stable
   error IDs, and short-circuit behavior.
...
- ADR-0021 coverage closure is recorded when characterization tests are added or
  materially changed;
- behavior identity is proven by focused tests and appropriate parser/API
  evidence;
