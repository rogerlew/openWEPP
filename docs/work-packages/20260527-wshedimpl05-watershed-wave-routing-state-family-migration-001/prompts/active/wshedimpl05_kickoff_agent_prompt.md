# WSHEDIMPL05 Kickoff Agent Prompt
...
Task: execute WSHED05 objective end-to-end for declared scope.
...
- Do not claim complete WS11 hydrology parity closure if unresolved
  `wshcqi/wshirs/wshrun` routine-chain surfaces remain outside this package
  write set.
- Promote WSHED03 WS11 wave-routing state-family vector from ignored to active
  only after runtime publication is production-true.
- Preserve fail-closed typed guard behavior for non-finite/domain-invalid
  wave-routing intermediate computations.
