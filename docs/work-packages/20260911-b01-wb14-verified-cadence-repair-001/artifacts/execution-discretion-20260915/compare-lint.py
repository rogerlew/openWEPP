...
def records(path):
    ...
    for line in (gzip.decompress(path.read_bytes()).decode() if path.suffix=='.gz' else path.read_text()).splitlines():
...
def comparison(package, candidate_label=None):
    ref_log=HERE.parent/'snowfree-recorder-20260915'/f'baseline-{package}-clippy.stdout.gz'
    cand_log=HERE/((candidate_label or f'candidate-{package}-clippy')+'.stdout')
    if not cand_log.exists():
        cand_log=cand_log.with_suffix(cand_log.suffix+'.gz')
    ...
    return {'evidence_class':'Ran: exact structured diagnostic comparison with exact-equal source-line relocation; manual review assigns acceptance',
            ...
            'candidate_log_sha256':hashlib.sha256(gzip.decompress(cand_log.read_bytes()) if cand_log.suffix=='.gz' else cand_log.read_bytes()).hexdigest(),
