...
def main():
    ...
    source_equal = latest['expected_sources']['accepted_operands'] == lineage['accumulated_operands']
    selected_equal = owner['state'] == trial['selected_ending_state']
    # Compare physical inventory explicitly; do not normalize identity bytes.
    def inventory(state):
        return [(ofe['ofe_id'], layer['layer_id'], bits(layer['temperature_k']), bits(layer['enthalpy_hi_j_m2_ofe_ground']), layer['enthalpy_carry']) for ofe in state['ofes'] for layer in ofe['ordered_layers']]
    inventory_equal = inventory(owner['state']) == inventory(trial['selected_ending_state'])
    transaction_markers = {'selected':trial['selected_ending_state']['last_accepted_transaction_id'], 'installed':owner['state']['last_accepted_transaction_id'], 'original_prepared':lineage['original_prepared_owner']['transaction_id']}
    ...
    result = {'input':str(args.stdout), 'input_sha256':hashlib.sha256(raw).hexdigest(),
              ...
              'physical_inventory_equal':inventory_equal, 'distinct_transaction_markers':transaction_markers,
              ...
              'pass':all((suffix_equal,source_equal,inventory_equal,receipt_owner_equal,support_equal,first_physical)),
              'limits':['Full selected and installed state identities differ; physical inventory comparison does not normalize or authorize transaction/replay identity.',
                        'Only selected child physical primitives are available; prior covered-prefix donor outputs are absent.',
