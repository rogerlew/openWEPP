...
fn covered_jacobian_probe_residuals(
    ...
    let result =
        covered_jacobian_probe_residuals_observed(base, probe, column_index, &mut observation);
...
fn solve_covered_column_observed(
    ...
    for iteration in 0..=MAX_NEWTON_ITERATIONS {
        ...
        if let Some((trial, steps, exponent)) = accepted {
            ...
            #[cfg(any(test, feature = "test-support"))]
            {
                // These are opt-in post-refusal observations.  They are not
                // canonical line-search candidates and never affect `failure`.
                if crate::lse_first_trial_diagnosis::enabled() {
                    for exponent in MAX_BACKTRACKING_HALVINGS + 1..=MAX_BACKTRACKING_HALVINGS + 3 {
                        let factor = 0.5_f64.powf(f64::from(exponent));
                        let trial: Vec<f64> = failure
                            .failed_solution
                            .iter()
                            .zip(delta.iter())
                            .map(|(value, change)| value + factor * change)
                            .collect();
                        let domain_valid = covered_trial_is_valid(
                            &trial,
                            beginning.occupancies.len(),
                            ground_uses_liquid_vapor_phase_domain,
                        );
                        let frozen_evaluation = domain_valid
                            .then(|| {
                                evaluate_covered_column_validated(
                                    &validated_evaluation_inputs,
                                    &trial,
                                    Some(&jacobian_base.frozen),
                                    None,
                                )
                            })
                            .transpose();
                        let unfrozen_evaluation = (exponent == MAX_BACKTRACKING_HALVINGS + 1
                            && domain_valid)
                            .then(|| {
                                evaluate_covered_column_validated(
                                    &validated_evaluation_inputs,
                            ...
                            .transpose();
                        crate::lse_first_trial_diagnosis::record(
                            "diagnostic_directional_probe",
                            || {
                                serde_json::json!({
                                    "frozen_canonical_evaluator_calls": u64::from(domain_valid),
                                    "unfrozen_canonical_evaluator_calls": u64::from(
                                        exponent == MAX_BACKTRACKING_HALVINGS + 1 && domain_valid),
                                    "iteration": iteration, "exponent": exponent, "factor": factor,
                                    "coordinates": trial, "domain_valid": domain_valid,
                                    "base_normalized_residuals": &detail.normalized_residuals,
                                    "applied_direction": delta.iter().map(|value| factor * value).collect::<Vec<_>>(),
                                    "frozen_evaluation": frozen_evaluation.as_ref().ok(),
                                    "unfrozen_evaluation": unfrozen_evaluation.as_ref().ok(),
                                })
                            },
                        );
                    }
