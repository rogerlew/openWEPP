...
- in ordinary non-test builds with the physical observer disabled, the added constructor pin is `None` and does not control physical computation; a `cfg(test)` recorder audit may retain a real pin even while its tested recorder mode is Disabled, and the current controls establish equivalent model behavior in either case;
...
- LP-C2/LP-C4 and WB14-RC1/RQ2 remain accepted only on their previously reviewed local frozen cuts; this review does not reopen them. RQ1/A-001, original-input regression, scientific/conservation/restart qualification, and cadence remain separate HOLD items outside this bounded feature repair.
