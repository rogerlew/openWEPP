    ...
    try:
        ...
        with source.open('rb') as raw, (destination/'events.provisional.jsonl').open('w') as output:
            ...
            for event, value in ijson.basic_parse(reader, use_float=False):
                if depth == 0:
                    if event != 'start_array' or root_done:
                        raise ValueError('expected one top-level array')
                    depth = 1
                    continue
                if depth == 1:
                    if event == 'end_array':
                    ...
                    top_key = None
                if selected:
                    if depth == 2 and event == 'map_key':
                        top_key = value
                    elif depth == 2 and event not in ('end_map', 'map_key'):
                        if top_key == 'kind':
                ...
                if event in ('start_map','start_array'):
                    ...
                    if depth == 1:
                        ...
                        rows += 1
            if not root_done or depth != 0:
                raise ValueError('incomplete root')
