...
src="$root/recovery-inputs"
test -d "$src" && test ! -e "$root/working145" && test ! -e "$root/baseline-red" && test ! -e "$root/candidate"
mv "$root/logs/reconstruct.log" "$root/logs/reconstruct-first-attempt.log"
log="$root/logs/reconstruct-continue.log"
exec > >(tee "$log") 2>&1
printf 'continued_utc=%s\n' "$(date -u +%FT%TZ)"
python3 - "$src" "$prior/PBC-final-build016-source.json" <<'PY'
import hashlib, json, sys
from pathlib import Path
root, manifest = Path(sys.argv[1]), json.loads(Path(sys.argv[2]).read_text())
files={item['path']: item['sha256'] for item in manifest['changed_files']}; mismatches=[]
for name, expected in files.items():
    p=root/name; actual=hashlib.sha256(p.read_bytes()).hexdigest() if p.is_file() else None
...
PY
patch -d "$src" -p1 < "$b01/correction145.patch"
...
python3 - "$src" "$root/build145-execution-identity.json" <<'PY'
import hashlib, json, sys
from pathlib import Path
