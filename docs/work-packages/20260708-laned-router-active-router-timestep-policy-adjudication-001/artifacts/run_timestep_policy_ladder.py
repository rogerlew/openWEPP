#!/usr/bin/env python3
"""Run the mn_corn_h4 day-792 active-router timestep-policy ladder."""
...
RUN_ROOT = ARTIFACTS / "timestep-policy-runs"
SUMMARY_JSON = ARTIFACTS / "timestep-policy-summary.json"
SUMMARY_MD = ARTIFACTS / "timestep-policy-summary.md"

RUNGS: dict[str, dict[str, float]] = {
    "dx1p25_dt300": {"target_dx_m": 1.25, "max_dt_s": 300.0},
    "dx1p25_dt150": {"target_dx_m": 1.25, "max_dt_s": 150.0},
    "dx1p25_dt75": {"target_dx_m": 1.25, "max_dt_s": 75.0},
    "dx0p625_dt300": {"target_dx_m": 0.625, "max_dt_s": 300.0},
    "dx0p625_dt150": {"target_dx_m": 0.625, "max_dt_s": 150.0},
    "dx0p625_dt75": {"target_dx_m": 0.625, "max_dt_s": 75.0},
}
...
def active_env(target_dx_m: float, max_dt_s: float) -> dict[str, str]:
    env = os.environ.copy()
...
def write_markdown(summary: dict[str, Any]) -> None:
    ...
    for rec in summary["runs"]:
        ...
        lines.append(
            "| {member} | {rung} | {dx} | {max_dt} | {status} | {failure_phase} | {failure_day} | {failure_ratio} | {wall} | {user} | {steps} | {trace_rows} | {outlet} | {storage} | {tail} | {tdet} |".format(
...
def main() -> None:
    ...
    for member in members:
        for rung, controls in rung_items:
            runs.append(
                run_member_rung(
                    member,
                    rung,
                    controls["target_dx_m"],
                    controls["max_dt_s"],
                )
            )
