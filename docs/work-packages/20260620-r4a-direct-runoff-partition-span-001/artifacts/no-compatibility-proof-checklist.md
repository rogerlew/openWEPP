...
| Explicit opt-in direct counters are positive | PASS | `cargo test -p openwepp-runner r2a_explicit_direct_skeleton_selection_runs_before_compatibility_outputs -- --nocapture`; R4A assertions require `phase_span_runs >= 4`, phase entries include R3A+R3B+R3C+R4A, and direct compute/state/downstream/shadow counts are `>= 4`. |
| Direct span compatibility-edge count remains zero | PASS | Focused R4A test asserts `report.compatibility_edge_invocation_count == 0`; aggregate direct skeleton test asserts report/audit compatibility-edge counts are zero inside direct runtime. |
