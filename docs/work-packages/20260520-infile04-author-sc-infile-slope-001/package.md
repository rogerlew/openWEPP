...
## Objective
Author and disposition `SC-INFILE-SLOPE-001` and its paired openWEPP-owned input
specification for surface `infile-slope-slp` (.slp).

## Why This Package Exists
The parser input-surface registry marks `infile-slope-slp` as `planned`.
...
- Update parser input-surface registry disposition for `infile-slope-slp`.
...
- Draft/refresh `docs/specifications/wepp-input-files/specs/slope-file.spec.md`.
- Draft/refresh `SC-INFILE-SLOPE-001` with required section set, propagation map,
...
## Exit Criteria
- `docs/specifications/wepp-input-files/specs/slope-file.spec.md` exists and covers this surface.
- `SC-INFILE-SLOPE-001` includes all required parser-contract sections.
...
- Dual verification artifacts are complete with no unresolved high-severity
  findings.
- Registry entry for `infile-slope-slp` is updated consistently with disposition.
