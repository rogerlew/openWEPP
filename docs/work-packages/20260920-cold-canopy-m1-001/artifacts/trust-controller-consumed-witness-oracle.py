...
def lambda_step(lam: float) -> list[float]:
    ...
    gain = sigma / denom
    # The canonical V-product starts every output accumulation at +0.0.
    # Preserve that sign for the exactly-zero coordinates rather than exposing
    # the intermediate coefficient's -0.0.
    step = [0.0] * N
    step[0] = -(gain * (-2.0**-25))
    step[3] = -(gain * (-0.5))
    return step
