...
struct Fixture {
    ...
    accounting_observations: Vec<crate::M1TrustRegionWorkObservation>,
...
impl Fixture {
    ...
    fn new(kind: FixtureKind) -> Self {
        ...
        Self {
            ...
            accounting_observations: Vec::new(),
            hydraulics: HydraulicLifecycleFixture {
...
impl Evaluator for Fixture {
    ...
    fn observe_controller_accounting(&mut self, work: crate::M1TrustRegionWorkObservation) {
        self.accounting_observations.push(work);
    }
}
...
fn m1_trust_region_refinement_controller_accounting_observes_all_solves_and_errors() {
    ...
    assert_eq!(
        rank_deficient_work.completed_face_pivots,
        natural_work.completed_face_pivots
    );
    ...
    assert_eq!(exact_root_work.stage1_calls, 1);
    assert_eq!(
        exact_root_work.stage1_calls,
        u32::from(exact_root_run.proposals)
    );
    assert!(exact_root_work.entered_svd_factorizations > 0);
    assert_eq!(
        exact_root_work.entered_svd_factorizations,
        u32::from(exact_root_run.entered_svd_factorizations)
    );
