#!/usr/bin/env python3
"""Nonphysical conformance controls for the private M1 provider authority.

This is a framing KAT and frozen-input control, not an instantiated provider,
parent, or simulation result.
"""
import hashlib
import json
import struct
...
def main():
    ...
    for bad in (receipts[:-1], receipts[1:], receipts[::-1], receipts[:1] + [(99, 0, 60_000_000_000)] + receipts[1:]):
        require(bad != receipts, "negative GSI control distinguishes missing/foreign/reordered")
    print("PASS m1 private-provider metadata controls (nonphysical)")
