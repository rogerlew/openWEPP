...
use openwepp_vegetation::cold_canopy_m1::{
    evaluate_m1_capacity_residual, m1_qsat_external, validate_m1_empty_surface, M1CapacityTrial,
    M1DrySurface, M1EmptySurfaceInput, M1Error, M1ExternalPhase, M1PhaseReservoir,
};
...
#[cfg(any(test, feature = "test-support"))]
const M1_DIAGNOSTIC_TRIAL_CAP: usize = M1_DIAGNOSTIC_BASE_CAP
    * (crate::numerics::MAX_BACKTRACKING_HALVINGS as usize + 1);
#[cfg(any(test, feature = "test-support"))]
...
#[cfg(any(test, feature = "test-support"))]
fn m1_diagnostic_begin(
    input: &M1CoupledColumnInput,
    generation: u64,
    seed_coordinates: [f64; 21],
) {
...
fn m1_diagnostic_begin_record(
    ...
    let capacity_ok = bases.try_reserve_exact(M1_DIAGNOSTIC_BASE_CAP).is_ok()
        && linear_solves.try_reserve_exact(M1_DIAGNOSTIC_LINEAR_CAP).is_ok()
...
fn m1_diagnostic_terminal(result: &Result<M1CoupledEvaluation, M1CoupledError>) {
    M1_DIAGNOSTIC_OBSERVATION.with(|observation| {
            ...
            && let Some(observation) = observation.as_mut()
        {
            ...
            observation.terminal_error_code = result.as_ref().err().map(|error| error.code().to_owned());
        }
    });
}
        ...
        if observation.bases.len() == M1_DIAGNOSTIC_BASE_CAP {
            ...
            observation.bases.push(M1DiagnosticNewtonBase {
                ...
                row_identities: (0..base.residuals.len())
                    ...
                    .collect(),
                row_units: (0..base.residuals.len()).map(m1_diagnostic_row_unit).collect(),
    ...
    m1_diagnostic_mutate(|observation| {
        if observation.trials.len() == M1_DIAGNOSTIC_TRIAL_CAP {
            ...
            observation.trials.push(M1DiagnosticTrial {
                iteration, exponent, factor, coordinates: candidate.coordinates,
...
impl Drop for M1DiagnosticObservationGuard {
    fn drop(&mut self) {
        M1_DIAGNOSTIC_PENDING_INPUT.with(|pending| {
            if let Ok(mut pending) = pending.try_borrow_mut()
                && pending.as_ref().is_some_and(|pending| pending.generation == self.generation)
            {
                *pending = None;
            }
        });
    ...
    if jacobian.is_none() {
        M1_ACCEPTED_CAPTURE.with(|capture| {
            if capture.borrow().is_none() {
                *capture.borrow_mut() = Some((input.clone(), trial.clone(), result.clone()));
            }
        });
    }
...
fn solve_selected_drainage_system(
    ...
    let (direction, pivot, norm, eliminated, dimension) =
        solve_selected_drainage_linear(
            matrix,
            rhs,
            raw,
            &rows,
    ...
    match poison {
        ...
        Some(poison) => match poison {
            ...
            M1AcceptedMaterializationPoison::OverCapacityOneUlp => {
                let liquid = M1PhaseReservoir::from_mass_enthalpy(
                    trial.coordinates[3],
                    trial.coordinates[4],
                )?
                .liquid_mass_kg_m2();
...
fn solve_m1_coupled_column_inner(
    ...
    let diagnostic_generation = M1_DIAGNOSTIC_OBSERVATION.with(|observation| {
        observation
            .try_borrow()
            .ok()
            .and_then(|observation| observation.as_ref().map(|observation| observation.generation))
    });
    #[cfg(any(test, feature = "test-support"))]
    ...
    for iteration in 0..=crate::numerics::MAX_NEWTON_ITERATIONS {
        ...
        for exponent in 0..=crate::numerics::MAX_BACKTRACKING_HALVINGS {
            ...
            if !fd_trial_is_admissible(&candidate) {
                #[cfg(any(test, feature = "test-support"))]
                if diagnostic_active {
                    m1_diagnostic_trial(iteration, exponent, factor, &candidate, None, input,
                        format!("pre_evaluator_domain:{:?}", fd_trial_rejection_reason(&candidate)));
                }
            ...
            if candidate_residuals.iter().any(|x| !x.is_finite()) {
                #[cfg(any(test, feature = "test-support"))]
                if diagnostic_active { m1_diagnostic_trial(iteration, exponent, factor, &candidate, Some(&candidate_core), input, "nonfinite_residual"); }
                continue;
            }
            ...
            let potentials = match hydraulic_potentials(input, &candidate_core) {
                ...
                Err(error) => {
                    #[cfg(any(test, feature = "test-support"))]
                    if diagnostic_active {
                        m1_diagnostic_trial(iteration, exponent, factor, &candidate, Some(&candidate_core), input,
            ...
            let Some(step_norms) = m1_candidate_step_norms(
                ...
                #[cfg(any(test, feature = "test-support"))]
                if diagnostic_active { m1_diagnostic_trial(iteration, exponent, factor, &candidate, Some(&candidate_core), input, "step_ineligible"); }
                continue;
            };
            M1_RUNTIME_ACTIVITY.with(|activity| {
            ...
            if no_update_witness {
                #[cfg(any(test, feature = "test-support"))]
                if diagnostic_active { m1_diagnostic_trial(iteration, exponent, factor, &candidate, Some(&candidate_core), input, "no_update_witness"); }
                if exponent > 0 {
                    M1_RUNTIME_ACTIVITY.with(|activity| {
            ...
            if candidate_norm < norm {
                #[cfg(any(test, feature = "test-support"))]
                if diagnostic_active { m1_diagnostic_trial(iteration, exponent, factor, &candidate, Some(&candidate_core), input, "accepted_strict_decrease"); }
                M1_RUNTIME_ACTIVITY.with(|activity| {
                    let mut activity = activity.borrow_mut();
            ...
            #[cfg(any(test, feature = "test-support"))]
            if diagnostic_active { m1_diagnostic_trial(iteration, exponent, factor, &candidate, Some(&candidate_core), input, "evaluated_non_decrease"); }
        }
