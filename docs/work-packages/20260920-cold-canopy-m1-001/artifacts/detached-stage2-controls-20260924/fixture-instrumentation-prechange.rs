...
use super::m1_trust_region_controller_interface::{
    M1TrustRegionControllerEvaluation as Evaluation, M1TrustRegionControllerEvaluator as Evaluator,
    M1TrustRegionStage1FaceEvent, M1_TRUST_REGION_CONTROLLER_DIMENSION as N,
};
...
impl Evaluator for Fixture {
    ...
    fn raw_jacobian(&mut self, e: &Evaluation<Payload>) -> Result<[[f64; N]; N], Self::Error> {
    ...
    fn select_phase(&mut self, _: &Evaluation<Payload>, _: [f64; N]) -> Result<(), Self::Error> {
        Ok(())
    }
    fn selected_side_probe(&mut self, _: &(), _: [f64; N]) -> Result<(), Self::Error> {
        Ok(())
    }
    fn materialize(&mut self, _: &Evaluation<Payload>) -> Result<Payload, Self::Error> {
        Ok(Payload)
    }
}
...
fn m1_stage2_raw_jacobian_and_frozen_merit_are_actual() {
    ...
    assert_eq!(
        f.raw_jacobian(&e).unwrap()[0][0].to_bits(),
        1.0f64.to_bits()
    );
    let t = super::m1_trust_region_controller_transition_for_test(&mut f).unwrap();
...
fn m1_stage2_constrained_stationarity_is_nonterminal_rejection() {
    ...
    close(t.kkt_gradient[5], 2.0);
    close(t.kkt_normal_gradient[5], 2.0);
    close(t.predicted_reduction, 0.0);
    assert_eq!(t.next_radius.to_bits(), 0.25f64.to_bits());
    assert!(!t.installed);
    assert_eq!(t.entered_materializations, 0);
    assert_eq!(
        t.stage1_face_events,
        vec![
