...
# Individual future GSI receipt bytes are not stored in caller snapshot;
# the authenticated parent receipt below binds all thirty derived members.
...
for base, lu in zip(observer['bases'], observer['linear_solves']):
    ...
    linear.append({'iteration': it, 'full_dimension': len(full), 'reduced_dimension': len(reduced), 'max_absolute_A_d_minus_rhs': max(map(abs, residuals)), 'relative_linear_residual': max(map(abs, residuals)) / scale, 'scaling': 'max(abs(A*d-b))/max_i(abs(b_i)+sum_j(abs(A_ij*d_j)))'})
...
reported_residuals = json.loads(re.search(r'normalized_residuals: (\[[^]]+\])', j['result_error']).group(1))
check('typed_error_is_terminal_current_base', list(map(bits, reported_residuals)) == bases[15]['normalized_residual_bits'] and 'kind: BacktrackingLimit, iterations: 15,' in j['result_error'])
check('reported_backtracking_count_is_previous_accepted_searches', sum(t['exponent'] for t in observer['trials'] if t['reason'] == 'accepted_strict_decrease') == 116)
