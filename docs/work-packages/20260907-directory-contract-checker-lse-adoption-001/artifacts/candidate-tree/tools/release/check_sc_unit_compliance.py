...
def lint_contract(
    ...
    try:
        ...
        return [Finding("SCUNIT-E-013", path_text, 1, str(error))]
    origins = {}
    if contract.directory:
        # Merge all declared normative unit/alias tables, retaining row locations.
        # Do not take only the first chapter or first distributed table.
        lines = []
        for fragment, headers in [
            ("Variables and Units", ["Symbol", "Units"]),
            ("Symbol Alias Map", ["Canonical symbol", "Boundary/API name", "Scope", "Units check"]),
        ]:
            selected = [(doc, table) for doc in contract.documents.values()
                        if doc.kind == "normative" for table in doc.tables
                        if fragment.lower() in table.heading.lower()]
            if not selected:
                continue
            lines.extend([f"## {fragment}", "| " + " | ".join(headers) + " |",
                          "|" + "---|" * len(headers)])
            for doc, table in selected:
                if not set(headers).issubset(table.headers):
                    return [Finding("SCUNIT-E-013", str(doc.path), table.line,
                                    f"{fragment} table lacks required columns")]
                for line_number, row in table.rows:
                    values = [row[table.headers.index(h)] for h in headers]
                    lines.append("| " + " | ".join(values) + " |")
                    origins[len(lines)] = (str(doc.path), line_number)
            lines.append("")
    else:
        lines = contract.documents[contract.entry].text.splitlines()
    ...
    return [Finding(f.code, origins.get(f.line, (f.path, f.line))[0],
                    origins.get(f.line, (f.path, f.line))[1], f.message) for f in findings]
