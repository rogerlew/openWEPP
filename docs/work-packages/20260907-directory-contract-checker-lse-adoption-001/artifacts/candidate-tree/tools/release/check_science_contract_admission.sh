...
from sc_contract_directory import ContractError, ContractSet
...
for entry_path in sorted(active_paths):
    try:
        contract = ContractSet(Path(entry_path))
    except ContractError as error:
        raise SystemExit(f"ERROR: {error}") from error
    if contract.directory:
        directory_owners[str(Path(entry_path).with_suffix("")) + "/"] = Path(entry_path).stem
        directory_members.update(str(path.relative_to(Path.cwd())) for path in contract.documents)

for changed_path in changed:
    if changed_path.startswith("docs/specifications/science-contracts/contracts/SC-"):
        contract_id = next(
            (owner for prefix, owner in directory_owners.items() if changed_path.startswith(prefix)),
            Path(changed_path).stem,
        )
...
if worktree:
    ...
    authority_paths = set(science_paths) | active_paths | directory_members | {
