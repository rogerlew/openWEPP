...
`crates/openwepp-watershed-orchestrator/src/lib_mod/kernel/routing/01_ws22_ws23_ws26_detachment.rs`
with CRAP above `30` to `<= 30`, or record an ADR-0021-style disposition when
a row is not safely reducible as behavior-preserving CQR. Preserve watershed
channel detachment, transport-capacity, deposition, detachment-capacity,
fall-velocity, shape, validation, and fail-closed semantics.
...
This package is science-sensitive watershed routing CQR. It must not change
published process authority, channel sediment formulas, thresholds, units,
floating-point operation grouping where material, validation guards, or
downstream output meaning. If a row cannot be closed without changing those
semantics, hold rather than changing authority.
...
assigned a bounded implementation fix in the target module,
`crates/openwepp-watershed-orchestrator/src/lib.rs`, focused integration tests,
or package-local artifacts.
...
- characterization tests required to make watershed detachment helpers safe to
  refactor;
...
- focused watershed-orchestrator unit or integration tests proving branch,
  validation, numeric, and fail-closed identity;
- package artifacts and prompt material.

Out of scope:

- formula, coefficient, threshold, unit, shape-flag, or finite-guard changes;
- public watershed output, serialization, CLI, or fixture semantic changes;
- contract amendments or new process authority;
- fallback wrappers that mask invalid watershed/channel state;
- broad fixture rewrites or unrelated watershed cleanup;
...
- `crates/openwepp-watershed-orchestrator/src/lib_mod/kernel/routing/01_ws22_ws23_ws26_detachment.rs`
- `crates/openwepp-watershed-orchestrator/src/lib.rs`
- focused integration tests under `tests/integration/**` only if unit tests
  cannot prove the required characterization surface;
...
Line-count note: the target starts at `867` lines, below the 2000-line WARN
...
3. If characterization tests are added or materially changed, record ADR-0021
   ...
   - applicable obligation-to-test binding for channel detachment/routing
     behavior without creating new authority.
...
1. Cover the high-CRAP helpers before decomposition:
   `ws23_detach_case4_iterative_closure`,
   `ws26_dcap_expanding_width_outcome`,
   `ws22_table_column2_to_column1`, and
   `ws23_validate_detach_input`.
2. Extract cohesive branch predicates, result constructors, or validation
   clusters one at a time.
3. Preserve formulas, branch ordering, short-circuit behavior, error variants,
   floating-point accumulation/order where material, and finite guards.
4. Run focused tests after each meaningful extraction.
5. Stop editing when every owned production function is `<= 30` or explicitly
   dispositioned.
6. Record the implementation in `artifacts/implementation.md`.
...
5. Run and record:
   ...
   - focused watershed-orchestrator tests
   - `cargo fmt --check`
...
- ADR-0021 coverage closure is recorded when characterization tests are added or
  materially changed;
- numeric/output identity is proven by focused tests and appropriate
  watershed-routing evidence;
