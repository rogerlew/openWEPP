# CQR35 - Lateral Drainage Complexity Closure
...
Close the current CQR35 target in
`crates/openwepp-hillslope-orchestrator/src/hydrology/kernel_phases_mod/hydrology_phase_lateral_drainage.rs`,
originally tracked as rank 29 with snapshot CRAP `239`, CC `64`, and
coverage `65%`, so the live target function and any newly extracted helpers
have CRAP `<= 30`.

## Rationale

The lateral-drainage phase owns WB19 lateral transfer, drainage, hourly tail,
and related WB12/WB13 hydrology handoff helpers. This is kernel-affecting code.
The CQR35 package must reduce or prove closure of complexity without changing
science-contract behavior, public API, runtime symbols, units, formulas, typed
guards, lane ordering, publication behavior, or writeback behavior.
...
- No science-contract threshold, unit, alias, symbol, formula, float expression
  order, typed guard, lane order, substep behavior, WB12/WB13 handoff,
  writeback, parser, output, or publication behavior changes.
...
hydrology helpers participate in state mutation, runoff, drainage, saturated
storage, and publication surfaces. The acceptance gate is strict behavior
preservation:
...
- preserve all formulas and float expression order unless no production edit is
  performed;
- preserve WB19-to-WB12/WB13 handoff order, hourly tail behavior, and realized
  lateral/drainage publication semantics;
...
- Preserve formula operand order, writeback order, typed errors, symbols,
  aliases, units, lane behavior, and science-contract behavior.
...
- Run the required closure gates:
  ...
  4. `cargo deny check`
  5. `markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260616-cqr35-lateral-drainage-complexity-001 --format json`
  6. `git diff --check`
...
- Current CQR35 target function and any newly extracted helpers have CRAP
  `<= 30`.
...
- No public API, runtime symbol, alias, unit, formula, threshold, typed guard,
  lane-order, writeback, parser compatibility, output, publication, or
...
lane-order, writeback, parser, output, publication, or science-contract
