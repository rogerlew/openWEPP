...
NAMES = ['interface','common-details','surface-energy','soil-coupling','water-vapor','nonlinear-solve','terminal-support','litter-phase','soil-custody','surface-custody','map-custody','dependency-replay','qualification','binding-index','history']
...
texts = {name:((ARCHIVE/(name+'.md')).read_text() if name=='binding-index' else compact((ARCHIVE/(name+'.md')).read_text())) for name in NAMES if name!='common-details'}
...
dep('interface','common-details.md#common-details','physical rules, solver correctness or accepted-primitive closure','shared physical symbols, state, algorithm, guards, aliases, units, tolerances and tests','common physical set listed in common-details introduction; source/guard-map/gap sections additionally for those claims')
...
texts['common-details']=texts['common-details'].replace('# Common details\n','# Common details\n\nFor physical rules, solver correctness and accepted-primitive closure read these complete sections: '+', '.join(f'[{a.split("-")[0]}]({"common-details.md#"+a})' for a in COMMON)+'. Read Authority Anchors for source-provenance claims, Invariant Guard Map for enforcement-path review, and Purpose/Scope/Gap Register for historical applicability or promotability adjudication. All universal canonical definitions are in interface.\n\n',1)
...
dep('surface-energy','nonlinear-solve.md#'+ORDER,'physical rule selection or accepted-primitive closure','ordered solve and error precedence; INV108-110')
...
for name in NAMES:
    if name in ['history','common-details','interface']:continue
...
short_sections={
    'nonlinear-solve': {ORDER:'solve','stage-3-identity-anchor-jacobian-amendment':'identity-anchor','covered-leaf-maximum-demand-exact-reuse-amendment':'leaf-reuse'},
    ...
    'surface-custody': {'version-16-lse-surface-enthalpy-exact-carry-amendment':'exact-surface','exact-surface-parent-local-chronology-amendment':'parent-chronology','topology-ranked-v16-exact-surface-owner-amendment':'topology'},
    ...
    'dependency-replay': {'component-temperature-jacobian-dependency-replay-amendment':'replay'},
    ...
    'map-custody': {'validated-in-memory-lse-custody-handoff-amendment':'handoff','snow-free-final-receipt-reseal-amendment':'reseal','covered-nonfinal-physical-only-map-amendment':'pending','carrier-parent-static-and-same-map-validation-once-amendment':'validation','canonical-stage-3-accepted-map-boundary-amendment':'accepted-map'},
}
