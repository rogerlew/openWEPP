...
model tokens, automatic context, quota and complete parent/child workflow totals are
UNOBSERVED. Requested exposure includes repetitions/truncation; recovery is assessed
separately. These are source-byte measurements, not scientific execution results.

The comparison baseline is the original264863-byte whole-LSE requirement, not an
observed baseline agent session. Each scientific task needs an adequate first answer
and actual LSE union<=211890; identity must be smaller than264863. Mandatory external
contracts, governance, tool-source requests and repetitions remain visible separately.
