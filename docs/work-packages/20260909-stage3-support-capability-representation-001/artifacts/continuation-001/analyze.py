...
def analyze(rows):
    ...
    costs = {name: {"exclusive_seconds": distribution([row[name]["ns"] / 1e9 for row in reconstructed]),
             ...
             for name in names}
    target_names = ["PreparedSupportClone", "LiveTransactionLookup", "LiveBindingRemainder",
                    "LiveSnowFreeCanonicalSeal", "LiveCoveredCanonicalSeal",
                    "LivePreparedSupportDrop"]
    target = [sum(row[name]["share"] for name in target_names) for row in reconstructed]
    target_seconds = [sum(row[name]["ns"] for name in target_names) / 1e9
    ...
    return {
        ...
        "live_clone_bind_seal_drop": {
            "buckets": target_names,
            "exclusive_seconds": distribution(target_seconds),
            ...
            "limitations": "Transaction lookup, changed-transaction mutation, complete canonical commitments, and destruction remain necessary; perfect elimination is deliberately optimistic."},
        "carrier_physical_value": costs["CarrierPhysicalValue"],
