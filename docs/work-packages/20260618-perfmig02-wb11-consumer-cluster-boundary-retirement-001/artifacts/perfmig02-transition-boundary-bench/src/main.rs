...
use openwepp_kernel_contract::{
    ...
    IndexedWritebackLogicalMaterialization, IndexedWritebackLogicalSurfaces,
    IndexedWritebackSurface, SymbolId, SymbolRegistry, WritebackDecisionOutcome,
    apply_indexed_kernel_writeback,
    apply_indexed_kernel_writeback_with_logical_materialization, evaluate_indexed_kernel_writeback,
};
use openwepp_sim_contract::status::SimulationPhase;

const STATE_UPDATE_COUNT: usize = 543;
const FLUX_UPDATE_COUNT: usize = 8;
const DEFAULT_REPETITIONS: usize = 50_000;
const H2637_OFE_DAYS: f64 = 235_961.0;

const PERFMIG02_RETIRED_STATE_SYMBOLS: &[&str] = &[
    "wb12_infiltration",
...
const PERFMIG02_RETIRED_FLUX_SYMBOLS: &[&str] = &["wb12_runoff_carryover"];
...
fn main() {
    ...
    let mut all_state_surface = fixture.state_surface.clone();
    let mut all_flux_surface = fixture.flux_surface.clone();
    let mut all_indexed_surface = fixture.indexed_surface.clone();
    report_metric("apply_indexed_payload_materialize_all", repetitions, || {
        let apply_result = apply_indexed_kernel_writeback(
            SimulationPhase::HillslopeKernel,
            &decision,
            &fixture.payload,
            &mut all_indexed_surface,
            &fixture.registry,
            &mut all_state_surface,
            &mut all_flux_surface,
        )
        .expect("indexed writeback should apply");
        black_box(apply_result);
    });

    let mut skip_state_surface = fixture.state_surface.clone();
    let mut skip_flux_surface = fixture.flux_surface.clone();
    let mut skip_indexed_surface = fixture.indexed_surface.clone();
    report_metric(
        "apply_indexed_payload_perfmig02_skip_6",
        repetitions,
        || {
            let apply_result = apply_indexed_kernel_writeback_with_logical_materialization(
                SimulationPhase::HillslopeKernel,
                &decision,
                &fixture.payload,
                &mut skip_indexed_surface,
                &fixture.registry,
                fixture.materialization_policy(),
                IndexedWritebackLogicalSurfaces::new(
                    &mut skip_state_surface,
                    &mut skip_flux_surface,
                ),
            )
            .expect("indexed writeback should apply");
            black_box(apply_result);
    ...
    report_metric(
        "evaluate_plus_apply_indexed_payload_perfmig02_skip_6",
        repetitions,
        || {
            let decision = evaluate_indexed_kernel_writeback(
                SimulationPhase::HillslopeKernel,
                &fixture.payload,
            )
            .expect("indexed writeback decision should construct");
...
struct Fixture {
    ...
    skip_state_ids: Vec<SymbolId>,
    skip_flux_ids: Vec<SymbolId>,
}

impl Fixture {
    fn new() -> Self {
        let mut state_symbols = PERFMIG02_RETIRED_STATE_SYMBOLS
            .iter()
            .map(|symbol| BoundarySymbol::from(*symbol))
            .collect::<Vec<_>>();
        state_symbols.extend(
            (state_symbols.len()..STATE_UPDATE_COUNT)
                .map(|index| BoundarySymbol::from(format!("state_{index:04}"))),
        );

        let mut flux_symbols = PERFMIG02_RETIRED_FLUX_SYMBOLS
            .iter()
            .map(|symbol| BoundarySymbol::from(*symbol))
            .collect::<Vec<_>>();
        flux_symbols.extend(
            (flux_symbols.len()..FLUX_UPDATE_COUNT)
        ...
        let state_surface = state_symbols
            .iter()
            .cloned()
            .map(|symbol| (symbol, BoundaryValue::scalar(-1.0)))
        ...
        let skip_state_ids = resolve_ids(&registry, PERFMIG02_RETIRED_STATE_SYMBOLS);
        let skip_flux_ids = resolve_ids(&registry, PERFMIG02_RETIRED_FLUX_SYMBOLS);
    ...
    fn materialization_policy(&self) -> IndexedWritebackLogicalMaterialization<'_> {
        IndexedWritebackLogicalMaterialization::skip_ids(&self.skip_state_ids, &self.skip_flux_ids)
    }
}

fn resolve_ids(registry: &SymbolRegistry, symbols: &[&str]) -> Vec<SymbolId> {
    let mut ids = symbols
        .iter()
        .map(|symbol| {
            registry
                .id_of(&BoundarySymbol::from(*symbol))
                .expect("PERFMIG02 retired symbol should be registered")
        })
        .collect::<Vec<_>>();
    ids.sort_unstable();
    ids
