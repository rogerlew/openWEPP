...
| `water_beginning_kg_m2` | kg m-2 OFE ground | compatibility template ignored in V10 | exact staged hydrology layer amount | interval-live exact amount/basis | hydrology snapshot/current transaction/OFE/lane key | differing-OFE/value/template poison | wrong demand/custody | root receipt uses the qualified live source; bare-ground LSE reads the tile-qualified live fact directly |
