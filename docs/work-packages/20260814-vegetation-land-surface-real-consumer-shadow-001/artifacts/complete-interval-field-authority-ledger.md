...
| `ground_albedo_vis` | 1 | caller template | per-tile `LSEcfg.tiles.ground_albedo_vis` | static, tile-local identity; scalar global form is inadequate | LSE config digest; tile order | heterogeneous/caller mutation | cross-tile optics alias | remove scalar from closure projection |
| `ground_albedo_nir` | 1 | caller template | per-tile `LSEcfg.tiles.ground_albedo_nir` | static, tile-local identity; scalar global form is inadequate | LSE config digest; tile order | heterogeneous/caller mutation | cross-tile optics alias | project per tile internally |
...
| `longwave_up_w_m2` | W m-2 | caller template | coupled LSE ground state/solver | V8 does not admit scalar forcing; solved per tile | LSE state/config digest; tile order | template mutation must be irrelevant | duplicate ground boundary | remove from closure path |
...
| `soil_layers` | vector | partial live overwrite | Hstate/Tstate/subsurface/V10cfg plus missing hydraulic owner | interval-live, per exact OFE/layer | all state/config digests; configured rooted-layer order | nested poisons | stale/caller root physics | **blocked by missing owner** |
