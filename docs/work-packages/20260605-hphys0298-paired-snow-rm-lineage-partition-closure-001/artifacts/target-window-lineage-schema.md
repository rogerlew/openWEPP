...
Evidence mode: static + retrospective correction

Static:

- Retrospective correction: the HPHYS0298 schema below is historical because it
  required `snow_hourly_snowfall_water_equiv_sum_m` for the openWEPP side of
  canonical `hrsnow`. That field is water equivalent, not snowfall depth.
  HPHYS0299 supersedes this schema for `hrsnow` parity with corrected
  `snow_hourly_snowfall_depth_sum_m` evidence.
...
- Historical openWEPP trace rows were post-WB13 rows from the opt-in JSONL
  trace with required fields `snow_hourly_melt_raw_m`,
  `snow_hourly_rain_sum_m`, `snow_hourly_snowfall_water_equiv_sum_m`,
  `snow_routed_melt_m`, `snow_post_winter_rain_m`, `wb13_rm_mm`, and
  `wb13_q_mm`. This exact schema is now rejected by the package-local unit
  guard for `hrsnow`; corrected continuation must use
  `snow_hourly_snowfall_depth_sum_m` for canonical `hrsnow` parity.
