...
| `area_m2` | `m^2` | generated mode: sum of contributing hillslope source slope geometry `fwidth * slplen`, converted from English square feet when declared by the source runfile; existing-pass mode: validated manifest `/wb13_publication/publication_area_m2` when no source runfile is present | committed `pN.source.run` plus parsed `inputs.slope` files, or the pass manifest after `area_policy = sum-ofe-geometry-area` validation | normalization basis when every contributing hillslope has finite positive area; otherwise null | Reject fake unit area, fixture count, number of hillslopes, output row count, or generated output path as area. Reject dropping manifest `publication_area_m2` when a valid existing-pass manifest supplies it. |
