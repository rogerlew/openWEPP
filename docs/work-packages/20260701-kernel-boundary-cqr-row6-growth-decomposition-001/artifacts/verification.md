...
```text
cargo test -p openwepp-hillslope-orchestrator --lib cqr_row6 -- --nocapture
```
...
- Full workspace LCOV tests passed and wrote `/tmp/openwepp-row6-after.lcov`.
- Full workspace CRAP JSON written to
  `/tmp/openwepp-crap-row6-after.json`.
- Row #6 owned functions above CRAP 30: `0`.

See `crap-after.md` for per-function after values from the row #6 before-list.
...
- `cargo nextest run --workspace --profile full`: `1257` tests run, `1257`
...
```text
markdown-doc lint --path docs/work-packages/20260701-kernel-boundary-cqr-row6-growth-decomposition-001 --path docs/work-packages/README.md
markdown-doc validate --path docs/work-packages/20260701-kernel-boundary-cqr-row6-growth-decomposition-001 --path docs/work-packages/README.md
```
...
/usr/bin/time -v target/release/openwepp-cli-hill \
  ...
  --output-dir /tmp/kernel-boundary-cqr-row6-h2637/output \
  --manifest-path /tmp/kernel-boundary-cqr-row6-h2637/output/manifest.json
```

Results:

- Release build exited 0.
- H2637 exited 0; wall `1:06.19`; max RSS `79828 KiB`.
...
- CLI output files were emitted through the runfile output path
  `/tmp/stage1-seed-authority/after-1b/output`; the explicit row #6 output
