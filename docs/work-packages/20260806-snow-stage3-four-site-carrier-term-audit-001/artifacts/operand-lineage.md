...
| complete latent | hourly/daily `J m^-2` | exported complete-arm turbulent latent flux times exact `3600 s` | vapor mass without latent conversion, CoE `C` |
| surface latent | daily `J m^-2` | exported surface-arm latent term; exact zero under the frozen disabled selector | complete latent, production Stage 3 hourly latent on evolving state |
| precipitation advection | hourly/daily `J m^-2` | exported complete-arm advected flux times exact `3600 s` | rainfall depth, CoE `D` |
| implemented external subset | `J m^-2` | independent sum of the five terms above | complete energy balance, producer total alone, residual, surface arm |
| surface arm | `J m^-2` | independent surface shortwave + surface longwave + surface latent | complete total or complete latent substituted for surface latent |
