...
- RTR-018 focused qualification: `package_admission_selects_exactly_one_independently_valid_authority` passes all one/zero/multiple admitted-candidate branches. Against the exact attempt-6 base and current 41-path diff, the recovery package validates `READY` with zero unauthorized paths while the workflow-qualification package validates `INVALID / UNDECLARED_CHANGED_PATH` with 41 unauthorized paths. The corrected pre-heavy selector therefore has one independently admitted authority and preserves fail-closed zero/multiple behavior.
