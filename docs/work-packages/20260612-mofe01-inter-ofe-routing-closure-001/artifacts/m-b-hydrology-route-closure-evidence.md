...
| `tools/owcmp/owcmp batch h1-h39-semantic ... --output-root /tmp/openwepp_mofe01_mb/owcmp_after_fix --start 1 --end 36` command execution | PASS | `summary.json`: `execution_verdict=PASS`. |
| `tools/owcmp/owcmp batch h1-h39-semantic ... --output-root /tmp/openwepp_mofe01_mb/owcmp_after_fix --start 1 --end 36` semantic comparison | FAIL | `summary.json`: `semantic_verdict=FAIL`, `semantic_pass_count=0/36`, `structural_row_key_failures=350720`, first divergent H1 key `[1,1,2000]`. Focus columns had zero numeric fail counts; the blocker is row-key/publication structure, assigned to M-C. |
