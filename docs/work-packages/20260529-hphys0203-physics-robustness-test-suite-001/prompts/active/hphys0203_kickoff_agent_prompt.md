...
- `tests/integration/**` (robustness/property/regression additions)
- `crates/openwepp-runner/src/hillslope/mod.rs` (only if contract tests require)

Task: execute HPHYS0203 objective end-to-end for declared scope:
build contract-derived robustness tests for targeted hydrology publication
families and validate promotability via process-correctness gates.
