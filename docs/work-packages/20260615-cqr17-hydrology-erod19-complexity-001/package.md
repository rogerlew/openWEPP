# CQR17 - Hydrology EROD19 Complexity Refactor

Status: complete pending package commit/push

Package type: code-quality refactor (behavior-preserving)

## Objective

Decompose the current CQR17 target in
...
originally tracked as rank 11 with snapshot CRAP `466`, CC `37`, and coverage
`32%`, so the live target function and any newly extracted helpers have CRAP
`<= 30`.

## Rationale

The EROD19 hydrology phase is kernel-affecting. CQR17 must reduce local
complexity without changing phase dispatch, typed status behavior, symbols,
units, formulas, float expression order, closure behavior, or science-contract
semantics.
...
- No symbol, alias, unit, formula, float expression order, parser
...
- Run the required closure gates:
  ...
  4. `cargo deny check`
  5. `markdown-doc lint --path docs/work-packages/README.md --path docs/work-packages/20260615-cqr17-hydrology-erod19-complexity-001 --format json`
  6. `git diff --check`
...
- Current CQR17 target function and any newly extracted helpers have CRAP
  `<= 30`.
