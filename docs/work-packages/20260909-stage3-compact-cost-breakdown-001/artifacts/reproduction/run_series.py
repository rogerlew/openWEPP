...
def main():
    ...
    for name in ("binary", "environment", "identity", "out", "cwd"):
    ...
    for ordinal, (kind, pair, posture) in enumerate(schedule):
        ...
        log = args.out / f"{ordinal:02d}-{kind}-{pair}-{posture}.log"
        receipt = {"ordinal": ordinal, "kind": kind, "pair": pair, "posture": posture,
                   "argv": command, "log": str(log.resolve()), "valid": False}
        ...
        try:
            with log.open("xb") as output:
                process = subprocess.run(command, cwd=args.cwd, env=env, stdout=output,
        ...
        if failure is not None:
            raise failure
    write(args.out / "complete.json", {"status": "PASS", "processes": 16, "measured_pairs": 6})
