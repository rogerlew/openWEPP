#!/usr/bin/env python3
"""Materialize the four-member plain-identity cohort for ADR-0037 removal."""
...
RUNS_DIR = ARTIFACTS / "plain-identity-runs"
...
def write_runfile(
    ...
    runfile_name = f"{stem}.{phase}.plain.run.toml"
    output_subdir = f"output-{phase}"
    lines = [
...
def write_phase_runfiles(
    ...
    return {
        "pre_run_file": write_runfile(
            run_dir, run_name, stem, include_pmetpara, wepp_ui, "pre"
        ),
        "post_run_file": write_runfile(
            run_dir, run_name, stem, include_pmetpara, wepp_ui, "post"
        ),
    }
...
def materialize_h2637(manifest: list[dict[str, Any]]) -> None:
    ...
    phase_runfiles = write_phase_runfiles(
        run_dir=dst,
        run_name="h2637-laned-seam-plain-identity",
        stem="p2637",
...
def main() -> None:
    ...
    manifest_path = ARTIFACTS / "plain-identity-materialization.json"
