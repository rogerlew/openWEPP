...
Evidence class: `Executed reproduction-input inventory`
...
- report tables and screenshots for equilibrium, 100-year means, and return
  periods.
...
If exact reconstruction is desired, request one archive per site containing
the full WEPP Windows project/run directory, executable/version receipt,
climate, soil, slope, both management variants, initial-condition and plant
records, all text outputs, graphical-data exports, and return-period inputs.
Ask Bill to identify whether the Hubbard screenshots used `dropfc=0.92` or the
