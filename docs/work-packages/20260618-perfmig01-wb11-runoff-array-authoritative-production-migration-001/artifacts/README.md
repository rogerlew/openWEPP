# Artifacts

Status: scaffolded 2026-06-18 (pending Codex execution).

Required deliverables:

- `perfmig01-migration.md` - what was flipped to dense array authority; the boundary placement.
- `perfmig01-bit-identity.md` - every touched branch validated `to_bits()`-identical; the fixture set.
- `perfmig01-endpoint-timing.md` - H2637 seconds + ratio + RSS vs PERFIDX06; transitional boundary
  broken out; trajectory projection to budget.
- `perfmig01-logical-free-proof.md` - no dual-write / no daily `BTreeMap` seam export / no hot-loop
  logical payload (perf or static evidence).
- `perfmig01_disposition.md` - CONTINUE + next-rung outline / REDIRECT + the named obstacle.

Separate deliverable (committed to the decisions tree, not here):

- `docs/decisions/0023-array-authoritative-hot-path-state.md` - ratified from the PERFARCH02 proposed text.
