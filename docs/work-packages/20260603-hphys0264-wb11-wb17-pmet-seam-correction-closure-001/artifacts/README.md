# Artifacts

Status: completed

Evidence artifacts for HPHYS0264. Each artifact must label evidence as
`Static:` or `Ran:`.

Disposition-grade artifacts:

- `contract-implementation-evidence.md`
- `contract-test-implementation-evidence.md`
- `pre-implementation-contract-gate.md`
- `implementation-test-evidence.md`
- `pmet-seam-diagnosis.md`
- `targeted-h1-h7-h39-pmet-seam-classification.md`
- `full-39-suite-metrics.md`
- `gate-results.md`
- `kernel-profile-compliance-checklist.md`
- `owned-file-manifest.md`
- `review_agent_a.md`
- `review_agent_b.md`
- `verification_agent_a.md`
- `verification_agent_b.md`
- `disposition.md`
- `worker-handoff.md`
