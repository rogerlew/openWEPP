...
|---|---|---|---|
| `CLIM02` | Climate Parser-to-Runtime Seam Adapters | Implement `HS-CLIM-SEAM-001` and `WS-CLIM-SEAM-001` adapters with typed `CLIM-RUNTIME-E-*` taxonomy and immutable runtime climate surfaces. | `SC-INFILE-CLIMATE-001`, ARCH17 seam pattern |
| `CLIM03` | Continuous-Daily Climate Runtime Kernel Port | Port baseline non-breakpoint event normalization and disaggregation semantics (`iclig`, unit conversions, `mxint`, invalid-duration behavior) into openWEPP climate runtime module, with policy gate `datver>=4.0` and `iclig=1` support only. | `CLIM02` |
| `CLIM04` | Breakpoint Runtime Port and Policy Reconciliation | Port breakpoint event semantics (`stmstr`, elapsed-time normalization, interval-intensity build) and implement the ratified breakpoint cardinality target (`1500`) with explicit compat controls. | `CLIM02`, `CLIM03` |
...
| `CLIM08` | Climate Governance Disposition Closeout | Close remaining CLIM HOLD items (`parser/runtime seam`, breakpoint zero-drain ambiguity) and update climate contracts/specs to promotable status. | `CLIM07` |
