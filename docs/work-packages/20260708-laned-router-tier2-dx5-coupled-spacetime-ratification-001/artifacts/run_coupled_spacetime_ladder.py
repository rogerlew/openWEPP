#!/usr/bin/env python3
"""Run the active-plain dx5 coupled space-time ladder."""
...
RUN_ROOT = ARTIFACTS / "coupled-spacetime-runs"
SUMMARY_JSON = ARTIFACTS / "coupled-spacetime-summary.json"
SUMMARY_MD = ARTIFACTS / "coupled-spacetime-summary.md"

RUNGS: dict[str, dict[str, float | None]] = {
    "baseline_fixed10_dt300": {"target_dx_m": None, "max_dt_s": 300.0},
    "dx5_dt300": {"target_dx_m": 5.0, "max_dt_s": 300.0},
    "dx2p5_dt300": {"target_dx_m": 2.5, "max_dt_s": 300.0},
    "dx1p25_dt300": {"target_dx_m": 1.25, "max_dt_s": 300.0},
    "dx5_dt75": {"target_dx_m": 5.0, "max_dt_s": 75.0},
    "dx2p5_dt75": {"target_dx_m": 2.5, "max_dt_s": 75.0},
    "dx1p25_dt75": {"target_dx_m": 1.25, "max_dt_s": 75.0},
}

COMPARISON_PAIRS: list[tuple[str, str, str]] = [
    ("fine_reference_adequacy_dt300", "dx2p5_dt300", "dx1p25_dt300"),
    ("fine_reference_adequacy_dt75", "dx2p5_dt75", "dx1p25_dt75"),
    ("candidate_vs_reference_dt300", "dx5_dt300", "dx2p5_dt300"),
    ("candidate_vs_reference_dt75", "dx5_dt75", "dx2p5_dt75"),
    ("timestep_control_dx5", "dx5_dt300", "dx5_dt75"),
    ("timestep_control_dx2p5", "dx2p5_dt300", "dx2p5_dt75"),
    ("timestep_control_dx1p25", "dx1p25_dt300", "dx1p25_dt75"),
]
...
def build_comparisons(runs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for member_id, rungs in by_member.items():
        for role, candidate_rung, reference_rung in COMPARISON_PAIRS:
            candidate = rungs.get(candidate_rung)
            reference = rungs.get(reference_rung)
            if has_trace(candidate) and has_trace(reference):
                comparison = compare_run_to_reference(candidate, reference)
                comparison["member_id"] = member_id
                comparison["comparison_role"] = role
                ...
                skips.append({
                    ...
                    "reason": "candidate or reference rung failed or lacks trace output",
                })
    comparisons.extend({"comparison_skip": skip} for skip in skips)
    return comparisons


def active_env(target_dx_m: float | None, max_dt_s: float) -> dict[str, str]:
    env = os.environ.copy()
...
def run_member_rung(
    ...
    with log_path.open("w") as fp:
        result = subprocess.run(
            command,
            cwd=Path.cwd(),
            env=active_env(target_dx_m, max_dt_s),
            stdout=fp,
...
def write_markdown(summary: dict[str, Any]) -> None:
    ...
    for rec in summary["runs"]:
        ...
        lines.append(
            "| {member} | {rung} | {status} | {target_dx} | {max_dt} | {failure_phase} | {failure_day} | {failure_ratio} | {wall} | {user} | {steps} | {trace_rows} | {outlet} | {storage} | {tail} | {tdet} |".format(
                member=rec["member_id"],
                rung=rec["rung"],
                status=rec["status"],
                target_dx=rec.get("target_dx_m") or "fixed10",
                max_dt=rec.get("max_dt_s", "n/a"),
    ...
    for comparison in summary.get("comparisons", []):
        if "comparison_skip" in comparison:
            ...
            lines.append(
                "| {member} | {role} | {candidate} | {reference} | n/a | n/a | n/a | n/a | n/a | n/a |".format(
                    member=skip["member_id"],
                    role=f"{skip['comparison_role']}: {skip['reason']}",
                    candidate=skip.get("candidate_rung", "SKIPPED"),
                    reference=skip.get("reference_rung", "SKIPPED"),
                )
            )
            continue
...
def main() -> None:
    ...
    for member in members:
        for rung, config in rung_items:
            runs.append(
                run_member_rung(
                    member,
                    rung,
                    config["target_dx_m"],
                    float(config["max_dt_s"] or 300.0),
                )
            )
