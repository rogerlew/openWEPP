...
## Objective
Author and disposition `SC-INFILE-TCR-001` and its paired openWEPP-owned input
specification for surface `infile-channel-tcr` (tcr.txt).

## Why This Package Exists
The parser input-surface registry marks `infile-channel-tcr` as `planned`.
...
- Update parser input-surface registry disposition for `infile-channel-tcr`.
...
- Draft/refresh `docs/specifications/wepp-input-files/specs/tcr.spec.md`.
- Draft/refresh `SC-INFILE-TCR-001` with required section set, propagation map,
...
## Exit Criteria
- `docs/specifications/wepp-input-files/specs/tcr.spec.md` exists and covers this surface.
- `SC-INFILE-TCR-001` includes all required parser-contract sections.
...
- Dual verification artifacts are complete with no unresolved high-severity
  findings.
- Registry entry for `infile-channel-tcr` is updated consistently with disposition.
