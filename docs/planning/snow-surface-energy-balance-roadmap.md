...
| 21G | [`SNOW-STAGE3-LIQUID-SIGNED-HOUR-TRACE-CLOSURE`](../work-packages/20260803-snow-stage3-liquid-signed-hour-trace-closure-001/package.md) behavior-neutral diagnostic publication — **queued** | Additive schema v4 will expose exact Stage-3 liquid closure plus existing signed-hour forcing/state/cold-content operands through the real JSONL consumer. | Independently close a real Snowbird trace, reject plausible aliases, prove WAT and pre-v4 trace neutrality, and complete critical gates before any physics adjudication. |
