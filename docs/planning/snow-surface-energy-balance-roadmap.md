...
| 21J | [`SNOW-ACCUMULATION-TARGET-FEASIBILITY-AND-INPUT-LOSS-DISCRIMINATION`](../work-packages/20260804-snow-accumulation-target-feasibility-input-loss-discrimination-001/package.md) target/input/loss discrimination — **scaffolded / queued** | Freeze annual all-phase input ceilings plus cold observed-accumulation and guarded dry-interval event operators across the four retained fixtures. | Distinguish input, modeled snowfall, observed-date storage, and modeled pack loss without treating PRCPSA or point SNOTEL data as independent hillslope truth; no correction is authorized. |
