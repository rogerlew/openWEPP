...
| 27 | [`SNOW-STAGE3-TURBULENT-CARRIER-LINEAGE-AND-OPERATOR-RECONCILIATION`](../work-packages/20260806-snow-stage3-turbulent-carrier-lineage-and-operator-reconciliation-001/package.md) — **active / result-blind protocol review** | Starting from byte-identical daily states and hourly forcing, add default-off schema-v6 primitive/state observability and reconcile every same-state versus sequential external term—especially longwave, sensible, and latent—while tracking subsequent state divergence. Bind surface temperature, vapor/dewpoint, wind, stability/exchange coefficients, `z_T/z_q/z_u/z_0`, latent/mass conversion, and sign convention. | Turbulence dominates the current same-state failure but is not yet the proven cause of the cross-operator contradiction. No fitting, persistence, physics correction, or ownership change; advance only with exact consumer lineage and independently reviewed results. |
