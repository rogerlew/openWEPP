...
| 22 | [`ASSURE-06` snow/frost flagship authority refresh](../work-packages/20260805-assure06-snow-frost-flagship-refresh-001/package.md) / `SNOW-SURFACE-EB-05` campaign assurance closeout — **DRAFT / review authority returned** | Refreshed the flagship against 21K-21N, then adopted v128 bounded evaluation-observability authority without changing the 188 retained values. The report remains governed `DRAFT` at generation `221f8e51`. | Public count, active review events, and approval roots are zero. Target and evaluation authority remain distinct from implemented physics, efficacy, noninferiority, default change, and cutover. A separately authorized exact-subject human review entry is required. |
...
| 24 | [`SNOW-STAGE3-EVALUATION-SHADOW-AUTHORITY`](../work-packages/20260806-snow-stage3-evaluation-shadow-authority-001/package.md) — **executed / evaluation authority admitted / runtime unchanged** | Contract v127 preserves one production snow-mass authority while admitting exactly two tagged, default-off, consumer-forbidden evaluation operators: same-state paired carriers and bounded sequential resolved shadows. The DRAFT snow/frost assurance source adopts the amendment at generation `34f2f80e`. | Persistent cross-day state, seasonal chronology, terminal meltout, receiving-surface disposition, runtime implementation, promotion, and cutover remain unauthorized. Solver extraction and behavior-neutral per-term observability are next; CoE remains authoritative. |
| 25 | [`SNOW-STAGE3-SHADOW-SOLVER-EXTRACTION-AND-OBSERVABILITY`](../work-packages/20260806-snow-stage3-shadow-solver-extraction-and-observability-001/package.md) — **executed / bounded evaluation realized / review and verification pass** | Contract v128 and runtime realize the two default-off evaluation operators with complete per-term, fingerprint, support, coverage, mass, cold-content, terminal-energy, and residual evidence through the real internal schema-v5 consumer. The Stage 3 solver is split into `1,233`, `1,797`, and `882`-line modules. | Disabled schema v4, production state and ledgers, legacy public API, and real WAT/HBP/PASS bytes remain exact. CoE stays authoritative. The frozen four-site carrier-term audit is next; persistence, physical seasonal chronology, terminal receipt, land-surface energy, promotion, and cutover remain held. |
...
| 29 | [`SNOW-STAGE3-EVOLVING-STATE-CARRIER-PLAUSIBILITY-RECONCILIATION`](../work-packages/20260807-snow-stage3-evolving-state-carrier-plausibility-reconciliation-001/package.md) — **executed / persistence HOLD / multifactor or inconclusive** | Attempt 004 reconstructs 154 eligible water years with zero bounded-transfer truncation and zero active-state failures. Sensible and latent evolution dominate the separately reduced reported Q−F terms, but this is mechanical attribution, not physical validation. | Wind exposure and physical magnitude/geometry envelopes remain unresolved. Paradise WY2015 retains its historical support failure but is now a non-blocking support-censored diagnostic that cannot carry a physical pass/fail claim. No persistence, correction, fitting, promotion, ownership change, or cutover. |
| 30 | [`SNOW-STAGE3-WIND-SOURCE-CUSTODY-AND-EXPOSURE-AUTHORITY`](../work-packages/20260807-snow-stage3-wind-source-custody-and-exposure-authority-001/package.md) — **complete / provider custody recovered / persistence HOLD** | Raw CLI/Stage 3 custody and PMET-local adjustment are proven; source 10 m and virtual 5 m geometry are separated. Surviving records directly recover byte-identical CLI lineage, retained centroids/GRIDMET flags, and parquet-to-CLI equality. Nearby source only reconstructs request/product/shared-series semantics. Deployed request/server pixel/version/time policy and forcing-to-forest exposure linkage remain absent, so all sites are still `AUTHORITY_MISSING`. | Independent neutral bounds reproduce 10.077% friction-velocity and 21.170% product sensitivity, insufficient alone for the larger magnitude concern and not correction authority. No canopy operator, attenuation, persistence, or cutover is licensed. |
| 31 | [`SNOW-STAGE3-PARADISE-WY2015-SUPPORT-RESOLUTION`](../work-packages/20260807-snow-stage3-paradise-wy2015-support-resolution-001/package.md) — **complete / support cause localized / non-blocking diagnostic** | Exact retained traces show 19 partial sequential hours terminate at `post_substep_no_resolved_surface`, followed by 183 same-state-only hours whose sequential status is `thin_pack_boundary_reached`. Omitted magnitude remains `98.0757 MJ m^-2`. | The direct cause explains support topology only. Preserve the historical `0.0621730192 > 0.05` failure; no noise label, threshold retune, physical pass/fail claim, persistence gate, physics, or cutover change. |
...
1. completed: scope a declared evaluation shadow under `INV-SNOWFREEZE-091`,
   freezing same-state carrier and sequential resolved-shadow operators;
...
5. completed with persistence `HOLD`: prospectively audited evolving-state
