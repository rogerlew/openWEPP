...
| 21M | [`SNOW-WARM-MIXED-COE-PHYSICS-AUTHORITY-AUDIT`](../work-packages/20260804-snow-warm-mixed-coe-physics-authority-audit-001/package.md) — **active / scaffolded** | Read-only first-principles audit of warm/mixed CoE generation against chapter, pinned legacy, and independent physical authority, prioritizing temperature/dewpoint/radiation/state interactions. | Preserve 21L as diagnostic evidence; adjudicate formula chronology, units, and authority before any implementation proposal. |
| 22 | `SNOW-SURFACE-EB-05` campaign assurance closeout — **deferred pending 21M** | Refresh `ASSURE-06` only after wet-compaction closure, corrected-state warm/mixed attribution, and CoE physics authority adjudication reach disposition. | Distinguish implemented physics, empirical efficacy, noninferiority, unresolved forcing/process ownership, development-only Snowbird normalization, and default status. Warm-maritime conifer transfer remains withheld. |
