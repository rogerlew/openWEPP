...
| 28 | [`SNOW-STAGE3-LEGACY-PREDECESSOR-BRIDGE-RECONCILIATION`](../work-packages/20260806-snow-stage3-legacy-predecessor-bridge-reconciliation-001/package.md) — **executing / prospective protocol correction** | Static custody proved the schema-v4 predecessor and schema-v6 current anchor used different Snowbird climate files. Execute a two-source by two-forcing endpoint matrix, paired disabled controls, independent primitive reconstruction, and per-WY factorial decomposition before accepting any source/state class. | Correct canonical forcing-matched predecessor authority before results. Run source checkpoints only if a fixed-forcing source effect remains. Physics, persistence, promotion, and ownership remain unchanged. |
