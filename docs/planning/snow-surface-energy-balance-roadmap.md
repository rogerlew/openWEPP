...
| 20B | `SNOW-SURFACE-EB-04W2B` warm-day new-snow activation and consumer closure — **next** | Reconcile the canonical contract and correct the public snow-partition activation path so positive typed hourly snowfall is conserved even with zero prior SWE and a nonnegative daily mean. Add mixed-event regression and fail-closed daily SWE closure through the real snowbench consumer. | Preserve the W2A cells, coefficients, and hypothesis rule. Only after exact closure may the legacy/albedo contrast rerun; no precipitation extension, coefficient fitting, or promotion is authorized. |
| 21 | `SNOW-SURFACE-EB-04X` Harvard depth–SWE geometry/interception investigation — **after EB-04W2B** | Use paired hardwood/open evidence to test algebraic SWE–density–depth closure, bulk-density trajectory, snowfall-input differences, interception storage/unloading/drip/sublimation, and the residual forest/open energy contrast. | The open lane anchors density/geometry. Only a residual paired forest/open contrast may identify interception or canopy energy. Freeze component-specific longwave, sublimation, and combined-interaction promotion gates before any successor candidate run. |
...
activation defect that invalidates its albedo harness. EB-04W2B now precedes
EB-04X and must close the real consumer before the unchanged contrast reruns. A
new result-bearing
