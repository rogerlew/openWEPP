...
| 22 | `SNOW-SURFACE-EB-05` campaign assurance closeout — **next / required by 21N hold** | Refresh `ASSURE-06` against 21K-21N and the admitted v7/v126 authority, then issue a new exact review root. | Distinguish target authority from implemented physics, empirical efficacy, noninferiority, unresolved forcing/process ownership, development-only Snowbird normalization, and default status. Warm-maritime conifer transfer remains withheld. |
