...
| 21I | [`SNOW-PREPEAK-MASS-TRANSITION-PHYSICS-ADJUDICATION`](../work-packages/20260804-snow-prepeak-mass-transition-physics-adjudication-001/package.md) exact-current cross-fixture correction-family adjudication — **queued / scaffold** | Reconstruct the linked mass boundaries for all four retained SNOTEL fixtures and quantify frozen signed-hour, cold-content, event, and downstream-disposition signatures without feeding diagnostics back into production state. | Four real CLI replays, exact closure and comparable-output identity, figures plus source sidecars, prospective adjudication, and dual review/verification. Any correction remains a separate contract-first package. |
