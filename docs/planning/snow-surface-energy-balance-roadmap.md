...
| 21H | [`SNOW-MASS-TRANSITION-LEDGER-PERSISTENCE`](../work-packages/20260803-snow-mass-transition-ledger-persistence-001/package.md) durable linked mass-accounting architecture — **queued / scaffolded** | Preserve the solid-to-liquid and liquid-disposition boundaries as two exact linked compact ledgers over one authoritative calculation, while separating production outcomes from the opt-in hourly research payload. | Prove disabled-path payload absence and bounded runtime/RSS, exact schema-v4 and ordinary-output compatibility, compact-ledger independent closure, no duplicate mass authority, and no physics change. |
