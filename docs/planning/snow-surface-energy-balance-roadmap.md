...
| 26 | [`SNOW-STAGE3-FOUR-SITE-CARRIER-TERM-AUDIT`](../work-packages/20260806-snow-stage3-four-site-carrier-term-audit-001/package.md) — **active / result-blind freeze** | Run a frozen paired-window per-term audit at Mica Creek, Niwot, Paradise, and Snowbird through the real schema-v5 consumer. Reconstruct the implemented external subset over water-year-stratified independent condition samples; Snowbird's development-only forcing is non-decisive. | Persisted chronology, seasonal efficacy, terminal receipt, receiving-surface energy, promotion, and cutover remain held. Admit and run persistent accumulation-season shadow state only if that bounded audit is interpretable and its prospective screen passes. |
...
3. active: run a frozen paired-window per-term audit at Mica Creek, Niwot,
