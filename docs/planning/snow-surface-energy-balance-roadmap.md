...
| 29 | [`SNOW-STAGE3-EVOLVING-STATE-CARRIER-PLAUSIBILITY-RECONCILIATION`](../work-packages/20260807-snow-stage3-evolving-state-carrier-plausibility-reconciliation-001/package.md) — **executed / persistence HOLD / multifactor or inconclusive** | Attempt 004 reconstructs 154 eligible water years with zero bounded-transfer truncation and zero active-state failures. Sensible and latent evolution dominate the separately reduced reported Q−F terms, but this is mechanical attribution, not physical validation. | Paradise WY2015 fails the frozen support threshold; wind exposure and physical magnitude/geometry envelopes remain unresolved. User-supplied GRIDMET sources identify nominal 10 m wind and 2 m temperature heights, but exact product/cell/aggregation/exposure custody is the first follow-on. No persistence, correction, fitting, promotion, ownership change, or cutover. |
| 30 | `SNOW-STAGE3-WIND-SOURCE-CUSTODY-AND-EXPOSURE-AUTHORITY` — **next / package required** | Admit exact GRIDMET product/version, grid cell, native cadence, aggregation into CLI `w-vl`, nominal 10 m wind height, and site exposure/roughness/displacement authority without fitting or attenuation. | Resolve the first authority-shaped blocker from row 29 before any new plausibility result. Nominal product height alone does not establish open-, above-, within-, or sub-canopy exposure. |
...
6. admit and run persistent accumulation-season shadow state only if that
   frozen plausibility gate later passes at all three canonical sites;
