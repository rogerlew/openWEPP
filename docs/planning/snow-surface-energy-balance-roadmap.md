...
| 2 | [`SNOW-SURFACE-EB-01A`](../work-packages/20260730-snow-surface-eb-01a-longwave-authority-research-001/package.md) — **complete / pass** | Resolved equation-level atmospheric longwave, hemispherical sky/canopy view, effective-unity treatment, canopy-temperature, and trunk-scope authority before EB-02. | EB-02 contract research is admitted; runtime implementation remains held on the deterministic canopy-state-to-sky-view mapping, cloud mapping, and active snow-surface temperature. No new user sky-view coefficient or remote-sensing dataset is required. |
| 3 | `SNOW-SURFACE-EB-02` sub-canopy longwave contract and opt-in implementation — **contract research admitted / runtime held** | Admit the selected displaced-sky/additive-canopy formulation, corrected Dilley-Unsworth estimator, an internally derived radiometric sky-view operand from existing canopy cover, LAI, structural cover, and scientifically relevant height, effective-unity treatment, active snow-surface temperature, and named air-temperature proxy limits; then implement independent nondefault physics. | Typed guards, contract-derived tests, no direct canopy-cover/sky-view alias, no new user coefficient or required remote data, real runtime consumer proof, approximation diagnostics, and independently reconstructed energy closure pass. |
| 4 | `SNOW-SURFACE-EB-03` energy-consistent sublimation composition — **admitted contract-first** | Reconcile the rejected Stage A/B mass-loss candidates with one snow surface-energy and state ledger. Preserve nonpromotion, use one turbulent exchange for vapor mass and latent heat, and make sublimation independently selectable from unrelated melt behavior. | Vapor mass and latent energy close independently; no liquid/melt aliasing or double debit exists; existing nonpromotion evidence remains visible; the `S` and `LS` cells are mechanically executable. |
...
Orders 3 and 4 may be implemented in either order after EB-01A, but they are
not result-bearing substitutes for Order 5. Order 5 starts only when the same
