...
| 27 | `SNOW-STAGE3-TURBULENT-CARRIER-LINEAGE-AND-OPERATOR-RECONCILIATION` — **next / package required** | Starting from byte-identical daily states and hourly forcing, reconcile every same-state versus sequential external term—especially longwave, sensible, and latent—while tracking subsequent state divergence. Bind surface temperature, vapor/dewpoint, wind, stability/exchange coefficients, `z_T/z_q/z_u/z_0`, latent/mass conversion, and sign convention. | Turbulence dominates the current same-state failure but is not yet the proven cause of the cross-operator contradiction. No fitting or persistence; advance only with exact consumer lineage, mutually interpretable operators, and a prospective screen. |
