...
| 21N | `SNOW-COE-STAGE3-MELT-OWNER-AUTHORITY-RECONCILIATION` — **next / unscaffolded** | Decide whether the empirical CoE production claim can be bounded and retained or whether melt ownership must consume existing state-resolved Stage-3 state. | Adjudicate authority before tests/code; prioritize `C_canopy`, daily midpoint gating, embedded albedo lineage, and cold-content satisfaction without preselecting a replacement. |
| 22 | `SNOW-SURFACE-EB-05` campaign assurance closeout — **deferred pending 21N** | Refresh `ASSURE-06` only after the CoE/Stage-3 melt-owner authority seam reaches contract disposition. | Distinguish implemented physics, empirical efficacy, noninferiority, unresolved forcing/process ownership, development-only Snowbird normalization, and default status. Warm-maritime conifer transfer remains withheld. |
