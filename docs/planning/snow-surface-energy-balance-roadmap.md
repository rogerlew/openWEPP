...
| 27 | [`SNOW-STAGE3-TURBULENT-CARRIER-LINEAGE-AND-OPERATOR-RECONCILIATION`](../work-packages/20260806-snow-stage3-turbulent-carrier-lineage-and-operator-reconciliation-001/package.md) — **executed / operator mechanics characterized / predecessor bridge hold** | Schema v6 closes primitive fluxes, exact support, state endpoints, and operator deltas across all four sites. Every site has `S<0`, `F<0`, `Q>0`; Snowbird is `-1086.47`, `-742.02`, and `+179.59 MJ m^-2`. Omitted support is below one percent and protected HBP/PASS/WAT bytes remain exact. | The frozen causal class is withheld because current legacy reconstruction is `+188.8725`, not the predecessor `+170.2536 MJ m^-2` (`10.94%` high). CoE stays authoritative; no fitting, persistence, physics correction, promotion, or cutover. |
| 28 | [`SNOW-STAGE3-LEGACY-PREDECESSOR-BRIDGE-RECONCILIATION`](../work-packages/20260806-snow-stage3-legacy-predecessor-bridge-reconciliation-001/package.md) — **executing / result-blind scaffold review** | Reproduce the schema-v5 predecessor from the schema-v6 source by separating source revision, daily reset cadence, first-state projection, same-hour transition preparation, and active-conduction lineage on the exact Snowbird cohort. Require term-by-term and state-endpoint closure before accepting any causal class. | If the bridge closes, adjudicate the prospectively observed four-site state-evolution pattern. If it does not, retain `PREDECESSOR_NOT_REPRODUCED` and repair evidence custody only. Physics, persistence, promotion, and ownership remain unchanged. |
