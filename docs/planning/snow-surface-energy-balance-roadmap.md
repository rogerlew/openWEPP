...
| ---: | --- | --- | --- |
| 1 | [`SNOW-SURFACE-EB-01`](../work-packages/20260729-snow-surface-eb-01-reconciliation-factorial-design-001/package.md) — **scaffolded / queued** | Reconcile canonical authority, current production and opt-in implementation, prior candidate outcomes, observations, selectors, and ledgers. Freeze the four-cell factorial, response variables, data roles, anti-double-counting rules, and stop-loss. | Complete only when every proposed equation/constant/state input is classified as already authoritative, authority-admissible, authority-missing, or out of scope; every factorial cell is implementable or has a named prerequisite; and observation roles and gates are frozen before new physics. |
| 2 | `SNOW-SURFACE-EB-02` sub-canopy longwave authority and opt-in implementation | Amend canonical authority as needed and implement the smallest authoritative explicit canopy–snow longwave term through the shared surface-energy balance. Keep it independently selectable and nondefault. | Typed guards, contract-derived tests, authoritative canopy/surface temperature treatment, real runtime consumer proof, and independently reconstructed energy closure pass. Missing temperature/process authority produces a named hold, not a surrogate formula. |
| 3 | `SNOW-SURFACE-EB-03` energy-consistent sublimation composition | Reconcile the rejected Stage A/B mass-loss candidates with the active snow surface-energy and multilayer state. Reuse existing physics where valid; change it only under canonical authority. Make sublimation independently selectable from unrelated melt behavior. | Vapor mass and latent energy close independently; no liquid/melt aliasing or double debit exists; existing nonpromotion evidence remains visible; the `S` and `LS` cells are mechanically executable. |
