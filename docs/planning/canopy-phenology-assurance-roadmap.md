...
| 4 | `CANOPY-CAL-04` process calibration and identifiability — [authority lifted](../work-packages/20260726-canopy-cal-04-05-authority-evidence-admission-001/package.md) | Evaluate GSI thresholds, `Bf,max`, `Bs`, `fe`, `xmxlai`, `Cs`, and `bb` in process order against Hubbard Brook calibration intervals, using Harvard Forest only as the independent holdout and a prior-free deterministic design. | Timing, amplitude, biomass partition, and canopy closure pass without hydrologic fitting; holdout observations do not influence fitting choices. |
