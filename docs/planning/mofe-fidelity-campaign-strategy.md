...
| D15 | `20260705-mofefid-d15-opt-in-production-activation-001` + rerun `20260706-mofefid-d15-opt-in-production-activation-rerun-001` + blocker resolution `20260706-mofefid-d15-blocker-resolution-001` + hold-lift `20260706-mofefid-d15-active-owner-optimization-001` | The opt-in production flip: routing owns the surface-water path, DC01 daily-lump runon is disabled for active lanes, the `INV-OFEROUTE-012` runtime closure hard-fail is live, `latqcc` bypass is included in closure, active outputs are published from the routed path, and subsystem-off protected outputs remain byte-identical. | This is the first package allowed to claim production activation only after source-authority, timing, and active-consumer blockers are closed. D9-D14 remove earlier known blockers, define the active consumer surfaces, and characterize/optimize the runtime cost. | **QUEUED HOLD-LIFT 2026-07-06:** the original D15 source-authority hold was unblocked by D10B (`GAP-OFEROUTE-005` resolved). The D15 rerun then held on `NegativeOutletBin` and missing active owner. The blocker-resolution package closed `NegativeOutletBin`: H2637 ignored shadow test passes and release shadow endpoint completes, but timing is `91.59 s` user / `1:31.67` wall, about `3.06x` D14's optimized budget. The timing regression is adjudicated as a hold. Phase B still finds no active production owner path: current code remains diagnostics-only shadow/candidate surfaces with DC01 still feeding production. `20260706-mofefid-d15-active-owner-optimization-001` is scaffolded to optimize/adjudicate timing and implement the active production owner with real consumer proof. |
...
source-authority hold (`GAP-OFEROUTE-005` resolved at rev 25/26), and the D15
blocker-resolution package closed the terminal-bin `NegativeOutletBin`.
Remaining blockers are the adjudicated timing regression and the absent active
production owner path; both are current-scope gates for
`20260706-mofefid-d15-active-owner-optimization-001`. D16 is explicitly
outside the opt-in activation package and remains blocked.
...
| ▶️ | `20260706-mofefid-d15-active-owner-optimization-001` + D16 sequence in §6.1 | D | remaining production-activation scope. D10B (2026-07-06) resolved the GAP-005 source-authority hold, and the D15 blocker-resolution package closed the H2637 `NegativeOutletBin`. D15A resolves the active production owner path and optimizes/adjudicates the `91.59 s` H2637 opt-in timing; D16 only after opt-in activation evidence exists. |
