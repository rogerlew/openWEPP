...
2026-07-06). Campaign code: **MOFEFID**.
...
> **MOFEFID A-C complete; Lane D is in activation-gate closure (2026-07-06).**
...
> opt-in runtime SHADOW + real-H2637 executed vector are merged. D10B/D11/D12/
> D13 closed the former source-authority, operand, melt-limb, and erosion-shape
> blockers. **Production activation is still not one remaining task.** The D15
> blocker-resolution package closed the D10B-corrected routed
> `NegativeOutletBin` terminal-bin failure, but the refreshed H2637 opt-in
> timing is `91.59 s` user / `1:31.67` wall and the active production owner
> path is still absent. The timing regression is a hold. Next package:
> `20260706-mofefid-d15-active-owner-optimization-001` resolves active
> production ownership and optimizes/adjudicates D15 timing with real consumer
> proof.
...
| D10B | `20260706-mofefid-d10b-gap005-source-authority-reconciliation-001` | The D10 hold-lift: reconcile `GAP-OFEROUTE-005` source authority. Leg A binds limiter/CFL/dissipation to the TVD family primaries (Mingham/Garcia-Navarro in hand; Davis 1984 + Tseng 2010 acquired from R-63's own citation chain); Leg B re-anchors Case-4 acceptance to the Iwagaki 1955 primary (characteristics oracle + experimental hydrographs + `n=0.009` via a named definitional mapping) and demotes the digitized enhanced-WEPP trace to an ADR-0017 comparator flag; Leg C reclassifies the OFE sampled handoff under the conservation hard gate and adjudicates the H2637 resolution sensitivity from a seam-decomposed ledger. | Under the clean-room posture, enhanced-WEPP implementation parity is permanently unclosable, so D10's hold cannot be lifted by more Papanicolaou material; it is lifted by re-anchoring the acceptance oracle to clean-room-legal authority. The handoff was never R-63-specified — it is openWEPP-owned machinery under the conservation invariant. | **EXECUTED-COMPLETE 2026-07-06** (Claude Code, operator-directed end-to-end run): `SC-OFEROUTE-001` rev 24 bound the scheme (R-63's printed limiter adjudicated a transcription error against Davis 1984/Mingham 2001; two-sided face dissipation; boundary/ledger/handoff under the conservation gate; Case-4 acceptance RE-ANCHORED to the Iwagaki-primary entropy-solution oracle, digitized enhanced-WEPP trace demoted to ADR-0017 flag; Manning `n=0.009` mapping) and rev 25 landed + validated the production correction: Case-4 oracle acceptance MET (peak <= 2.6% vs 5% tolerance, `t_peak` <= 0.09 s, rise <= 0.11 s), 19-OFE H2637-class cascade conservation IDENTICALLY ZERO across the recorded sweep grid (pre: 9-54% anti-convergent), 5/5 contract-derived tests flipped from failing, `GAP-OFEROUTE-005` RESOLVED, `GAP-OFEROUTE-001` CLOSED. Named bounded residuals recorded (strict-TVD transient bound, boundary-flux ripple, shock-peak wobble). **D15 rerun precondition: D14 endpoint-timing REFRESH REQUIRED** (true-celerity evaluation + conservative handoff changed the profiled path). |
...
| D14 | `20260705-mofefid-d14-laned-runtime-profile-optimization-001` | Profile and optimize the Lane D runtime physics path before the opt-in flip: break the H2637 shadow overhead into solver math, per-day/OFE setup, allocation, hydrograph sampling/interpolation, handoff, and diagnostics; land behavior-preserving optimizations that keep numerical authority and closure evidence intact. | The current H2637 shadow estimate is about `+207 s` CPU over the default path, mostly in `ofe_routing` cascade work. That is too large to bury inside the activation package, and activation needs a fresh endpoint budget after D10-D13 settle the candidate path. | **EXECUTED-COMPLETE 2026-07-06:** release-grade baseline re-pinned the overhead at `+65.3 s` user CPU on the 2-year H2637 fixture (the `+207 s` figure was a dev-profile estimate); slot + perf attribution: ~97 % solver math, 51 % of the run in redundant per-step alpha recomputation, libm `pow` 36 %, allocation ~2 %, everything runner-side < 0.2 %. Landed three bit-identical optimizations (single alpha evaluation per cell per step; per-solver step scratch replacing 8 per-step allocations; hoisted skin rain term `3393 I^0.407` per step): shadow-on `67.6 s → 29.9 s` wall (2.26×), overhead −58 % to `+27.6 s`, shadow multiplier ~29× → ~13×. Protected outputs and the `laned_shadow` manifest diagnostics are bit-identical pre/post, with identical trajectory counters (10,334,879 steps) as the preservation witness. Persistent opt-in slot diagnostics live in `ofe_routing::profile` (`OPENWEPP_LANED_SHADOW_PROFILE=1`). No numerical-method, tolerance, resolution, or activation change. |
| D15 | `20260705-mofefid-d15-opt-in-production-activation-001` + rerun `20260706-mofefid-d15-opt-in-production-activation-rerun-001` + blocker resolution `20260706-mofefid-d15-blocker-resolution-001` + hold-lift `20260706-mofefid-d15-active-owner-optimization-001` | The opt-in production flip: routing owns the surface-water path, DC01 daily-lump runon is disabled for active lanes, the `INV-OFEROUTE-012` runtime closure hard-fail is live, `latqcc` bypass is included in closure, active outputs are published from the routed path, and subsystem-off protected outputs remain byte-identical. | This is the first package allowed to claim production activation only after source-authority, timing, and active-consumer blockers are closed. D9-D14 remove earlier known blockers, define the active consumer surfaces, and characterize/optimize the runtime cost. | **EXECUTED 2026-07-06 (D15A, Claude Code end-to-end):** BOTH holds lifted at `SC-OFEROUTE-001` rev 27. Timing: five bit-identical solver optimizations (OPT-5..9) cut the shadow endpoint `92.4 → 78.8 s` user; the remainder decomposes to ~1 % into the two contract-mandated mechanisms (steps ×1.639 from the rev-26 true-celerity CFL; per-step ×1.645 from the mandated perturbed celerity evaluation), so the D14 `29.9 s` budget — priced on the pre-rev-24 latently-unstable scheme — is superseded and `78.8 s` is adjudicated the corrected-scheme budget (flagged for operator ratification). Active owner: `OPENWEPP_LANED_ACTIVE=1` runs the two-phase active day loop (all-lane hydrology → per-lane route → erosion → ledger over the shared day window); DC01 SURFACE admission disabled with a live INV-009 double-feed guard (lateral unchanged); the rev-27 closure hard-fails are LIVE (they caught two real seam defects during implementation — the mesh-basis aliasing and the full-mesh-hold erosion degeneracy, both resolved contract-first); D13 erosion consumes the routed shape everywhere; rev-21 operands consumed by the active step. Executed H2637 active evidence: `37.4 s` user endpoint, 610/731 days routed, supply reconstruction 7.3e-16, day cascade residual ≤ 2.5e-13, day identity residual ≤ 2.4e-13, 1 counted degenerate lane-day; default/off byte-identical throughout. NAMED follow-on gates (not silent): watershed-facing HBP outlet re-pointing (SC-RUNOFFPART/SC-ROUTE authority) and active-mode erosion water-magnitude coupling (active `tdet` drops ~250x because downstream lanes no longer re-infiltrate runon). D16/default promotion remains blocked. |
...
The ordering is strict at D15: production activation must not start while a
current activation precondition is failing. D10B lifted the original
source-authority hold (`GAP-OFEROUTE-005` resolved at rev 25/26), the D15
blocker-resolution package closed the terminal-bin `NegativeOutletBin`, and
`20260706-mofefid-d15-active-owner-optimization-001` (D15A) closed BOTH
remaining blockers: the timing regression (optimized + adjudicated at rev 27)
and the active production owner (landed with live closure hard-fails and real
consumer proof). D16 is explicitly outside the opt-in activation package and
remains blocked; its inputs now exist (executed opt-in activation evidence),
plus two named follow-on gates: HBP outlet re-pointing and active-mode erosion
water-magnitude coupling.
...
| ✅ | `20260706-mofefid-d15-active-owner-optimization-001` | D | **executed 2026-07-06:** opt-in production activation LANDED (`SC-OFEROUTE-001` rev 27) — timing optimized `92.4 → 78.8 s` (bit-identical) + adjudicated (mandated-mechanism decomposition), active owner live (two-phase day loop, DC01-surface-disable + double-feed guard, live closure hard-fails, D13 routed erosion consumer, rev-21 operands), H2637 active endpoint `37.4 s` with machine-exact day closures. Follow-on gates named: HBP outlet re-pointing; active-mode erosion water magnitude. |
| ▶️ | D16 sequence in §6.1 | D | default-promotion adjudication — now unblocked on inputs (executed opt-in activation evidence exists) but remains a separate policy/release gate. |

Stop conditions (current): A/B findings are dispositioned and Lane C proceeded
only after its envelope was ratified. The §6.1 D10 source-authority hold is
CLOSED (D10B, 2026-07-06). The H2637 terminal-bin/day-boundary blocker is also
CLOSED by `20260706-mofefid-d15-blocker-resolution-001`. Lane D opt-in
production activation now waits on
`20260706-mofefid-d15-active-owner-optimization-001`, where the rev-21 D11
operand path, D12 source-shape closure, and D13 routed-hydrograph erosion shape
must be proven in the active consumer and the `91.59 s` timing regression must
be optimized/adjudicated. Lane D default activation is D16 or later and remains
blocked.
...
1. ~~**D14 runtime budget**~~ — resolved 2026-07-06: the overhead is ~97 %
   solver math (redundant alpha recomputation + `pow`), not
   setup/allocation/sampling/handoff; three bit-identical optimizations cut
   it 58 % (release-grade `+27.6 s` on the 2-year H2637 fixture, shadow
   multiplier ~13×). D15 receives the budget in the D14 worker handoff;
   remaining headroom under the bit-identity constraint is small (~1–3 %
   candidates recorded and deferred).
2. **D15 opt-in surface:** queued in
   `20260706-mofefid-d15-active-owner-optimization-001`; exact user/runtime
   selector, output-publication scope, active consumer proof, and timing
   adjudication for production-active routed water.
