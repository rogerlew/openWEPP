...
| **SNOWDENSITY-05H Low-Canopy Mixed-Forest Melt Adjudication** | Future melt-value fork: before retiring or reopening activation for `coe_shortwave_albedo_v1`, build canopy-stratified mixed-forest fixtures such as Marcell/Harvard and rerun melt adjudication across open/deciduous/conifer regimes. The harness must use real per-day seasonal `cancov`, not the single representative evergreen value used by 05G. | Decisive only for melt value in low-canopy or leaf-off regimes. No density compensation, no default activation, and no site-specific tuning. This package is not required to start SNOWDENSITY-06 density work if 06 keeps fixed melt boundaries. |
...
| **SNOWDENSITY-06B CoE-Bound Density Replay** | Couple the accepted density-compaction state update to a fixed CoE melt/liquid boundary in snowbench/offline replay before runtime activation. This should consume `legacy_coe` and optionally `coe_shortwave_albedo_v1` diagnostic melt outputs as fixed upstream liquid/SWE-loss operands rather than reusing the old physics-bulk degree-day melt surrogate. | Must preserve no-site-tuning and fixed radiation/canopy/albedo/melt coefficients; must beat or clearly classify whole-rubric and density-cell profiles before SNOWDENSITY-07 runtime opt-in. |
| **SNOWDENSITY-07 Runtime Opt-In** | Couple the accepted melt + density model into the winter column behind `snow_model = physics_bulk`; preserve `legacy_wepp` default. | Requires a prior offline CoE-bound density replay that clears the profile gate, plus conservation, publication, anti-alias, direct/compatibility rollback, and full workspace gates. |
