...
| **SNOWDENSITY-05F Melt Closure / Density Handoff** | Freeze the accepted opt-in melt API, diagnostics, rollback policy, cold-start albedo state policy, and coefficient/default envelope. Decide whether the melt path is ready for SNOWDENSITY-06 density compaction consumption or must remain held. | Handoff names the exact accepted melt selector, required operands, activation baseline, and residual risks. SNOWDENSITY-06 may start only if melt is either accepted opt-in or explicitly held with a bounded interface that density can consume without retuning melt. |
