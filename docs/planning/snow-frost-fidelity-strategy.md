...
| **SNOWDENSITY-05A Melt Contract + Sign Reconciliation** | Scaffolded as `docs/work-packages/20260625-snowdensity-05-melt-modernization-contract-first-001/`. Amend `SC-SNOWFREEZE-001` for CoE melt modernization, opt-in `snow_melt_model`, shortwave/albedo operands, no-radiation-tuning guard, and explicit negative-benchmark disposition for degree-day snowbench variants. Reconcile WEPP Chapter 3 `amelt - bmelt + cmelt + dmelt` prose with the current signed `melt_bmelt_in` trace convention before code. | Contract-derived tests pass; no production code until the pre-implementation contract gate is green. Close `HOLD` if the sign/alias convention or `SC-CLIMATE-001#INV-CLIMATE-013` cannot be reconciled. |
| **SNOWDENSITY-05B Shortwave Source Binding** | Identify and bind the canonical gridded daily shortwave/radiation source available to openWEPP, its units, slope/aspect transformation, hourly distribution, and provenance relationship to ET forcing. This package owns source selection and unit/anti-alias evidence; it does not tune radiation to snow. | A typed source/provenance ledger proves one conversion path into `winter.hourly.rad_mj_m2_####`, rejects raw Langleys/day and fitted radiation scalars, and demonstrates ET and snow see the same forcing authority. Close `HOLD` if the required gridded source is outside openWEPP ownership or cannot be passed without duplicating wepppy concerns. |
| **SNOWDENSITY-05C Albedo State Core** | Ratify and implement the opt-in albedo state update behind `coe_shortwave_albedo_v1`: Brock-2000 style temperature/age decay, fresh-snow refresh, bounded albedo domain, model id/provenance, and no hidden default when opt-in state is required. | Contract/unit tests prove albedo bounds, monotonic age decay, fresh-snow reset, missing-state fail-closed behavior, and no effect on `legacy_coe` default runs. No routed-melt acceptance yet. |
| **SNOWDENSITY-05D Opt-In CoE Melt Implementation** | Wire `coe_shortwave_albedo_v1` into the existing production CoE melt-term path. Preserve `bmelt` sign convention, `cmelt`/rain terms, signed raw melt, corrected daily redistribution, density gate, and routed-melt publication. | Default `legacy_coe` identity passes; opt-in runs change only accepted melt operands; independent reconstruction proves raw melt, redistributed melt, `wmelt`, SWE loss, and WB12/WB13 liquid forcing close without aliasing SWE/depth/density surfaces. |
| **SNOWDENSITY-05E Melt Rubric Adjudication** | Run SNOTEL and non-SNOTEL snow/frost rubric profiles for `legacy_coe` vs `coe_shortwave_albedo_v1`, reading forcing-robust melt timing/ablation/regime cells as the verdict surface and reporting forcing-limited magnitude cells separately. | No SNOTEL-fitted defaults. Promotion requires forcing-robust improvement or a documented non-promotion route; any claimed defect needs independent energy/conservation authority under ADR-0017, not observation disagreement alone. |
| **SNOWDENSITY-05F Melt Closure / Density Handoff** | Freeze the accepted opt-in melt API, diagnostics, rollback policy, and coefficient/default envelope. Decide whether the melt path is ready for SNOWDENSITY-06 density compaction consumption or must remain held. | Handoff names the exact accepted melt selector, required operands, and residual risks. SNOWDENSITY-06 may start only if melt is either accepted opt-in or explicitly held with a bounded interface that density can consume without retuning melt. |
| **SNOWDENSITY-06 Density Compaction** | Add the Anderson-1976/SNOBAL overburden + metamorphism compaction to the `physics_bulk` density path, constants carried from cited authority, on top of (not instead of) the modernized melt. | Conserves SWE; beats legacy on forcing-robust density/densification signatures without site tuning; no melt change substituting for density. |
...
- Which shortwave source and transformation openWEPP can own without duplicating
  wepppy orchestration. SNOWDENSITY-05B owns this source/provenance decision.
- Which albedo decay scheme to ratify. Brock-2000 accumulated-positive-
  temperature is the leading temperature-only candidate; CLASS/ISBA/Crocus are
  fuller alternatives. SNOWDENSITY-05C owns this decision.
