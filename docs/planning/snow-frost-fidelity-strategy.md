...
| **SNOWDENSITY-07 Runtime Opt-In** | Couple the accepted CoE-bound melt + density model into the winter column behind an explicit opt-in snow-density selector; preserve legacy snow default/rollback. | Requires conservation, publication, anti-alias, direct/compatibility rollback, no-site-tuning, default-disabled isolation, and full workspace gates. 06B clears the offline profile prerequisite but does not itself authorize runtime activation. |
