...
| **SNOWDENSITY-06 Density Compaction** | Entry gate now starts from 05G representative-regime evidence: use `legacy_coe` as default/rollback baseline and treat `coe_shortwave_albedo_v1` only as an opt-in diagnostic boundary unless a later activation package reopens melt. Add Anderson-1976/SNOBAL overburden + metamorphism compaction to the `physics_bulk` density path, constants carried from cited authority, on top of fixed melt boundaries rather than substituting melt for density. | Conserves SWE; beats legacy on forcing-robust density/densification signatures without site tuning; no melt change substituting for density; no default activation. |
