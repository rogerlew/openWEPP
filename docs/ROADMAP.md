...
flush still live. Deep migration remains fail-closed; the next cut is scaffolded
as PERFDEEP05 lane-dense transfer authority and sync removal.
...
| **P** | **PERF - comprehensive array-native re-architecture (HillslopeDayFrame)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP04 profiled the no-go and identified the dominant current boundary: daily lane-dense resync from logical/indexed surfaces. | Execute `PERFDEEP05 - Lane-Dense Transfer Authority and Sync Removal` (`work-packages/20260619-perfdeep05-lane-dense-transfer-authority-sync-removal-001/`): apply transfer input directly to dense lane state, cache hot slot metadata, avoid symbol-id lookup in dense apply, and remove `sync_from_writeback_surface` from the opt-in H2637 daily hot loop. No default activation until an opt-in H2637 endpoint beats `669.97 s` and identity/full gates pass. | 🧭 **NEXT: PERFDEEP05 scaffolded** - PERFDEEP04 measured opt-in `1164.31 s` under profiler vs default `704.82 s`; `sync_from_writeback_surface` is `33.49%` inclusive and absent from default. Keep PERFDEEP02/03 opt-in and fail-closed; do not run another writeback-only, materialization-retirement, or blind expansion rung. |
