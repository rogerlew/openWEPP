...
| **P** | **PERF - comprehensive array-native re-architecture (HillslopeDayFrame)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing now also taxes the default-disabled path (`701.95 s` vs `669.97 s`). The ratified runtime spec treats more compatibility-edge shaving as insufficient, and PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. PERFDEEP07 then improved but did not close that P0 gate (`685.85 s` retained vs `<= 676.67 s`). | Before any R2+ direct-frame runtime implementation, either close the PERFDEEP07 disabled-path hold with zero-cost-when-disabled proof or explicitly supersede it with a ratified performance/architecture decision. The completed R0/R1 planning package provides the direct schema envelope, type-boundary decision, constructor/projection plan, publication-ledger promotion plan, and no-compatibility proof plan only. No default activation until a future opt-in implementation beats `669.97 s` and identity/full gates pass. | **BLOCKED / PLANNING COMPLETE** - R0/R1 planning package `work-packages/20260619-r0-r1-array-native-schema-frame-planning-001/` is complete. PERFDEEP07 package `work-packages/20260619-perfdeep07-zero-cost-disabled-direct-frame-hydrology-001/` remains `HOLD`; R2+ runtime implementation is blocked. |
