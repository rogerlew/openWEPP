...
| 2 | **Monolith line-count split** | Behavior-preserving split by domain responsibility of the WARN-band files (measured 2026-06-18: **10 files >2000, 0 over the 3000 required threshold** — advisory hygiene, not the stale "3 files") | Target tier (4 files >2500) under 2000 WARN; bit-identical outputs | ▶️ **`REFACTOR022` scaffolded, Codex-ready** — scoped to the 4 files closest to the 3000 hard threshold (routing.rs 2807, scheduler_seed_and_runtime.rs 2672, core_types.rs 2671, lateral_drainage.rs 2549); 2000-2500 tier deferred; coherent-seam splits only (a documented WARN beats a bad split). |
