...
| [`NATIVE-FOREST-ECOHYDROLOGY-AUTHORITY-REFRAME`](work-packages/20260809-native-forest-ecohydrology-authority-reframe-001/package.md) | Vegetation/ecohydrology authority | `complete / exact-head full-workspace pass` | The v4 caller-value/native-flux authority reframe is complete. Advance only through the held coupled successor's complete constitutive authority and contract-first gates; do not reopen site-value selection. |
| [`RHESSYS-EAST-COAST-VEGETATION-AUTHORITY-CLOSURE`](work-packages/20260809-rhessys-east-coast-vegetation-authority-closure-001/package.md) | Vegetation/ecohydrology authority | `executed-hold / premise superseded in part` | Retain its complete source search and gap evidence. The reframe supersedes its demand for selected universal site values and a jointly observed pine/oak state, but not its finding that complete constitutive cycles remain missing. |
| [`COUPLED-C3-FOREST-VEGETATION-STATE-MACHINE`](work-packages/20260811-coupled-c3-forest-vegetation-state-machine-implementation-001/package.md) | Vegetation/ecohydrology architecture | `queued / implementation authority released` | Implement the indivisible `OPENWEPP_C3_WOODY_V1` stack default-off; no partial endpoint or runtime cutover. |
| [`SOIL-BIOGEOCHEMISTRY-TRANSFORMATIONS`](work-packages/20260811-soil-biogeochemistry-transformations-implementation-001/package.md) | Soil biogeochemistry | `queued / dependency authority bounded` | Admit and implement decomposition, mineralization, and mineral-N transformations; until then transformation delta is zero or the run fails explicitly. |
| [`RHESSYS-EAST-COAST-COUPLED-VEGETATION-SLICE`](work-packages/20260808-rhessys-east-coast-coupled-vegetation-slice-001/package.md) | Vegetation/ecohydrology architecture | `prospectively superseded` | Do not execute; replaced by the whole-state successor above. |
| `LAND-SURFACE-ENERGY-CONSTITUTIVE-AUTHORITY` | Cross-domain land-surface architecture | `queued / vegetation boundary admitted` | Close `GAP-LANDSURFACEENERGY-001/002/003/006` with a snow-free ground/surface solver that consumes typed vegetation and hydrology handoffs. Keep canopy radiation, interception, wet-canopy evaporation, conductance, and transpiration owned by vegetation rather than embedding another ET model in the land-surface subsystem. |
| `VEGETATION-HYDROLOGY-ARBITRATION-SHADOW` | Vegetation + hillslope hydrology | `queued / depends on first vegetation slice` | Implement default-off Stage A potential ecosystem response, Stage B hydrologic allocation, and Stage C vegetation finalization with layer-resolved shared-withdrawal closure, reason-coded limits, atomic failure, and a named compatibility reduction adapter. |
| `LAND-SURFACE-ENERGY-RUNTIME-SHADOW` | Cross-domain land-surface architecture | `queued / depends on constitutive authority and arbitration protocol` | Close the implementation portion of `GAP-LANDSURFACEENERGY-004` with persistent typed surface state, component and liquid ledgers, explicit interval support, and an independent real internal consumer. Reject snow-present and censored terminal payloads; do not change the production default. |
| `VEGETATION-LAND-SURFACE-REAL-CONSUMER-SHADOW` | Vegetation + land surface + hydrology | `queued / depends on both shadows` | Run the coupled default-off path through real hillslope consumers, proving exact-one canopy/ground radiation, latent energy, soil withdrawal, infiltration/runoff, frost boundary, and compatibility-adapter consumption across admitted vegetation classes. Promotion requires conservation and out-of-sample coupled-response evidence, not RHESSys comparator agreement alone. |
| `SNOW-COE-STAGE3-ATOMIC-CUTOVER` | Snow + vegetation + land-surface campaign | `held / terminal increment` | Advance only after the vegetation/land-surface sequence above, Stage 3 carrier exposure and physical-efficacy holds, persistent and terminal mechanics, two-contract receiving-surface authority, exact-one ownership, real-consumer publication, defaults, rollback, assurance, and release gates pass. In one stable increment, authorize the terminal tuple, recompute the remaining interval through the coupled snow-free regime, make Stage 3 the sole melt owner, and retire CoE; keep CoE authoritative until then. |
