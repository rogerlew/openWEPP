...
equivalence.
R6 direct publication cutover is active-held at
...
behavior. The current blocker is
`HOLD-R6E-HBP-DIRECT-PROCESS-PARITY-MISMATCH`: HBP byte comparison is reached,
but current direct process operands are not parity-grade. The next package must
close contract-backed HBP direct process parity before WAT/PASS/loss/manifest
parity can honestly resume. R6F is now scaffolded at
...
as a defect-closure ExecPlan for that work. It explicitly forbids stopping at a
named parity mismatch, missing fixture/helper, manifest wiring gap, complex
fix, or newly discovered blocker; closure requires full
`DirectPublicationFrameCutover` public output writes with HBP/WAT/PASS/loss/
manifest parity, anti-alias fixtures, independent reconstruction,
no-compatibility proof, and default-disabled isolation.
...
| **P** | **PERF - comprehensive array-native re-architecture (direct runtime)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing also taxed the default-disabled path (`701.95 s` vs `669.97 s`). PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. PERFDEEP07 improved but did not close that gate (`685.85 s` retained vs `<= 676.67 s`); PERFDEEP08 tested disabled diagnostic-hook caching and was slower (`691.93 s`). PERFDEEP09 closed the disabled-path blocker, R2A created the separate direct-runtime skeleton without phase math or publication cutover, R3A proved the first complete direct phase span, R3B proved a richer direct water-ledger span, R3C proved run-level multi-lane transfer/topology propagation, R4A through R4P/Q/Z closed the direct hydrology path through shadow-only projection, R5A through R5E completed full OFE-day direct endpoint readiness while preserving no-publication/no-default/no-scheduler boundaries, and R6A added the missing run-bound direct publication frame plus direct HBP/WAT/PASS/loss/manifest projection consumers. R6 has promoted the publication operand ledger into architecture authority and added a fail-closed cutover candidate. R6B proved the candidate still lacked a production typed operand bridge; R6C corrected that failure mode so cutover failed before skeleton capture; R6D added a cutover-only retained production `DirectRunPublicationFrame` in the climate lifecycle and moved the hold to `HOLD-R6D-PARITY-GRADE-PUBLICATION-PRODUCERS-ABSENT`. R6E resolved direct-runtime input binding for parsed climate and moved the hold to `HOLD-R6E-HBP-DIRECT-PROCESS-PARITY-MISMATCH`. R6F is scaffolded to iteratively close that blocker and every subsequent in-envelope R6 publication blocker. | Next: execute R6F to reduce HBP byte mismatch to fields/operands/producers/authority, implement all in-envelope corrections, then continue through WAT/PASS/loss/manifest parity, anti-alias fixtures, independent reconstruction, direct manifest checksum cutover, default-disabled isolation, endpoint/RSS gates, and no-compatibility proof. Complete closure requires real `DirectPublicationFrameCutover`, not compatibility-row wrapping or another diagnostic hold. | **SCAFFOLDED: R6F direct publication cutover blocker closure** - no direct public output is shipped while gates fail. |
