...
| --- | --- | --- | --- |
| [`SNOW-STAGE3-LEGACY-PREDECESSOR-BRIDGE-RECONCILIATION`](work-packages/20260806-snow-stage3-legacy-predecessor-bridge-reconciliation-001/package.md) | Snow surface-energy-balance campaign | `executing / prospective protocol correction` | Static custody proved the schema-v4 `+170.2536089` and schema-v6 `+188.8725288 MJ m^-2` anchors used different Snowbird forcings. Execute the prospectively frozen two-source by two-forcing endpoint matrix and per-WY decomposition before any source/state attribution; localize checkpoints only if a fixed-forcing source effect remains. No fitting, physics change, persistence, promotion, or ownership change. |
