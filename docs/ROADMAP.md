...
| `SNOW-STAGE3-PARADISE-WY2015-SUPPORT-RESOLUTION` | Snow surface-energy-balance campaign | `queued / non-blocking diagnostic resolution` | Localize the 183 unmatched and 19 partial hours and `98.0757 MJ m^-2` omitted magnitude by date, term, operator status, and snow-state transition. Preserve the historical `0.0621730192 > 0.05` gate failure, but do not let the single support-censored row independently block persistence or carry a physical pass/fail claim. Admit noise/censoring only with direct cause evidence; do not retune the threshold. |
