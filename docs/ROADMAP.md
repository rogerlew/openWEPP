...
| --- | --- | --- | --- |
| `GATE-LINT-GOV-01` advisory-linter governance alignment | Agent tooling | `active` | Execute [Order 1](work-packages/20260727-gate-planner-governance-authority-alignment-001/package.md): apply exact governance, guard-test, history-identity, and frozen-status amendments through the ADR-0043 direct manual landing rule. |
...
| `CANOPY-ASSURE-01` | Execute the [canopy phenology assurance roadmap](planning/canopy-phenology-assurance-roadmap.md): reproduce Bill Elliot's analysis, calibrate and adjudicate native canopy/litter processes, prove canopy-gradient congruence and independent Southern Hemisphere robustness, then publish a dedicated canopy assurance report. | Complete the ordered `CANOPY-CAL-01..07` evidence ladder; add contract-governed physics only if litter-source adjudication requires it; bind exact research objects; pass independent reproduction, accountable scientific and publication review, finding disposition, approval, and release transfer. | `queued`; `CANOPY-PHENOLOGY-02` mechanics are complete, while empirical calibration and assurance remain prospective. This row does not displace `SNOW-SURFACE-EB` without an explicit priority update. |
