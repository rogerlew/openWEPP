...
| **K** | **Kernel-boundary typing program (symbol-map-free terminal state)** | Eliminate the symbol-map carrier types (`HillslopeKernelRequest`/`KernelWritebackPayload`/`HillslopeWritebackSurface`/`SymbolRegistry`) — the kernel-invocation boundary — by **typing the kernel request/writeback interface**: typed phase context, typed phase result/mutation APIs, typed diagnostic/trace sinks, and typed audit/publication event streams (authority-bearing typed state, never symbol-map wrappers). Migrate consumers first (diagnostics/trace → tests → phase return paths → event streams), **delete `scheduler.rs`/`day_frame.rs`/carriers last**. The binding 8-step sequence is in [array-native spec §8.2](architecture/array-native-runtime-specification.md). | Every live consumer reads typed boundaries; carrier types deleted (or documented as genuine I/O adapters only); H2637 + multi-OFE + Wave-2 identity; `compatibility_edge_invocations=0`; static call-graph/source-scan proof; RSS/time no-regress + the `<=5x` re-measure; full workspace gates. | ⏳ **runtime deletion complete (2026-06-30, `a381702b`); coverage-restoration CQR burndown ACTIVE — item NOT closed.** `20260630-kernel-boundary-terminal-typing-001` deleted the compiled symbol-map scheduler/day-frame runtime, carrier exports, scheduler trace/publication/seed support, and scheduler-only tests. Production hillslope execution is **direct-only** (`compatibility_edge_invocations=0`, H2637 byte-identical at `1:10.69` / `79284 KiB`, `1221` nextest passed). **However the wholesale deletion removed 85 test files (~657 tests, 1,878 → 1,221) with zero replacements, including real ratified-contract coverage** (FC/WP policy, BASECOND01 harmonic-`ssc`, WB11–19 closure, input parsers, MOFE conservation, hydrology, growth). That coverage gap is **tracked and being burned down** at [kernel-boundary-cqr-burndown-execplan.md](work-packages/kernel-boundary-cqr-burndown-execplan.md) via contract-anchored CQR packages (`SC-*#INV-*`/WB-identity coverage, no silent retirement, net-coverage gate). Item K closes only when the suite returns to **≥1,878** with the ratified-contract coverage restored. |
...
delete), not an incremental burn-down (which held twice). The ≤5× ideal remains
future performance work after terminal deletion; it is not a compatibility
runtime cleanup blocker.
