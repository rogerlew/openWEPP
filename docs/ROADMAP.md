...
| **WSHED-W11** | **Channel-network hourly water/sediment routing** | Close M-T3's explicit leaf-channel scope limit by adding contract-authorized typed channel-hourly water and particle-class sediment state, stateful channel routing, and downstream dependency consumption. Preserve no-hourly behavior and fail closed at unsupported branches/topologies; do not reconstruct channel hourly output from scalar summaries. | Canonical time-grid/projection and particle-class authority; typed routed channel hourly outputs; at least two dependency-ordered channels consuming HBP `V_h`/`S_h`; equal-daily-total/different-shape downstream sensitivity; per-channel/network water and sediment closure; production CLI consumer proof; protected minor-0 identity; full review/verification/gates. | **executed hold 2026-07-10; hold lifted same day** — water-series routing/dependency authority is established for `ipeak` 3-5, and ADR-0036 authorizes the limited uniform class reconstruction. WSHED-W11A (complete 2026-07-10) ratified per-interval channel sediment sequencing and state carry in `SC-ROUTE-001` v51; resume W11 at Phase B per the W11A `w11-handoff.md` (contract-first: contract-derived tests before production code). Package: [work-packages/20260710-wshedw11-channel-network-hourly-water-sediment-routing-001/package.md](work-packages/20260710-wshedw11-channel-network-hourly-water-sediment-routing-001/package.md). |
| **WSHED-W11A** | **Channel-hourly sediment sequencing authority** | Lift `WSHED-W11-HOLD-001` by acquiring and ratifying, or explicitly rejecting, canonical authority for time-resolved WS18-WS26 channel sediment routing coupled to routed channel water series. | Temporal quantum; water/sediment coupling; channel geometry/profile/bed state carry; per-class ingress/egress and detachment/deposition/storage closure; guards/tolerances/test vectors; dual review/verification; exact W11 implementation handoff. | ✅ **complete 2026-07-10 UTC (`EXECUTED-COMPLETE-AUTHORITY`; Codex re-confirmation pending)** — amended `SC-ROUTE-001` v51/v52: per-`dtchr`-interval quasi-steady sequencing (`INV-ROUTE-015..020`, biconditional lane activation, unique hydraulic-profile operand map, monotonic geometry carry, pinned-`dcap.for` widening realization with `t_exp`/`t_norm` operands, per-interval/day class mass closure, eleven contract-derived vectors) on convergent WEPP/CREAMS + KINEROS ARS-77 + HEC-RAS quasi-unsteady authority (references vendored); dual review (21 findings) + dual verification passed; Codex post-hoc review REOPEN dispositioned same day with all five closure amendments applied in v52 (`GAP-ROUTE-014` records the two migrated-lane terminals W11 must correct before interval reuse). Package: [work-packages/20260710-wshedw11a-channel-hourly-sediment-authority-001/package.md](work-packages/20260710-wshedw11a-channel-hourly-sediment-authority-001/package.md). |
