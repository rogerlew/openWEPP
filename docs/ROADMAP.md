...
| --- | --- | --- | --- |
| [`VEGETATION-RADIATION-INTERCEPTION-CONDUCTANCE-SLICE`](work-packages/20260808-vegetation-radiation-interception-conductance-slice-001/package.md) | Vegetation/ecohydrology architecture | `next / package scaffolded` | Execute the authority-first package: admit one coherent independent constitutive chain, then create the separately testable `openwepp-vegetation` crate and implement the default-off Stage A radiation/interception/conductance and layer-demand slice. The crate requests withdrawals and never mutates hydrologic soil state. |
