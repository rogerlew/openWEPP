...
| --- | --- | --- | --- |
| `LAND-SURFACE-ENERGY-CONSTITUTIVE-AUTHORITY` | Cross-domain land-surface architecture | `next / V7 canopy boundary available` | Close `GAP-LANDSURFACEENERGY-001/002/003/006` with a snow-free ground/surface solver that consumes typed vegetation and hydrology handoffs. Keep canopy radiation, interception, wet-canopy evaporation, conductance, and transpiration owned by vegetation rather than embedding another ET model in the land-surface subsystem. |
| `VEGETATION-HYDROLOGY-ARBITRATION-SHADOW` | Vegetation + hillslope hydrology | `queued / V7 vegetation prerequisite satisfied` | Connect the public V7 Stage A/B/C transaction to the real hydrology owner in a default-off, snow-free, positive-wind neutral shadow. Preserve layer-resolved D/A/F identity, shared-withdrawal closure, reason-coded limits, and atomic failure; do not change a production selector. |
| `LAND-SURFACE-ENERGY-RUNTIME-SHADOW` | Cross-domain land-surface architecture | `queued / depends on constitutive authority and arbitration shadow` | Close the implementation portion of `GAP-LANDSURFACEENERGY-004` with persistent typed surface state, component and liquid ledgers, explicit interval support, and an independent real internal consumer. Reject snow-present and censored terminal payloads; do not change the production default. |
| `VEGETATION-LAND-SURFACE-REAL-CONSUMER-SHADOW` | Vegetation + land surface + hydrology | `queued / depends on both shadows` | Run the coupled default-off path through real hillslope consumers, proving exact-one canopy/ground radiation, latent energy, soil withdrawal, infiltration/runoff, frost boundary, and compatibility-adapter consumption across admitted vegetation classes. Quantify representative forcing outside the admitted positive-wind neutral domain; do not add a hidden wind floor. Promotion requires conservation and out-of-sample coupled-response evidence, not RHESSys comparator agreement alone. |
