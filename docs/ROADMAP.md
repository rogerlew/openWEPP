...
| **P** | **PERF — array-authoritative production migration** | PERFARCH03 closed the missing floor measurement: a full array-native WB11 runoff branch hot loop measured **0.959423 us/OFE-day** (`0.024823x` legacy us/OFE-day), with boundary materialization measured separately. PERFMIG01 ratified ADR-0023 and migrated the production WB11 warm-rain writeback branch to dense `SymbolId` authority with exact identity. PERFMIG02 migrated hot scalar helpers to dense-first reads, but final H2637 timing was flat/negative and the conservative six-symbol apply-boundary retirement did not get cheaper. | Pivot to a deep WB11/WB12 single-phase array-native path that captures dense read + compute + write. Preserve exact identity and remeasure H2637 endpoint + RSS. | ⏭️ **NEXT — post-PERFMIG02 REDIRECT package needed**. PERFMIG02 is `executed-redirect`: endpoint was **669.97s -> 672.14s / 675.00s** (73.46× -> 73.70× / 74.01×) and identity held, but the strict `apply_indexed` attribution subgate failed (**105.461 us/payload** skip-six vs **104.752 us/payload** materialize-all). Do not run another writeback-only or tiny materialization-retirement rung. The next package should implement the PERFARCH03-style deep array-native read+compute+write pivot. |
