...
| --- | --- | --- | --- |
| [`SNOW-STAGE3-TURBULENT-CARRIER-LINEAGE-AND-OPERATOR-RECONCILIATION`](work-packages/20260806-snow-stage3-turbulent-carrier-lineage-and-operator-reconciliation-001/package.md) | Snow surface-energy-balance campaign | `active / result-blind protocol review` | Turbulent terms dominate the negative same-state failure, but causation of the positive independently reinitialized predecessor contradiction remains unresolved. Add default-off primitive/state observability, then reconcile every external term—especially longwave, sensible, and latent—from byte-identical initial states/forcing while tracking state divergence. No fitting, persistence, physics correction, or ownership change. |
| `SNOW-STAGE3-PERSISTENT-ACCUMULATION-SHADOW` | Snow surface-energy-balance campaign | `held / turbulent reconciliation and authority dependent` | Advance only after same-state and sequential carrier terms are mutually interpretable and the implemented external subset passes a prospectively frozen plausibility gate. Persist the admitted evaluation state across days with snowfall, liquid, restart, reappearance, and exact linked mass/energy custody; censor claims at unresolved terminal or receiving-surface boundaries. |
...
| [`ASSURANCE-SINGLE-APPROVER-DRAFT-RETURN`](work-packages/20260805-assurance-single-approver-draft-return-001/package.md) | Assurance governance | `executing / DRAFT returned / full rerun pending` | The one-approver language and typed immutable return are implemented; the later v128 bounded-observability source adoption now binds generation `221f8e51`. Close after exact-head full correctness and its own terminal evidence pass. |
...
| [`ASSURE-06` snow/frost flagship authority refresh](work-packages/20260805-assure06-snow-frost-flagship-refresh-001/package.md) | Scientific assurance | `DRAFT / review authority returned` | The v128 bounded-observability source remains unpublished at generation `221f8e51`; active review and approval roots are empty. A separately authorized, exact-subject human review entry is required before any review or approval can resume. Public report count is zero. |
