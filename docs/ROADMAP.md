...
| **P** | **PERF — array-authoritative production migration** | PERFARCH03 closed the missing floor measurement: a full array-native WB11 runoff branch hot loop measured **0.959423 us/OFE-day** (`0.024823x` legacy us/OFE-day), with boundary materialization measured separately. PERFMIG01 ratified ADR-0023 and migrated the production WB11 warm-rain writeback branch to dense `SymbolId` authority with exact identity. | Keep dense authority across adjacent hydrology consumers so the PERFMIG01 compatibility materialization boundary moves outward; remeasure H2637 endpoint + RSS after each rung; preserve exact identity and no hot-loop logical payload construction. | ⏭️ **NEXT — `PERFMIG02` contiguous WB11-consumer cluster**. `PERFMIG01` completed `CONTINUE`: H2637 `669.97s`, `228144 KB`, semantically identical outputs, transition apply boundary `107.531649 us/payload`; the single-rung endpoint regression is a named retireable boundary offset, not a redirect. |
