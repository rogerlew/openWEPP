...
| --- | --- | --- | --- |
| [`SNOW-SURFACE-EB`](planning/snow-surface-energy-balance-roadmap.md) additive sublimation and sub-canopy longwave | Snow/frost science | `EB-04U diagnostic design complete; EB-04V next` | EB-04U freezes the exact `9/2/5` density, Harvard geometry/interception, and open-control under-persistence cohorts plus observed-anchored primary operators, diagnostic-only existing data roles, materiality/noninferiority rules, and successor gates. Retained outputs provide 16 direct, one derived, one semantic-proof, and one partial operand; seven mechanism-critical operands remain missing. EB-04V–04X are admitted for bounded operator/observability diagnostics with prerequisites, not a result-bearing efficacy or promotion factorial. EB-04S nonpromotion/default-off remains unchanged. |
