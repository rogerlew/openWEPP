...
| **M** | **MOFEFID — MOFE fidelity campaign** | Four lanes on the closed MOFE routing foundation: (A) proactive MOFE defect-review sweep; (B) adjudication-first audit against the eleven defect classes in the wepp-forest stakeholder water-balance brief; (C) lateral-flow observed-authority rubric from `tests/fixtures/forest_lateral_flow_authority/`; (D) Papanicolaou 2018 OFE-by-OFE routing (space/time-variant resistance, TVD-MacCormack KWE). A-C are complete; Lane D is merged through D1-D8, subsurface seam design, solver/identity seam implementation, opt-in runtime SHADOW + real-H2637 executed vector, D9-D13 disposition/authority work, the rev-21 opt-in-shadow friction operand closure, rev-22 melt-limb source-shape closure, rev-23/rev-53 routed-hydrograph erosion-shape closure, D14 runtime profiling/optimization, D15 activation preflight, D10B hold-lift, D15 rerun hold, and D15 terminal-bin blocker resolution. | Strategy: [planning/mofe-fidelity-campaign-strategy.md](planning/mofe-fidelity-campaign-strategy.md). D15A ([work-packages/20260706-mofefid-d15-active-owner-optimization-001/package.md](work-packages/20260706-mofefid-d15-active-owner-optimization-001/package.md), executed 2026-07-06) closed both remaining D15 holds at `SC-OFEROUTE-001` rev 27: the H2637 opt-in timing was optimized `92.4 → 78.8 s` bit-identically and the residual regression adjudicated as contract-mandated (the D14 budget priced the pre-rev-24 defective scheme), and the OPT-IN ACTIVE OWNER is landed — `OPENWEPP_LANED_ACTIVE=1` routes production surface water (DC01-surface-disable + live double-feed guard, live day-closure hard-fails, routed erosion shape into the D13 consumer, rev-21 operand consumption), with the executed H2637 active endpoint at `37.4 s` and machine-exact day closures. | ▶️ **active 2026-07-06** — A-C complete; Lane D opt-in activation EXECUTED. Next actionable Lane D item is D16 default-promotion adjudication, plus the two named rev-27 follow-on gates: watershed-facing HBP outlet re-pointing (SC-RUNOFFPART/SC-ROUTE authority) and active-mode erosion water-magnitude coupling. |
