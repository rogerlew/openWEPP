...
| --- | --- | --- | --- |
| [`VEGETATION-SOURCE-PROVENANCE-AND-BOUNDARY-AUTHORITY`](work-packages/20260808-vegetation-source-provenance-and-boundary-authority-001/package.md) | Vegetation/ecohydrology architecture | `active / scaffolded authority package` | Produce the sanitized RHESSys process/state inventory, provenance and licensing dispositions, native-stratum contract, canopy/snow ownership boundary, and typed Stage A/B/C soil-water arbitration contract. Preserve current vegetation, ET, canopy, litter, and hydrology authority until the replacement boundaries are explicit. |
| `VEGETATION-RADIATION-INTERCEPTION-CONDUCTANCE-SLICE` | Vegetation/ecohydrology architecture | `queued / depends on vegetation boundary authority` | Create the separately testable `openwepp-vegetation` crate and implement one contract-authorized, default-off vertical slice for canopy radiation/interception, conductance, and layer-resolved water demand. The crate requests withdrawals and never mutates hydrologic soil state. |
| `LAND-SURFACE-ENERGY-CONSTITUTIVE-AUTHORITY` | Cross-domain land-surface architecture | `queued / depends on vegetation boundary authority` | Close `GAP-LANDSURFACEENERGY-001/002/003/006` with a snow-free ground/surface solver that consumes typed vegetation and hydrology handoffs. Keep canopy radiation, interception, wet-canopy evaporation, conductance, and transpiration owned by vegetation rather than embedding another ET model in the land-surface subsystem. |
