...
| **P** | **PERF - comprehensive array-native re-architecture (direct runtime)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing also taxed the default-disabled path (`701.95 s` vs `669.97 s`). PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. PERFDEEP07 improved but did not close that gate (`685.85 s` retained vs `<= 676.67 s`); PERFDEEP08 tested disabled diagnostic-hook caching and was slower (`691.93 s`). PERFDEEP09 closed the disabled-path blocker, R2A created the separate direct-runtime skeleton without phase math or publication cutover, R3A proved the first complete direct phase span, R3B proved a richer direct water-ledger span, R3C proved run-level multi-lane transfer/topology propagation, R4A proved the first narrow direct hydrology-process span for SC-RUNOFFPART runoff partition, R4B proved a downstream direct WB12 storage-reconciliation consumer of R4A runoff, R4C proved the direct storage-input producer feeding R4B while splitting storage direct-runtime code below the WARN band, R4D proved the direct WB18/WB12 deep-seepage handoff producer feeding R4B, R4E-H proved the remaining direct storage-budget handoff producers for `Qd`, aggregate ET, and signed snow coupling, R4I-L proved the remaining direct runoff-path input producers for liquid input, runon/carry, infiltration, depression storage, and saturation addback, R4M/O promoted the subsurface handoffs into direct WB18/WB19 compute for `D`, `Pe`, per-layer percolation fluxes, lateral `q`, tile drainage `Qdd`, final `Qd`, and carry diagnostics, and R4N promoted the aggregate ET handoff into direct WB17 evapotranspiration/root-uptake compute. | Execute grouped R4P/Q/Z: build direct aggregate storage recomputation, direct hydrology publication-projection shadow, full-chain no-compatibility proof, and R4 closure while preserving no-publication/no-default/no-scheduler boundaries. | **NEXT: R4P/Q/Z direct hydrology projection and R4 closure** - scaffold from `docs/work-packages/r4-burndown-execplan.md`; do not claim public WB13/WAT/PASS/loss cutover. |
