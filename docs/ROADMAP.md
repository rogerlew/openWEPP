...
| [`SNOW-STAGE3-PARADISE-WY2015-SUPPORT-RESOLUTION`](work-packages/20260807-snow-stage3-paradise-wy2015-support-resolution-001/package.md) | Snow surface-energy-balance campaign | `active / non-blocking diagnostic resolution` | Localize the 183 unmatched and 19 partial hours and `98.0757 MJ m^-2` omitted magnitude by date, term, operator status, and snow-state transition. Preserve the historical `0.0621730192 > 0.05` gate failure, but do not let the single support-censored row independently block persistence or carry a physical pass/fail claim. Admit noise/censoring only with direct cause evidence; do not retune the threshold. |
| `SNOW-STAGE3-PERSISTENT-ACCUMULATION-SHADOW` | Snow surface-energy-balance campaign | `held / evolving-state authority dependent` | Advance only after wind exposure/applicability, canopy relevance, raw/bounded vapor transfer, geometry, and energy/mass closure are resolved for the supported canonical evidence. Paradise WY2015 remains support-censored and cannot carry a physical pass/fail claim, but does not independently block persistence; Snowbird remains non-decisive. Then persist the admitted evaluation state across days with snowfall, liquid, restart, reappearance, and exact linked mass/energy custody; censor claims at unresolved terminal or receiving-surface boundaries. |
