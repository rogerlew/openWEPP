...
| --- | --- | --- | --- |
| `SNOW-STAGE3-TURBULENT-CARRIER-LINEAGE-AND-OPERATOR-RECONCILIATION` | Snow surface-energy-balance campaign | `next / contract and diagnostic package required` | Turbulent terms dominate the negative same-state failure, but causation of the positive independently reinitialized predecessor contradiction remains unresolved. Reconcile every external term—especially longwave, sensible, and latent—from byte-identical initial states/forcing while tracking state divergence; bind surface/air/vapor/wind/stability/exchange geometry and sign convention before persistence or correction. |
