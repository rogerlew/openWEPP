...
| **P** | **PERF — comprehensive array-native re-architecture (HillslopeDayFrame)** | **Incrementalism is abandoned** (PERFMIG01 +0.47%, PERFMIG02 REDIRECT — both net-negative because partial dual-representation bookkeeping dominates = PERFIDX05 ceiling). Operator-directed pivot to a **comprehensive array-native rewrite**: replace the symbol-keyed `BTreeMap` hot path with a typed dense `HillslopeDayFrame` that all 14 phases mutate in place — **phase-to-phase seams vanish by construction**, so the 108 µs seam tax that sank the rungs doesn't exist. Spec authored (commit `e2ca694b`): [`architecture/array-native-runtime-specification.md`](architecture/array-native-runtime-specification.md). Key finding: the I/O edge is ~5 HBP scalars (`wat`/`pass` parquet already typed), so the frame stays dense all-run. Model: ~14–20 µs/OFE-day ≈ 0.5× legacy. | Ratify **ADR-0025** (adopt the HillslopeDayFrame; supersedes ADR-0023's *incremental application*, not its dense-authority principle). Then execute the **`PERFDEEP0N`** staged series, each identity-gated (shadow-run `to_bits` diff + per-branch fixtures) and H2637 endpoint+RSS-measured: Stage 0 frame scaffold → Stage 1 hydrology island (first measured win) → Stage 2 close edges + falsification check → Stage 3 erosion → Stage 4 growth/decomp → Stage 5 delete the logical hot path (the ≤10×/≤5× measurement). | ⏭️ **NEXT — ratify ADR-0025, then scaffold `PERFDEEP01` (Stage 0 frame scaffold)**. The spec is the design authority; do **not** run another writeback-only or materialization-retirement rung (proven dead twice). PERFMIG02 production code was reverted; main is at the clean PERFMIG01 baseline (`6b54db2d`, 669.97 s). |
