...
[active-work locator](work-packages/active.md). The owner-authorized Stage-3
successor reached its frozen Phase-A boundary in the contract-first
[residual/Jacobian prototype](work-packages/20260908-stage3-residual-jacobian-prototype-001/artifacts/worker-handoff.md).
...
| --- | --- | --- | --- |
| [`STAGE3-RESIDUAL-JACOBIAN-PROTOTYPE`](work-packages/20260908-stage3-residual-jacobian-prototype-001/package.md) | Stage 3/LSE coupled-evaluation architecture | `terminal correction / INCONCLUSIVE_WITH_BOUND proposed / production HOLD` | Frozen Phase-A oracle admitted 0/16 active-stem columns, so J and downstream measurements were not run. Any successor must prospectively revise reference-oracle conditioning/precision authority and reject early below 12/16 admitted columns. |
| [`STAGE3-NATIVE-VEGETATION-LANED-WATERSHED-THROUGHPUT-RECOVERY`](work-packages/20260901-stage3-native-vegetation-laned-watershed-throughput-recovery-001/package.md) | Snow + native vegetation + Lane D runtime architecture | `executed / terminal HOLD / nonqualifying` | ADR-0044 replacement is active: the v33–v57 recovery cascade is deleted and one canonical solver remains. The prior 1/10/19 matrix, exact-workspace correctness, orchestrator warnings-denied Clippy, and release/performance qualification remain failed or unestablished. Execution continued beyond the first rejected HOLD through revision 61 and independently reviewed revision-31 component-dependency replay; both failed prospectively frozen release conjuncts and were fully reverted. Further progress requires owner authority to change the workload/budgets or select a materially different canonical solver/evaluation architecture. |
| [`STAGE3-AUTHENTIC-COVERAGE-MEMORY-ATTRIBUTION`](work-packages/20260904-stage3-authentic-coverage-memory-attribution-001/package.md) | Diagnostic coverage and process-memory attribution | `executed / TERMINAL HOLD / v31 discriminator closed; v61 historical evidence limitation` | Authentic topology and retained consumer path are established. Direct debugger capture of the exact historical v31 binary identifies the boundary-aware one-sided stencil shortfall (2,000 sweeps; zero `58/14/16/28` targets) with no collector/lifecycle loss. Revision-61 causal memory contrast remains unsupported because no authenticated equivalent candidate source/binary or matched treatment exists after automatic, manual, and reverse-cleanup recovery routes. R1 same-binary observer controls and bounded gates pass; no production mechanism or acceptance rule changed. No successor is authorized without new owner authority, exact candidate custody, matched memory attribution, parity, and full correctness gates. |
| [`NIX-AGENT-DEVBOX-FEASIBILITY`](work-packages/20260814-nix-agent-devbox-feasibility-001/package.md) | Developer infrastructure | `active / branch scaffolded / forest workload protected` | Build the pinned Nix dev shell and isolated Cargo/sccache layout on `ow-dev-01`; freeze the exact openWEPP commit after the current `forest` work lands; benchmark identical interactive, heavy-gate, and realistic concurrent workloads only while `forest` is idle; cut over primary agent development as soon as the evidence meets the package criteria, retaining `forest` as a noninteractive heavy-run fallback until decommission is separately authorized. |
| [`SNOW-STAGE3-PRODUCTION-CUTOVER-CAMPAIGN`](work-packages/20260819-snow-stage3-production-cutover-campaign-001/package.md) | Snow + vegetation + land-surface campaign | `held / throughput replacement authorized` | The [parent V11 covered-consumer/runner package](work-packages/20260821-snow-stage3-v11-covered-consumer-runner-closure-001/package.md) and its terminal research successors remain historical `EXECUTED HOLD`/`NO-GO` evidence. Owner direction on 2026-09-01 authorizes only the new throughput-recovery package above; it does not resume the old endpoint-root, LTE, DAE, Hermite, complementarity, frost-subtype, search-method, Child 3, receiver, or cutover continuations. Advance this campaign only after the replacement meets its non-CoE Stage 3, native-vegetation, Lane D, conservation, and long-run throughput gates. |
| `SNOW-STAGE3-REAL-CONSUMER-CUTOVER-QUALIFICATION` | Snow production-candidate qualification | `held / Child 3` | Start only after Children 1 and 2 pass. Run the actual scheduler and complete owner stack over representative full snow seasons, restart boundaries, routed water, output comparisons, resource budgets, and typed rejection census. CoE comparison is diagnostic, not a parity target. |
| [`SOIL-BIOGEOCHEMISTRY-TRANSFORMATIONS`](work-packages/20260811-soil-biogeochemistry-transformations-implementation-001/package.md) | Soil biogeochemistry | `queued / dependency authority bounded` | Admit and implement decomposition, immobilization, mineralization, nitrification, denitrification, leaching, and persistent litter/SOM transformations. This does not block the first snow-free integration shadow, but must precede multi-year calibration or forest C/N interpretation. |
| `RICHARDS-COUPLED-V1` | Soil--surface--plant hydrology replacement | `deferred / post-snow-cutover campaign` | Refresh the frozen phase inventory against current main, then author `SC-RICHARDS-001`. Require `CoupledAdaptiveSupportV1`, persistent Lane D state inside the same owner transaction, signed top-face exchange, staged vegetation coupling, adaptive common slabs, restartable controller state, and atomic interval commit; reject legacy fixed schedule, whole-day nonpersistent Lane D, and legacy R4L mutation tuples. |
| [`ASSURANCE-DRAFT-PUBLICATION-DEFECT-CLOSURE`](work-packages/20260809-assurance-draft-publication-defect-closure-001/package.md) | Assurance governance | `complete / dual terminal verified` | Root confinement was correct; external scratch passed 2,325/2,325 selected tests, both Rust reviews and both terminal verifiers pass, and the prompt is archived. |
| [`RHESSYS-EAST-COAST-VEGETATION-AUTHORITY-CLOSURE`](work-packages/20260809-rhessys-east-coast-vegetation-authority-closure-001/package.md) | Vegetation/ecohydrology evidence | `held / historical evidence retained` | V7 resolves the former missing-constitutive-cycle implementation premise. Retain the source-search and parameter/data-gap evidence for a future calibration-readiness campaign; it is not a blocker for the snow-free integration shadows. |
| `SNOW-COE-STAGE3-ATOMIC-CUTOVER` | Snow + vegetation + land-surface campaign | `held / Child 4 terminal increment` | Start only after Children 1--3 pass and the final-candidate `ASSURE-06` receives independent human scientific review and approval. Make Stage 3 the sole snow/melt owner and retire CoE generation in the same stable increment; no partial or dual-owner deployment is permitted. |
| [`ASSURANCE-SINGLE-APPROVER-DRAFT-RETURN`](work-packages/20260805-assurance-single-approver-draft-return-001/package.md) | Assurance governance | `executing / DRAFT returned / full rerun pending` | The one-approver language and typed immutable return are implemented; the later v130 predecessor-bridge source adoption now binds generation `7d1a3ba1`. Close after exact-head full correctness and its own terminal evidence pass. |
| [`JENNINGS-DRYAD-HOURLY-LFS` vendoring](work-packages/20260805-jennings-dryad-hourly-archive-lfs-vendoring-001/package.md) | External observed-data custody | `executing / awaiting independent review` | Exact LFS pointer/object, DOI/version/checksum, and deterministic current-consumer evidence pass; complete dual review and terminal verification without pushing the object. |
| [`ASSURE-06` snow/frost flagship authority refresh](work-packages/20260805-assure06-snow-frost-flagship-refresh-001/package.md) | Scientific assurance | `DRAFT / review authority returned` | The v130 predecessor-bridge source remains unpublished at generation `7d1a3ba1`; active review and approval roots are empty. A separately authorized, exact-subject human review entry is required before any review or approval can resume. Public report count is zero. |
| [`SNOW-COE-STAGE3-MELT-OWNER-AUTHORITY-RECONCILIATION`](work-packages/20260804-snow-coe-stage3-melt-owner-authority-reconciliation-001/package.md) | Snow surface-energy-balance campaign | `held / authority admitted / assurance DRAFT` | Stage 3 is admitted as the future sole melt owner; current CoE runtime remains byte-identical on implementation hold. The refreshed assurance source is governed DRAFT. Runtime implementation follows the revised evaluation-shadow sequence; cutover remains separately unauthorized. |
...
| `ASSURE-06` | Carry the refreshed snow/frost flagship from governed DRAFT through accountable human scientific review, approval, release transfer, and publication when separately authorized. | Enter an exact-subject human review only after the next science refresh selected by the report lead; then complete independent scientific and reproduction/publication review, finding disposition, three-role approval, release transfer, and publication gates. | `DRAFT / review authority returned` under [`20260805-assure06-snow-frost-flagship-refresh-001`](work-packages/20260805-assure06-snow-frost-flagship-refresh-001/package.md); generation `7d1a3ba1` is current and public report count remains zero. |
