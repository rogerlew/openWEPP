...
|---|---|---|---|---|
| **INTVAL-20260713** | **Integrated production validation campaign** | Execute one fixed-source end-to-end matrix after CQR `GO-INTEGRATED-VALIDATION`: native hillslope hydrology/groundwater, p61/p102 erosion and MOFE continuity, production snow/frost, sediment-active watershed serial/parallel publication, two-channel hourly water/sediment/baseflow routing, fail-closed inputs, and release/required-authority lanes. | Follow [the integrated-validation package](work-packages/20260713-integrated-validation-campaign-001/package.md). Require real consumers, independent water/sediment reconstruction, rejected-alias proof, hashes, comparator-as-flag review, release/workspace gates, dual review, and dual verification. Semantic failure opens a separate DC-ExecPlan and forces a fixed-source restart. | **executed HOLD 2026-07-13 — `INTVAL-REL-001`** — all focused domain/consumer lanes passed, but the default release script's threaded workspace libtest violated the H2637 nextest-only selector isolation contract and failed three guards. Execute [the bounded release-isolation DC package](work-packages/20260713-dc-intval-release-nextest-isolation-001/package.md), then restart the entire campaign at its correction commit. |
