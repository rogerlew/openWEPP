...
| **WSHED-W11A** | **Channel-hourly sediment sequencing authority** | Lift `WSHED-W11-HOLD-001` by acquiring and ratifying, or explicitly rejecting, canonical authority for time-resolved WS18-WS26 channel sediment routing coupled to routed channel water series. | Temporal quantum; water/sediment coupling; channel geometry/profile/bed state carry; per-class ingress/egress and detachment/deposition/storage closure; guards/tolerances/test vectors; dual review/verification; exact W11 implementation handoff. | ✅ **complete 2026-07-10 UTC (`EXECUTED-COMPLETE-AUTHORITY`; Codex cycle-2 re-confirmation pending)** — amended `SC-ROUTE-001` v51-v53: per-`dtchr`-interval quasi-steady sequencing (`INV-ROUTE-015..020`, biconditional lane activation, unique hydraulic-profile operand map with the distinct `qlat(it)` wave-total / derived `qlat_eff(it)` per-unit-length symbols, monotonic geometry carry, pinned-`dcap.for` widening realization with `t_exp`/`t_norm` operands, per-interval/day class mass closure, eleven contract-derived vectors) on convergent WEPP/CREAMS + KINEROS ARS-77 + HEC-RAS quasi-unsteady authority (references vendored); dual review (21 findings) + dual verification passed; two Codex post-hoc REOPEN cycles dispositioned same day (v52: five closure amendments incl. `GAP-ROUTE-014` — the two migrated-lane terminals W11 must correct before interval reuse; v53: the total-vs-per-length lateral operand fix). Package: [work-packages/20260710-wshedw11a-channel-hourly-sediment-authority-001/package.md](work-packages/20260710-wshedw11a-channel-hourly-sediment-authority-001/package.md). |
