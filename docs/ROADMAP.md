...
PERFDEEP07's `685.85 s`, and was reverted.
...
| **P** | **PERF - comprehensive array-native re-architecture (HillslopeDayFrame)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing now also taxes the default-disabled path (`701.95 s` vs `669.97 s`). The ratified runtime spec treats more compatibility-edge shaving as insufficient, and PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. PERFDEEP07 then improved but did not close that P0 gate (`685.85 s` retained vs `<= 676.67 s`); PERFDEEP08 tested disabled diagnostic-hook caching and was slower (`691.93 s`). | Before any R2+ direct-frame runtime implementation, first profile or micro-benchmark the retained default path and close the disabled-path hold with a concrete zero-cost-when-disabled proof, identity, and a three-run H2637 median `<= 676.67 s`; otherwise explicitly supersede the hold with a ratified performance/architecture decision. The completed R0/R1 planning package provides the direct schema envelope, type-boundary decision, constructor/projection plan, publication-ledger promotion plan, and no-compatibility proof plan only. | **BLOCKED / NEEDS ATTRIBUTION** - PERFDEEP08 package `work-packages/20260619-perfdeep08-disabled-path-hard-isolation-001/` is `HOLD`; candidate reverted. R2+ runtime implementation is still blocked. Next work should profile or micro-benchmark the retained default path before another cleanup patch. |
