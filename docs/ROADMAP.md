...
| **P** | **PERF - comprehensive array-native re-architecture (HillslopeDayFrame)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing also taxed the default-disabled path (`701.95 s` vs `669.97 s`). PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. PERFDEEP07 improved but did not close that gate (`685.85 s` retained vs `<= 676.67 s`); PERFDEEP08 tested disabled diagnostic-hook caching and was slower (`691.93 s`). PERFDEEP09 closed the disabled-path blocker by collapsing repeated perennial decomposition indexed-overflow scans. | Execute R2A from the completed R0/R1 planning envelope: direct-frame type namespace, no-op/shadow direct executor skeleton selected once at setup, executable no-compatibility proof hooks, and PERFDEEP09 default-disabled regression gate. Do not port phase math or cut over publication in R2A. | **READY: R2A scaffolded** - package `work-packages/20260619-r2a-direct-runtime-skeleton-001/` is queued. PERFDEEP09 final default-disabled H2637 median `635.65 s` cleared `<= 676.67 s`; R2A must preserve that gate while creating the direct skeleton. |
