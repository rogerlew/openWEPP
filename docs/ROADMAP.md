...
| [`ROOT-ZONE-HYDRAULIC-OWNER-AUTHORITY`](work-packages/20260819-root-zone-hydraulic-owner-authority-001/package.md) | Soil/plant hydraulic boundary | `held / exact input and constitutive authority missing` | Admit and populate a distinct per-occupancy/per-layer root-tissue path plus current-unsaturated conductivity relation/parameters. `lateral_root_length_m`, WB14 suction/conductivity, fixture values and caller templates are forbidden substitutes. |
| [`ROOT-ZONE-HYDRAULIC-OWNER-IMPLEMENTATION`](work-packages/20260819-root-zone-hydraulic-owner-implementation-001/package.md) | Cross-domain land-surface architecture | `queued / blocked on authority release` | Implement only after the root-zone authority receives complete contract profile, vectors, reviews and dual terminal PASS. No selector/default/output activation is authorized. |
| [`SNOW-FREE-LAND-SURFACE-REAL-HYDROLOGY-INTEGRATION`](work-packages/20260814-snow-free-land-surface-real-hydrology-integration-001/package.md) | Cross-domain land-surface architecture | `active HOLD / root-zone hydraulic owner authority open` | Children 1--3, forcing and persisted restart are complete. Child 4 cannot remove caller hydraulic physics until the explicit root-path and current-unsaturated conductivity owner is released and implemented. Production remains unchanged. |
