...
| --- | --- | --- | --- |
| [`SNOW-STAGE3-EVALUATION-SHADOW-AUTHORITY`](work-packages/20260806-snow-stage3-evaluation-shadow-authority-001/package.md) | Snow surface-energy-balance campaign | `active / contract-first` | Amend `SC-SNOWFREEZE-001#INV-SNOWFREEZE-091` to distinguish a declared, tagged, default-off, consumer-forbidden evaluation shadow from a prohibited undeclared second production mass state. Freeze same-state carrier and sequential resolved-shadow operators, claim boundaries, custody, restart prohibition, and no-cutover semantics before implementation. |
| `SNOW-STAGE3-SHADOW-SOLVER-EXTRACTION-AND-OBSERVABILITY` | Snow surface-energy-balance campaign | `queued / mechanical then behavior-neutral` | First extract the shadow solver so `runoff_reconciliation.rs` falls below `3,000` lines with exact behavior retained. Then expose shadow-specific shortwave, longwave, sensible, latent, precipitation-advection, conduction, vapor/cold-content export, available-ice, total, residual, and coverage operands through the declared evaluation surface. |
| `SNOW-STAGE3-FOUR-SITE-CARRIER-TERM-AUDIT` | Snow surface-energy-balance campaign | `queued / prospective result-bearing` | Before execution, freeze identical paired-window support, aggregation, literature comparison ranges, coverage rules, and no-tuning disposition. Independently reconstruct term sums and distributions at Mica Creek, Niwot, Paradise, and Snowbird; separate internal active/lower conduction from the external surface/full-column budget and report the absent snow-ground boundary explicitly. |
| `SNOW-STAGE3-PERSISTENT-ACCUMULATION-SHADOW` | Snow surface-energy-balance campaign | `held / audit and authority dependent` | Advance only if the four-site carrier audit is physically interpretable. Persist the admitted evaluation state across days with snowfall, liquid, restart, reappearance, and exact linked mass/energy custody; censor claims at unresolved terminal or receiving-surface boundaries. Reevaluate the frozen melt/peak-date directions without tuning. |
...
| [`ASSURANCE-SINGLE-APPROVER-DRAFT-RETURN`](work-packages/20260805-assurance-single-approver-draft-return-001/package.md) | Assurance governance | `executing / DRAFT returned / full rerun pending` | The one-approver language and typed immutable return are implemented; the later v127 evaluation-shadow refresh now binds generation `34f2f80e`. Close after exact-head full correctness and its own terminal evidence pass. |
...
| [`ASSURE-06` snow/frost flagship authority refresh](work-packages/20260805-assure06-snow-frost-flagship-refresh-001/package.md) | Scientific assurance | `DRAFT / review authority returned` | The v127-refresh source remains unpublished at generation `34f2f80e`; active review and approval roots are empty. A separately authorized, exact-subject human review entry is required before any review or approval can resume. Public report count is zero. |
