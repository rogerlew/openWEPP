...
stale-logical removal outweighed six avoided inserts. Deep migration remains authorized; the next rung
should pivot to single-phase array-native read+compute+write.
...
| **P** | **PERF — comprehensive array-native re-architecture (HillslopeDayFrame)** | **Incrementalism is abandoned** (PERFMIG01 +0.47%, PERFMIG02 REDIRECT — both net-negative because partial dual-representation bookkeeping dominates = PERFIDX05 ceiling). Operator-directed pivot to a **comprehensive array-native rewrite**: replace the symbol-keyed `BTreeMap` hot path with a typed dense `HillslopeDayFrame` that all 14 phases mutate in place — **phase-to-phase seams vanish by construction**, so the 108 µs seam tax that sank the rungs doesn't exist. Spec authored (commit `e2ca694b`): [`architecture/array-native-runtime-specification.md`](architecture/array-native-runtime-specification.md). Key finding: the minimal HBP edge is ~5 scalars, but WB13/WAT/PASS publication assembly still reads runtime-surface operands and must be lifted to typed publication projection in Stage 0/1 before logical hot-path deletion. Model hypothesis: ~14–20 µs/OFE-day ≈ 0.5× legacy, pending staged endpoint measurement. | Ratify **ADR-0025** (adopt the HillslopeDayFrame; supersedes ADR-0023's *incremental application*, not its dense-authority principle). Then execute the **`PERFDEEP0N`** staged series, each identity-gated (shadow-run `to_bits` diff + per-branch fixtures) and H2637 endpoint+RSS-measured: Stage 0 frame scaffold → Stage 1 hydrology island lifecycle fix and measured default activation → Stage 2 close edges + falsification check → Stage 3 erosion → Stage 4 growth/decomp → Stage 5 delete the logical hot path (the ≤10×/≤5× measurement). `PERFDEEP01` must also deliver a publication-operand lineage ledger, dynamic-guard parity policy (static + runtime-derived bounds), and kernel-contract transition compatibility map before Stage 1 authority cutover. | 🚧 **`PERFDEEP02` NO-GO (2026-06-19, commit `fa29c34b`) — kept opt-in as a verified negative benchmark; `PERFDEEP03` scaffolded, Codex-ready.** PERFDEEP02 regressed H2637 to **2417 s (3.6×)**: it kept logical/indexed surfaces as the real runtime state and rebuilt a full-registry (~4038-slot) `Vec<Option<BoundaryValue>>` frame **per OFE-day** — a temporary dense mirror (PERFIDX dual-representation ceiling in frame form). Identity-clean; deferred PERFDEEP01 round-trip gate **closed** (235,961 rows, 0 mismatch); fail-closed behind `OPENWEPP_PERFDEEP02_FRAME_ISLAND=1`. **`PERFDEEP03` (`20260619-perfdeep03-persistent-lane-owned-dense-state-001/`) moves OWNERSHIP**: the lane runtime owns a *persistent* dense frame (hot working set, forcing borrowed), scheduler phases borrow views, materialize to logical only at true boundaries — reusing PERFDEEP02's verified mechanics. **Hard gate: opt-in dense path must BEAT 669.97 s on the real H2637 endpoint** (not a microbenchmark) before default activation; spec §4.6/§8 amended. Do **not** run another writeback-only/materialization-retirement or temporary-mirror rung. |
