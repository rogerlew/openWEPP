...
| --- | --- | --- | --- |
| [`SNOW-STAGE3-WIND-SOURCE-CUSTODY-AND-EXPOSURE-AUTHORITY`](work-packages/20260807-snow-stage3-wind-source-custody-and-exposure-authority-001/package.md) | Snow surface-energy-balance campaign | `complete / provider custody recovered / persistence HOLD` | Raw CLI wind reaches Stage 3 unchanged; PMET-local 2 m wind does not. Surviving records directly prove byte-identical CLI lineage, retained centroids/GRIDMET flags, and parquet-to-CLI equality. Nearby source only reconstructs request/product/shared-series semantics. Deployed request/server pixel-version-time semantics and forest-exposure linkage remain absent, so every site is still `AUTHORITY_MISSING`. No fitted attenuation, canopy operator, production correction, persistence, or cutover. |
| [`SNOW-STAGE3-PARADISE-WY2015-SUPPORT-RESOLUTION`](work-packages/20260807-snow-stage3-paradise-wy2015-support-resolution-001/package.md) | Snow surface-energy-balance campaign | `complete / support cause localized / non-blocking diagnostic` | Exact retained traces show 19 partial sequential hours terminate at `post_substep_no_resolved_surface`, followed by 183 same-state-only hours whose sequential status is `thin_pack_boundary_reached`. The `98.0757 MJ m^-2` omission and historical `0.0621730192 > 0.05` failure remain exact. This explains support topology, not physical correctness; no threshold, persistence gate, physics, or cutover changed. |
| [`SNOW-STAGE3-PERSISTENT-ACCUMULATION-SHADOW`](work-packages/20260807-snow-stage3-persistent-accumulation-shadow-001/package.md) | Snow surface-energy-balance campaign | `complete / continuity mechanics implemented / physical efficacy held` | Default-off per-lane persistence, snowfall/liquid custody, dormancy/reappearance, fingerprinted restart, and schema-v7 mass/energy reconstruction are implemented without production reachability. Wind/exposure, canopy, geometry, magnitude, terminal-recipient, physical-validation, promotion, ownership, and cutover claims remain held. |
| [`SNOW-TERMINAL-ENTHALPY-EVENT-NUMERICS`](work-packages/20260807-snow-terminal-enthalpy-event-numerics-001/package.md) | Snow and land-surface numerics | `executing / contract-first` | Admit and implement an error-controlled shallow-snow enthalpy solve and localized solid-exhaustion event independently of the receiving land-surface subsystem. Event-local snow ledger closure does not authorize post-snow energy disposition. |
