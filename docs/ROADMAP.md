...
| `QUALITY-OBSERVATORY-01` merged measurement and optional forest1 QA | Test/gate architecture | `complete` | Orders 3-4 completed: merged `full` plus `science-manual` evidence and the TESTGATE-first manual forest1 workflow passed dual verification. |
| `QUALITY-CQR-HANDOFF-01` exact QA evidence consumption | Test/gate architecture | `complete` | Order 5 completed exact current evidence intake and typed explicit recollection authorization with dual verification. |
| `TESTGATE-QUALITY-QUALIFY-01` functional workflow qualification | Test/gate architecture | `next` | Order 6 changed-head TESTGATE qualification passed; execute Order 7 optional QA and selection-only exact-report CQR intake. |
