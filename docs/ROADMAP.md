...
| --- | --- | --- | --- |
| [`RHESSYS-EAST-COAST-CODE-LITERATURE-AUTHORITY-AUDIT`](work-packages/20260808-rhessys-east-coast-code-literature-authority-audit-001/package.md) | Vegetation/ecohydrology scientific authority | `active / precursor audit` | Audit the vegetation-relevant source/call/state closure and all 71 GIS2RHESSys fields/32 profiles against primary literature, units, domains, physical invariants, and openWEPP ownership. Close admissible authority gaps, explicitly block unresolved required rows, and amend the coupled successor before Rust work. |
| [`RHESSYS-EAST-COAST-COUPLED-VEGETATION-SLICE`](work-packages/20260808-rhessys-east-coast-coupled-vegetation-slice-001/package.md) | Vegetation/ecohydrology architecture | `queued / depends on authority audit` | Begin only after the precursor passes dual review/verification, amends the exact coherent boundary, and leaves no required `BLOCK_SUCCESSOR` row. Then implement existing vegetation-file compatibility and deciduous, evergreen, and explicit mixed multistratum behavior through contract-first gates. |
