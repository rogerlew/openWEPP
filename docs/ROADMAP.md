...
fail-closed; the next cut is the PERFDEEP06 fast-path inventory/API planning
gate: enumerate the complete typed H2637 hot-loop frame, publication operands,
direct-frame APIs, and layout/allocation risks before any direct-frame
...
| **P** | **PERF - comprehensive array-native re-architecture (HillslopeDayFrame)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, so the remaining measured boundary is the partial dense island's cached daily refresh, logical dense writeback apply, symbol lookup, and flush edge. The ratified runtime spec now treats more seam shaving as insufficient; the next step is a Stage-3 fast-path inventory/API planning gate. | Execute PERFDEEP06 to enumerate the complete typed H2637 hot-loop frame, publication operands, persistent lane state, borrowed forcing, direct-frame phase APIs, layout/type-size and allocation risks, no-hot-loop-map proof, and follow-on direct-frame package sequence. No default activation until a future opt-in implementation beats `669.97 s` and identity/full gates pass. | **READY: PERFDEEP06 scaffolded** - package `work-packages/20260619-perfdeep06-array-native-fast-path-inventory-001/` is queued as the planning gate before PERFDEEP07 direct-frame implementation. Keep PERFDEEP02/03/05 opt-ins fail-closed; do not revert to full sync or expand blindly. |
