...
| --- | --- | --- | --- |
| [`SNOW-SURFACE-EB`](planning/snow-surface-energy-balance-roadmap.md) additive sublimation and sub-canopy longwave | Snow/frost science | `active` | Execute EB-03 contract-first: select the coherent hourly snow/canopy temperature and cold-content provider, bind polar-night cloud behavior and `R_a,min`, and compose exact-one sublimation mass/latent energy with the shared `B/L/S/LS` carrier. EB-02 contract authority is complete; runtime longwave remains `HOLD` until EB-03 closes those interfaces. |
