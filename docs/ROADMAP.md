...
superseded. PERFDEEP08 is scaffolded as the next narrow hard-isolation package:
close the default-disabled gate first, or hold with a precise remaining
blocker.
...
| **P** | **PERF - comprehensive array-native re-architecture (HillslopeDayFrame)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing now also taxes the default-disabled path (`701.95 s` vs `669.97 s`). The ratified runtime spec treats more compatibility-edge shaving as insufficient, and PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. PERFDEEP07 then improved but did not close that P0 gate (`685.85 s` retained vs `<= 676.67 s`). | Execute PERFDEEP08 as a hard-isolation package before any R2+ direct-frame runtime implementation. It must close the default-disabled hold with zero-cost-when-disabled proof, identity, and a three-run H2637 median `<= 676.67 s`, or hold with the remaining blocker named. The completed R0/R1 planning package provides the direct schema envelope, type-boundary decision, constructor/projection plan, publication-ledger promotion plan, and no-compatibility proof plan only. No default activation until a future opt-in implementation beats `669.97 s` and identity/full gates pass. | **READY: PERFDEEP08 scaffolded** - package `work-packages/20260619-perfdeep08-disabled-path-hard-isolation-001/` is queued. PERFDEEP07 remains `HOLD`; R2+ runtime implementation is still blocked until PERFDEEP08 closes or a supersession decision lands. |
