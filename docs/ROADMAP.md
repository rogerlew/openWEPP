...
delta to day-2 `Es`, `Total-Soil`, and `SoilWaterTotal`. R6H is now
scaffolded at
...
to replace the precomputed PMET day-input vector with an interleaved direct
day-input builder, prove lane-dimensional direct input authority, resolve WAT
id semantics, add allowlisted direct symbol lineage, and close WAT parity
without compatibility-row wrapping.
