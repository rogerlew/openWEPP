...
| **W** | **R7 terminal cleanup: compatibility runtime deletion** | The ADR-0026 winter-column sub-solver, frost observed-data ratification/default activation, and the direct cutover correction are complete. ADR-0030 amends the R7 terminal contract: compatibility frost bit-parity is no longer the acceptance target, direct production is the no-env hillslope default, and silent compatibility fallback is a defect. ADR-0031 supersedes the retained-seam clause and authorizes full deletion. | Delete obsolete skeleton/shadow/cutover transition modes, remove the public `--compatibility-runtime` selector, and delete production hot-loop symbol-map machinery under no-regression/static-proof gates. | **PARTIAL — public seam removed; symbol-surface support boundary still held.** `20260630-compatibility-runtime-deletion-001` removed obsolete skeleton/shadow/cutover transition modes; the direct-publication RSS packages made the endpoint run-length-flat (1.13 GiB → 110/51 MiB, byte-identical); `20260630-typed-day-zero-seed-computation-001` cut production seeding to typed `DirectProductionSeedAuthority`; and `20260630-compatibility-runtime-full-deletion-001` ratified ADR-0031 and removed the public `--compatibility-runtime` API/CLI/harness selector. Full deletion of `scheduler.rs`, `day_frame.rs`, and carrier types is held because the remaining symbol-keyed scheduler lifecycle, WB13 scheduler publication, HPHYS trace, audit/shadow, and legacy scheduler tests are still compiled as a support surface. The next package must delete or replace that support boundary coherently, preserving only documented I/O serialization adapters. |
