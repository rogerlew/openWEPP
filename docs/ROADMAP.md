...
| --- | --- | --- | --- |
| [`SNOW-SURFACE-EB`](planning/snow-surface-energy-balance-roadmap.md) additive sublimation and sub-canopy longwave | Snow/frost science | `EB-04X next` | EB-04W completed behavior-neutral accumulation/melt observability and 16 B/L/S/LS cells over four open mountain lanes. All five chronology failures have large storage deficits at the observed SWE peak, but ownership remains mixed among the realized input pathway, liquid retention, and pre-peak modeled loss. No calibration or promotion is admitted. EB-04X next isolates Harvard paired hardwood/open geometry and interception. |
