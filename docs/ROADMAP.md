...
| --- | --- | --- | --- |
| `ASSURE-MAINT-01` mechanical amendments | Scientific assurance maintenance | `next` | Implement the typed amendment transaction, generated identity locks, acyclic review roots, receipt-driven gates, and measured fast path under `20260716-assure-maint01-mechanical-amendment-workflow-001` before another review freeze, `IN_REVIEW` attribution/role amendment, or embedded-hash report admission. Scientific drafting and ongoing independent human review may continue. |
...
| --- | --- | --- | --- |
| `ASSURE-MAINT-01` | Make bounded attribution, role, lifecycle, and normalization revisions transactional, mechanically identity-bound, and proportionally gated without agent-maintained hashes or unnecessary scientific reruns. | Generated-lock migration, acyclic-root/event and typed-transaction contracts, no-package/no-agent focused receipts, fast-lane timing limits, full one-time implementation closure, fresh CRAP, dual review, and dual terminal verification pass without changing report science or public state. | `next`; canonical specification and queued implementation package are scaffolded. |
