...
| --- | --- | --- | --- |
| [`SNOW-SURFACE-EB`](planning/snow-surface-energy-balance-roadmap.md) additive sublimation and sub-canopy longwave | Snow/frost science | `active` | Scaffold EB-02 canonical-contract research to bind a deterministic radiometric sky-view operator derived from existing canopy state, Dilley-Unsworth cloud mapping, effective-unity exchange, and the active snow-temperature provider; require no new user sky-view coefficient or remote-sensing dataset. EB-02 runtime implementation remains `HOLD` until those prerequisites are contract-bound. EB-03 exact-one vapor/latent composition may proceed in parallel. |
