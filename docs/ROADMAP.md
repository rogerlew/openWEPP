...
| --- | --- | --- | --- |
| `SNOW-STAGE3-LEGACY-PREDECESSOR-BRIDGE-RECONCILIATION` | Snow surface-energy-balance campaign | `next / required before persistence` | Reproduce the frozen Snowbird schema-v5 `+170.2536089 MJ m^-2` legacy estimator from byte-identical daily source and forcing, isolating daily reset cadence, first projection, substep transition, and active-conduction lineage against schema v6's `+188.8725288 MJ m^-2`. Only after the bridge closes may the campaign adjudicate the four-site `S<0, F<0, Q>0` state-evolution pattern. No fitting, physics change, persistence, promotion, or ownership change. |
