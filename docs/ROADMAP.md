...
| **P** | **PERF - comprehensive array-native re-architecture (direct runtime)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing also taxed the default-disabled path (`701.95 s` vs `669.97 s`). PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. PERFDEEP07 improved but did not close that gate (`685.85 s` retained vs `<= 676.67 s`); PERFDEEP08 tested disabled diagnostic-hook caching and was slower (`691.93 s`). PERFDEEP09 closed the disabled-path blocker, R2A created the separate direct-runtime skeleton without phase math or publication cutover, R3A proved the first complete direct phase span, R3B proved a richer direct water-ledger span, R3C proved run-level multi-lane transfer/topology propagation, R4A through R4P/Q/Z closed the direct hydrology path through shadow-only projection, R5A completed full-day direct executor lifecycle, R5B promoted direct normalization/storage-bounds phase calls, R5C promoted direct decomposition/residue phase calls, and R5D promoted direct annual/perennial growth phase calls while preserving no-publication/no-default/no-scheduler boundaries. | Execute R5 full OFE-day direct path / endpoint activation readiness from [r5-burndown-execplan.md](work-packages/r5-burndown-execplan.md): integrate the completed R4 direct hydrology projection into a full direct-day execution surface, prove opt-in endpoint behavior and no-compatibility call graph/runtime counters, and decide whether R6 publication cutover is ready or blocked. | **ACTIVE: R5E full 14-phase direct endpoint readiness and R5 closure** - scaffold and execute the R5E package from `docs/work-packages/r5-burndown-execplan.md`; do not claim public WB13/WAT/PASS/loss cutover until an R6 package authorizes it. |
