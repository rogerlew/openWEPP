...
| --- | --- | --- | --- |
| [`SNOW-SURFACE-EB`](planning/snow-surface-energy-balance-roadmap.md) additive sublimation and sub-canopy longwave | Snow/frost science | `EB-04V complete; EB-04W next` | EB-04V now publishes a behavior-neutral density-process ledger through the real JSONL consumer and closes all 36 diagnostic cells to `3.411e-13 kg m^-3`. Wet compaction is active and largest in both retained bias groups, while mixing, structure, and caps oppose it; the result resolves observability but not a uniquely identifiable coefficient or efficacy claim. EB-04W next investigates the five open-control mountain under-persistence failures. Existing observations remain diagnostic-only and EB-04S nonpromotion/default-off is unchanged. |
