...
| --- | --- | --- | --- |
| [`SNOW-STAGE3-WIND-SOURCE-CUSTODY-AND-EXPOSURE-AUTHORITY`](work-packages/20260807-snow-stage3-wind-source-custody-and-exposure-authority-001/package.md) | Snow surface-energy-balance campaign | `complete / authority reconciled / persistence HOLD` | Raw CLI wind reaches Stage 3 unchanged; PMET-local 2 m wind does not. Nominal GRIDMET 10 m source height is now distinct from virtual Stage 3 5 m geometry. Retained generator/pixel/transform and forest-exposure linkage are absent, so every site is `AUTHORITY_MISSING`. Independent neutral bounds are 10.077% and 21.170%; no fitted attenuation, canopy operator, production correction, persistence, or cutover. |
