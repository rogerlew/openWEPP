...
| **M** | **MOFEFID — MOFE fidelity campaign** | Four lanes on the closed MOFE routing foundation: (A) proactive MOFE defect-review sweep; (B) adjudication-first audit against the eleven defect classes in the wepp-forest stakeholder water-balance brief; (C) lateral-flow observed-authority rubric from `tests/fixtures/forest_lateral_flow_authority/`; (D) Papanicolaou 2018 OFE-by-OFE routing (space/time-variant resistance, TVD-MacCormack KWE). A-C are complete; Lane D is merged through D1-D8, subsurface seam design, solver/identity seam implementation, opt-in runtime SHADOW + real-H2637 executed vector, D9-D13 disposition/authority work, the rev-21 opt-in-shadow friction operand closure, rev-22 melt-limb source-shape closure, rev-23/rev-53 routed-hydrograph erosion-shape closure, D14 runtime profiling/optimization, D15 opt-in activation, D16 selected-cohort materialization/active-suite blockers, ADR-0037 hybrid abandonment/removal, the Tier-2 target-`dx` mesh-policy rescope, the rev-41 WA active-router positivity-preserving solver correction, the rev-41 Tier-2 re-adjudication, the `dx0p625` fine-reference hold-lift, the `mn_corn_h4` day-792 shape attribution hold, the day-792 raw-hydrograph CFL timestep-transition hold, the active-router timestep-policy adjudication (`SC-OFEROUTE-001` rev 43), and the coupled space-time `dx5` ratification hold. | Strategy: [planning/mofe-fidelity-campaign-strategy.md](planning/mofe-fidelity-campaign-strategy.md). D15A ([work-packages/20260706-mofefid-d15-active-owner-optimization-001/package.md](work-packages/20260706-mofefid-d15-active-owner-optimization-001/package.md), executed 2026-07-06) landed the opt-in ACTIVE OWNER at `SC-OFEROUTE-001` rev 27. ADR-0037 ([decisions/0037-abandon-hybrid-implicit-stepping.md](decisions/0037-abandon-hybrid-implicit-stepping.md)) abandoned the hybrid implicit stepper and retained plain Lane D active routing as the surviving production path. The coupled `dx5` ratification package ([work-packages/20260708-laned-router-tier2-dx5-coupled-spacetime-ratification-001/package.md](work-packages/20260708-laned-router-tier2-dx5-coupled-spacetime-ratification-001/package.md)) held before production promotion: `dx5` candidate comparisons pass, but the fine-reference basis remains open on `mn_corn_h4` shape and WA refined-75 annual sediment. | ⏭️ **next** — scaffold a narrow WA annual pass-sediment fine-reference adequacy attribution package (`wa_cascades_forest_h1`, `dx2p5_dt75` vs `dx1p25_dt75`, annual sediment max relative `0.022131684 > 0.0066666667` on `tdep:4`). Do not promote `dx5` or widen tolerances until that miss is attributed and the fine-reference basis closes. |
