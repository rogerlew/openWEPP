...
| **W** | **R7 terminal cleanup: compatibility runtime deletion** | The ADR-0026 winter-column sub-solver, frost observed-data ratification/default activation, and the direct cutover correction are complete. ADR-0030 amends the R7 terminal contract: compatibility frost bit-parity is no longer the acceptance target, direct production is the no-env hillslope default, and silent compatibility fallback is a defect. | Delete obsolete skeleton/shadow/cutover transition modes and production hot-loop symbol-map machinery under no-regression/static-proof gates; retain explicit `--compatibility-runtime` as deprecated replay-only seam until a later full-deletion package. | **PARTIAL — transition-mode deletion + RSS arc COMPLETE (merged to `main` `5b139058`, full-suite green).** `20260630-compatibility-runtime-deletion-001` removed the obsolete skeleton/shadow/cutover transition modes; `20260630-direct-publication-rss-reduction-001` + `20260630-direct-publication-streaming-sink-001` made the direct endpoint **run-length-flat** (1.13 GiB → 110/51 MiB, byte-identical; the dominant cause was a 909 MiB per-day×OFE constructor pre-alloc, not the symbol-map carrier). **NEXT (the full compat deletion):** make the direct input/setup/seeding path **typed end-to-end** to eliminate the symbol-map `runtime_surface` carriers (`scheduler.rs` ~3177 lines + `HillslopeWritebackSurface`/`HillslopeKernelRequest`/`SymbolRegistry` across ~29 orchestrator files), then delete the symbol-map runtime, retaining only the explicit `--compatibility-runtime` replay seam. **This is a single-authority RE-ARCHITECTURE, not a clean deletion** (corrected 2026-06-30): two incremental attempts (`...elimination-001`, `...carrier-deletion-001`, `...seed-authority-migration-001`) held after migrating 1 seed read (208→207), because the day-zero seed authority is a **computed ordered pipeline** (WB11 day-zero projection → soil → ET → plant → stress → snow/frost) producing the `HillslopeWritebackSurface`, not independent reads that can be peeled off. The durable end state (ADR-0025) is the typed `DirectRunFrame` as the **sole authority from parse to output**. Approach = **one shadow-first re-architecture**: build a typed per-lane seed-authority carrier (typed WB11 day-zero + typed seed state), shadow-prove identical seed values, **cut the whole production seeding over at once** (all 207 reads together), then delete the symbol-map seed surface + `scheduler.rs` + carriers (compat survives only as the explicit comparator seam). Gated on byte-identity at cutover + full suite + a **≤5× re-measure** (the most likely remaining lever; setup overhead is paid per-hillslope, so it is multiplicative at watershed scale). |
...
(1.13 GiB → 110/51 MiB, byte-identical). Explicit `--compatibility-runtime`
remains only as a deprecated replay seam. The remaining forward item is the
**full compatibility-runtime deletion**: making the direct setup/seeding path
typed end-to-end to eliminate the symbol-map `runtime_surface` carriers, then
