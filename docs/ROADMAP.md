...
costs about **108 us/payload**. Deep migration remains authorized; the next rung should migrate a
contiguous WB11-consumer cluster so that boundary moves outward.
...
| **P** | **PERF — array-authoritative production migration** | PERFARCH03 closed the missing floor measurement: a full array-native WB11 runoff branch hot loop measured **0.959423 us/OFE-day** (`0.024823x` legacy us/OFE-day), with boundary materialization measured separately. PERFMIG01 ratified ADR-0023 and migrated the production WB11 warm-rain writeback branch to dense `SymbolId` authority with exact identity. | Migrate the **readers** of the WB11 outputs to dense `SymbolId` reads so the PERFMIG01 dense→logical apply-boundary (`107.5 us/payload`) **retires** (not relocates); remeasure H2637 endpoint + RSS; preserve exact identity. The hard gate is now a **measured endpoint improvement**, since writeback-only is net-negative by construction. | ⏭️ **NEXT — `PERFMIG02` scaffolded, Codex-ready** (`20260618-perfmig02-wb11-consumer-cluster-boundary-retirement-001/`). `PERFMIG01` completed `CONTINUE` but endpoint went **+0.47%** (73.12×→73.46×): a writeback-only rung adds a round-trip boundary without removing a logical read. PERFMIG02 must show a **measured** endpoint win by migrating the WB11 *readers*; **two consecutive net-negative rungs = REDIRECT** (pivot to deep single-phase array-native read+compute+write, per Claude review). Writeback-flip alone cannot reach the 0.96 µs floor — the internal-compute lever is still needed. |
