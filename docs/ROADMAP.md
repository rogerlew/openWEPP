...
fail-closed; the next cut should plan lane-dense edge cost removal / indexed
dense writeback authority before any broader island expansion.
...
| **P** | **PERF - comprehensive array-native re-architecture (HillslopeDayFrame)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, so the remaining measured boundary is the partial dense island's cached daily refresh, logical dense writeback apply, symbol lookup, and flush edge. | Plan and execute a follow-on package for lane-dense edge cost removal / indexed dense writeback authority. It must start from PERFDEEP05's profile, preserve the default-disabled posture, and prove whether the partial dense island can become endpoint-flat before any broader hydrology island expansion or kernel-body rewrite. No default activation until an opt-in H2637 endpoint beats `669.97 s` and identity/full gates pass. | **NEXT: PERFDEEP06 planning needed** - PERFDEEP05 final opt-in was `911.11 s`, still slower than final default `701.95 s`. Keep PERFDEEP02/03/05 opt-ins fail-closed; do not revert to full sync, but also do not expand blindly. |
