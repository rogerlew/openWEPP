...
WSHED-W7 resume -> Lane D active baseflow export closure -> Lane D watershed
HBP hourly water/sediment consumption -> remaining WSHED hardening.
...
| **WSHED-W7R** | **Resume W7 sediment-active watershed fixture/publication closure** | Resume W7 on current main after active Tier 1 local numerics closes. First prove the W7DC01 producer-side blocker is gone on current binaries, then adopt/generate the sediment-active watershed fixture and run the full typed watershed supervisor/publication path. | Current-main nonzero multi-OFE HBP producer proof (`tdet`/`tdep`/`sedcon_*`); committed sediment-active watershed fixture or generated fixture with provenance; full watershed typed dispatch/publication; `--jobs 1` vs parallel schema/row identity; nonzero sediment public outputs; independent sediment reconstruction; source guards proving publication remains on `WatershedPublicationFrame`. | next WSHED action after M-T1 executed-hold closure. |
