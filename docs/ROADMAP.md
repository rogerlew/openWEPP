...
| [`SNOW-STAGE3-PRODUCTION-CUTOVER-CAMPAIGN`](work-packages/20260819-snow-stage3-production-cutover-campaign-001/package.md) | Snow + vegetation + land-surface campaign | `next / coordinator and Child 1 scaffolded` | Execute the four ordered closure children: terminal meltout into the admitted V10/LSE-V2/direct-hydrology receiver; turbulent-carrier and forest-exposure authority; actual-scheduler production-candidate qualification; then atomic Stage 3 cutover and CoE retirement. `ASSURE-06` human review and approval must pass between Children 3 and 4. Canopy-intercepted snow is excluded. |
| [`SNOW-STAGE3-TERMINAL-MELTOUT-LSE-HANDOFF-IMPLEMENTATION`](work-packages/20260819-snow-stage3-terminal-meltout-lse-handoff-implementation-001/package.md) | Snow-to-land transition | `next / queued` | Connect the admitted terminal solid-exhaustion event to the complete real snow-free owner stack, route terminal liquid exactly once, recompute the remaining interval without snow operands, and prove restart plus atomic rollback. This closes mechanical handoff only; CoE remains production-authoritative. |
| `SNOW-STAGE3-TURBULENT-CARRIER-AUTHORITY-CLOSURE` | Snow turbulent-exchange authority | `queued / Child 2` | Contract-first resolution of wind source/exposure, transfer geometry, displacement and roughness, stability coordinates, canopy structure, applicability, flux envelopes, and deposition/sublimation bounds. Fixed forest-wind multipliers are prohibited. Advance before physical-efficacy or cutover qualification. |
