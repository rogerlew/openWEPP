...
PERFDEEP03 default-disabled sat in the `697-708 s` band. The next cut must first
make accumulated opt-in plumbing zero-cost when disabled, then implement the
direct-frame hydrology fast path.
...
| **P** | **PERF - comprehensive array-native re-architecture (HillslopeDayFrame)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing now also taxes the default-disabled path (`701.95 s` vs `669.97 s`). The ratified runtime spec treats more seam shaving as insufficient, and PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. | Execute PERFDEEP07 as a cleanup-plus-implementation package: first remove or bypass dense-first/direct-frame compatibility work when PERFDEEP opt-ins are off, then implement a bounded direct-frame hydrology fast path over typed frame/view APIs. No default activation until a future opt-in implementation beats `669.97 s` and identity/full gates pass. | **READY: PERFDEEP06 complete** - package `work-packages/20260619-perfdeep06-array-native-fast-path-inventory-001/` closed `READY-FOR-PERFDEEP07`. Keep PERFDEEP02/03/05 opt-ins fail-closed; PERFDEEP07 must not leave the default-disabled regression sitting on `main`. |
