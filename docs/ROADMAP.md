...
boundary materialization measured separately. Deep re-architecture is now authorized, but full
H2637 endpoint closure still requires a production array-authoritative migration and real endpoint
timing after each rung.
...
| **P** | **PERF — array-authoritative production migration** | PERFARCH03 closed the missing floor measurement: a full array-native WB11 runoff branch hot loop measured **0.959423 us/OFE-day** (`0.024823x` legacy us/OFE-day), with boundary materialization measured separately. The branch-floor result clears both the ≤5× and ≤10× viability budgets; the remaining work is production migration, not another input-only pilot. | Ratify ADR-0023, then port production WB11 runoff first with exact identity fixtures, no logical payload construction in migrated hot loops, H2637 endpoint timing + RSS after each rung, and workspace gates. The transitional materialize boundary is measured separately and retired as adjacent phases migrate. | ⏭️ **NEXT — `PERFMIG01` scaffolded, Codex-ready** (`20260618-perfmig01-wb11-runoff-array-authoritative-production-migration-001/`; Claude review confirmed PERFARCH03 GO). Ratify ADR-0023 + migrate WB11 runoff to dense `SymbolId` authority; a modest first-rung endpoint delta is acceptable if identity-clean and trajectory projects to budget; do **not** claim full H2637 endpoint closure until migrated production phases are measured. |
