...
| [`SNOW-STAGE3-PRODUCTION-CUTOVER-CAMPAIGN`](work-packages/20260819-snow-stage3-production-cutover-campaign-001/package.md) | Snow + vegetation + land-surface campaign | `active / Child 1 HOLD; Child 2A next` | Child 1 stopped before runtime edits because modeled time lacks exact-one custody and V10 cannot consume partial support. Execute 2A--2C continuously: coupled-time authority and runtime, V11 segmented vegetation, then snow-covered turbulent carrier; resume Child 1 only after all three pass. Qualification, assurance, and atomic cutover remain held. |
| [`COUPLED-TIME-AUTHORITY-IMPLEMENTATION`](work-packages/20260820-coupled-time-authority-implementation-001/package.md) | Cross-domain coupled-time authority | `next / Child 2A` | Admit `SC-COUPLEDTIME-001` and implement exact integer support, one staged clock owner, segment/slab validation, attempt/accept/reject rollback, adaptive-controller restart state, atomic multi-owner acceptance, and parent-buffered publication through a real orchestrator reference consumer. A type-only or vegetation-only seam does not pass. |
| `C3-WOODY-V11-SEGMENTED-SUPPORT` | Vegetation transaction authority | `queued / Child 2B` | After 2A passes, supersede only V10 transaction-time integration: preserve immutable V10, deterministically migrate to V11, prove full-support physical/owner compatibility, sequential segment chronology, one parent finalization/increment, and one atomic commit. Scaling V10 output, overriding `dt_s`, or rehashing configuration is prohibited. |
| `SNOW-STAGE3-TURBULENT-CARRIER-AUTHORITY-CLOSURE` | Snow turbulent-exchange and vegetation-support authority | `queued / Child 2C` | After 2A/2B pass, resolve wind source/exposure, transfer geometry, displacement/roughness, stability coordinates, canopy structure, applicability, flux envelopes, deposition/sublimation bounds, and snow-covered pre-event V11 evolution with a shared canopy-air/snow-surface carrier. Fixed forest-wind multipliers and independently evolved aerodynamic carriers are prohibited. |
