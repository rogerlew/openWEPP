...
equivalence.
R6 direct publication cutover is scaffolded at
...
ledger into canonical architecture authority and then held with
`HOLD-R6-DIRECT-PUBLICATION-FRAME-ABSENT`: the runner has no run-bound direct
publication frame carrying the promoted HBP/WAT/PASS/loss/manifest operands.
Next work must build that direct publication frame from typed direct state
before output-family cutover.
...
| **P** | **PERF - comprehensive array-native re-architecture (direct runtime)** | Comprehensive array-native migration remains the perf direction, but PERFDEEP03 and PERFDEEP05 falsified the current partial hydrology-island shape as a production endpoint win. PERFDEEP05 removed the PERFDEEP04 full-sync hotspot, while the accumulated plumbing also taxed the default-disabled path (`701.95 s` vs `669.97 s`). PERFDEEP06 converted that into a direct-frame plan with a zero-cost-disabled P0 gate. PERFDEEP07 improved but did not close that gate (`685.85 s` retained vs `<= 676.67 s`); PERFDEEP08 tested disabled diagnostic-hook caching and was slower (`691.93 s`). PERFDEEP09 closed the disabled-path blocker, R2A created the separate direct-runtime skeleton without phase math or publication cutover, R3A proved the first complete direct phase span, R3B proved a richer direct water-ledger span, R3C proved run-level multi-lane transfer/topology propagation, R4A through R4P/Q/Z closed the direct hydrology path through shadow-only projection, and R5A through R5E completed full OFE-day direct endpoint readiness while preserving no-publication/no-default/no-scheduler boundaries. R6 has now promoted the publication operand ledger into architecture authority, but public output cutover is held until a run-bound direct publication frame exists. | Close the R6 direct-publication-frame blocker, then resume output-family cutover from [20260621-r6-direct-publication-cutover-001](work-packages/20260621-r6-direct-publication-cutover-001/package.md): HBP, WAT, PASS, loss JSON, and run manifest must read typed direct projection only under byte/Arrow identity, metadata parity, anti-alias fixtures, and independent operand reconstruction. | **HELD: R6 direct publication frame blocker** - ledger authority is promoted, but direct output cutover cannot start until typed direct run/lane/day state populates the promoted `PublicationFrame`; compatibility WB13/runtime-row wrappers are explicitly forbidden. |
