...
| --- | --- | --- | --- |
| `SNOW-POST-MELTOUT-LAND-SURFACE-ENERGY-AUTHORITY` | Snow surface-energy-balance campaign | `next / authority admission required` | Admit the tolerance-localized terminal event, actual-state receiving-regime selector, complete post-snow component equations, unfrozen-soil/surface-water energy recipients, and independent ledgers. Pinned `tmpadj` is frost-boundary provenance only; direct transfer of snow-computed excess remains prohibited. |
| `SNOW-STAGE3-COUPLED-SHADOW-STATE-AUTHORITY` | Snow surface-energy-balance campaign | `queued / authority admission required` | Reconcile `SC-SNOWFREEZE-001#INV-SNOWFREEZE-091` and define bounded parallel snow/surface/soil/frost/cover persistence, restart, reappearance, noninterference, and performance before any seasonal shadow claim. |
