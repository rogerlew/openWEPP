...
`--compatibility-runtime` selector from API/CLI/harness entrypoints. The
remaining forward item is the **symbol-surface support-boundary deletion**:
deleting or replacing the compiled scheduler/day-frame/carrier support surface,
then
deleting `scheduler.rs` and the symbol-map machinery. This is a **single-authority
re-architecture, not a clean deletion**: the day-zero seed authority is a computed
ordered pipeline, so it is done as **one shadow-first typed-seed-carrier package**
(build typed carrier → shadow-prove seed identity → cut over all reads at once →
delete), not an incremental burn-down (which held twice). It is the most likely
remaining lever for the ≤5× ideal.
