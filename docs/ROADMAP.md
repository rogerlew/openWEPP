...
| 2 | **Array-authoritative WB11 integrated pilot** | PERFARCH02 closed with a CONDITIONAL GO: the artifact-local floor prototype preserved exported-map identity for the prototyped writeback/guard flow and measured the array-authoritative path at about 49.9x faster than the current logical writeback/guard path, but it did not execute the complete WB11 hot path. The next lever is a real WB11 pilot that removes the PERFIDX03 export seam and PERFIDX05 dual-write in production-shaped execution, while keeping logical surfaces at publication/diagnostic boundaries. | Bit-identical logical materialization for the piloted WB11 flow; same-machine H2637 no-UI timing; evidence that normal execution has no per-day full `BTreeMap` export and no logical + array dual-write; decision whether <=10x remains credible before expanding kernel families. | ▶️ **`PERFARRAY01` scaffolded, Codex-ready** — Stage A contract shell + Stage B WB11 pilot (flag-gated, bit-identity load-bearing) under **proposed ADR-0023**; the integrated floor it measures is the real **5×** answer; two structural proofs (no per-day export, no dual-write) are make-or-break; NO-GO ("73× is the honest floor") is a valid outcome. First production code in the arc. *5× still unproven until this lands; irrigation stays deferred -> `backlog/20260617-...`.* |
