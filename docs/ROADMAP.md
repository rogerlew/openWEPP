...
|---|---|---|---|---|
| **WSHED-W7** | **Sediment-active watershed fixture and publication closure** | Adopt or generate a committed full watershed fixture with actual nonzero sediment response from hillslope pass artifacts, then run the full watershed supervisor through typed dispatch and typed publication. No subsets, no synthetic sediment fills, no surrogate physics. | Full committed fixture evidence; `--jobs 1` vs parallel parquet schema/row identity across all required outputs; nonzero detachment/deposition/sediment-yield rows; independent reconstruction of sediment/publication operands where conservation-sensitive; source guards proving public publication remains on the typed frame path. | ⏸️ **executed-hold 2026-07-02** — package: `work-packages/20260702-wshedw7-sediment-active-watershed-fixture-publication-closure-001/`. Blocker: production hillslope HBP sediment emission remains zero for inspected sediment-active source substrates. |
| **WSHED-W7DC01** | **Hillslope sediment production hold lift** | Close `WSHED-W7-HOLD-001`: diagnose and correct production hillslope sediment emission so real multi-OFE source substrates can produce nonzero HBP sediment when contract-backed physics produces it. | Contract-first correction or authority hold; before/after pass parquet evidence with nonzero `tdet`/`tdep`/`sedcon_*`; no surrogate sediment physics; handoff back to W7 fixture adoption. | ▶️ **queued 2026-07-02** — package: `work-packages/20260702-wshedw7dc01-hillslope-sediment-production-hold-lift-001/`. **Diagnosis (2026-07-04): the zero is the *multi-OFE* EROD14 Wave-2 path; the fix is erosion §E.3 (multi-OFE Wave-1 chaining, retiring EROD14), not a watershed-side defect.** The single-OFE Wave-1 flip (1b-C, merged) proves the continuity engine produces sediment (p61 HBP `tdet=20.9`) but only enables single-OFE no-tillage; W7DC01 needs that engine wired across OFEs. Blocked on §E.3. |
