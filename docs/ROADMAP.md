...
| 2 | **Hot-path state redesign — array-authoritative scoping + floor prototype** | PERFIDX06 measured the PERFIDX04 endpoint at **73.12×** legacy no-UI (`666.82s / 9.12s`), profile spread across the symbol-keyed map machinery as a class (no single lever). The fix is to promote the dense indexed rep from *mirror* to **authority** (kernels read/write a `Vec<BoundaryValue>` by `SymbolId`; `BTreeMap` only at I/O boundaries), solving the PERFIDX03 export seam + PERFIDX05 dual-write + typed-guards-without-string-cost. Changes the kernel writeback payload shape (the unlock every PERFIDX held out of scope) | GO/NO-GO decision from a **floor prototype**: measure openWEPP's intrinsic floor (≤10× needs −86%, 5× needs −93% of 667s) before any migration; if GO, staged plan + ADR superseding/amending ADR-0022 | ▶️ **`PERFARCH02` scaffolded, Codex-ready** — floor-prototype-first feasibility (a candid NO-GO "≤Nx is the ceiling" is a valid outcome). Migration is downstream packages gated on GO. *Irrigation stays deferred → `backlog/20260617-…`.* |
