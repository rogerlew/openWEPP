...
current binding order: single-OFE and Lane D MOFE groundwater/baseflow
implementation -> Lane D active baseflow export closure -> Lane D watershed HBP
hourly water/sediment consumption -> remaining WSHED hardening. M-T2P closed as
a projection-authority hold, and M-T2Q locked `ow-lanuse-1` or later native
management files as the canonical new-physics production datver, so this queue
keeps pre-native legacy datvers on protected legacy/off paths unless they are
explicitly migrated.
