...
| --- | --- | --- | --- |
| [`SNOW-SURFACE-EB`](planning/snow-surface-energy-balance-roadmap.md) additive sublimation and sub-canopy longwave | Snow/frost science | `EB-04C complete; EB-04D next` | EB-04C closes all 22 captured thermal-domain failures using the fixed libsnobal boundary while retaining CoE mass authority. Execute EB-04D for the two independently assigned geometry failures, then run 04E corrected-population qualification before a fresh preregistered 04R factorial. No coefficient tuning, guard relaxation, promotion, or rewriting of EB-04 is authorized. |
