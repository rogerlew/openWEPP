...
| --- | --- | --- | --- |
| [`STAGE3-NATIVE-VEGETATION-LANED-WATERSHED-THROUGHPUT-RECOVERY`](work-packages/20260901-stage3-native-vegetation-laned-watershed-throughput-recovery-001/package.md) | Snow + native vegetation + Lane D runtime architecture | `next / scaffolded / predecessor suspended` | Apply [ADR-0044](decisions/0044-prohibit-accretive-production-solver-dispatch.md): freeze the preserved r151 source/diff, establish measured OFE-day and watershed workload budgets plus physically justified numerical tolerances before production edits, then replace—not extend—the accretive v33–v57 solver dispatch. Retain Lane D per-OFE MOFE execution, the `SC-VEGETATION-001` native RHESSys-derived process model and ET, non-CoE Stage 3 snow, and admitted frozen-litter authority. One canonical solver per physical regime is mandatory; nonconvergence returns a typed failure or canonical adaptive-step response and may not invoke an older solver. |
