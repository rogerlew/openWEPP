...
| `C3-WOODY-V11-SEGMENTED-SUPPORT` | Vegetation transaction authority | `queued / Child 2B / coupled-time prerequisite satisfied` | Import `SC-COUPLEDTIME-001@2` and supersede only V10 transaction-time integration: preserve immutable V10, deterministically migrate to V11, prove full-support physical/owner compatibility, sequential segment chronology, one parent finalization/increment, and one atomic commit. Scaling V10 output, overriding `dt_s`, or rehashing configuration is prohibited. |
