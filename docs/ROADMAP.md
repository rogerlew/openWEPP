...
| **P** | **PERF — array-authoritative production migration** | PERFARCH03 closed the missing floor measurement: a full array-native WB11 runoff branch hot loop measured **0.959423 us/OFE-day** (`0.024823x` legacy us/OFE-day), with boundary materialization measured separately. The branch-floor result clears both the ≤5× and ≤10× viability budgets; the remaining work is production migration, not another input-only pilot. | Author the follow-on migration package / ADR-0023 revival, then port production WB11 runoff first with exact identity fixtures, no logical payload construction in migrated hot loops, H2637 timing after each rung, and workspace gates. | ⏭️ **NEXT — Codex-ready to scaffold** (`PERFMIG01` suggested). PERFARCH03 is complete; do **not** claim full H2637 endpoint closure until migrated production phases are measured. |
