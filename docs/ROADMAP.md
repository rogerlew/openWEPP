...
| [`RHESSYS-EAST-COAST-COUPLED-VEGETATION-SLICE`](work-packages/20260808-rhessys-east-coast-coupled-vegetation-slice-001/package.md) | Vegetation/ecohydrology architecture | `held / constitutive-authority dependency` | Site values/state are caller-supplied and no longer release blockers. Begin production only after the schema and selected constitutive families are complete and contract-first tests pass; implement independent native-forest flux components plus admitted acquisition. |
