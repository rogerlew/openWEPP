...
| **M** | **MOFEFID — MOFE fidelity campaign** | Four lanes on the closed MOFE routing foundation: (A) proactive MOFE defect-review sweep; (B) adjudication-first audit against the eleven defect classes in the wepp-forest stakeholder water-balance brief; (C) lateral-flow observed-authority rubric from `tests/fixtures/forest_lateral_flow_authority/`; (D) Papanicolaou 2018 OFE-by-OFE routing (space/time-variant resistance, TVD-MacCormack KWE). A-C are complete; Lane D is merged through D1-D8, subsurface seam design, solver/identity seam implementation, and the opt-in runtime SHADOW + real-H2637 executed vector. | Strategy: [planning/mofe-fidelity-campaign-strategy.md](planning/mofe-fidelity-campaign-strategy.md). Remaining Lane D **opt-in production activation** is sequenced in strategy §6.1 as D9-D16: D-val disposition, `GAP-OFEROUTE-005` shock numerics, `GAP-OFEROUTE-007` friction operand authority, melt-limb hourly-source coverage, ADR-0036 erosion hourly-shape switch, Lane D runtime profiling/optimization, opt-in production flip, then default-promotion adjudication. | ▶️ **active 2026-07-05** — A-C complete; Lane D production activation blocked by the enumerated `SC-OFEROUTE-001` flip preconditions and the dedicated runtime-cost gate, not by missing scaffold. |
