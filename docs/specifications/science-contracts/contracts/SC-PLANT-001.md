...
1. Derive daily mean VPD from the existing climate `Tmax`, `Tmin`, and dewpoint
   ...
   negative values without clamping.
...
3. The first realization publishes no allocation/litter regardless of aggregate
   PL live biomass, and two identical annual forcing cycles return bit-identical
   endpoint canopy, foliar state, and annual transfer totals without pending
   fixed-date litter.
4. A full cyclic NH year and its 182-day wrapped sequence at negated latitude
   produce corresponding post-warmup SH canopy values and leaf-on/leaf-off limb
   ordering within one transformed calendar day.
5. Missing/partial YAML, invalid thresholds/fractions/masses, zero/non-finite
   `bb`, non-continuous native schedules, any negative/non-finite VPD, and
   nonconsecutive dates fail before state mutation.
