...
| B | legacy no datver line | Strict reject, compat optional branch only. | Compatibility branch must be explicit and typed. | `[DIRECT][E-SPEC-SLP-01]` |
| C | explicit older datver >= compatibility threshold | Compat-only candidate. | Requires explicit policy flag. | `[DIRECT][E-SPEC-SLP-01]` |
| D | unsupported datver (< threshold or unknown) | Reject. | Emit typed `UnsupportedDatver`. | `[DIRECT][E-SPEC-SLP-01]` |
...
azm_fwidth_line = azm fwidth ;
nslpts_slplen_line = nslpts slplen ;
slope_pairs = (xinput slpinp){nslpts} ;
```
...
| `azm,fwidth` | `ofe[i].geom` | `slope.ofe[i].geom` | `input::slope` | init,daily | immutable | hydrology geometry setup | `G-SLP-003` |
...
| `azm,fwidth,nslpts,slplen` | `slope.ofe[*].geom`, `slope.ofe[*].shape_hdr` | runtime topology/geometry boundary | aliases from Section 3 (`aspect_deg`,`width_m`,`point_count`,`length_m`) with unit continuity | no parser-side rescaling |
...
- Strict mode:
  ...
  - reject explicit datver values other than `97.5`;
...
| `SLP-GAP-003` | Non-usersum utility extensions (`wepppy` `2023*` variant) remain undispositioned for canonical parser acceptance. | `[DIRECT][E-SPEC-SLP-01]`, `[DIRECT][E-WP-SLP-01]` | `HOLD` |
