...
| GAP-ROUTE-009 | Full watershed channel sediment routing process families (`chnero/chnrt/detach`) are not yet migrated into openWEPP watershed production kernels. WSHED06 closed WS11 publication-family symbols (`ws10_channel_{id}_qsed`, `ws10_channel_{id}_tc`), WSHEDIMPL15 added required WS15 runtime control projection + baseline conversion scaffold publication (`crsh/depmid/depsid`) with fail-closed guard continuity, WSHEDIMPL16 added fail-closed contributor `particle_diameter_m` payload projection for transport-capacity lineage ingress, and WSHEDIMPL17 added fail-closed WS17 segment/hydraulic scaffold projection/guard closure (`nslpts`, segment `x/slope/depa/depb/wida/widb`). Segment-level detachment/deposition/transport-capacity process parity remains open. | Channel sediment continuity/detachment/deposition parity and downstream sediment publication claims remain blocked pending full `chnero/chnrt/detach` process-family migration and end-to-end validation closure. | non-promotable | `[DIRECT][Static] + [INFERENCE][Static]` |
