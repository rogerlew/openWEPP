...
| GAP-ROUTE-014 | Migrated-lane terminal divergence (Codex post-hoc review `WSHED-W11A-POSTHOC-001` H2, 2026-07-10): two WS20/WS21 detachment terminals diverge from pinned `dcap.for` — the capped-widening terminal returns uncapped width with unchanged depth instead of deriving geometry from the reconstructed capped erosion (`dcap.for:238-261`), and the post-contact subcritical-boundary-shear terminal returns unchanged `depmid` instead of re-entering the incision terminal with `timsh = timpot` (`dcap.for:210-215, 173-190`); existing lane tests lock both divergent behaviors in. INV-ROUTE-018 binds pinned `dcap.for` as the realization; the interval lane MUST NOT reuse these terminals uncorrected (vector 10(b)/(c) are the correction gates). CLOSED by `20260710-wshedw11b-channel-interval-sediment-implementation-001` (2026-07-10): both terminals and their lock-in tests corrected to pinned-baseline behavior (`GAP-ROUTE-014-A/-B` in the W11B disposition), vector 10(b)/(c) gates passing in the W11B gate record (full workspace 1,677/1,677; erosion profile 312/312; dual verifier EXECUTED-PASS); the capped-erosion reconstruction verified present in the corrected lane (`01_ws22_ws23_ws26_detachment.rs` `eros = dct * tb * wflow / wtdsoi` lineage with capped-terminal tests). | Both terminal state transitions now match pinned `dcap.for`; the event-scalar-lane parity flag is retired with the correction. | closed | `[DIRECT][Static] + [Ran]` |
