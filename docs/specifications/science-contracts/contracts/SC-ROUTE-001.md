...
| Operand | Basis |
|---|---|
| Hydraulic profile operands | `qe(it) := q1(it)` (outlet anchor), `qt(it) := qin(it)` (top inflow rate), `qlat(it) :=` the wave-routing lateral series — the unique INV-ROUTE-016 realization of Eq. [13.5.6]-[13.5.12]; event-peak fractions and event-duration rates are invalid on this lane; routed storage change stays a water-routing surface (outlet-anchored steady profile per interval) |
...
1. **Single-interval equivalence**: a day whose routed series has exactly one
   ...
   `qsed_top := interval class mass / dtchr`, hydraulic profile
   `(qe, qt, qlat) := (q1(it), qin(it), qlat(it))`, erosion exposure
...
11. **Hydraulic-profile anti-alias**: an interval constructed so that the
    four candidate profile constructions diverge (event-peak-fraction
    partition, `qin(it)`-anchored, `q1(it)`-anchored, and
    projected-lateral-only) — assert the solve's shear/transport surfaces
    match the pinned INV-ROUTE-016 operand map
    `(qe, qt, qlat) = (q1(it), qin(it), qlat(it))` and none of the
    aliases; include nonzero routed storage change so
    `qin + qlat*lc != q1` distinguishes the candidates.
