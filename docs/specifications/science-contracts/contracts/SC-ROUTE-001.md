...
| REF-ROUTE-ARS77-SAMEGRID | `references/vendorable/kineros/703.pdf` (+ `703.md` conversion; Woolhiser, Smith & Goodrich 1990, USDA-ARS ARS-77) | External canonical same-grid coupling authority: the sediment mass-balance equation (Bennett 1974 lineage, restated with citation) is solved on the same time/space grid as the kinematic water solution; a zero upper-boundary transport capacity invokes deposition mode on that grid (`703.md:1065-1075`). The whole-reach dry-interval all-mass deposition rule of INV-ROUTE-020(a) is a contract **inference** extending this boundary condition, graded mixed at its rows. Also the recorded unsteady-advection fallback form. | `[DIRECT][Static]` |
| REF-ROUTE-HECRAS-QUS | `references/vendorable/HEC_RAS_1D_Sediment_Transport_UserManual_20260710.pdf` (USACE web capture; user-manual text carrying the Technical Reference Manual's quasi-unsteady semantics — treated as **class corroboration** co-anchored with REF-ROUTE-ARS77-SAMEGRID and REF-ROUTE-CREAMS-CH3-QS; the formal TRM citation remains pinned for acquisition, bibliography R-107) | External canonical quasi-steady-sequence class authority: a flow hydrograph approximated by a series of steady profiles; the computational increment is the hydraulic and sediment-transport time step; **bed-change state advances and carries each increment** (HEC-RAS gates the cross-section *refresh* behind a minimum-bed-change threshold, vendored manual PDF p. 178 — the per-increment claim here is state carry, not unconditional cross-section refresh), justified when per-increment bed change does not alter hydrodynamics appreciably. Geometry-*update* authority for the interval lane is the Chapter-13/lineage carry (REF-ROUTE-CH13-GEOMCARRY, REF-ROUTE-GULLY-STATE), not HEC cross-section refresh policy. | `[DIRECT][Static]` |
...
| `qt`, `qlat`, `qu`, `ql` | `ft^3 s^-1` | Top, lateral (total), upper, and lower segment discharges (Eq. [13.5.6]-[13.5.10], [13.5.12]). | discharge-distribution routine | spatially-varied flow and erosion solver |
| `qlat_eff` | `ft^3 s^-1 ft^-1` | Effective lateral inflow **per unit length** (Eq. [13.5.11], `qlat_eff = qpo/leff`) — dimensionally distinct from the total `qlat`; the baseline `chnrt.for:233-242` local variable named `qlat` is this quantity. | effective-length routine | segment erosion solver (`qlat_cfs_per_ft` slot) |
...
| `rho_soil` | `lbm ft^-3` | In-place soil **bulk mass density** (CREAMS "mass density of the soil in place"; baseline `wtdsoi` numeric provenance, lbm convention — the variable name's "weight density" is a legacy misnomer); the [I-131]/[I-136] denominator and the detached-mass construction factor. | channel soil-input projection | incision/widening rates and detached-mass derivation |
| `d_i` | `lbm ft^-2 s^-1` | Maximum-shear mass detachment rate: baseline `di = excess * Kch * (tau - taucr)` (`dcap.for:166` lineage) — the CREAMS `e_m` realization; the `timpot` denominator and incision-rate numerator (`d_ch = d_i / rho_soil`). | detachment-capacity evaluator | `timpot` partition, incision, detached-mass derivation |
| `t_exp(it)`, `t_norm(it)` | `s`, `s` | Interval erosion-exposure time (fills every legacy `timsh` slot; `dtchr` gated by shear, `timpot`/`timex`-partitioned at layer contact) and interval flux-normalization basis (`:= dtchr`; fills every legacy `tb` denominator slot — the triangular factor `2` retires with the surrogate). | channel-interval sediment lane | detachment/widening flux and mass construction (INV-ROUTE-018) |
| `timpot`, `timex` | `s` | Erosion-time budget partition at nonerodible-layer contact: incision-completion time (`timpot = depmid * rho_soil / d_i`) and residual widening time within the contact interval (`dcap` lineage). | widening-clock evaluator | INV-ROUTE-018 budget accounting |
...
| INV-ROUTE-016 | Per-interval quasi-steady solve-form invariant (W11A): each hydraulically active interval executes the complete Chapter-13 quasi-steady spatially-varied segment solve (the REF-ROUTE-CH13-SVF/EFFLEN/SHEAR/CONT/DETDEP machinery in the WSHEDIMPL18-41 migrated segment-solve lanes — the WS20/WS21 runtime families) at that interval's operands. **Hydraulic profile operand map (unique realization):** the profile anchor is `qe(it) := q1(it)` (the routed interval outlet discharge — the legacy `qe = peakot` slot interval-ized); the top inflow rate is `qt(it) := qin(it)` (the routed superposed inflow on the shared grid — the Eq. [13.5.6] slot); the **total** lateral inflow rate is the published wave-routing lateral series `qlat(it)` (`m^3 s^-1` for the whole reach — `RoutedChannelWaveState::qlat_m3_s` lineage; the Eq. [13.5.7] slot), which is used ONLY to build the effective-length partition after the unit bridge (Eq. [13.5.8]-[13.5.9]: `leff(it) = lc * (1 + qt(it)/qlat(it))` when `qlat(it) > 0`, the no-lateral branch otherwise). The segment solve's lateral operand is the **effective per-unit-length** rate `qlat_eff(it) := qe(it) / leff(it)` (Eq. [13.5.11], `ft^3 s^-1 ft^-1` — the quantity the baseline `chnrt.for:233-242` local variable named `qlat` and the migrated `qlat_cfs_per_ft` slot consume), with `qu(it) := qe(it) * ltop(it) / leff(it)` (Eq. [13.5.10]) and segment discharges per Eq. [13.5.12]. Passing the raw total `qlat(it)`, or `qlat(it)/lc`, into the per-unit-length solve slot is invalid — the total and per-length quantities are dimensionally distinct and must never share a symbol binding. Event-peak fractions, event-duration-derived rates, or any profile reconstruction from event scalars are invalid operands on the interval lane. Routed water-storage change (`qt(it) + qlat(it) - q1(it) != 0`, all total rates) is deliberately NOT reconciled into the sediment hydraulics: the profile is outlet-anchored steady per interval, exactly the legacy event-scale posture (legacy anchors the steady profile at routed `peakot` while inflow volumes set the split) and the quasi-steady-sequence class posture (each increment's profile is steady); storage is a water-routing surface, and sediment mass closure (INV-ROUTE-019) is flux-mass-based and unaffected. Sediment operands: inlet sediment flux = interval-projected inlet class mass / interval duration (replacing the event-scalar `qsed_top = qsed_tot / durrof` reduction of Eq. [13.5.18]); upstream-channel sediment ingress = the upstream channel's same-interval egress on the shared grid. Published daily channel sediment surfaces are the interval sums. This quasi-steady-sequence form is a labeled refinement beyond legacy event-scalar source-intent (REF-ROUTE-CREAMS-CH3-QS documents the time-term deletion as a compute-cost reduction); the recorded fallback if per-interval quasi-steady proves untenable is the unsteady advection continuity form (REF-ROUTE-ARS77-SAMEGRID lineage) — never a return to the single event-peak solve. | hard-fail | REF-ROUTE-HECRAS-QUS, REF-ROUTE-CREAMS-CH3-QS, REF-ROUTE-CH13-CONT, REF-ROUTE-CH13-DETDEP, REF-ROUTE-CH14-TIMESTEP | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| Operand | Basis |
|---|---|
| Hydraulic profile operands | `qe(it) := q1(it)` (outlet anchor), `qt(it) := qin(it)` (top inflow rate), `qlat(it)` (published wave total, `m^3 s^-1`) partition-only into `leff(it)` per Eq. [13.5.8]-[13.5.9], then the solve's lateral operand `qlat_eff(it) := qe(it)/leff(it)` (per unit length, Eq. [13.5.11]) and `qu(it) := qe(it)*ltop(it)/leff(it)` — the unique INV-ROUTE-016 realization; raw-total or total/`lc` substitution into the per-length slot, event-peak fractions, and event-duration rates are all invalid; routed storage change (`qt + qlat - q1`, totals) stays a water-routing surface (outlet-anchored steady profile per interval) |
...
| Erosion exposure `t_exp(it)` | `dtchr` when the interval's gate shear exceeds `taucr` (detachment gate: average soil shear `tau`; widening gate: boundary shear `tau_b = tau * f(x_b)`), else zero; partitioned per `timpot`/`timex` within a layer-contact interval — fills every legacy `timsh` slot (INV-ROUTE-018) |
| Flux normalization `t_norm(it)` | `:= dtchr` — fills every legacy `tb` denominator slot in the detachment/widening flux and capped-erosion constructions; the triangular `tb = 2*rundur` surrogate (both the factor 2 and the event duration) retires wholesale (INV-ROUTE-018) |
...
(kg) side after the inverse conversion. The lateral chain has one
additional named normalization: the wave total `qlat(it)` (m^3/s) enters
the effective-length partition after the SI-to-English bridge, and the
per-unit-length solve operand is then **derived** as
`qlat_eff(it) = qe(it)/leff(it)` (ft^3/s/ft) — a division by effective
length, not a unit conversion of the total. No new conversion constants
are introduced by this lane.
...
1. **Single-interval equivalence**: a day whose routed series has exactly one
   ...
   `qsed_top := interval class mass / dtchr`, hydraulic profile from the
   pinned map (`qe := q1(it)`, `qt := qin(it)`, `leff` from the interval
   partition, `qlat_eff := qe/leff`), erosion exposure
   `t_exp := dtchr`, and normalization `t_norm := dtchr` (i.e., the
   event-scalar path minus the triangular surrogate and minus interval
   sequencing) — per-class egress equal within TOL-ROUTE-006. This vector
   verifies sequencing/wiring identity, not operand-law differences (the
   surrogate retirement is a deliberate operand change covered by the
...
10. **Mid-interval layer contact and baseline-divergent terminals**: (a) an
    interval in which incision reaches the nonerodible layer partway
    through partitions its erosion-time budget per `timpot`/`timex` (no
    double-count: incision through the residual `depmid` plus widening
    only over `timex`), and the contact interval's detached mass matches
    the constructive geometry derivation of INV-ROUTE-019; (b) the
    **capped-widening terminal**: when `dct` caps at `maxe`, geometry
    derives from the reconstructed capped erosion (`dcap.for:238-261`
    semantics), and detached mass independently reconstructed from the
    resulting geometry matches the capped flux; (c) the **post-contact
    subcritical-boundary-shear terminal**: boundary shear below `taucr`
    with `depmid > 0` re-enters the incision terminal with
    `t_exp = timpot` semantics and decrements `depmid`
    (`dcap.for:210-215, 173-190`), with the same independent
    geometry-mass reconstruction check.
11. **Hydraulic-profile anti-alias**: an interval constructed so that the
    candidate profile constructions diverge — event-peak-fraction
    partition, `qin(it)`-anchored, `q1(it)`-anchored with raw-total
    lateral, `q1(it)`-anchored with `qlat(it)/lc`, and the pinned map —
    with nonzero routed storage change (`qt(it) + qlat(it) != q1(it)`,
    totals) and a channel length chosen so `qlat(it)/lc != qe(it)/leff(it)`.
    Assert the solve's shear/transport surfaces match the pinned
    INV-ROUTE-016 map (`qe := q1(it)`, `qt := qin(it)`,
    `qlat_eff := qe/leff` from the interval partition) and none of the
    aliases — in particular that the per-unit-length slot never receives
    the raw total or the total divided by `lc`.
...
| GAP-ROUTE-014 | Migrated-lane terminal divergence (Codex post-hoc review `WSHED-W11A-POSTHOC-001` H2, 2026-07-10): two WS20/WS21 detachment terminals diverge from pinned `dcap.for` — the capped-widening terminal returns uncapped width with unchanged depth instead of deriving geometry from the reconstructed capped erosion (`dcap.for:238-261`), and the post-contact subcritical-boundary-shear terminal returns unchanged `depmid` instead of re-entering the incision terminal with `timsh = timpot` (`dcap.for:210-215, 173-190`); existing lane tests lock both divergent behaviors in. INV-ROUTE-018 binds pinned `dcap.for` as the realization; the interval lane MUST NOT reuse these terminals uncorrected (vector 10(b)/(c) are the correction gates). | W11 Phase B must correct both terminals (and their tests) to pinned-baseline behavior before activating the interval lane; until then the divergence also stands as an event-scalar-lane parity flag (Investigation tier, ADR-0017). | open | `[DIRECT][Static] + [Ran]` |
