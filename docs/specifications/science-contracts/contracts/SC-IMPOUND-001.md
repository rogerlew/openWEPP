...
3. Active-projection vector: active branch payloads seed runtime full
   function-family coefficients (`f01..f15` over `a,b,c,d,e,ha`) without
   projection-gap hard failure (`WS-RUNTIME-E-012`), and kernel `qo` equals
   the legacy 15-function min-controller composition from those families.
