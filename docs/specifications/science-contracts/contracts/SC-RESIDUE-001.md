...
| INV-RESIDUE-021 | CP-GSI02 native leaf-off closure: the residue-domain litter input for a native GSI forest equals the same day's `L_leaf` from `SC-PLANT-001#INV-PLANT-035`; it is added once to surface and ground litter pools before decomposition, residue-cover derivation, residue-depth conversion, and frost consumption. Pending-date accumulation, duplicate aggregate-live-biomass loss, and delayed publication are invalid. | hard-fail | REF-RESIDUE-CH8-COUPLING, REF-RESIDUE-PHYS-BOUNDS, SC-PLANT-001#INV-PLANT-035 | `[DIRECT][Static] + [INFERENCE][Static]` |
