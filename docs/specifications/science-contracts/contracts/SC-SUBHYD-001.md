...
| REF-SUBHYD-OBSERVED-LATERAL-ENVELOPE | External field-observation corpus: HJ Andrews WS10 (McGuire & McDonnell 2010, WRR 46 W10543, R-67; Harr 1977, R-86), Maimai M8 (Woods & Rowe 1996, R-89; McGlynn 2002, R-88; Mosley 1979, R-87), Panola (Tromp-van Meerveld & McDonnell 2006, R-64; threshold-shape only), Weiler 2005 synthesis (R-68); observed datasets vendored at `tests/fixtures/forest_lateral_flow_authority/`. Steep-wet-forest annual streamflow ratio 0.54–0.60 (2,200–2,610 mm), event commencement threshold 15–35 mm, above-threshold event ratio 0.25–0.80, forest ET 500–1,000 mm/yr. External-authority envelope per ADR-0011 (magnitude judged last, as a flag) and the correctness re-anchoring scheme (level-4/5 external suite; test a law with a bounded magnitude, not a point). Derivation and page-cited extraction: `docs/work-packages/20260702-mofefid-c01-lateral-flow-envelope-promotion-001/artifacts/`. | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `Keff_ksatadj` | `m s^-1` | Conductivity after reference-intent `ksatadj` adjustment, before frost limitation. | soil hydraulic state routine | infiltration, runoff reconciliation, and downstream lateral magnitude |
...
| `BR-SUBHYD-KSATADJ-GUARD` | `ksatadj` branch missing/non-finite/out-of-range | source-intent operands required by `BR-SUBHYD-KSATADJ-EXECUTE` | runtime | typed hard-fail |
...
| INV-SUBHYD-032 | REFINTENT001 forest `ksatadj` effective-conductivity invariant: when `ksatadj = 1`, conductivity consumed by infiltration/runoff reconciliation and downstream lateral-magnitude paths must be formed from the ADR-0024 source-intent algorithm in `REF-SUBHYD-KSATADJ-INTENT`: top-two tillage-layer total water over averaged porosity and rock correction for `sat_frac`, source-intent 9001/9002+/9003 branch formulas, explicit unit conversion between `mm h^-1` and `m s^-1`, and typed rejection for missing/non-finite/ambiguous operands. `Keff_ksatadj` is the pre-frost soil-conductivity surface; when a positive active frost infiltration-capacity surface is present, the WB14 effective conductivity must be the lower of `Keff_ksatadj` and the frost capacity. Legacy binary output magnitude, disabled branches, and non-conservation artifacts are non-authoritative. | hard-fail | REF-SUBHYD-KSATADJ-INTENT, REF-SUBHYD-PHYS-BOUNDS | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-SUBHYD-033 | MOFEFID-C01 forest lateral-flow observed-authority envelope: absolute lateral/water-yield magnitude for the steep-wet-forest class (H2637-like: gradient ≥0.45, annual precipitation ≥2,000 mm, thin conductive soil over an impeding layer, no modeled Horton overland flow) is judged against `REF-SUBHYD-OBSERVED-LATERAL-ENVELOPE`, NOT against legacy binary output (ADR-0017). Four tiers, each a law-with-bounded-magnitude judged only after conservation, routing, and export closure hold: (ENV-Y) annual combined water-yield fraction `(Σ runvol + Σ latqcc_outlet·A_outlet)/(Σ P·A_total)` ∈ [0.55, 0.72] — where the lateral term is the `latqcc` LEAVING the hillslope toe (the outlet OFE only), NOT summed over internal OFEs (internal lateral is re-received downslope under `INV-RUNOFFPART-031` and would double-count); (ENV-T) a detectable event commencement threshold in 15–40 mm; (ENV-E) above-threshold (>50 mm, wet antecedent) event combined-export ratio ∈ [0.25, 0.80] with an ascending shape in storm size and antecedent wetness; the event tiers (ENV-T, ENV-E) are judged on the QUICKFLOW-separated (Hewlett-Hibbert) event-responsive component, NOT total daily export, because continuous baseflow lateral drainage contaminates the total (MOFEFID-C03: total-export gives physically impossible >100% small-storm ratios and inverts the ascending law); (ENV-ET) annual evapotranspiration ∈ [500, 1,000] mm/yr as the complementary plausibility check that yield is not achieved by suppressing ET. A value inside a tier is NOT-CONTRADICTED (not validated); a value outside a tier is an INVESTIGATION FLAG that opens magnitude adjudication, not a hard-fail and not an automatic defect. The envelope is conditioned on the wet-steep class and does not transfer to drier/gentler sites; Panola contributes ENV-T shape only, Coweeta is context-only (no `latqcc` verdict), and sub-hillslope per-trough coefficients and single-channel-vs-two-channel comparisons are excluded as bounds. | investigation-flag | REF-SUBHYD-OBSERVED-LATERAL-ENVELOPE, REF-SUBHYD-PHYS-BOUNDS, SC-RUNOFFPART-001#INV-RUNOFFPART-031, ADR-0011, ADR-0017 | `[DIRECT][Ran] + [INFERENCE][Static]` |
## Invariant Guard Map
...
| `INV-SUBHYD-032` | runtime | `ksatadj` effective-conductivity branch evaluator, source-intent operand-lineage validator, and frost-cap composition validator | Typed hard error when `sat_frac`, `avthetafc`, `avthetadr`, branch parameters, unit conversion, or final frost-cap composition diverge from `REF-SUBHYD-KSATADJ-INTENT` | REFINTENT001 H2637 forest-conductivity closure gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `Keff_ksatadj` | `wb14_soil_conductivity_m_s`; `wb14_effective_conductivity_m_s = min(Keff_ksatadj, frost_infcap_m_s)` when a positive frost cap is present | Conductivity after source-intent `ksatadj` adjustment before frost/runoff storage effects | `m s^-1` preserved | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| Forest `ksatadj` source-intent authority (`INV-SUBHYD-032`) | effective-conductivity branch evaluator, operand-lineage validator, and frost-cap composer | Hard error on source-intent saturation-fraction, exponent, branch-parameter, unit-conversion, or frost-cap composition divergence | REFINTENT001 H2637 forest-conductivity closure gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
|---|---|---|---|---|---|---|
| `WB12-RECONCILIATION-COUPLING-ADDENDUM` | `SC-SUBHYD-001.md#wb12-reconciliation-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-009, INV-SUBHYD-021, INV-SUBHYD-022` | `none` | SCSTRUCT07 map-in-core: `Qd` closure handoff maps to `INV-SUBHYD-009`; same-pass WB19 flux handoff maps to `INV-SUBHYD-021`; carryover precedence maps to `INV-SUBHYD-022`. |
| `WB13-DAILY-OUTPUT-COUPLING-ADDENDUM` | `SC-SUBHYD-001.md#wb13-daily-output-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-009, INV-SUBHYD-021` | `none` | SCSTRUCT07 map-in-core: WB13 subsurface/drainage publication preserves `Qd` coupling semantics through `INV-SUBHYD-009` and flux-authoritative same-pass handoff through `INV-SUBHYD-021`. |
| `HPHYS0203-SUBSURFACE-WB13-ROBUSTNESS-VALIDATION-ADDENDUM` | `SC-SUBHYD-001.md#hphys0203-subsurface-wb13-robustness-validation-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-009, INV-SUBHYD-014, INV-SUBHYD-021` | `none` | SCSTRUCT07 map-in-core: finite/non-negative publication and no-surrogate WB13 subsurface checks map to coupling `INV-SUBHYD-009`, WB19 guard `INV-SUBHYD-014`, and same-pass handoff `INV-SUBHYD-021`; percolation-owned `Dp` context remains core narrative, not relocated. |
| `HPHYS0234-WB13-SUBSURFACE-FLUX-AUTHORITY-ANTI-SHADOW-ADDENDUM` | `SC-SUBHYD-001.md#hphys0234-wb13-subsurface-flux-authority-anti-shadow-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-021` | `none` | SCSTRUCT07 map-in-core: WB13 `q`/`Qdd`/`Qd` flux-over-state anti-shadow authority is directly exposed by the WB19->WB12/WB13 handoff invariant. |
| `HPHYS0208-COUPLED-SUBSURFACE-RESIDUAL-CLOSURE-ADDENDUM` | `SC-SUBHYD-001.md#hphys0208-coupled-subsurface-residual-closure-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-012, INV-SUBHYD-014, INV-SUBHYD-019` | `none` | SCSTRUCT07 map-in-core: WB19 layer-aware lateral execution and `drfc` lineage map to `INV-SUBHYD-012`; fail-closed missing/domain checks map to `INV-SUBHYD-014`; FC/WP + COCA water-yield lineage maps to `INV-SUBHYD-019`; cross-domain seed context remains core narrative. |
| `HPHYS0218-WB19-DRFC-THRESHOLD-LINEAGE-ADDENDUM` | `SC-SUBHYD-001.md#hphys0218-wb19-drfc-threshold-lineage-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-012, INV-SUBHYD-014, INV-SUBHYD-019, INV-SUBHYD-025, INV-SUBHYD-026` | `none` | SCSTRUCT07 map-in-core: `drfc_i` threshold, `coca` guard posture, FC/WP + COCA coupling, frozen-adjusted floors, and daily lane threshold branch semantics are exposed by the cited invariants; detailed threshold text remains core. |
| `HPHYS0221-WB19-WATER-YIELD-AND-SATURATED-DEPTH-COUPLING-ADDENDUM` | `SC-SUBHYD-001.md#hphys0221-wb19-water-yield-and-saturated-depth-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-015, INV-SUBHYD-019, INV-SUBHYD-024` | `none` | SCSTRUCT07 map-in-core: `solwpv` saturated-depth mutation maps to `INV-SUBHYD-015`; `avfca`/`watyld` theta lineage maps to `INV-SUBHYD-019`; HPHYS0247 layer selection maps to `INV-SUBHYD-024`. |
...
| `HPHYS0238-WB19-HOURLY-ITERATIVE-LATERALDRAINAGE-ADDENDUM` | `SC-SUBHYD-001.md#hphys0238-wb19-hourly-iterative-lateraldrainage-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-020` | `none` | SCSTRUCT07 map-in-core: hourly per-substep recomputation and accumulated `q`/`Qdd` publication are exposed by `INV-SUBHYD-020`; vector detail stays core-resident. |
| `HPHYS0239-WB19-WB12WB13-HANDOFF-ORDERING-ADDENDUM` | `SC-SUBHYD-001.md#hphys0239-wb19-wb12wb13-handoff-ordering-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-021, INV-SUBHYD-023` | `none` | SCSTRUCT07 map-in-core: deterministic WB19->WB12/WB13 same-pass handoff and anti-shadow semantics map to `INV-SUBHYD-021`; hourly drainage/lateral order maps to `INV-SUBHYD-023`. |
| `HPHYS0240-HOURLY-RUNOFF-CARRYOVER-HANDOFF-ADDENDUM` | `SC-SUBHYD-001.md#hphys0240-hourly-runoff-carryover-handoff-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-022` | `none` | SCSTRUCT07 map-in-core: same-pass `wb12_runoff_carryover` precedence, anti-shadow semantics, and malformed-boundary validation are exposed by `INV-SUBHYD-022`. |
| `HPHYS0242-HOURLY-DRAINAGELATERALSATURATION-TAIL-ADDENDUM` | `SC-SUBHYD-001.md#hphys0242-hourly-drainagelateralsaturation-tail-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-023` | `none` | SCSTRUCT07 map-in-core: hourly `Drainage -> LateralTransfer`, final `Qd`, `ui_LfCrf`, and `ui_SCrunf` carry obligations are exposed by `INV-SUBHYD-023`. |
| `HPHYS0247-WB19-BASELINE-SATURATED-ZONE-CAPACITY-ADDENDUM` | `SC-SUBHYD-001.md#hphys0247-wb19-baseline-saturated-zone-capacity-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-024, INV-SUBHYD-025, INV-SUBHYD-027` | `none` | SCSTRUCT07 map-in-core: hourly `meblfc`, `fffx`, and legacy multiplier authority maps to `INV-SUBHYD-024`; frozen-floor interaction maps to `INV-SUBHYD-025`; modern hourly conductivity lineage maps to `INV-SUBHYD-027`. |
| `HPHYS0252-WB19-FROZEN-ADJUSTED-LATERAL-STORAGE-ADDENDUM` | `SC-SUBHYD-001.md#hphys0252-wb19-frozen-adjusted-lateral-storage-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-024, INV-SUBHYD-025` | `none` | SCSTRUCT07 map-in-core: frozen-adjusted `fzdrfc` capacity and withdrawal floors are exposed by `INV-SUBHYD-025`, while unfrozen `drfc` conductivity weighting remains governed by `INV-SUBHYD-024`. |
| `HPHYS0256-WB19-DAILY-LATERAL-LANE-BRANCH-ADDENDUM` | `SC-SUBHYD-001.md#hphys0256-wb19-daily-lateral-lane-branch-addendum` | `active` | `maps-to-existing-INV` | `INV-SUBHYD-026` | `none` | SCSTRUCT07 map-in-core: daily-vs-hourly WB19 lane selection, daily `hk`/`fzul` conductivity weighting, and `solwpv` branch behavior are exposed by `INV-SUBHYD-026`. |
...
| GAP-SUBHYD-002 | Concrete openWEPP runtime-field aliases for remaining non-`ksatadj` subsurface/drainage surfaces are not yet fixed. | `Keff_ksatadj` aliases are fixed by version 34; remaining alias map work stays pending boundary finalization. | non-promotable | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `2026-07-03` | `34` | `Codex` | DFF-WS2 amendment: lifted `BR-SUBHYD-KSATADJ-GUARD` from governance `HOLD` to runtime typed hard-fail, fixed `Keff_ksatadj` aliases for the direct-runtime port, and recorded positive frost-cap composition as `min(Keff_ksatadj, frost_infcap_m_s)`. |
...
| `2026-07-02` | `13` | `Claude Code` | MOFEFID-C01 amendment: added `REF-SUBHYD-OBSERVED-LATERAL-ENVELOPE` (WS10/Maimai/Panola/Weiler field corpus + vendored observed datasets) and `INV-SUBHYD-033` (four-tier forest lateral-flow observed-authority envelope — annual yield, event threshold, event ratio, ET plausibility — as an ADR-0011/0017 investigation flag, magnitude judged last). Promotes the FARPOINT01 external-authority gap (`docs/backlog/20260618-forest-lateral-flow-absolute-magnitude-authority.md`) to a ratified envelope; judgment run is MOFEFID-C03. |
| `2026-07-02` | `14` | `Claude Code` | MOFEFID-C03 amendment: `INV-SUBHYD-033` event tiers (ENV-T/ENV-E) judged on quickflow-separated response, not total export (baseflow-contamination finding). H2637 post-DC01 not-contradicted on all four tiers (ENV-Y 0.673, ENV-ET 863, ENV-E 0.46 + ascending shape, ENV-T shape present ~10-20 mm wet-end); FARPOINT01 magnitude flag resolved. |
| `2026-07-02` | `15` | `Claude Code` | MOFEFID-C03 review disposition: clarified `INV-SUBHYD-033` ENV-Y lateral term as outlet-OFE export (not internal-OFE sum, which double-counts under `INV-RUNOFFPART-031`); ENV-E robustness to the quickflow separation slope recorded (in-band 0.30–0.58 across the range; parameter-free surface anchor 0.48). |
