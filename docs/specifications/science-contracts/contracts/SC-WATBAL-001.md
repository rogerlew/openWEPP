...
| INV-WATBAL-098 | MOFE01 M-F-REDO per-OFE publication anti-clone and active surface-handoff invariant: multi-OFE WB13/H.wat publication may clear only when OFE-keyed records are sourced from lane-local dynamic state and active routed-day evidence proves at least one non-zero adjacent surface handoff (`downstream UpStrmQ == upstream QOFE` after declared area scaling) with independently stored sent/received operands. On routed multi-OFE hillslopes, the published hydrology vector (`Q`, `QOFE`, `UpStrmQ`, `Es`, `Ep`, `Dp`, `Total-Soil`, `SoilWaterTotal`, and contracted successors) must not be all-OFE-identical for active days unless an explicit physics/no-difference rationale is recorded and independently verified. Conservation residual closure is necessary but insufficient: aggregate-duplicated records, row-cardinality-only evidence, zero-on-zero transfer acceptance, public-WAT synthesis of `QOFE`/`UpStrmQ`, or all-OFE clone hydrology hard-fails publication promotion. | hard-fail | REF-WATBAL-LEGACY-HOURLY-CARRY, REF-WATBAL-LEGACY-WB13, INV-WATBAL-096, INV-WATBAL-097, SC-SYSTEM-001#INV-SYSTEM-031 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `MOFE01-M-F-REDO-PER-OFE-PUBLICATION-ANTI-CLONE-ADDENDUM` | `SC-WATBAL-001.md#mofe01-m-f-redo-per-ofe-publication-anti-clone-addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-098` | `none` | MOFE01 M-F-REDO: per-OFE WB13/H.wat publication requires lane-local source lineage, nonzero active adjacent surface handoff, and anti-clone hydrology-vector evidence; conservation closure alone is not acceptance evidence. |
...
| `INV-WATBAL-098` | runtime + governance | Per-OFE WB13/H.wat publication validator spanning hydrology-vector distinctness, active nonzero surface handoff, anti-clone comparison, and public-row source lineage | Typed hard error / explicit `HOLD` when multi-OFE records are aggregate clones, all active-day OFE hydrology vectors are identical without independent physical rationale, downstream `UpStrmQ == upstream QOFE` is proven only by zero-on-zero rows, or `QOFE`/`UpStrmQ` are synthesized from public WAT rows instead of transfer state | MOFE01 M-F-REDO anti-clone publication gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
2. The storage delta used by frost-active per-OFE checks is
   `Δ(Total-Soil + frozwt)` from WB11/frost runtime state. A named frost
   fixture is required before M-E4 can clear.
