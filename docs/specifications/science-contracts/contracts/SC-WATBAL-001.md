...
| INV-WATBAL-098 | MOFE01 M-F-REDO per-OFE publication anti-clone and active surface-handoff invariant: multi-OFE WB13/H.wat publication may clear only when OFE-keyed records are sourced from lane-local dynamic state and active routed-day evidence proves at least one non-zero adjacent surface handoff (`downstream UpStrmQ == upstream QOFE` after declared area scaling) with independently stored sent/received operands. On routed multi-OFE hillslopes, the published hydrology vector (`Q`, `QOFE`, `UpStrmQ`, `Es`, `Ep`, `Dp`, `Total-Soil`, `SoilWaterTotal`, and contracted successors) and local surface runoff (`Q - UpStrmQ`) must not be all-OFE-identical for active days unless an explicit physics/no-difference rationale is recorded and independently verified. WB14 runoff reconciliation on declared multistep MOFE/hourly lanes must not accept seeded or stale `wb12_infiltration` as producer-published infiltration; it must compute lane-local infiltration unless one-step WB18 lineage is declared. Conservation residual closure is necessary but insufficient: aggregate-duplicated records, row-cardinality-only evidence, zero-on-zero transfer acceptance, public-WAT synthesis of `QOFE`/`UpStrmQ`, or all-OFE clone hydrology hard-fails publication promotion. | hard-fail | REF-WATBAL-LEGACY-HOURLY-CARRY, REF-WATBAL-LEGACY-WB13, INV-WATBAL-096, INV-WATBAL-097, SC-SYSTEM-001#INV-SYSTEM-031 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `MOFE01-M-F-REDO-PER-OFE-PUBLICATION-ANTI-CLONE-ADDENDUM` | `SC-WATBAL-001.md#mofe01-m-f-redo-per-ofe-publication-anti-clone-addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-098` | `none` | MOFE01 M-F-REDO/M-F-REDO-CLONE: per-OFE WB13/H.wat publication requires lane-local source lineage, nonzero active adjacent surface handoff, local-runoff anti-clone evidence, WB14 multistep infiltration lineage, and anti-clone hydrology-vector evidence; conservation closure alone is not acceptance evidence. |
...
| `INV-WATBAL-098` | runtime + governance | Per-OFE WB13/H.wat publication validator spanning hydrology-vector distinctness, local `Q - UpStrmQ` runoff distinctness, active nonzero surface handoff, WB14 multistep infiltration lineage, anti-clone comparison, and public-row source lineage | Typed hard error / explicit `HOLD` when multi-OFE records are aggregate clones, all active-day OFE hydrology vectors or local runoff values are identical without independent physical rationale, declared multistep MOFE/hourly WB14 lanes accept seeded/stale `wb12_infiltration`, downstream `UpStrmQ == upstream QOFE` is proven only by zero-on-zero rows, or `QOFE`/`UpStrmQ` are synthesized from public WAT rows instead of transfer state | MOFE01 M-F-REDO anti-clone publication gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
2. Active routed-day evidence must include at least one nonzero adjacent
   surface handoff where the downstream `UpStrmQ` matches the upstream `QOFE`
   after declared area scaling. Exact-zero transfer residuals are acceptable
   only when paired with nonzero sent/received operands elsewhere in the same
   acceptance set.
...
6. M-F-REDO-CLONE acceptance must include local runoff (`Q - UpStrmQ`)
   distinctness on active routed multi-OFE days and must reject WB14
   multistep-lane use of seeded/stale `wb12_infiltration` as a
   producer-published infiltration result.
...
|---|---|---|---|
| `2026-06-13` | `158` | `Codex` | MOFE01 M-F-REDO-CLONE amendment: tightened `INV-WATBAL-098` to require local runoff (`Q - UpStrmQ`) anti-clone evidence and prohibit WB14 multistep MOFE/hourly lanes from accepting seeded or stale `wb12_infiltration` as producer-published infiltration; clarified `INV-WATBAL-096` frost-active per-OFE WB12/frozwt exchange terms. |
