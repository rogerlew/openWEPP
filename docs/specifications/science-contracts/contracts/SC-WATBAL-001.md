...
| `INV-WATBAL-103` | Hourly source custody: routed melt/liquid and surface/lateral runon are hourly liquid supplies admitted to WB14 infiltration and depression-storage partition exactly once, not post-partition runoff limbs. WB14 preserves each resulting excess in its modeled hour after depression-storage and same-pass-infiltration debits; frost-retained local liquid is proportionally debited from those positive WB14 runoff bins because no finer producer clock exists. Each nonnegative `ui_SCrunf(h)`-lineage surface-saturation return remains in its WB19-produced hour when assembling `q_hourly(h)`. None of these sources may be collapsed to a daily amount, reassigned to positive rainfall intervals, spread over a storm duration, or assigned a uniform fallback. Saturation-only, melt-supply-only, and runon-supply-only positive runoff therefore retains its modeled hourly timing. | hard-fail | REF-WATBAL-HOURLY-PEAK, REF-WATBAL-LEGACY-HOURLY-CARRY, REF-WATBAL-LEGACY-HOURLY-FIN, SC-RUNOFFPART-001#INV-RUNOFFPART-031 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| TOL-WATBAL-009 | WB14 hourly-to-daily post-partition runoff-ledger reconciliation | `abs(Σ q_wb14_hourly - frost_retained_local_liquid - Q_partition) <= 24 * 1e-9 m * max(1, abs(Σ q_wb14_hourly), abs(frost_retained_local_liquid), abs(Q_partition))` | The WB14 interval solve permits `1e-9 m` arithmetic closure per each of 24 intervals. This bounded aggregate allowance reconciles the independently accumulated daily partition scalar to the authoritative hourly ledger; it does not absorb a missing source, invent timing, or authorize loss of a positive hourly source. Material mismatch hard-fails. A within-bound floating residual is applied once to the largest runoff bin after the frost-retention debit so hourly depth sums reconstruct exactly. |
