...
| `INV-WATBAL-103` | Hourly source custody: routed melt/liquid and surface/lateral runon are hourly liquid supplies admitted to WB14 infiltration and depression-storage partition exactly once, not post-partition runoff limbs. WB14 is the sole owner of both cumulative same-pass infiltration and each resulting hourly excess. A later daily-only snow or same-pass infiltration reconstruction cannot increase infiltration or debit any positive hourly bin; doing so would manufacture peak timing even for local-only supply. A daily-only frost-retention debit may clear the complete local hourly series when no positive local runoff remains; partial retention that leaves positive runoff requires a producer-timed hourly frost-retention ledger and otherwise hard-fails. Neither daily debit may be distributed across positive bins to manufacture a peak. Each nonnegative `ui_SCrunf(h)`-lineage surface-saturation return remains in its WB19-produced hour when assembling `q_hourly(h)`. None of these sources may be collapsed to a daily amount, reassigned to positive rainfall intervals, spread over a storm duration, or assigned a uniform fallback. Saturation-only, melt-supply-only, and runon-supply-only positive runoff therefore retains its modeled hourly timing. | hard-fail | REF-WATBAL-HOURLY-PEAK, REF-WATBAL-WB14-INTERVAL-CLOSURE, REF-WATBAL-LEGACY-HOURLY-CARRY, REF-WATBAL-LEGACY-HOURLY-FIN, SC-RUNOFFPART-001#INV-RUNOFFPART-031 | `[DIRECT][Static] + [INFERENCE][Static]` |
