...
| `INV-WATBAL-098` | runtime + governance | Per-OFE WB13/H.wat publication validator spanning hydrology-vector distinctness, raw local-runoff operand distinctness, active nonzero surface handoff, WB14 multistep infiltration lineage, public `QOFE = runoff * efflen / slplen`, public `Q = runoff * efflen / totlen`, anti-clone comparison, and public-row source lineage | Typed hard error / explicit `HOLD` when multi-OFE records are aggregate clones, all active-day OFE hydrology vectors or raw local runoff values are identical without independent physical rationale, declared multistep MOFE/hourly WB14 lanes accept seeded/stale `wb12_infiltration`, handoff is proven only by zero-on-zero rows, `QOFE`/`UpStrmQ` are synthesized from public WAT rows instead of transfer state, or downstream multi-OFE `QOFE` aliases cumulative `Q` where `slplen != totlen` | MOFE01 M-F-REDO anti-clone and M-F-REDO2 runoff-publication gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
6. M-F-REDO-CLONE acceptance must include raw local runoff distinctness on
   ...
   result.
7. M-F-REDO2 acceptance must publish the same routed runoff with two
   baseline-authoritative normalizations: public `QOFE = runoff * efflen /
   slplen` and public `Q = runoff * efflen / totlen`. Internal water-balance
   identity checks must continue to use raw transfer/runoff operands, not the
   geometry-normalized public `Q` value.
