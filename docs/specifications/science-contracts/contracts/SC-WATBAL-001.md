...
| `RM` | `m` | WB13 daily rainfall + irrigation + snowmelt publication term. Baseline publication uses post-winter `rain(iplane) + wmelt(iplane) + irdept(iplane) + iraplo(iplane)`, not raw precipitation plus a SWE-delta proxy. | climate/winter/runoff/irrigation coupling | WB13 daily water-balance output |
...
| `RM` | `post-winter rain + wmelt + Irr` | `SC-WATBAL-001` `INV-WATBAL-064`; `SC-RUNOFFPART-001` `INV-RUNOFFPART-019`; `SC-SNOWFREEZE-001` `INV-SNOWFREEZE-022` | `crates/openwepp-runner/src/hillslope/mod.rs` consumes `snow.routed_melt_m` plus post-winter rain branch -> `("RM", rm)` | `HKERNEL-WB13-HWAT-E-001..003`, `HS-SIMOUT-E-001` |
