...
| REF-WATBAL-HOURLY-PEAK | `SC-RUNOFFPART-001#INV-RUNOFFPART-031`, ADR-0036 shared hourly runoff-shape authority, and dimensional/conservation invariants | A peak asserted at hourly resolution must consume the post-partition modeled hourly runoff depths, preserve hourly saturation-return timing, close to event runoff, and convert depth rate to volume rate exactly once with area. | `[DIRECT][Static] + [INFERENCE][Static]` |
| REF-WATBAL-WB14-INTERVAL-CLOSURE | `SC-RUNOFFPART-001#TOL-RUNOFFPART-007` and its 24-bin hourly WB14 execution basis | Each WB14 interval may require at most `1e-9 m` infiltration-boundary roundoff normalization. Summing the independently accumulated interval ledger therefore permits at most `24 * 1e-9 m` aggregate arithmetic difference from the daily scalar; this authority does not supply missing process timing. | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `Interception` | `mm` | WB13/WAT publication alias of daily interception `I` after the named `m`-to-`mm` conversion. | WB13/WAT publication | hillslope WAT output |
...
| `UpStrmQ` | `m` runtime; WB13/WAT publication `mm` | Upstream surface-runoff carry component derived from `Σui_SUrunf(ii)` with area-ratio scaling. | MOFE hourly carry resolver | downstream OFE hourly `xfin`, WB12 closure, hillslope WAT output |
| `SubRIn` | `m` runtime; WB13/WAT publication `mm` | Upstream lateral-flow carry component derived from `Σui_LfUrf(ii)` with area-ratio scaling. | MOFE hourly carry resolver | downstream OFE hourly `xfin`, WB12 closure, hillslope WAT output |
...
| `frozwt` | `mm` | WB13/hillslope WAT frozen-water publication term sourced from the frost/water exchanged frozen store (`frost.runtime_frwatc_frozen_water_after_m`), additive to `Total-Soil` for frost-active storage audits. | frost/water-balance publication | hillslope WAT output |
...
| `q_hourly(h)` | `m` per one-hour bin | Post-partition modeled surface-runoff depth for hour `h`: WB14 hourly excess after infiltration, depression-storage debit, same-pass infiltration debit, and frost-retention reconciliation, plus the WB19 hourly surface-saturation return. Routed melt/liquid and runon enter the WB14 hourly supply exactly once and are not added again as runoff limbs. | WB14/WB19 post-partition hourly-runoff authority | WB16 maximum-hourly-mean peak and hourly erosion/transfer/interchange consumers |
...
| MOFE hourly carry-array outputs | `ui_SCrunf_0001..0024`, `ui_LfCrf_0001..0024`, copy-forward `ui_SUrunf_0001..0024`, `ui_LfUrf_0001..0024`, plus separated `UpStrmQ` and `SubRIn` component totals when MOFE hourly carry is enabled |
...
| INV-WATBAL-012 | PL14 replay-candidate emission invariant: WB13 candidate rows staged for strict Tier-A replay must preserve canonical 25-column schema and deterministic `(Y, J, OFE)` ordering; additive interchange parquet extensions must be versioned and identified as non-canonical replay columns. Missing required symbols/artifacts or schema/arity violations must hard-fail replay staging without truncation, padding, or legacy-surface substitution. | hard-fail | REF-WATBAL-CH5-BAL, REF-WATBAL-PHYS-BOUNDS | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| INV-WATBAL-059 | HPHYS0284/SNOWSCI-S1 spring snow-retention and conservation invariant: WB13 `RM`, signed `S`, WB12 liquid forcing, WB13 `Snow-Water`, and runtime snow carry-state must consume the same authoritative snow-storage transition from `SC-SNOWFREEZE-001#INV-SNOWFREEZE-019`. `Snow-Water` is derived from the post-hourly depth/density store, and the snowpack routed-melt component of `RM`/`S` must equal `old SWE + snowfall water equivalent + retained rain - new SWE` within `SC-SNOWFREEZE-001#TOL-SNOWFREEZE-006`. Water-balance closure must not force `Snow-Water` from a separate SWE debit, reconstruct `RM` from raw precipitation/SWE deltas, preserve a negative SWE ledger behind a clamp, or allow mixed positive/negative hourly melt diagnostics to make snow water vanish from both storage and routed melt. | hard-fail | INV-WATBAL-055, INV-WATBAL-056, INV-WATBAL-058, SC-SNOWFREEZE-001#INV-SNOWFREEZE-019, REF-WATBAL-LEGACY-WB13, REF-WATBAL-CH5-SNOW | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| INV-WATBAL-071 | HPHYS0296/HPHYS0303 snow/`RM` acceptance invariant: after cumulative accounting assigns dominant residual ownership to snow/`RM`, fixed-comparator negative-melt evidence, candidate WB13 publication identity, and snow-state closure are diagnostic evidence only and may not by themselves accept a residual as semantic-not-bit divergence. Before any window leaves the failing set, water-balance closure must reference the `SC-SNOWFREEZE-001#INV-SNOWFREEZE-027` per-window defective-model verdict: mechanistic `file:line` root cause in both openWEPP and the active fixed comparator or archived original comparator as applicable, reconstruction controlled experiment to named tolerance, independent correctness adjudication, and ADR0017 peer verdict disposition (`HARNESS-SURFACE-MISMATCH`, `LEGACY-DEFECTIVE`, `OPENWEPP-DEFECTIVE`, or `UNRESOLVED`). `LEGACY-DEFECTIVE` windows may be auditable documented-legacy-defective re-tierings; `HARNESS-SURFACE-MISMATCH`, `OPENWEPP-DEFECTIVE`, and `UNRESOLVED` windows remain failing/owned `HOLD` unless explicitly corrected or superseded. No WB17/WB18/WB19/WB13 downstream compensation is allowed. | governance-hold | INV-WATBAL-070, INV-WATBAL-068, INV-WATBAL-087, SC-SNOWFREEZE-001#INV-SNOWFREEZE-027, SC-RUNOFFPART-001#INV-RUNOFFPART-024 | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-WATBAL-072 | HPHYS0297 snow/`RM` defect-ledger water-balance invariant: water-balance residual accounting must consume the HPHYS0297 ledger before re-tiering any snow/`RM` window or returning focus to WB17/WB18/WB19. A valid ledger row must include observed candidate/baseline `RM`, reconstructed `/workdir/wepp-forest_260430_baseline` branch `RM`, reconstruction residual to named tolerance, closed `Q` and producer-consumer identity evidence, independent correctness rationale, and a final ADR0017 peer verdict: `HARNESS-SURFACE-MISMATCH`, `LEGACY-DEFECTIVE`, `OPENWEPP-DEFECTIVE`, or `UNRESOLVED`. Rows with reconstruction residual outside tolerance remain failing/owned `HOLD`; rows with unit/surface mismatch remain `HARNESS-SURFACE-MISMATCH`; neither can be excluded from semantic accounting without correction or supersession. | governance-hold | INV-WATBAL-071, INV-WATBAL-070, INV-WATBAL-087, SC-SNOWFREEZE-001#INV-SNOWFREEZE-028, SC-RUNOFFPART-001#INV-RUNOFFPART-025 | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-WATBAL-073 | HPHYS0298 paired snow/`RM` source-partition invariant: water-balance residual accounting must consume the HPHYS0298 partition ledger before re-tiering any of the nine H1/H7/H39 target windows or returning focus to WB17/WB18/WB19/WB13. A valid ledger row must include baseline observe identity status, full H1..H39 same-HEAD metrics, candidate/baseline `RM` and residual, ordered first-divergent cut-point, canonical symbol values and units, baseline and openWEPP source-line provenance, closed `Q` and WB13 `RM` identity status, prohibited compensation paths, independent correctness rationale, and a final ADR0017 peer verdict: `HARNESS-SURFACE-MISMATCH`, `LEGACY-DEFECTIVE`, `OPENWEPP-DEFECTIVE`, or `UNRESOLVED`. Observe-identity failure, missing first-divergence evidence, missing verdict, or unit/surface mismatch keeps the window failing/owned `HOLD` or `HARNESS-SURFACE-MISMATCH` and prevents downstream WB17/WB18/WB19/WB13 compensation. | governance-hold | INV-WATBAL-072, INV-WATBAL-071, INV-WATBAL-087, SC-SNOWFREEZE-001#INV-SNOWFREEZE-029, SC-RUNOFFPART-001#INV-RUNOFFPART-026 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| INV-WATBAL-096 | MOFE01 M-B inter-OFE water-balance conservation invariant: multi-OFE hourly lanes must preserve three conservation identities before WB13/WAT promotion. Per-element identity: `local_liquid + UpStrmQ + SubRIn = infiltration + Q_partition + Δdepression_storage + ε` before current-OFE `surdra` addback. Transfer identity: each upstream OFE's `ui_SCrunf(ii)` and `ui_LfCrf(ii)` must become the downstream OFE's scaled `ui_SUrunf(ii)` and `ui_LfUrf(ii)`, with `UpStrmQ = Σui_SUrunf(ii)` and `SubRIn = Σui_LfUrf(ii)` after scaling. Hillslope-total identity: inter-OFE transfers cancel internally, leaving only external precipitation/melt/irrigation inputs, outlet runoff, lateral/drain/percolation/ET losses, and storage change at the declared hillslope boundary. Frost-active per-OFE evidence must carry WB12 final liquid storage, `frost.runtime_frwatc_net_liquid_delta_m`, `frost.runtime_watbtm_m`, `frost.runtime_watpdg_m`, and the named internal frost adjustment term into `Δ(Total-Soil + frozwt)` rather than absorbing frost exchange as tolerance. Acceptance evidence must compare independently sourced state/flux/transfer terms; WB13 row aliases or values rebuilt from the same validation operand are structural checks only, not conservation identities. Missing component surfaces, aggregate-only transfer substitution, non-zero single-OFE upstream carry, tautological identity evidence, unaccounted frost exchange, or identity residuals beyond tolerance hard-fail. Runon membership in the same-pass infiltration supply is governed by `SC-RUNOFFPART-001#INV-RUNOFFPART-031` (DC01); the per-element closure identity here is unchanged by that admission. | hard-fail | REF-WATBAL-LEGACY-HOURLY-CARRY, REF-WATBAL-CH5-BAL, SC-RUNOFFPART-001#INV-RUNOFFPART-028, INV-WATBAL-033, INV-WATBAL-034, INV-WATBAL-095 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| INV-WATBAL-098 | MOFE01 M-F-REDO per-OFE publication anti-clone, active surface-handoff, and M-F-REDO2 runoff-normalization invariant: multi-OFE WB13/H.wat publication may clear only when OFE-keyed records are sourced from lane-local dynamic state and active routed-day evidence proves at least one non-zero adjacent surface handoff from independently stored sent/received transfer operands. On routed multi-OFE hillslopes, the published hydrology vector (`Q`, `QOFE`, `UpStrmQ`, `Es`, `Ep`, `Dp`, `Total-Soil`, `SoilWaterTotal`, and contracted successors) and raw lane-local surface runoff from transfer/internal operands must not be all-OFE-identical for active days unless an explicit physics/no-difference rationale is recorded and independently verified. Public `QOFE` equals public `Q` on all canonicalized WB13 rows (MOFE04 canonicalized policy items 1/6; `SC-RUNOFFPART-001#INV-RUNOFFPART-032`, MOFEFID-B02); the retained per-OFE local-length basis feeds only `H.pass.runvol`/peak, which stay byte-invariant. **Superseded (2026-07-02):** the former `QOFE == Q where slplen != totlen` rejection was an anti-clone proxy; per-OFE genuineness is now proven solely by the hydrology-vector distinctness, raw local-runoff distinctness, active surface-handoff, and lane-local lineage evidence below — NOT by `QOFE != Q`. WB14 runoff reconciliation on declared multistep MOFE/hourly lanes must not accept seeded or stale `wb12_infiltration` as producer-published infiltration; it must compute lane-local infiltration unless one-step WB18 lineage is declared. Conservation residual closure is necessary but insufficient: aggregate-duplicated records, row-cardinality-only evidence, zero-on-zero transfer acceptance, public-WAT synthesis of transfer fields, all-OFE clone hydrology (raw local runoff or hydrology vectors identical across active OFEs without recorded rationale) hard-fails publication promotion. | hard-fail | REF-WATBAL-LEGACY-HOURLY-CARRY, REF-WATBAL-LEGACY-WB13, INV-WATBAL-096, INV-WATBAL-097, SC-SYSTEM-001#INV-SYSTEM-031 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `INV-WATBAL-102` | WB16 hourly peak authority: for positive reconciled runoff, the production peak operator consumes the same post-partition 24-bin runoff depths used by hourly erosion, inter-OFE transfer, and HBP serialization; hourly depths sum to `Q`, and `peakro_depth = max_h(q_hourly(h)/3600 s)`. The closing depths are authoritative; normalized weights may be derived only after closure and may not be used to manufacture the depths from a daily scalar. Rainfall elapsed duration, maximum rainfall intensity, APPMTH dimensionless branches, and a uniform synthetic-time fallback cannot carry the production peak claim. Dry runoff emits zero peak and zero rectangular-equivalent duration. Positive runoff without reconstructible hourly timing hard-fails. The only positive-side runoff canonicalizations are the source-informed `TOL-WATBAL-006` roundoff case and the bounded interval-ledger reconciliation in `TOL-WATBAL-009`; any positive hourly source preserves its runoff, however small. | hard-fail | REF-WATBAL-HOURLY-PEAK, SC-RUNOFFPART-001#INV-RUNOFFPART-031 | `[DIRECT][Static] + [INFERENCE][Static]` |
| `INV-WATBAL-103` | Hourly source custody: routed melt/liquid and surface/lateral runon are hourly liquid supplies admitted to WB14 infiltration and depression-storage partition exactly once, not post-partition runoff limbs. WB14 preserves each resulting excess in its modeled hour after depression-storage and same-pass-infiltration debits. A local-only daily same-pass infiltration correction may operate on a local-only hourly ledger; if runon is also present, source-tagged hourly debit custody is required and otherwise hard-fails rather than debiting the merged series. A daily-only frost-retention debit may clear the complete local hourly series when no positive local runoff remains; partial retention that leaves positive runoff requires a producer-timed hourly frost-retention ledger and otherwise hard-fails. Neither daily debit may be distributed across positive bins to manufacture a peak. Each nonnegative `ui_SCrunf(h)`-lineage surface-saturation return remains in its WB19-produced hour when assembling `q_hourly(h)`. None of these sources may be collapsed to a daily amount, reassigned to positive rainfall intervals, spread over a storm duration, or assigned a uniform fallback. Saturation-only, melt-supply-only, and runon-supply-only positive runoff therefore retains its modeled hourly timing. | hard-fail | REF-WATBAL-HOURLY-PEAK, REF-WATBAL-WB14-INTERVAL-CLOSURE, REF-WATBAL-LEGACY-HOURLY-CARRY, REF-WATBAL-LEGACY-HOURLY-FIN, SC-RUNOFFPART-001#INV-RUNOFFPART-031 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
Status: `phase2-hold-preconsolidation`
Evidence mode: `Static`

This index is intentionally conservative. It does not relocate addendum narrative. Rows marked `unpromoted-binding` or `undecidable` are retained in the binding core and block sidecar consolidation until semantic mapping, flagged binding promotion, or science-review follow-on resolves them. This preserves binding semantics by failing closed rather than moving live obligations to a non-binding sidecar.

| Entry ID | Source | Status | Binding classification | Canonical binding IDs | Review gate | Notes |
|---|---|---|---|---|---|---|
| `HPHYS0298-WATER-BALANCE-DISPOSITION-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0298-water-balance-disposition-addendum` `HPHYS0298 Water-Balance Disposition Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-073, INV-WATBAL-087` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `HPHYS0299-WATER-BALANCE-UNIT-PROVENANCE-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0299-water-balance-unit-provenance-addendum` `HPHYS0299 Water-Balance Unit/Provenance Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-074, INV-WATBAL-087` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `HPHYS0300-WATER-BALANCE-RAW-POST-RAW-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0300-water-balance-raw-post-raw-addendum` `HPHYS0300 Water-Balance Raw/Post-Raw Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-075` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `HPHYS0301-H39-RAIN-RELEASE-WATER-BALANCE-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0301-h39-rain-release-water-balance-addendum` `HPHYS0301 H39 Rain-Release Water-Balance Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-076` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `HPHYS0302-COMPARATOR-SURFACE-WATER-BALANCE-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0302-comparator-surface-water-balance-addendum` `HPHYS0302 Comparator-Surface Water-Balance Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-077, INV-WATBAL-087` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `HPHYS0305-PAIRED-MELT-TERM-STATE-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0305-paired-melt-term-state-addendum` `HPHYS0305 Paired Melt-Term/State Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-078` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `HPHYS0306-BRANCH-ACTIVE-MELT-TERM-OBSERVE-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0306-branch-active-melt-term-observe-addendum` `HPHYS0306 Branch-Active Melt-Term Observe Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-079` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `HPHYS0307-MELT-CALL-BRANCH-ACTIVATION-LINEAGE-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0307-melt-call-branch-activation-lineage-addendum` `HPHYS0307 Melt-Call Branch Activation Lineage Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-080` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `HPHYS0308-BRANCH-EXTRA-STATE-ORDERING-ADDENDUM` | `provenance/SC-WATBAL-001-provenance.md#hphys0308-branch-extra-state-ordering-addendum` `HPHYS0308 Branch-Extra State-Ordering Addendum` | `historical` | `historical-or-superseded` | `INV-WATBAL-081` | `none` | SCSTRUCT03 batch 2: snow/melt comparator narrative relocated; live residue conserved by cited WATBAL governance invariants and ADR0017 where applicable. |
| `EROD12-CROSS-DOMAIN-OWNERSHIP-AND-GUARD-CLOSURE-ADDENDUM` | `lines 608-900` `EROD12 Cross-Domain Ownership and Guard Closure Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 6 narrower HOLD: broad cross-domain ownership/guard table carries live multi-contract boundary obligations not exactly exposed by a single WATBAL mapping. Owner: SCSTRUCT03-CROSSDOMAIN-BEI-PROMOTION. Next gate: promote/map each cross-domain lane to precise WATBAL/consumer binding IDs before relocation. |
| `WB12-RECONCILIATION-AUTHORITY-ADDENDUM` | `lines 901-913` `WB12 Reconciliation Authority Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 3 narrower HOLD: live WB12 required surfaces, formulas, guard codes, and test vectors are not fully exposed by current invariant rows. Owner: SCSTRUCT03-WB12-BEI-PROMOTION. Next gate: promote precise WB12 runoff/storage reconciliation invariant(s) with guard map, or prove complete mapping to INV-WATBAL-001/013/016/031/032/034 plus TOL-WATBAL-006 before relocation. |
| `WB12-DETERMINISTIC-RECONCILIATION-RULES` | `lines 914-952` `WB12 Deterministic Reconciliation Rules` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 3 narrower HOLD: live WB12 deterministic reconciliation equations and lane-scoped closure-delta semantics require explicit canonical binding exposure. Owner: SCSTRUCT03-WB12-BEI-PROMOTION. Next gate: promote/map WB12 equation and guard/test-vector authority before relocation. |
| `CLIM05-SNOW-COUPLED-RECONCILIATION-ADDENDUM` | `lines 953-954` `CLIM05 Snow-Coupled Reconciliation Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-013` | `none` | SCSTRUCT03 batch 4: mapped to existing signed-S WB12 storage closure invariant; retained in core with adjacent CLIM05 test-vector context. |
| `CLIM05-REQUIRED-COUPLING-SURFACE` | `lines 955-960` `CLIM05 Required Coupling Surface` | `active` | `maps-to-existing-INV` | `INV-WATBAL-013` | `none` | SCSTRUCT03 batch 4: signed `S` required surface maps to `INV-WATBAL-013`; retained in core. |
| `CLIM05-DETERMINISTIC-COUPLING-RULE` | `lines 961-977` `CLIM05 Deterministic Coupling Rule` | `active` | `maps-to-existing-INV` | `INV-WATBAL-013` | `none` | SCSTRUCT03 batch 4: deterministic signed-S storage equation and hard-fail posture map to `INV-WATBAL-013`; retained in core. |
| `CLIM06-FROZEN-SOIL-INFILTRATION-COUPLING-ADDENDUM` | `lines 978-979` `CLIM06 Frozen-Soil Infiltration Coupling Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 4 narrower HOLD: live frozen-soil infiltration coupling is not fully exposed by current WATBAL invariant rows. Owner: SCSTRUCT03-CLIM06-BEI-PROMOTION. Next gate: promote/map CLIM06 active frost controls, runtime frozen-state surfaces, WB14 consumption, and typed guard posture before relocation. |
| `CLIM06-REQUIRED-COUPLING-SURFACES` | `lines 980-987` `CLIM06 Required Coupling Surfaces` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 4 narrower HOLD: CLIM06 required parsed frost/runtime/output surfaces require exact binding exposure. Owner: SCSTRUCT03-CLIM06-BEI-PROMOTION. Next gate: flagged binding promotion or complete mapping proof before relocation. |
| `CLIM06-DETERMINISTIC-COUPLING-RULES` | `lines 988-1021` `CLIM06 Deterministic Coupling Rules` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 4 narrower HOLD: CLIM06 deterministic active-coupling rules and domain guards require exact canonical exposure. Owner: SCSTRUCT03-CLIM06-BEI-PROMOTION. Next gate: promote/map before relocation. |
| `WB14-INFILTRATION-AND-HYETOGRAPH-COUPLING-ADDENDUM` | `lines 1022-1023` `WB14 Infiltration and Hyetograph Coupling Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 4 narrower HOLD: live WB14 hyetograph/infiltration and ksatadj coupling authority is not fully exposed by current WATBAL rows. Owner: SCSTRUCT03-WB14-BEI-PROMOTION. Next gate: promote/map WB14 required surfaces, formulas, tolerances, guard codes, and test vectors before relocation. |
| `WB14-REQUIRED-COUPLING-SURFACES` | `lines 1024-1033` `WB14 Required Coupling Surfaces` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 4 narrower HOLD: WB14 required coupling surfaces require exact binding exposure. Owner: SCSTRUCT03-WB14-BEI-PROMOTION. Next gate: flagged binding promotion or complete mapping proof before relocation. |
| `WB14-DETERMINISTIC-COUPLING-RULES` | `lines 1034-1096` `WB14 Deterministic Coupling Rules` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 4 narrower HOLD: WB14 deterministic hyetograph, runoff, same-pass carryover, and closure-delta semantics require exact canonical exposure. Owner: SCSTRUCT03-WB14-BEI-PROMOTION. Next gate: promote/map before relocation. |
| `WB15-CANOPY-INTERCEPTION-COUPLING-ADDENDUM` | `lines 1097-1098` `WB15 Canopy Interception Coupling Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 4 narrower HOLD: live canopy interception coupling carries equation-input caps, ordering, closure, guard, and vector obligations beyond current `INV-WATBAL-003`. Owner: SCSTRUCT03-WB15-BEI-PROMOTION. Next gate: promote/map WB15 authority before relocation. |
| `WB15-REQUIRED-COUPLING-SURFACES` | `lines 1099-1106` `WB15 Required Coupling Surfaces` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 4 narrower HOLD: WB15 required plant/runtime/closure surfaces require exact binding exposure. Owner: SCSTRUCT03-WB15-BEI-PROMOTION. Next gate: flagged binding promotion or complete mapping proof before relocation. |
...
| `HPHYS0206-CORRECTED-LAYER-NORMALIZATION-AND-MAPPING-CLOSURE-HISTORICAL` | `provenance/SC-WATBAL-001-provenance.md#hphys0206-corrected-layer-normalization-and-mapping-closure-historical` `HPHYS0206 Corrected-Layer Normalization and Mapping Closure (Historical)` | `superseded` | `historical-or-superseded` | `INV-WATBAL-041, INV-WATBAL-042` | `none` | SCSTRUCT03 batch 1: narrative relocated; live residue conserved by cited binding IDs and retained HPARITY02 core rules. |
| `HPHYS0207-FC-WP-DEPTH-AUTHORITY-AND-NORMALIZED-TAIL-CLOSURE` | `lines 1541-1562` `HPHYS0207 FC/WP Depth-Authority and Normalized-Tail Closure` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT02 routed: binding status or precise canonical mapping requires science review; narrative remains in core and is not sidecar-eligible. |
| `HPHYS0216-PROFILEFC-LAYER-AUTHORITY-REALIGNMENT` | `provenance/SC-WATBAL-001-provenance.md#hphys0216-profilefc-layer-authority-realignment` `HPHYS0216 ProfileFC Layer-Authority Realignment` | `superseded` | `historical-or-superseded` | `INV-WATBAL-041, INV-WATBAL-042` | `none` | SCSTRUCT03 batch 1: narrative relocated; live residue conserved by cited binding IDs and retained HPARITY02 core rules. |
| `HPHYS0216D-PROFILEFC-LAYER-TAIL-AUTHORITY-RECONCILIATION` | `provenance/SC-WATBAL-001-provenance.md#hphys0216d-profilefc-layer-tail-authority-reconciliation` `HPHYS0216D ProfileFC Layer+Tail Authority Reconciliation` | `historical` | `historical-or-superseded` | `INV-WATBAL-041, INV-WATBAL-042` | `none` | SCSTRUCT03 batch 1: narrative relocated; live residue conserved by cited binding IDs and retained HPARITY02 core rules. |
| `HPHYS0203-PHYSICS-ROBUSTNESS-VALIDATION-ADDENDUM` | `lines 1601-1624` `HPHYS0203 Physics-Robustness Validation Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT02 routed: binding status or precise canonical mapping requires science review; narrative remains in core and is not sidecar-eligible. |
| `HPHYS0208-FC-THRESHOLD-COUPLED-RESIDUAL-CLOSURE-ADDENDUM` | `lines 1625-1654` `HPHYS0208 FC-Threshold Coupled Residual Closure Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT02 routed: binding status or precise canonical mapping requires science review; narrative remains in core and is not sidecar-eligible. |
| `HPHYS0218-WB19-DRFC-EQUIVALENT-THRESHOLD-CLOSURE-ADDENDUM` | `lines 1655-1670` `HPHYS0218 WB19 `drfc`-Equivalent Threshold Closure Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-009` | `none` | SCSTRUCT03 batch 5: mapped to existing WB17/WB18/WB19 production execution invariant for WB19 threshold/coupling state; retained in core pending exact SUBHYD cross-contract exposure. |
| `HPHYS0221-WB19-WATER-YIELD-SATURATED-DEPTH-COUPLING-ADDENDUM` | `lines 1671-1694` `HPHYS0221 WB19 Water-Yield + Saturated-Depth Coupling Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-009` | `none` | SCSTRUCT03 batch 5: mapped to existing WB17/WB18/WB19 production execution invariant; retained in core with branch/coupling context. |
| `HPHYS0222-WB19-SOLWPV-BRANCH-AUTHORITY-CORRECTION-ADDENDUM` | `lines 1695-1711` `HPHYS0222 WB19 `solwpv` Branch-Authority Correction Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-009` | `none` | same-section invariant references provide a preliminary binding exposure map; relocation still requires conservation crosswalk |
| `HPHYS0224-WB19-REALIZED-WITHDRAWAL-SOIL-WATER-CAP-ADDENDUM` | `lines 1712-1728` `HPHYS0224 WB19 Realized-Withdrawal Soil-Water Cap Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 5 narrower HOLD: live WB19 non-clamping realized-withdrawal cap authority requires exact cross-contract WATBAL/SUBHYD binding exposure. Owner: SCSTRUCT03-WB19-CAP-BEI-PROMOTION. Next gate: promote/map realized withdrawal cap, no-clamp posture, and guard/test obligations before relocation. |
| `HPHYS0225-WB19-LAYER-POOL-AVAILABLE-CAP-AUTHORITY-ADDENDUM` | `lines 1729-1743` `HPHYS0225 WB19 Layer-Pool Available-Cap Authority Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 5 narrower HOLD: live WB19 layer-pool available-cap authority requires exact canonical exposure and external-suite linkage check. Owner: SCSTRUCT03-WB19-CAP-BEI-PROMOTION. Next gate: promote/map before relocation. |
| `HPHYS0226-WB19-LATERAL-SATURATED-THICKNESS-RESPONSE-ADDENDUM` | `lines 1744-1756` `HPHYS0226 WB19 Lateral Saturated-Thickness Response Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 5 narrower HOLD: live WB19 lateral saturated-thickness behavioral authority requires exact binding exposure and Level-4 suite linkage disposition. Owner: SCSTRUCT03-WB19-LATERAL-LAW-BEI-PROMOTION. Next gate: promote/map before relocation. |
| `HPHYS0227-WB19-FC-WP-COCA-WATER-YIELD-COUPLING-ADDENDUM` | `lines 1757-1773` `HPHYS0227 WB19 FC/WP + COCA Water-Yield Coupling Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 5 narrower HOLD: live WB19 FC/WP + COCA water-yield coupling authority requires exact binding exposure and suite linkage. Owner: SCSTRUCT03-WB19-WATYLD-BEI-PROMOTION. Next gate: promote/map before relocation. |
| `HPHYS0234-WB13-SUBSURFACE-FLUX-AUTHORITY-ANTI-SHADOW-ADDENDUM` | `lines 1774-1789` `HPHYS0234 WB13 Subsurface Flux-Authority Anti-Shadow Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-031` | `none` | SCSTRUCT03 batch 5: mapped to existing WB19->WB12->WB13 flux-authority handoff invariant; retained in core. |
| `HPHYS0235-WB18-HOURLY-ITERATIVE-LANE-REANCHORING-ADDENDUM` | `lines 1790-1802` `HPHYS0235 WB18 Hourly Iterative-Lane Reanchoring Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-034, INV-WATBAL-036` | `none` | SCSTRUCT03 batch 5: mapped to hourly cadence/ordering and hourly restrictive-bottom authority; retained in core. |
| `HPHYS0248-WB18-HOURLY-RESTRICTIVE-BOTTOM-ADDENDUM` | `lines 1803-1820` `HPHYS0248 WB18 Hourly Restrictive-Bottom Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-036` | `none` | SCSTRUCT03 batch 5: mapped to H39 hourly Dp/Pe restrictive-bottom invariant; retained in core. |
| `HPHYS0209-PROFILEWP-NEAR-CLOSED-ADJUDICATION-ADDENDUM` | `lines 1821-1840` `HPHYS0209 ProfileWP Near-Closed Adjudication Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT02 routed: binding status or precise canonical mapping requires science review; narrative remains in core and is not sidecar-eligible. |
| `AUTH03-LEVEL-4-CONSTITUTIVE-GATE-BOOTSTRAP-ADDENDUM` | `lines 1841-1863` `AUTH03 Level-4 Constitutive Gate Bootstrap Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-006` | `none` | same-section invariant references provide a preliminary binding exposure map; relocation still requires conservation crosswalk |
| `MOFE04-MULTI-OFE-WB13-WAT-PUBLICATION-POLICY-ADDENDUM` | `lines 1864-1895` `MOFE04 Multi-OFE WB13/WAT Publication Policy Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 6 narrower HOLD: MOFE WB13/WAT publication policy carries live row-id, contributor-cardinality, area, and storage-lineage semantics not fully exposed by current mapped IDs. Owner: SCSTRUCT03-MOFE04-BEI-PROMOTION. Next gate: promote/map exact publication policy obligations before relocation. |
| `ARCH22-TYPED-PRODUCTION-SURFACE-ADDENDUM` | `lines 1896-1916` `ARCH22 Typed Production-Surface Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 6 narrower HOLD: typed production-surface migration authority spans covered WB11/WB12/WB14/WB15/WB16 interfaces and needs exact binding exposure. Owner: SCSTRUCT03-ARCH22-BEI-PROMOTION. Next gate: promote/map typed-boundary obligations before relocation. |
| `SIMIMPL03-PRODUCTION-EXECUTION-OWNERSHIP-AND-INTAKE-GUARDRAIL-ADDENDUM` | `lines 1917-1948` `SIMIMPL03 Production Execution Ownership and Intake Guardrail Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-018, INV-WATBAL-019, INV-WATBAL-020, INV-WATBAL-021` | `none` | SCSTRUCT03 batch 6: mapped to production execution ownership, mode-selection, simulation-owned output, and selective consolidated-intake invariants; retained in core. |
| `EROD13-WAVE-1-ACTIVE-PRODUCER-COUPLING-ADDENDUM` | `lines 1949-1961` `EROD13 Wave-1 Active Producer-Coupling Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT03 batch 6 narrower HOLD: Wave-1 erosion producer-coupling surfaces are live cross-domain ingress obligations. Owner: SCSTRUCT03-EROD13-BEI-PROMOTION. Next gate: promote/map WATBAL producer and SED consumer binding IDs before relocation. |
| `EROD14-WAVE-2-ACTIVE-PRODUCER-COUPLING-ADDENDUM` | `lines 1962-1975` `EROD14 Wave-2 Active Producer-Coupling Addendum` | `deleted-historical` | `none` | `none` | `none` | E.3 stage 2e: the runtime arm is DELETED (SC-SED-001 rev 45); the addendum is retained as historical specification only — no live obligations, the SCSTRUCT promotion gate is void. The Wave-1 chain (`SC-SED-001#INV-SED-016`) owns all inter-OFE erosion coupling. |
| `SIMIMPL14-CONTINUOUS-RUNNER-AND-WB13-SPAN-KEY-ADDENDUM` | `lines 1976-1991` `SIMIMPL14 Continuous Runner and WB13 Span/Key Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-022` | `none` | SCSTRUCT03 batch 6: mapped to continuous WB13 span/key invariant; retained in core. |
| `SIMIMPL15-REPLAY-COMPARATOR-TOOLING-ALIGNMENT-ADDENDUM` | `lines 1992-2009` `SIMIMPL15 Replay Comparator Tooling Alignment Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-023, INV-WATBAL-024` | `none` | SCSTRUCT03 batch 6: mapped to strict-lane/source-provenance and parquet semantic-alias diagnostics invariants; retained in core. |
| `SIMIMPL16-REPLAY-CONTRACT-DERIVED-TEST-COVERAGE-CLOSURE-ADDENDUM` | `lines 2010-2023` `SIMIMPL16 Replay Contract-Derived Test-Coverage Closure Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-025` | `none` | SCSTRUCT03 batch 6: mapped to replay contract-derived test-coverage invariant; retained in core. |
| `SIMIMPL18-RAIN-SNOW-PARTITION-AND-STORAGE-STATE-MUTATION-CLOSURE-ADDENDU` | `lines 2024-2039` `SIMIMPL18 Rain/Snow Partition and Storage-State Mutation Closure Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-026, INV-WATBAL-027` | `none` | SCSTRUCT03 batch 6: mapped to day-key rain/snow publication-source and mutable storage-state invariants; retained in core. |
| `SIMIMPL21-WB11-ET-SOIL-WATER-BASELINE-ORDERING-AND-LINEAGE-ADDENDUM` | `lines 2040-2055` `SIMIMPL21 WB11 ET/Soil-Water Baseline Ordering and Lineage Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-028, INV-WATBAL-029` | `none` | SCSTRUCT03 batch 6: mapped to baseline execution-order and aggregate-lineage invariants; retained in core. |
| `HPHYS0238-WB19-HOURLY-ITERATIVE-LATERAL-DRAINAGE-ADDENDUM` | `lines 2056-2068` `HPHYS0238 WB19 Hourly Iterative Lateral/Drainage Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-030` | `none` | SCSTRUCT03 batch 5: mapped to WB19 hourly iterative execution invariant; retained in core. |
| `HPHYS0239-WB19-WB12-WB13-ORDERING-AND-FLUX-AUTHORITY-HANDOFF-ADDENDUM` | `lines 2069-2085` `HPHYS0239 WB19->WB12->WB13 Ordering and Flux-Authority Handoff Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-031, INV-WATBAL-034` | `none` | SCSTRUCT03 batch 5: mapped to hydrology-tail handoff and hourly cadence/ordering invariants; retained in core. |
| `HPHYS0240-HOURLY-RUNOFF-CARRYOVER-ADDENDUM` | `lines 2086-2097` `HPHYS0240 Hourly Runoff Carryover Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-032` | `none` | SCSTRUCT03 batch 5: mapped to hourly runoff-carryover invariant; retained in core. |
...
| `MOFE01-M-E4-REDO-INTERNAL-WB13-IDENTITY-ACCEPTANCE-ADDENDUM` | `SC-WATBAL-001.md#mofe01-m-e4-redo-internal-wb13-identity-acceptance-addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-096, INV-WATBAL-097` | `none` | MOFE01 M-E4-REDO: internal per-OFE WB13 records are acceptance evidence only when per-element storage and adjacent transfer identities compare independently sourced operands under `TOL-WATBAL-007`; alias/self-built checks are non-acceptance structural checks. |
| `MOFE01-M-F-REDO-PER-OFE-PUBLICATION-ANTI-CLONE-ADDENDUM` | `SC-WATBAL-001.md#mofe01-m-f-redo-per-ofe-publication-anti-clone-addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-098` | `none` | MOFE01 M-F-REDO/M-F-REDO-CLONE/M-F-REDO2: per-OFE WB13/H.wat publication requires lane-local source lineage, nonzero active adjacent surface handoff, raw local-runoff anti-clone evidence, WB14 multistep infiltration lineage, anti-clone hydrology-vector evidence, and public `Q`/`QOFE` length-normalization evidence; conservation closure alone is not acceptance evidence. |
| `MOFE01-M-G-EROSION-QIN-WATER-BALANCE-BOUNDARY-ADDENDUM` | `SC-WATBAL-001.md#mofe01-m-g-erosion-qin-water-balance-boundary-addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-099` | `none` | MOFE01 M-G: water-balance transfer closure and water-derived EROD14 `qin` compatibility provenance do not close downstream sediment-coupled `qin`; manifests must expose water-transfer-only source policy until the SED-owned prior-OFE `qout` and class-fraction handoff follow-on closes. |
| `MOFE01-M-I-HILLSLOPE-TOTAL-CLOSURE-ADDENDUM` | `SC-WATBAL-001.md#mofe01-m-i-hillslope-total-closure-addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-100` | `none` | MOFE01 M-I: per-hillslope conservation closure must be independently computed from internal per-OFE records and areas, exclude internal transfers, publish `hillslope_total_identity_max_abs_mm`, close under `TOL-WATBAL-008`, and keep multi-OFE execution on the persistent per-OFE lifecycle without a discarded aggregate scheduler run. |
...
| `HPHYS0251-SWU-UPTAKE-MAGNITUDE-COUPLING-ADDENDUM` | `lines 2190-2207` `HPHYS0251 SWU Uptake Magnitude Coupling Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT02 routed: binding status or precise canonical mapping requires science review; narrative remains in core and is not sidecar-eligible. |
| `HPHYS0252-WB19-STORAGE-AVAILABILITY-CONTINUATION-ADDENDUM` | `lines 2208-2231` `HPHYS0252 WB19 Storage-Availability Continuation Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-040, INV-WATBAL-043, INV-WATBAL-044` | `none` | SCSTRUCT03 batch 5: mapped to WB19 storage-availability, lane-branch, and realized lateral publication invariants; retained in core. |
| `HPHYS0254-WB11-INITIAL-STORAGE-PROJECTION-ADDENDUM` | `lines 2232-2255` `HPHYS0254 WB11 Initial-Storage Projection Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT02 routed: binding status or precise canonical mapping requires science review; narrative remains in core and is not sidecar-eligible. |
| `HPHYS0255-MOFE-STORAGE-PROJECTION-ADDENDUM` | `lines 2256-2279` `HPHYS0255 MOFE Storage Projection Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT02 routed: binding status or precise canonical mapping requires science review; narrative remains in core and is not sidecar-eligible. |
| `HPHYS0256-WB19-LATQCC-LANE-BRANCH-ADDENDUM` | `lines 2280-2293` `HPHYS0256 WB19 Latqcc Lane-Branch Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-043` | `none` | SCSTRUCT03 batch 5: mapped to WB19 latqcc lane-branch invariant; retained in core. |
| `HPHYS0258-WB19-REALIZED-LATERAL-PUBLICATION-ADDENDUM` | `lines 2294-2309` `HPHYS0258 WB19 Realized Lateral Publication Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-044` | `none` | SCSTRUCT03 batch 5: mapped to realized lateral publication invariant; retained in core. |
| `HPHYS0259-WB19-TRACE-RESIDUAL-LOCALIZATION-ADDENDUM` | `lines 2310-2324` `HPHYS0259 WB19 Trace Residual Localization Addendum` | `active` | `maps-to-existing-INV` | `INV-WATBAL-045` | `none` | SCSTRUCT03 batch 5: mapped to WB19 trace-localization invariant; retained in core. |
| `HPHYS0260-WB17-WB18-STORAGE-TRACE-RESIDUAL-LOCALIZATION-ADDENDUM` | `lines 2325-2504` `HPHYS0260 WB17/WB18/Storage Trace Residual Localization Addendum` | `active` | `undecidable` | `none` | `science-review-follow-on` | SCSTRUCT02 routed: binding status or precise canonical mapping requires science review; narrative remains in core and is not sidecar-eligible. |


### ADR0017 Comparator-Flag Water-Balance Addendum

Comparator agreement is a water-balance investigation flag, not a production
target. A water-balance residual may be assigned `OPENWEPP-DEFECTIVE` only
after paired evidence proves the compared surfaces are the same physical
quantity in the same units at the same lineage stage, and after independent
correctness authority proves the openWEPP branch is wrong. Depth-vs-SWE,
raw-vs-released, branch-mask, or absent-key/default comparisons must remain
`HARNESS-SURFACE-MISMATCH` or `UNRESOLVED`. Criterion C is not waivable, and
all `HOLD` dispositions must name a scoped owner and follow-on gate.
...
| `INV-WATBAL-096` | runtime + governance | MOFE hourly inter-OFE transfer validator spanning separated `UpStrmQ`/`SubRIn` components, `ui_SCrunf`/`ui_LfCrf` copy-forward, per-element closure, frost-active WB12/frozwt exchange terms, and hillslope-total cancellation | Typed hard error / explicit `HOLD` when upstream carry components are missing/collapsed, single-OFE upstream carry is non-zero, downstream arrays do not match upstream current arrays after scaling, frost liquid/overflow exchange is absorbed as tolerance, or per-element/transfer/hillslope-total residuals exceed tolerance | MOFE01 M-B inter-OFE route-conservation gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `INV-WATBAL-098` | runtime + governance | Per-OFE WB13/H.wat publication validator spanning hydrology-vector distinctness, raw local-runoff operand distinctness, active nonzero surface handoff, WB14 multistep infiltration lineage, public runoff normalization (`Q = runoff * efflen / totlen`; `QOFE == Q` canonical per MOFEFID-B02), anti-clone comparison, and public-row source lineage | Typed hard error / explicit `HOLD` when multi-OFE records are aggregate clones, all active-day OFE hydrology vectors or raw local runoff values are identical without independent physical rationale, declared multistep MOFE/hourly WB14 lanes accept seeded/stale `wb12_infiltration`, handoff is proven only by zero-on-zero rows, or `QOFE`/`UpStrmQ` are synthesized from public WAT rows instead of transfer state (the `QOFE == Q` rejection is superseded by MOFEFID-B02) | MOFE01 M-F-REDO anti-clone and M-F-REDO2 runoff-publication gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| HPHYS0318 `stmtim` control-surface diagnostics | `snow_hourly_stmtim_rain_m`, `snow_hourly_stmtim_stmdur_s`, `snow_hourly_stmtim_wntdur_h`, `snow_hourly_stmtim_wnttim_h`, `snow_hourly_stmtim_hrtemp_c`, `snow_hourly_stmtim_rst_c`, `snow_hourly_stmtim_hrrain_m`, `snow_hourly_stmtim_hrsnow_m`, `snow_hourly_stmtim_active_interval`, `snow_hourly_stmtim_rain_branch`, `snow_hourly_stmtim_snow_branch` | Required trace evidence to classify whether the 2013 day 11 hour 11 snowfall mismatch is controlled by OpenWEPP SIMIMPL28 partition inputs/branching before assigning snow-producer or downstream water-balance ownership | depths `m`, durations `s`/`h`, temperatures `degC`, flags `0/1` | `[DIRECT][Static] + [INFERENCE][Static]` |
...
- OBL-WATBAL-P-020: HPHYS0296/HPHYS0303 closure must preserve snow/`RM` acceptance
  ...
  adjudication, and explicit ADR0017 peer defective-model disposition before
  any residual leaves the failing set.
  `[DIRECT][Static] + [INFERENCE][Static]`
- OBL-WATBAL-P-021: HPHYS0297 closure must publish the snow/`RM` defect ledger
  ...
  identity, correctness rationale, and final ADR0017 peer verdict. `[DIRECT][Static]
  + [INFERENCE][Static]`
- OBL-WATBAL-P-022: HPHYS0298 closure must publish the paired snow/`RM`
  ...
  final verdict using `HARNESS-SURFACE-MISMATCH`, `LEGACY-DEFECTIVE`,
  `OPENWEPP-DEFECTIVE`, or `UNRESOLVED`. `[DIRECT][Static] +
  [INFERENCE][Static]`
- OBL-WATBAL-P-023: Comparator and water-balance ledger producers after
  ADR0017 must publish same-quantity unit pairing, lineage-stage pairing,
  independent correctness authority, owner/follow-on-gate fields for `HOLD`,
...
- OBL-WATBAL-P-025: HPHYS0315 and later water-balance continuation producers
  must consume the paired hourly snowfall input-lineage ledger and must not
  authorize WB13/WB17/WB18/WB19/WB12, melt-term, branch-predicate, or snow
  producer edits unless the ledger proves same-unit `hrsnow`/snowfall-depth
  pairing plus the controlling fixed-baseline/openWEPP input surfaces and
  independent correctness authority. `[DIRECT][Static] + [INFERENCE][Static]`
- OBL-WATBAL-P-026: HPHYS0316 and later water-balance continuation producers
  must consume a 2013 terminal carry-recursion ledger for the spring-2016
  inherited rows, preserve `33` carried rows, publish owner/follow-on routing
...
- OBL-WATBAL-P-027: HPHYS0317 and later water-balance continuation producers
  ...
  `[DIRECT][Static] + [INFERENCE][Static]`
- OBL-WATBAL-P-028: HPHYS0318 water-balance continuation producers must
  consume OpenWEPP `snow_hourly_stmtim_*` trace maps, preserve the `57`
  carried-row route, and keep WB13/WB17/WB18/WB19/WB12 edits in `HOLD` when
  fixed-baseline paired `stmtim` observe values remain unavailable.
  `[DIRECT][Static] + [INFERENCE][Static]`
- OBL-WATBAL-P-029: HPHYS0319 water-balance continuation producers must
  consume the paired fixed-baseline/OpenWEPP `stmtim` observe ledger at H1/H7/H39
  2013 day 11 hour 11, preserve the `57` carried-row route, record production
...
| TOL-WATBAL-009 | WB14 hourly-to-daily post-partition runoff-ledger reconciliation | `abs(Σ q_wb14_hourly - frost_retained_local_liquid - Q_partition) <= 24 * 1e-9 m * max(1, abs(Σ q_wb14_hourly), abs(frost_retained_local_liquid), abs(Q_partition))` | `REF-WATBAL-WB14-INTERVAL-CLOSURE` bounds each of the 24 WB14 interval solves at `1e-9 m`. This aggregate allowance only adjudicates agreement between two present, independently accumulated ledgers. It never mutates an hourly bin, supplies an empty hourly ledger for positive daily runoff, absorbs a missing source, invents frost timing, or authorizes loss of positive hourly runoff. The exact boundary is accepted; any greater mismatch hard-fails. Full frost retention may clear the series only when the reconciled local runoff is zero; partial positive frost retention without an hourly producer hard-fails. |
...
4. D2 continuation must establish freeze/thaw exchange wiring from
   ...
   hydrology storage logic. WB13/WAT `frozwt` publication must source
   `frost.runtime_frwatc_frozen_water_after_m`, and that diagnostic must equal
   the legacy `soilf` layer sum
   `Σ(wb18_perc_frzw_#### + thetdr_#### * wb18_perc_frozen_depth_####)`.
   Publication from `frdp`, `frdp * scalar`, or a non-layer
   `frost.runtime_ws_frz` aggregate is a depth-derived proxy and is
   non-authoritative. D3 depth-runaway and independent `p2` defects remain
   downstream of this trustworthy D2 gate.
...
| Surface | Symbols |
|---|---|
| Required inputs | reconciled `Q`; post-partition `q_hourly(1:24)` on the shared ADR-0036 time basis |
...
1. WB16 executes after reconciled runoff and hourly source producers. It uses
   the same post-partition 24-bin runoff depths as hourly erosion, transfer,
   and HBP serialization. Routed melt and runon have already entered WB14 as
   hourly supply and are not appended again.
...
3. For `Q > 0`, all 24 hourly depths must be finite and nonnegative and their
   sum must reconstruct `Q` after `TOL-WATBAL-009` reconciliation.
4. Each hourly depth is produced by the WB14/WB19 post-partition ledger. A
   normalized shared shape may be derived only after that ledger closes; the
   accepted event scalar may not be redistributed to manufacture timing.
   Hourly WB19 saturation return remains in its producer hour. No daily-return
   retiming or synthetic uniform fallback is allowed for a production peak.
...
2. Saturation-only, melt-supply-only, and runon-supply-only vectors retain the
   modeled source hour after partition and select the corresponding peak hour.
...
| `frozwt` | `Σ(wb18_perc_frzw_#### + thetdr_#### * wb18_perc_frozen_depth_####) -> frost.runtime_frwatc_frozen_water_after_m -> frozwt` (legacy `soilf` frozen store) | `SC-WATBAL-001` `INV-WATBAL-095`; `SC-SNOWFREEZE-001` frost/water exchange lineage | `crates/openwepp-runner/src/hillslope/02_output_and_climate_helpers.rs` (`require_runtime_surface_scalar("frost.runtime_frwatc_frozen_water_after_m")` -> `("frozwt", frozwt)`) | `HKERNEL-WB14-RUNOFF-E-001..003`, `HKERNEL-WB13-HWAT-E-001..003`, `HS-SIMOUT-E-001` |
...
5. `frozwt` publication must consume the exchanged frozen store diagnostic,
   and that diagnostic must be derived from the legacy `soilf` layer sum
   `Σ(wb18_perc_frzw_#### + thetdr_#### * wb18_perc_frozen_depth_####)`, not
   `frdp`, `frdp * scalar`, or any depth-scaled proxy.
...
7. Daily aggregate carry surfaces such as `wb12_runoff_carryover` may summarize
   the explicit `ui_SUrunf + ui_LfUrf` array payload for WB12/WB13 closure only
   after array provenance is preserved. Stale aggregate carry from a prior
   scheduler lifecycle must be purged before array-enabled execution, and no
   aggregate may replace or synthesize the array payload in MOFE hourly lanes.
...
### MOFE01 M-B Inter-OFE Conservation Addendum

MOFE01 M-B promotes the hydrology-route seam from array availability to
component-preserving conservation.

1. Upstream carry into an OFE is component-preserving. `UpStrmQ` is the
   area-scaled daily sum of `ui_SUrunf(1:24)`, and `SubRIn` is the area-scaled
   daily sum of `ui_LfUrf(1:24)`. WB12/WB14 `runon_input` is their sum.
2. Per-element closure must expose runon explicitly before publication:
   `local_liquid + UpStrmQ + SubRIn = infiltration + Q_partition +
   Δdepression_storage + ε`. The current-OFE surface-saturation addback
   `surdra = Σui_SCrunf(ii)` remains a same-pass `Q` term under
   `INV-WATBAL-034` and cannot mask a transfer residual.
3. Transfer closure is hourly and directional. For each adjacent OFE pair,
   upstream `ui_SCrunf(ii)` feeds downstream `ui_SUrunf(ii)`, and upstream
   `ui_LfCrf(ii)` feeds downstream `ui_LfUrf(ii)`, after the declared
   area-ratio scaling. Aggregate `wb12_runoff_carryover` may summarize the
   transfer but cannot replace the explicit arrays.
4. Hillslope-total closure must cancel internal inter-OFE transfers. A
   hillslope-total residual may include only external climate/irrigation inputs,
   ET, percolation, lateral/drain exports, outlet runoff, and storage change at
   the declared boundary.
5. Contract-derived vectors must cover a two-OFE surface carry, a two-OFE
...
### MOFE01 M-E4-REDO Internal WB13 Identity Acceptance Addendum

1. M-E4 internal per-OFE WB13 records are valid conservation evidence only when
   per-element residuals compare the current row to the independently captured
   pre-day OFE dynamic storage snapshot. `SoilWaterTotal == Total-Soil` is an
...
3. For runner-owned WB13 rows, `RM` is the local liquid-input publication term
   ...
   M-E4 internal residual.
4. Adjacent transfer identity evidence must compare OFE `i` sent transfer
   (`current_transfer_output`/`ui_SCrunf`/`ui_LfCrf`) with OFE `i+1` received
   transfer (`upstream_transfer_input`/`ui_SUrunf`/`ui_LfUrf`). Validators may
   compute an expected value from one side, but fixtures must prove the other
   side can diverge independently; rebuilding both operands from the same side
   is not acceptance evidence.
5. M-E4-REDO residual closure uses `TOL-WATBAL-007`. Runtime evidence should
   show nonzero-at-noise residuals for at least one real per-element case; an
   all-exact-zero residual suite is a hold for tautology review.

### MOFE01 M-F-REDO Per-OFE Publication Anti-Clone Addendum

1. Public per-OFE WB13/H.wat rows are acceptance evidence only when they are
   sourced from the lane-local dynamic record collection, not from aggregate
   WB13 rows stamped with OFE keys.
2. Active routed-day evidence must include at least one nonzero adjacent
   surface handoff where independently stored sent and received transfer
   operands match after declared scaling. Exact-zero transfer residuals are
   acceptable only when paired with nonzero sent/received operands elsewhere in
   the same acceptance set.
3. Conservation identities from `INV-WATBAL-096` remain required but do not
   prove per-OFE genuineness. A multi-OFE publication whose active-day
   hydrology vector is all-OFE-identical is a clone signal and must hold unless
   independently justified by same-input/same-physics evidence.
4. `QOFE`, `UpStrmQ`, and `SubRIn` must be published from transfer state, not
   reverse-built from public WAT rows or aggregate `Q` values.
5. Contract-derived tests must include an anti-clone assertion and a nonzero
   active surface-handoff assertion before M-F publication can clear.
6. M-F-REDO-CLONE acceptance must include raw local runoff distinctness on
   active routed multi-OFE days and must reject WB14 multistep-lane use of
   seeded/stale `wb12_infiltration` as a producer-published infiltration
   result.
7. M-F-REDO2 runoff normalization: public `Q = runoff * efflen / totlen`,
   with public `QOFE == Q` the canonical published convention (MOFEFID-B02 /
   `SC-RUNOFFPART-001#INV-RUNOFFPART-032`; the OFE-local `runoff * efflen /
   slplen` basis is retained only for the byte-invariant `H.pass.runvol`/peak).
   Internal water-balance identity checks must continue to use raw
   transfer/runoff operands, not the geometry-normalized public `Q` value.

### MOFE01 M-G Erosion `qin` Water-Balance Boundary Addendum

1. Water-balance transfer closure exposes EROD14 water operands, but it is not
   sediment coupling acceptance.
2. `UpStrmQ`, `SubRIn`, `TransferInput`, `TransferOutput`, hourly carry arrays,
   aggregate `Q`, `QOFE`, and water-derived `erod14_qin` compatibility
   provenance may be used only as water-side evidence until `SC-SED-001`
   closes prior-OFE erosion `qout` and class-fraction handoff lineage.
3. Public WB13/WAT rows cannot be used to synthesize accepted downstream
   `erod14_qin`; acceptance vectors must compare against an independently
   produced prior-OFE erosion `qout`.
4. Operator-facing manifests must expose
   `erod14_qin_source_policy =
   "water-transfer-only-mofe01-mg-sediment-coupling-follow-on"` and
   `erod14_qin_sediment_coupled = false` whenever Wave-2 runs on the current
   water-transfer-only source.
5. M-G emits the sediment-coupling follow-on instead of broadening MOFE01 water
   closure into partial erosion-routing acceptance.

### MOFE01 M-I Hillslope-Total Closure Addendum

1. M-I acceptance requires an in-runner hillslope-total residual computed from
   internal per-OFE WB13 records and OFE areas before public WAT/WB13
   publication. Published WAT rows may audit the result, but they are not the
   source of the identity.
2. The normalized residual is:
   `Σ_i((external_in_i - external_out_i - ΔS_i) * area_i) / Σ_i area_i`.
   `external_in_i` is the local liquid input plus the explicit frost internal
   adjustment. `external_out_i` includes interception, `Ep`, `Es`, `Er`, `Dp`,
   `Tile`, frost upper overflow, and only the outlet OFE's surface/lateral
   hillslope exports. Internal `UpStrmQ`, `SubRIn`, non-outlet routed surface
   runoff, and non-outlet lateral handoff are excluded because they are
   internal transfers. Downstream transfer input depths must be scaled by
   upstream-area/current-area from the same OFE areas used in the residual.
3. The M-I residual uses `TOL-WATBAL-008` (`<= 1e-9 mm`). Active-suite
   evidence must include nonzero-at-noise residuals; exact 0.0 across every
   active MOFE case is a hold for tautology review.
4. Manifests that publish per-OFE WB13 records must expose
   `hillslope_total_identity_max_abs_mm` alongside the per-element, adjacent
   transfer, and internal-transfer cancellation residuals.
5. Multi-OFE execution must run the persistent per-OFE scheduler lifecycle as
   the sole source of WB13/WAT rows and identity provenance. The aggregate
   scheduler lifecycle may remain the single-OFE specialization, but must not
   execute for multi-OFE days merely to compute and discard an aggregate
   surface.
...
| `2026-06-13` | `158` | `Codex` | MOFE01 M-F-REDO-CLONE amendment: tightened `INV-WATBAL-098` to require raw local-runoff anti-clone evidence and prohibit WB14 multistep MOFE/hourly lanes from accepting seeded or stale `wb12_infiltration` as producer-published infiltration; clarified `INV-WATBAL-096` frost-active per-OFE WB12/frozwt exchange terms. |
| `2026-06-13` | `157` | `Codex` | MOFE01 M-F-REDO amendment: added `INV-WATBAL-098` and the per-OFE publication anti-clone gate requiring lane-local source lineage, nonzero active surface-handoff evidence, and hydrology-vector distinctness beyond conservation closure. |
| `2026-06-13` | `156` | `Codex` | MOFE01 M-E4-REDO amendment: pinned `TOL-WATBAL-007` at `<= 1e-11 mm` and required non-tautological internal per-OFE WB13 identity evidence from pre-day storage snapshots, adjacent sent/received transfer records, and a frost-active fixture before M-E4 clears. |
...
| `2026-06-11` | `151` | `Codex` | FDHP01 D2 publication amendment: bound WB13/WAT `frozwt` to the exchanged frozen-store diagnostic `frost.runtime_frwatc_frozen_water_after_m` and rejected depth-derived `frdp * scalar`/`frost.runtime_ws_frz` publication leakage. |
