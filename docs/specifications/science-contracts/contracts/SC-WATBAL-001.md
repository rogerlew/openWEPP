...
| `INV-WATBAL-086` | governance | HPHYS0313 split-route snowpack settling/carry recursion ledger covering full-precision settling reconstruction and 2014 terminal carry-state recursion | Explicit `HOLD` when settling reconstruction lacks source-owned proof, earlier carry-state inheritance remains unresolved, rounded observe output is treated as sufficient, or downstream compensation is asserted | HPHYS0313 split-route snowpack water-balance gate | `[DIRECT][Static] + [INFERENCE][Static]` |
