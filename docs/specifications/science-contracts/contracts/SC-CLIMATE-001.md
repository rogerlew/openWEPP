...
| INV-CLIMATE-013 | HPHYS0272/HPHYS0277 winter hourly radiation unit and physical-flux invariant: runtime climate projection must preserve legacy `radly` as Langleys/day at the daily parser seam, convert exactly once to `radmj = radly * 0.04184` for `hr_tmp`, and publish `winter.hourly.rad_mj_m2_####` in `MJ m^-2 h^-1`. Active SIMIMPL28 synthesis must fail closed at the daily source seam when finite `radly` exceeds the baseline `sunmap` horizontal daily potential `rpoth`/`r3` for the same date and latitude; this is `INV-CLIMATE-006` source-bound evidence, not an hourly normalization opportunity. Every finite hourly `hradmj` that reaches `hr_tmp` must additionally satisfy `0 <= hradmj <= E0h_max(sdate)`, where `E0h_max` is the hourly extraterrestrial normal-incidence energy bound derived from the baseline `radcur.for` solar constant (`0.082 MJ m^-2 min^-1`), earth-sun distance term `rdsun = 1 + 0.033*cos(2π*sdate/365)`, and one-hour hour-angle integral `2*sin(π/24)`. Evidence or code paths that treat daily `rad`/`radly` as already `MJ m^-2 d^-1`, re-convert an MJ value back to Langleys, silently clip physically impossible daily/hourly radiation, or publish Langley-scale finite radiation under `MJ` labels are non-authoritative. | hard-fail | REF-CLIMATE-WF-RADLY-RADMJ, INV-CLIMATE-006, REF-CLIMATE-CH3-COUPLING, REF-CLIMATE-PHYS-BOUNDS, SC-SNOWFREEZE-001#INV-SNOWFREEZE-017 | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-CLIMATE-014 | HPHYS0299 winter hourly snowfall unit/provenance invariant: canonical `hrsnow` from pinned-baseline `stmtim.for` is a snowfall-depth increment, produced by `hrsnow(hour) = rain / wntdur * 10.0` when `hrtemp <= rst`, after `winter.for:296-300` calls `stmtim`. openWEPP parity diagnostics for `hrsnow` must compare against `snow_hourly_snowfall_depth_sum_m`/`snow.hourly.snowfall_m_####`, not `snow_hourly_snowfall_water_equiv_sum_m`, which is a derived water-equivalent summary using snow density. Evidence that maps canonical `hrsnow` directly to water-equivalent snowfall is non-authoritative and cannot justify production precipitation-partition migration. | governance-hold | REF-CLIMATE-CH3-COUPLING, SC-SNOWFREEZE-001#INV-SNOWFREEZE-030, SC-WATBAL-001#INV-WATBAL-074, `/workdir/wepp-forest_260430_baseline/src/winter.for:296-300`, `/workdir/wepp-forest_260430_baseline/src/stmtim.for:43-95` | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `INV-CLIMATE-013` | runtime + governance | SIMIMPL28 hourly winter radiation projection and HPHYS0271/HPHYS0272/HPHYS0277 melt-forcing diagnostics | Typed hard error for missing/non-finite radiation, daily `radly` above baseline `sunmap` `rpoth`/`r3`, or finite `hradmj` above the physical hourly extraterrestrial bound; explicit `HOLD` when radiation evidence is Langley-scale under `MJ` labels or exceeds baseline unit lineage without provenance | HPHYS0272/HPHYS0277 radiation-unit closure gate | `[DIRECT][Static] + [INFERENCE][Static]` |
| `INV-CLIMATE-014` | governance | HPHYS0299 paired partition diagnostics and SIMIMPL28 snowfall trace provenance | Explicit `HOLD` when canonical `hrsnow` is compared to water-equivalent snowfall, when `stmtim.for` provenance is missing, or when production migration is proposed from a depth-vs-water-equivalent mismatch | HPHYS0299 unit/provenance gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `stmtim` control surfaces | `snow.hourly.stmtim.{rain_m,stmdur_s,wntdur_h,wnttim_h,hrtemp_c,rst_c,hrrain_m,hrsnow_m,active_interval,rain_branch,snow_branch}_####` | HPHYS0318 OpenWEPP trace diagnostics for SIMIMPL28 hourly precipitation partition control surfaces | `m`, `s`, `h`, `degC`, and `0/1` flag domains preserved | `[DIRECT][Static] + [INFERENCE][Static]` |
...
- Finite `winter.hourly.rad_mj_m2_####` above the physical hourly extraterrestrial normal-incidence energy bound derived from `radcur.for`. `[DIRECT][Static] + [INFERENCE][Static]`
...
- OBL-CLIMATE-P-008: SIMIMPL28 winter forcing projection must keep daily
  ...
  `MJ m^-2 h^-1`.
  `[DIRECT][Static] + [INFERENCE][Static]`
- OBL-CLIMATE-P-009: SIMIMPL28 winter forcing projection must fail closed with
  a typed runtime error when finite daily `radly` exceeds baseline `sunmap`
  horizontal daily potential `rpoth`/`r3`, or when a finite hourly radiation
  value exceeds the physical hourly extraterrestrial normal-incidence bound
  derived from `radcur.for`; it must not clip, cap, renormalize, or compensate
  downstream.
  `[DIRECT][Static] + [INFERENCE][Static]`
- OBL-CLIMATE-P-010: HPHYS0317 and later paired hourly snowfall input-surface
  producers must publish the controlling `stmtim` lane (`rain`, `stmdur`,
  rounded `wntdur`, adjusted `wnttim`, `hrtemp`, `rst`, `hrsnow`, `hrrain`,
  active interval membership, and branch choice) for fixed-baseline and
  openWEPP at the same year/day/hour before authorizing precipitation-phase
  production edits.
  `[DIRECT][Static] + [INFERENCE][Static]`
- OBL-CLIMATE-P-011: HPHYS0318 OpenWEPP runtime producers must emit the
  `snow.hourly.stmtim.*_####` control-surface diagnostic family from the
  existing SIMIMPL28 partition helper and preserve the existing
  `snow.hourly.rain_m_####`/`snow.hourly.snowfall_m_####` partition outputs.
  Missing fixed-baseline paired `stmtim` observe values remain a governance
  `HOLD` and do not authorize production precipitation-phase edits.
  `[DIRECT][Static] + [INFERENCE][Static]`
- OBL-CLIMATE-P-012: HPHYS0319 fixed-baseline observe recovery producers must
  use the pinned baseline commit
  `dac3c950d8b16cc73774bf5ce2e7e11f80baac70`, record any temporary observe
  patch, publish paired H1/H7/H39 2013 day 11 hour 11 `stmtim` control
  surfaces, and keep production precipitation-phase edits unauthorized unless
...
| HPHYS0272/HPHYS0277 hourly radiation unit and physical-flux closure (`INV-CLIMATE-013`) | SIMIMPL28 hourly winter forcing projection and downstream snowmelt diagnostics | Hard error for missing/non-finite radiation, daily `radly` above baseline `sunmap` `rpoth`/`r3`, or finite hourly radiation above the physical hourly extraterrestrial bound; `HOLD` when emitted radiation is Langley-scale under `MJ` labels or exceeds baseline unit lineage | HPHYS0272/HPHYS0277 radiation-unit gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| TOL-CLIMATE-005 | Hourly radiation physical-bound runtime tolerance | relative `1e-9` plus absolute `1e-12 MJ m^-2 h^-1` | Allows floating-point roundoff in `radcur`-derived bound evaluation only; not a clipping allowance. |
...
6. HPHYS0277 physical-flux closure requires typed hard-fail guards before
   radiation publication: finite `radly` must not exceed baseline `sunmap`
   horizontal daily potential `rpoth`/`r3` for the same date and latitude, and
   emitted `hradmj` after `hr_tmp` hourly radiation synthesis must be finite,
   non-negative, and no greater than the physical hourly extraterrestrial
   normal-incidence bound
   `((12*60)/π) * 0.082 * rdsun * 2*sin(π/24)` with `rdsun` from `radcur.for`.
   The guards are invariant checks only and must not mutate `radly` or
   `hradmj`.
7. Frost heat-flow hourly families (`frost.hourly.*`) remain SIMIMPL29 scope;
...
6. HPHYS0277 high-flux vector confirms an overlarge finite `radly` payload
   fails with a typed runtime error at the daily `radly` source seam or, for
   source-valid daily radiation that later overflows the hourly guard,
   `winter.hourly.rad_mj_m2_####`, instead of publishing, clipping, or
   compensating.
...
|---|---|---|---|
| `2026-06-06` | `23` | `Codex` | WBVAL02 amendment: clarified active SIMIMPL28 daily `radly` source-bound fail-closed behavior against baseline `sunmap` horizontal potential before hourly publication. |
...
| `2026-06-05` | `18` | `Codex` | HPHYS0299 amendment: added `INV-CLIMATE-014` requiring paired hourly snowfall diagnostics to compare pinned-baseline `stmtim.for` `hrsnow` depth against openWEPP snowfall-depth traces, not derived snowfall water-equivalent summaries. |
| `2026-06-03` | `17` | `Codex` | HPHYS0277 amendment: extended `INV-CLIMATE-013` with the `radcur.for`-derived physical hourly radiation flux bound and required typed fail-closed guard without clipping or downstream compensation. |
