...
| REF-RUNOFFPART-BASELINE-RUNON-FIN | `/workdir/wepp-forest_260430_baseline/src/watbal_hourly.for:358-366,468-476` (`dac3c950d8b16cc73774bf5ce2e7e11f80baac70`) | Baseline infiltration supply admits inter-OFE runon on receiving OFEs: daily `fin` gains the net upstream surface runon scaled by effective/slope lengths (`:361-363`), and the per-hour supply `xfin` adds the 24-slot upstream surface and lateral carry `(ui_SUrunf(ii) + ui_LfUrf(ii))` scaled by upstream/current plane area (`:471-473`), making runon water available for same-pass infiltration and layer-storage ingress. This is the promotion the `REF-RUNOFFPART-LEGACY-FIN-INFIL` note anticipated. | `[DIRECT][Static]` |
...
| `UpStrmQ`, `SubRIn` | `m` at runtime; WB13/WAT publication `mm` | Separated upstream saturation-runoff and upstream lateral-flow carry totals, derived from `Σui_SUrunf(ii)` and `Σui_LfUrf(ii)` with area-ratio scaling. | MOFE hourly carry-array resolver | downstream OFE runon closure and WAT carry publication |
...
| INV-RUNOFFPART-030 | MOFE01 M-G erosion-`qin` producer-boundary invariant: MOFE water-transfer surfaces (`TransferInput`, `TransferOutput`, `UpStrmQ`, `SubRIn`, hourly carry arrays, aggregate `Q`, `QOFE`, and `wb12_runoff_carryover`) can prove water-routing closure but cannot by themselves prove accepted sediment-coupled EROD14 downstream `qin`. For downstream OFEs, accepted erosion `qin` requires lineage to the prior OFE erosion `qout` and the sediment/class-fraction handoff governed by `SC-SED-001`; public WAT rows or aggregate water surfaces may not synthesize or replace that handoff. Runtime seeding that uses water-transfer provenance only must be manifest-labeled as compatibility/follow-on scope and cannot close sediment coupling acceptance. DISPOSITION (E.3 / SC-SED-001 rev 44): the Wave-1 hourly handoff (`SC-SED-001#INV-SED-016` (b)) carries the required prior-OFE erosion `qout` + sediment/class-fraction lineage — multi-OFE runs on the Wave-1 chain SATISFY this invariant (`erod14_qin_sediment_coupled = true`, `erod14_qin_source_policy = wave1-hourly-sediment-coupled-handoff`); the hold continues to bind any future non-Wave-1 seeding path. | governance-hold | REF-RUNOFFPART-CH4-MULTIOFE, REF-RUNOFFPART-LEGACY-HOURLY-CARRY, INV-RUNOFFPART-028, INV-RUNOFFPART-029, SC-WATBAL-001#INV-WATBAL-099, SC-SED-001#INV-SED-012 | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-RUNOFFPART-031 | DC01 runon re-infiltration invariant: on a lane whose resolved upstream transfer (`INV-RUNOFFPART-028` components, area-scaled) is non-zero, the WB14 event liquid supply must include the upstream surface and lateral carry distributed over the event time base per `REF-RUNOFFPART-BASELINE-RUNON-FIN`; a lane with material resolved runon but no positive-duration local hyetograph must synthesize an hourly time base rather than bypassing infiltration. The R4A partition consumes the enlarged same-pass infiltration under the unchanged `INV-RUNOFFPART-028` event-closure identity (no new partition terms), and same-pass infiltrated runon enters layer storage through the existing WB18 lineage. Zero-upstream (single-OFE) lanes must remain bit-identical to pre-DC01 behavior. Decreasing-flow erosion consequences (`qout < qin`) were governed by the `INV-RUNOFFPART-030` hold; with its E.3 disposition the Wave-1 chain solves decreasing-flow hours as ordinary deposition quanta with inflow lineage (`SC-SED-001#INV-SED-016`), superseding the interim-clamp scope (no clamp is present on the chain). | hard-fail | REF-RUNOFFPART-BASELINE-RUNON-FIN, REF-RUNOFFPART-LEGACY-FIN-INFIL, REF-RUNOFFPART-LEGACY-HOURLY-CARRY, REF-RUNOFFPART-CH4-MULTIOFE, INV-RUNOFFPART-028, INV-RUNOFFPART-029, SC-WATBAL-001#INV-WATBAL-096 | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-RUNOFFPART-032 | MOFEFID-B02 QOFE publication-convention invariant: the published `QOFE` runoff column adopts the post-`wepp_260516` ecosystem convention `QOFE == Q` on ALL WB13 rows (not only single-OFE) — both normalized by cumulative slope length down to the OFE (`totlen`), completing the 2008-era parallel `slplen -> totlen` denominator alignment. The canonical per-OFE runoff VOLUME (`H.pass.runvol`) and peak-runoff operands MUST remain byte-invariant across this change: they are computed from the retained per-OFE local-length runoff basis, NOT from the published `QOFE` value, so `runvol = runoff_local_basis x per_OFE_area` is unchanged. Single-OFE lanes are byte-identical (cumulative_length == ofe_length). Downstream consumers recovering canonical hillslope runoff volume from `H.wat` must use `Q(outlet) x A_total` (or `H.pass.runvol` directly), NOT `QOFE x per_OFE_area`, which under `QOFE == Q` no longer carries the legacy cancellation. | hard-fail | REF-RUNOFFPART-CH4-MULTIOFE, REF-RUNOFFPART-PHYS-BOUNDS, INV-RUNOFFPART-028, INV-RUNOFFPART-029 | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-RUNOFFPART-033 | SNOW-SURFACE-EB-04W2B shared snow-consumer invariant: WB12/WB14 typed snow partition activation must include positive typed hourly snowfall as well as prior pack and cold-day triggers. Before the signed snow term or liquid input is returned, the partition must independently reconstruct and hard-fail daily snow storage closure under `SC-SNOWFREEZE-001#INV-SNOWFREEZE-089`; an inactive zero-accumulation result cannot mask positive typed snowfall. Warm all-rain, zero-pack forcing remains on the inactive compatibility path. | hard-fail | SC-SNOWFREEZE-001#INV-SNOWFREEZE-089, INV-RUNOFFPART-001, INV-RUNOFFPART-015, INV-RUNOFFPART-017 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `INV-RUNOFFPART-030` | runtime + governance | EROD14 `qin` producer-boundary provenance checker plus manifest/acceptance gate | Explicit `HOLD` when water-routing transfer closure is treated as sediment-coupled `qin` acceptance, downstream `erod14_qin` is synthesized from public `UpStrmQ`/aggregate runoff without prior-OFE erosion `qout` and class-fraction lineage, or compatibility seeding lacks operator-visible follow-on provenance | MOFE01 M-G erosion `qin`/sediment coupling boundary gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `UpStrmQ` | `UpStrmQ` state/output symbol | scaled upstream saturation-runoff carry component from `ui_SUrunf` | `m` runtime, `mm` WAT publication | `[DIRECT][Static] + [INFERENCE][Static]` |
| `SubRIn` | `SubRIn` state/output symbol | scaled upstream lateral-flow carry component from `ui_LfUrf` | `m` runtime, `mm` WAT publication | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `QOFE` | Runoff output alias `QOFE = Q` on all rows (MOFEFID-B02); cumulative-length normalized. Recover canonical volume via `Q(outlet)*A_total` or `H.pass.runvol`, not `QOFE*per_OFE_area`. | `mm` |
...
2. `QOFE` must be equal to `Q` for ALL WB13 rows (MOFEFID-B02, `INV-RUNOFFPART-032`; formerly single-OFE only).
...
2. `wb12_runoff_carryover` remains an aggregate publication of the explicit
   hourly array payload after array provenance is preserved. Stale daily
   aggregate carry from a prior scheduler lifecycle must be purged before
   array-enabled execution; any same-lifecycle aggregate retained after
   resolution must match the array-derived carry within declared numeric
   tolerance or hard-fail.
...
## MOFE01 M-B Inter-OFE Runoff/Lateral Transfer Addendum

1. MOFE01 M-B refines `INV-RUNOFFPART-013` by requiring component-preserving
   upstream carry. The resolver must retain `Σui_SUrunf(ii)` as the surface
   upstream component and `Σui_LfUrf(ii)` as the lateral upstream component,
   applying the same `Aupstream/Acurrent` scaling to both components.
2. `runon_input` for WB12/WB14 closure is the sum of the separated components:
   `runon_input = UpStrmQ + SubRIn`. `UpStrmQ` and `SubRIn` are runtime
   depth surfaces in meters and WAT publication terms in millimeters.
3. Current-OFE carry publication is a transfer identity: for each hour,
   downstream `ui_SUrunf(ii)` must equal upstream OFE `ui_SCrunf(ii)` and
   downstream `ui_LfUrf(ii)` must equal upstream OFE `ui_LfCrf(ii)` after the
   declared area-ratio transformation. Daily aggregate carry may summarize this
   payload, but cannot replace or collapse it.
4. Per-element closure must include runon explicitly:
   `local_liquid + runon_input = infiltration + Q_partition +
   Δdepression_storage + ε`. The `surdra` addback remains governed by
   `INV-RUNOFFPART-014` and must be carried into the water-balance `Q`
   publication without hiding transfer residuals.
5. Contract-derived vectors must include a two-OFE surface carry case, a
   two-OFE lateral handoff case proving `SubRIn`, and a mixed 1/2/3/4/5-OFE
...
## MOFE01 M-G Erosion `qin` Producer-Boundary Addendum

1. MOFE01 water-routing closure exports the transfer state that a later
   erosion-coupling increment needs, but that export is not itself sediment
   coupling acceptance.
2. `UpStrmQ`, `SubRIn`, hourly carry arrays, aggregate `Q`, `QOFE`, and
   `wb12_runoff_carryover` are water-transfer operands. They may not synthesize
   accepted downstream `erod14_qin` unless the sediment lane also proves prior
   OFE erosion `qout` and class-fraction handoff lineage under `SC-SED-001`.
3. Single-OFE and OFE-1 zero-upstream cases may emit zero `erod14_qin` only with
   explicit zero-upstream provenance.
4. Existing water-transfer-only runtime seeding is compatibility provenance for
   Wave-2 continuity; manifests must label it as
   `water-transfer-only-mofe01-mg-sediment-coupling-follow-on` and must not
   claim sediment-coupled `qin` closure.
5. M-H ladder acceptance may consume MOFE01 water-routing closure evidence, but
   must not mark erosion `qin`/sediment coupling complete until the follow-on
   provides non-water-only sediment handoff evidence.
...
Rows mapped to existing IDs are resolved for binding exposure, but remain
core-resident when they carry active runtime, guard, producer, vector, or
cross-domain context. SCSTRUCT09 found no sidecar-eligible historical row and no
row requiring new binding promotion.

| Entry ID | Source | Status | Binding classification | Canonical binding IDs | Review gate | Notes |
|---|---|---|---|---|---|---|
| `EROD12-CROSS-DOMAIN-OWNERSHIP-AND-GUARD-CLOSURE-ADDENDUM` | `SC-RUNOFFPART-001.md#erod12-cross-domain-ownership-and-guard-closure-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-007, INV-RUNOFFPART-008, INV-RUNOFFPART-009, INV-RUNOFFPART-011` | `none` | SCSTRUCT09 map-in-core: runoff/peak-duration and multi-OFE producer ownership maps to existing case, aggregation, coupling, and forward-lane invariants; cross-contract consumer detail remains active core context. |
| `WB12-RUNOFF-RECONCILIATION-ADDENDUM` | `SC-RUNOFFPART-001.md#wb12-runoff-reconciliation-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-001, INV-RUNOFFPART-009, INV-RUNOFFPART-011, INV-RUNOFFPART-012` | `none` | SCSTRUCT09 map-in-core: WB12 closure equation maps to event partition and daily coupling invariants; forward-lane closure-delta semantics map to `INV-RUNOFFPART-011`; carryover precedence maps to `INV-RUNOFFPART-012`. |
| `WB13-DAILY-OUTPUT-COUPLING-ADDENDUM` | `SC-RUNOFFPART-001.md#wb13-daily-output-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-009, INV-RUNOFFPART-019, INV-RUNOFFPART-020, INV-RUNOFFPART-021` | `none` | SCSTRUCT09 map-in-core: WB13 runoff/runon publication maps to downstream coupling, routed-melt publication, post-winter-rain publication, and same-day snow publication lifecycle invariants. |
| `WB14-INFILTRATION-AND-SUBDAILY-HYETOGRAPH-KERNEL-AUTHORITY-ADDENDUM` | `SC-RUNOFFPART-001.md#wb14-infiltration-and-subdaily-hyetograph-kernel-authority-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-001, INV-RUNOFFPART-002, INV-RUNOFFPART-004, INV-RUNOFFPART-011, INV-RUNOFFPART-012, INV-RUNOFFPART-015, INV-RUNOFFPART-016, INV-RUNOFFPART-017, INV-RUNOFFPART-027` | `none` | SCSTRUCT09 map-in-core: hyetograph, Green-Ampt, runoff reconciliation, snow-liquid guard, same-pass infiltration handoff, carryover, and top-two-layer storage-limit semantics are exposed by existing invariants. |
| `WB15-CANOPY-INTERCEPTION-RUNTIME-COUPLING-ADDENDUM` | `SC-RUNOFFPART-001.md#wb15-canopy-interception-runtime-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-001, INV-RUNOFFPART-002, INV-RUNOFFPART-009` | `none` | SCSTRUCT09 map-in-core: interception-before-infiltration reduces liquid supply inside the existing event partition, rainfall-excess, and downstream coupling invariants; biomass cap/guard detail remains core-resident. |
| `IRRIG10-IRRIGATION-RUNTIME-COUPLING-ADDENDUM` | `SC-RUNOFFPART-001.md#irrig10-irrigation-runtime-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-001, INV-RUNOFFPART-009, INV-RUNOFFPART-016` | `none` | SCSTRUCT09 map-in-core: irrigation forcing participates in event closure and daily `Q` coupling, while same-pass infiltration/runoff availability maps to `INV-RUNOFFPART-016`; schedule guard detail stays core-resident. |
| `CLIM05-SNOW-RUNTIME-COUPLING-ADDENDUM` | `SC-RUNOFFPART-001.md#clim05-snow-runtime-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-001, INV-RUNOFFPART-015, INV-RUNOFFPART-017` | `none` | SCSTRUCT09 map-in-core: signed snow liquid forcing maps to event closure and routed-melt infiltration forcing, while projected snow-state guard posture maps to `INV-RUNOFFPART-017`. |
| `CLIM06-FROZEN-SOIL-RUNTIME-COUPLING-ADDENDUM` | `SC-RUNOFFPART-001.md#clim06-frozen-soil-runtime-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-001, INV-RUNOFFPART-004, INV-RUNOFFPART-009` | `none` | SCSTRUCT09 map-in-core: frozen infiltration-capacity consumption and domains are exposed by event closure, infiltration-domain, and downstream coupling invariants; frost control guard detail remains core. |
| `WB16-PEAK-RUNOFF-KERNEL-ADDENDUM` | `SC-RUNOFFPART-001.md#wb16-peak-runoff-kernel-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-005, INV-RUNOFFPART-009` | `none` | SCSTRUCT09 map-in-core: peak/runoff-duration branch equations, near-zero behavior, and `m`/`ealpha` producer provenance are exposed by peak-discharge and runoff-surface coupling invariants. |
| `ARCH22-TYPED-PRODUCTION-SURFACE-ADDENDUM` | `SC-RUNOFFPART-001.md#arch22-typed-production-surface-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-009, INV-RUNOFFPART-011` | `none` | SCSTRUCT09 map-in-core: typed production surfaces preserve required runoff boundary payloads and WB12/WB16 guard behavior already exposed by coupling and forward-lane invariants; migration detail remains core. |
| `EROD13-WAVE-1-ACTIVE-PRODUCER-COUPLING-ADDENDUM` | `SC-RUNOFFPART-001.md#erod13-wave-1-active-producer-coupling-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-005, INV-RUNOFFPART-009` | `none` | SCSTRUCT09 map-in-core: Wave-1 mandatory `Q`, `peakro`, `watdur`, and branch surfaces map to existing peak-discharge and downstream coupling invariants; enabled-path guard detail remains core. |
| `EROD14-WAVE-2-ACTIVE-PRODUCER-COUPLING-ADDENDUM` | `SC-RUNOFFPART-001.md#erod14-wave-2-active-producer-coupling-addendum` | `deleted-historical` | `none` | `none` | `none` | E.3 stage 2e: the runtime arm is DELETED (SC-SED-001 rev 45); the addendum is retained as historical specification only — no live obligations, the SCSTRUCT promotion gate is void. The Wave-1 chain (`SC-SED-001#INV-SED-016`) owns all inter-OFE erosion coupling. The mapped runoff invariants (INV-RUNOFFPART-007/008/009) remain live in their own right. |
| `HPHYS0240-HOURLY-RUNOFF-CARRYOVER-ADDENDUM` | `SC-RUNOFFPART-001.md#hphys0240-hourly-runoff-carryover-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-012` | `none` | SCSTRUCT09 map-in-core: same-pass carryover precedence, republished carryover, anti-shadow behavior, and malformed flux rejection are directly exposed by `INV-RUNOFFPART-012`. |
...
| `MOFE01-M-G-EROSION-QIN-PRODUCER-BOUNDARY-ADDENDUM` | `SC-RUNOFFPART-001.md#mofe01-m-g-erosion-qin-producer-boundary-addendum` | `active` | `maps-to-existing-INV` | `INV-RUNOFFPART-030` | `none` | MOFE01 M-G: water-transfer closure exposes EROD14 inputs but does not close sediment-coupled `qin`; runtime water-transfer-only seeding is compatibility provenance and must remain manifest-labeled until the sediment handoff follow-on closes. |
...
|---|---|---|---|
| `2026-07-04` | `47` | `Claude Code` | E.3 stage 2e (Codex round-2): the EROD14/EROD15 binding-exposure REGISTRY rows flipped `active` → `deleted-historical` (the body-addendum banners landed in the prior revision; the registry posture now matches — no live obligations, SCSTRUCT promotion gates void). |
