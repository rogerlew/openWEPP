...
| INV-RUNOFFPART-024 | HPHYS0296 snow/`RM` runoff-consumer acceptance invariant: `RM` acceptance must prove that WB12/WB14 consumed the same producer-owned `snow.routed_melt_m` and `snow.post_winter_rain_m` surfaces published to WB13, and that residual `Q` remains closed before a snow/`RM` residual is accepted or assigned to snow producer migration debt. If candidate `RM` publication identity closes but candidate/baseline `RM` magnitude remains open without material negative-melt evidence, runoff partitioning remains excluded and the package must hold on snow/winter producer authority rather than patching infiltration, runoff, or downstream storage. | governance-hold | INV-RUNOFFPART-021, INV-RUNOFFPART-022, INV-RUNOFFPART-023, SC-SNOWFREEZE-001#INV-SNOWFREEZE-027, SC-WATBAL-001#INV-WATBAL-071 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `INV-RUNOFFPART-024` | governance | HPHYS0296 runoff-consumer classifier tying WB12/WB14 liquid forcing, residual `Q`, and WB13 `RM` publication identity to the same snow producer surfaces | Explicit `HOLD` when `RM` magnitude residuals are reassigned to runoff/storage despite closed `Q` and closed producer-consumer identity | HPHYS0296 snow/`RM` acceptance gate | `[DIRECT][Static] + [INFERENCE][Static]` |
...
- OBL-RUNOFFPART-P-010: For HPHYS0296 acceptance, preserve `RM` producer-consumer identity evidence showing that WB12/WB14 liquid forcing and WB13 `RM` publication consume the same `snow.routed_melt_m` and `snow.post_winter_rain_m` surfaces, with residual `Q` closed before snow/`RM` residual acceptance or producer hold. `[DIRECT][Static] + [INFERENCE][Static]`
