...
| WB11 ET-demand branch lineage | `pmetpara.mode.sidecar_present`, `pmetpara.mode.iflget`, selected `kcb`, selected `rawp`, selected PMET crop row, actual ET seed branch |
...
| GAP-EVAP-008 | HPHYS0262 makes PMET demand-seeding lineage observable, but full production closure remains non-promotable until baseline-authoritative `evappm.for` demand equations are migrated when `pmetpara.mode.iflget != 1`. | PMET sidecar discovery and crop lookup evidence can classify the residual, but cannot close WB17 parity with PT or proxy demand under PMET mode. | non-promotable | `[DIRECT][Static]` |
