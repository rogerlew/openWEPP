...
| `pmet.es_storage_return_m` | `m` | Positive water-depth returned to top-layer storage when migrated EVAPPM condensation computes negative raw soil/residue evaporation. | PMET demand seed pathway | WB17 top-layer storage return branch |
...
- PMET condensation return follows pinned `evappm.for:461-472`: when the
  migrated PMET seed computes a negative soil/residue evaporation component,
  publish non-negative `pmet.es_m = 0` and carry the positive magnitude as
  `pmet.es_storage_return_m` so the WB17 ET phase returns that water to
  top-layer `wb18_perc_theta_0001` before any soil extraction.
- PMET transpiration publication remains a non-negative demand seam: when
  condensation produces negative raw `ep`, publish `pmet.ep_m = 0` and
  `wb11_et_demand = 0` rather than forwarding a material-negative demand into
  the WB17 guard.
...
| `pmet.es_storage_return_m` | identity name | PMET condensation return applied to top-layer `wb18_perc_theta_0001` before soil extraction | `m` preserved | `[DIRECT][Static] + [INFERENCE][Static]` |
...
- OBL-EVAP-P-009: Preserve HPHYS0295 cumulative-budget observability for
  `Ep`, `Es`, `Er`, `Etp`, `ΣUi`, `Ws`, layer `Ui`, post-ET layer storage,
  and same-day storage/lateral/percolation/snow masks before asserting WB17 as
  the owner of H1/H7/H39 `Total-Soil` or `SoilWaterTotal` residuals.
  `[DIRECT][Static] + [INFERENCE][Static]`
...
| `2026-06-04` | `24` | `Codex` | HPHYS0281 follow-up: clarified that negative raw `ep` canonicalization is an openWEPP guard-consistency decision under semantic-not-bit parity, not a bit-faithful baseline clamp port. |
...
| `2026-06-07` | `26` | `Codex` | FQ3-DC Corn ET amendment: tightened `INV-EVAP-016` to require day-local annual pre-plant PL skips to preserve carried activation sentinels for post-plant `rtd`/`Ep` lineage. |
