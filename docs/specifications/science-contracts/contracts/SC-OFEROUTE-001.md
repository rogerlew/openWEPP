...
| GAP-OFEROUTE-004 (WITHDRAWN 2026-07-02) | **Withdrawn on correction.** The D7 execution first attributed a ~5-6 s Iwagaki shock-timing lag to the solver, but that rested on a forcing bug: `run_iwagaki` fed the lateral-supply rate into the skin-term rainfall intensity `I`, although the Iwagaki experiment has NO rain (`I = 0`). With `I = 0` corrected, the timing and 10-90 % rise reproduce at `k_o ~ 200` (t_peak ~28 s vs 26 s; rise ~20.6 s vs 20.9 s); the residual is peak magnitude (~20 % low) and moderate `NS_trace` (~0.30), consistent with the UNSPECIFIED flume `k_o` (operand-limited), NOT a confirmed solver defect. No shock-capture GAP is asserted. See `docs/work-packages/20260702-mofefid-d7-enhanced-wepp-dval-001/artifacts/execution-report.md`. | n/a (withdrawn) | withdrawn — the attribution was an artifact of a corrected forcing bug. | `[DIRECT][Static]` |
