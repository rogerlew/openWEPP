...
| GAP-OFEROUTE-004 | Shock-capture fidelity: the D7 D-val (Iwagaki Case 4) found openWEPP reproduces the enhanced-WEPP shock peak MAGNITUDE (order of magnitude; numerically noisy — peak non-monotonic in `k_o`) but LAGS the shock timing ~5-6 s (`NS_trace` 0.13-0.18; lag-corrected ~0.52 at -5.5 s → dominantly a phase lag). The lag survives the full `k_o` scan and is worse as a 3-OFE cascade, so it is SOLVER-SIDE (not operand-limited): openWEPP behaves like the paper's Original WEPP and does not sharpen the concave-curvature shock catch-up that enhanced-WEPP captures. Candidate root cause: TVD-MacCormack front diffusion / interface celerity. Evidence: `docs/work-packages/20260702-mofefid-d7-enhanced-wepp-dval-001/artifacts/execution-report.md`. | Blocks Case-4 D-val reproduction; blocks `INV-OFEROUTE-011` closure on the curvature-shock case. | attributed (D7); a solver/cascade correction is a separate future package (write-set exceeds D-val analysis) per the D7 split-rule. | `[DIRECT][Static]` |
