...
| REF-OFEROUTE-TVD-MACCORMACK | Garcia-Navarro et al. (1992), JHE 118(10) (R-81, primary in hand); Mingham et al. (2001) (R-82, primary in hand); Papanicolaou et al. (2018) §2.3 (R-63) | TVD-corrected MacCormack predictor/corrector, flux limiter `phi`, dissipative term, and CFL posture. D10 confirms the primaries are now locally available, but they do not by themselves authorize a production correction of the reduced Papanicolaou KWE + lateral-source + OFE sampled-handoff implementation; `GAP-OFEROUTE-005` records the remaining reconciliation boundary. | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `INV-OFEROUTE-011` | D-val Ef + Zone taxonomy vs the four cases | validation | Ef out of tolerance -> investigate; after D9, only Case 4 / `GAP-OFEROUTE-005` remains the current blocker | D-val + Figure 9 taxonomy harness | `[DIRECT][Static] + [DIRECT][Ran]` |
...
`INV-OFEROUTE-011` D10 note (2026-07-05): the Case-4 residual is now
adjudicated under `GAP-OFEROUTE-005` as
`EXECUTED-HOLD-SOURCE-AUTHORITY`. Cases 1-3 and the Zone taxonomy stay closed
...
| `OFEROUTE-ACTIVATION-VALIDATION` | `SC-OFEROUTE-001.md#invariants` | `active` | `maps-to-existing-INV` | `INV-OFEROUTE-010, INV-OFEROUTE-011, INV-OFEROUTE-012` | `science-review-follow-on` | Opt-in default-byte-flat gate + D-val Ef/Zone acceptance + the subsurface-coupling/baseflow activation gate (`INV-OFEROUTE-012`; `GAP-OFEROUTE-006` design-RESOLVED rev 11, seam machinery + fixtures rev 13; runtime SHADOW + real-H2637 vector rev 15; ACTIVATION wiring outstanding). The byte-flat gate is enforceable now (subsystem-off). D9 closes the non-numerics `INV-OFEROUTE-011` surface (Cases 1-3 dispositions + Zone taxonomy); D10 holds the remaining Case 4 / `GAP-OFEROUTE-005` surface on source-authority reconciliation. The subsurface-coupling gate blocks production activation until subsurface-excess-to-runoff is implemented + fixture-verified. |

## Gaps

| Gap ID | Statement | Impact | Disposition | Evidence |
|---|---|---|---|---|
| GAP-OFEROUTE-001 | TVD-MacCormack numerics primaries (Garcia-Navarro 1992, Mingham 2001) were frozen-library secondary through R-63; D10 acquired/read the local primary PDFs and updated `REF-OFEROUTE-TVD-MACCORMACK`. | The primary acquisition gap is closed, but the production correction remains source-bound because those primaries do not directly specify the reduced Papanicolaou KWE + lateral-source + OFE sampled-handoff implementation, nor the Iwagaki Case-4 friction-operand mapping used by D-val. | primary-acquired; residual shock-fidelity closure remains under `GAP-OFEROUTE-005`. Shadow physics tests remain promotable only as shadow diagnostics; Case 4 is not D-val acceptance evidence. | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| GAP-OFEROUTE-007 | **Friction operands are not sourced from WEPP inputs.** The runtime shadow parameterizes every OFE as a BARE cell with `k_o = 500` (the R-63 base grain roughness used across the paper's validation cases) and threads no rainfall-intensity operand into the skin term (`I = 0`); form/wave/vegetation resistance elements (`C_d`, `D_r`, `lambda`, `LAI`, `h_c`) have no WEPP-input lineage at all. | Blocks treating shadow (or future active) hydrographs as friction-faithful on vegetated/rough real hillslopes; the routed VOLUME ledger is unaffected (friction shapes timing, not mass). | declared first cut (2026-07-05, labeled in `laned_shadow.rs`): activation requires either a sourcing rule (management/residue → resistance-element operands) or a recorded operator-approved default set; joins the D-val Case-2/3 operand-limited findings as the operand-authority family. | `[INFERENCE][Static]` |
| GAP-OFEROUTE-005 | Iwagaki shock sampled-hydrograph / resolution sensitivity. D8 corrected the solver sampler so hydrograph samples are interpolated to the actual sample time rather than stamped with the step-end value. D10 reran Case 4 at `k_o=200`: baseline `cells=120`, `sample_dt=1.0 s`, `max_dt=0.5 s` gives `NS_trace=0.262677`, peak ratio `0.836598`, sampled `t_peak=37.0 s` vs reference `25.98 s`, and 10-90% rise `29.41 s` vs reference `20.88 s`. D10 resolution controls show no accepted convergence: `cells=240`, `sample_dt=0.25 s`, `max_dt=0.25 s` gives `NS_trace=0.193296`, peak ratio `1.069855`, `t_peak=39.0 s`; `cells=480`, `sample_dt=0.125 s`, `max_dt=0.125 s` gives `NS_trace=0.101244`, peak ratio `0.844099`, `t_peak=36.125 s`. | Blocks using Case 4 as D-val shock-capture confirmation or default-activation evidence; blocks active routed-water publication/default promotion. H2637 remains a production-shaped diagnostic reproduction, not acceptance. | **EXECUTED-HOLD-SOURCE-AUTHORITY (D10, 2026-07-05):** Garcia-Navarro 1992 and Mingham 2001 are now primary-in-hand and confirm TVD-MacCormack shock-capture families and CFL posture, but they do not by themselves bind the reduced Papanicolaou KWE implementation, the ambiguous R-63 limiter branch, the implicit `alpha` update/boundary treatment, the sampled OFE handoff, or the Iwagaki Manning `n=0.009` primary setup to the D-val `k_o` operand. A source-shaped limiter-branch flip was considered and rejected after worsening Case 4 and failing focused `ofe_routing` tests; `k_o` scans are diagnostic only and are rejected as tuning authority. First actionable closure is a source-authority reconciliation package that binds limiter/CFL/dissipation, lateral source and boundary/handoff treatment, and Iwagaki friction mapping into named tolerances before any production solver/cascade correction. | `[DIRECT][Static] + [DIRECT][Ran] + [INFERENCE][Static]` |
