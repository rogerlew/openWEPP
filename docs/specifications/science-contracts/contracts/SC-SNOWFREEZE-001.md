...
| `snow_stage3_liquid_routing_model` | `enum` | Package-bound Paradigm 2 Stage 3 selector. Accepted values are empty/default `disabled` and explicit opt-in `layered_thermal_liquid_v1`. It is not a parser/runfile/user CLI surface and must fail closed on unsupported non-empty values. | direct-production winter snow/liquid diagnostic path | opt-in rollback and trace proof |
...
- `layered_thermal_liquid_v1` selected by default, exposed through parser/runfile/user or `.run` controls, running without `physics_bulk_multilayer_density_v1` layer state when snow is present, replacing the CoE melt mass path with energy-balance melt, changing public snow/WAT schemas, producing layer temperatures above freezing, publishing untyped or unbounded meltwater temperature, silently discarding refrozen liquid, or leaving mass/liquid/energy residuals materially nonzero. `[DIRECT][Static] + [INFERENCE][Static]`
