...
| `snow_accumulation_melt_diagnostics` | typed diagnostic record | Behavior-neutral hourly ledger carrying active precipitation water depth, rain depth, physical snowfall depth, snowfall SWE (`snowfall_depth * 0.1`), rain/snow fractions, phase-model identity, optional hydrometeor temperature, the four exact CoE empirical melt-depth contributions converted from legacy inches to `m` water equivalent, their uncapped sum, cap adjustment, applied raw melt, and explicit modeled wind-redistribution status/contribution. Depth terms are `m`; fractions are dimensionless; temperature is `degC`. | SIMIMPL28 forcing plus SIMIMPL29 CoE melt evaluation | internal direct-production JSONL trace and diagnostic analysis only |
...
| `snow_stage3_liquid_trace_diagnostics` | typed diagnostic record | Exact Stage-3 daily incoming liquid, routed liquid, signed retained-liquid change, refrozen liquid, and producer residual. All values are `m` water equivalent; retained liquid means the Stage-3 layer-store delta, not the CoE retained store. | `DirectSnowStage3Diagnostics` | additive internal direct-production JSONL trace v4 and independent liquid-closure reconstruction only |
