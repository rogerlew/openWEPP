...
| GAP-SNOWFREEZE-002 | Frost-depth heat-flow executable parity remains open after FDHP01 Increment De. The D2 layered-store continuation closed additive storage for years 2-6 on all 43 prefixes and cleared `p2`; Da/Db re-pinned the accepted gate to the independent WAT flux ledger at WAT-publication numerical texture. C1b/C2 landed fine-layer capacity/overflow and thaw-arm state-machine ownership without reopening D2. Db landed `frzng` in-hour surface-resistance feedback and temporarily put maximum depths inside the legacy envelope. The failed Dc pass proved that seasonal lower-front heat plus thaw feedback cannot be accepted if it reopens storage. Dc1 then landed the seasonal `tmpbl` lower-front heat, in-hour thaw resistance feedback, and fine-theta lower-bound roundoff handling while restoring the independent years 2-6 `Total-Soil + frozwt` residual to `6.471338602487275e-07 mm` max abs across the clean `43/43` cohort. Dd showed that forced legacy snow was only a partial F4 contributor and assigned the next lower-front heat discriminator to the missing `frostn.for:430-458` `bdcons`/`slsw` harmonic unfrozen conductivity path. De landed that content-dependent `Qdry` path and kept storage closed, but the forced legacy-snow cohort still missed the accepted physical envelope (`0/43` prefixes inside `240..503.2 mm`, median duration residual `+186` days). The remaining defect is no longer frozen-water publication, capacity/overflow ownership, missing thaw-arm storage plumbing, unit conversion, stale in-hour surface resistance, seasonal lower-front heat wiring, thaw feedback, D2 accounting, or a constant lower-front-conductivity surrogate. | Blocks frost-depth heat-flow closure and MOFE advancement until the single-OFE cohort runs 43/43, annual `Total-Soil + frozwt` closure stays at numerical noise, depth enters a physical heat-flow envelope without profile-bound pinning, duration residual materially collapses, and the year-7 boundary effect is explained or eliminated without comparator tuning. | active-defect | `[DIRECT][Ran] + [INFERENCE][Static]` |
