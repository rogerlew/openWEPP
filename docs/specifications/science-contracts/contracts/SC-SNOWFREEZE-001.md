...
| INV-SNOWFREEZE-084 | Frost direct-production default activation: the no-env hillslope `DefaultCandidate` runtime policy must select `DirectProductionExecutor` for the supported modern single-OFE direct-production surface after `INV-SNOWFREEZE-047/048/050` ratification and the `GAP-SNOWFREEZE-002` attributed/bounded disposition. Unsupported no-regression surfaces, including current multi-OFE/Wave-2 public-output runs and legacy sidecar-discovery runs, must fall back to `Compatibility` with an explicit fallback reason until a separate package promotes those direct surfaces. Explicit `Compatibility` runtime selection remains the production rollback path and must select compatibility regardless of the default activation gate; explicit `DirectProductionExecutor` remains available and fail-closed for callers that deliberately request direct execution outside the default support set; unknown or conflicting runtime selector values must fail closed. Activation is authorized by the frost Policy B analog in `INV-SNOWFREEZE-050`: bounded residuals plus full-surface no-regression and conservation evidence, not zero paired observation failures. The activation does not authorize Qwet, new frost physics, observation-fixture edits, public output-schema changes, compatibility deletion, multi-OFE direct/Wave-2 promotion, legacy sidecar-discovery direct promotion, or reinterpretation of legacy as an acceptance target. Acceptance requires current full gates (`cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `cargo deny check`), source-level anti-evasion guards when authority-suite posture changes, and runtime evidence that supported no-env runs use direct production while explicit and unsupported compatibility rollback still works. | hard-fail | INV-SNOWFREEZE-047, INV-SNOWFREEZE-048, INV-SNOWFREEZE-050, REF-SNOWFREEZE-FROST-RATIFICATION-20260629, ADR-0011, ADR-0017 | `[DIRECT][Ran] + [INFERENCE][Static]` |
...
- `layered_thermal_liquid_v1` selected by default, exposed through parser/runfile/user or `.run` controls, running without either valid Stage 1 layers or valid Stage 3-Decouple bulk-equivalent layers when snow is present, replacing the CoE melt mass path with energy-balance melt, changing public snow/WAT schemas except for the ratified nullable `MeltwaterTemperature` WAT parquet field, producing layer temperatures above freezing, publishing untyped or unbounded meltwater temperature, silently discarding refrozen liquid, leaving mass/liquid/energy residuals materially nonzero, changing the bulk snow-density rubric under the Stage 3-Decouple arm, or serializing meltwater temperature through HBP/watershed paths before a later stream-temperature package authorizes that boundary. `[DIRECT][Static] + [INFERENCE][Static]`
...
|---|---|---|---|
| `2026-06-29` | `114` | `Codex` | FROST RATIFICATION + DEFAULT ACTIVATION amendment: ratified `INV-SNOWFREEZE-047`, `INV-SNOWFREEZE-048`, and `INV-SNOWFREEZE-050` as accepted observation-validation authority, accepted `TOL-SNOWFREEZE-007..011` as noise-floor evaluation bands, added `REF-SNOWFREEZE-FROST-RATIFICATION-20260629` and `INV-SNOWFREEZE-084`, documented the H1b `frdp` bottom-extent / `thdp` top-thaw correspondence, adjudicated the Step 1 `>0.25` systematic-timing-fraction cutoff as diagnostic-local, and re-dispositioned `GAP-SNOWFREEZE-002` as open-but-attributed/bounded under the frost Policy B analog. The amendment authorizes supported modern single-OFE direct-production frost as the no-env default with explicit compatibility rollback and unsupported multi-OFE/Wave-2 plus legacy sidecar-discovery fallback under full no-regression/conservation gates; it does not authorize Qwet, new frost physics, multi-OFE direct/Wave-2 promotion, or legacy sidecar-discovery direct promotion. |
