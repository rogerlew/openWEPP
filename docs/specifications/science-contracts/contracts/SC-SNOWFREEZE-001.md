...
The required follow-on migration target is therefore the pinned baseline
`/workdir/wepp-forest_260430_baseline/src/winter.for:410-412` hourly
rain/snow partition behavior, with openWEPP source-line provenance preserved in
the continuation ledger. Paired instrumented baseline observation using
...
- OBL-SNOWFREEZE-P-016: For HPHYS0297, publish a per-window defect ledger for H1/H7/H39 that reconstructs `/workdir/wepp-forest_260430_baseline/src/winter.for:434-448` negative-melt behavior from openWEPP hourly trace fields, records the corrected openWEPP source lineage, compares reconstructed `RM` to the pinned comparator using a named tolerance, and assigns `LEGACY-DEFECTIVE`, `OPENWEPP-DEFECTIVE`, or `UNRESOLVED` without downstream compensation. `[DIRECT][Static] + [INFERENCE][Static]`
