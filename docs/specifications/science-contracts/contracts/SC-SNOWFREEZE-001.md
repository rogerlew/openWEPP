...
| INV-SNOWFREEZE-099 | SNOW-STAGE3-WIND-SOURCE-CUSTODY-AND-EXPOSURE authority: retained CLI `w-vl` passes unchanged through raw `vwind`/`vwind_m_s` into Stage 3, while PMET's `10 m`-to-`2 m` `fwv_m_s` is local and cannot feed snow. GRIDMET metadata defines distributed daily `vs` as nominal `10 m` wind on an approximately `4 km` grid; Stage 3 `z_T=z_q=z_u=5 m` remains virtual transfer geometry relative to instantaneous snow, never measurement height. Every site requires immutable product/version/status, request/pixel/sampling, cadence/day boundary, missing policy, transformation/rounding, source datum, forcing exposure, target canopy/snow class, and authoritative linkage before `APPLICABLE` or `INAPPLICABLE`; otherwise emit `AUTHORITY_MISSING`. Values, residuals, height conversion, and desired energy balance cannot infer exposure, fit attenuation, license canopy physics, or authorize production correction. The independent neutral diagnostic may bound geometry sensitivity only. | evidence hard-fail + governance hold | INV-SNOWFREEZE-098, SC-SNOWENERGY-001#INV-SNOWENERGY-031/033, REF-SNOWFREEZE-STAGE3-WIND-SOURCE-CUSTODY, ADR-0042 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `INV-SNOWFREEZE-099` | result-blind custody freeze, exact CLI identities, source/consumer alias checks, neutral-height diagnostic, and exposure matrix | raw CLI-to-Stage-3 custody; PMET-local adjustment nonreachability; nominal source height versus virtual geometry; complete generator/source/exposure fields; two-sided applicability authority; no fitting/canopy/correction | Evidence hard-fail on identity or alias error; governance `HOLD`/`AUTHORITY_MISSING` on absent source or exposure linkage and on any inferred/fitted or production claim | contract-derived literal/source checks, custody ledger, independent calculation, exposure review, and terminal verification | `[DIRECT][Static] + [INFERENCE][Static]` |
...
Stage 3 tuples. Fixture documentation identifies GRIDMET wind but does not bind
native product version, grid support, cadence transformation, reference height,
or exposure. Therefore open-site, above-canopy, within-canopy, and sub-canopy
...
| GAP-SNOWFREEZE-009 | Retained forest-site CLI files omit immutable GRIDMET generator/product/pixel/transform receipts and authoritative forcing-to-target exposure linkage. | Blocks wind applicability, physical carrier passage, and persistence; does not authorize canopy attenuation or production correction. | `AUTHORITY_MISSING`; non-promotable until direct custody and two-sided exposure authority exist | `[DIRECT][Static] + [INFERENCE][Static]` |
