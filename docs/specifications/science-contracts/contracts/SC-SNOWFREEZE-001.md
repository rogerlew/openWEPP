...
- OBL-SNOWFREEZE-P-031: Any density or activation producer consuming
  ...
  consumes live snow-season canopy cover instead of `cancov = 0.0` and uses
...
3. Activation evidence baseline: 05E is promotion-candidate evidence relative
   ...
   live snow-season canopy cover and the native/proven 05B shortwave source.
...
5. Density handoff: SNOWDENSITY-06 may consume the opt-in melt boundary without
   ...
   from the real per-day growth state and either consume native openWEPP
   shortwave or prove the PySnobal-bridge radiation inversion like-for-like.
