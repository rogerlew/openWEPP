...
| `snow_stage3_liquid_routing_model` | `enum` | Package-bound Paradigm 2 Stage 3 selector. Accepted values are empty/default `disabled` and explicit opt-in `layered_thermal_liquid_v1`. It is not a parser/runfile/user CLI surface and must fail closed on unsupported non-empty values. After Stage 3-Decouple, the selector does not require `physics_bulk_multilayer_density_v1`; when the selected density model is bulk, the thermal/liquid arm uses bulk-equivalent layers. | direct-production winter snow/liquid diagnostic path | opt-in rollback and trace proof |
...
| `snow_meltwater_flux_temperature` | `degC` | Typed intensive temperature assigned to the daily meltwater flux when positive routed liquid leaves the pack. It is diagnostic for the future stream-temperature program and must not by itself activate in-stream temperature routing or public schema changes. | `layered_thermal_liquid_v1` candidate / winter-column sub-solver | stream-temperature source diagnostic |
...
- `layered_thermal_liquid_v1` selected by default, exposed through parser/runfile/user or `.run` controls, running without either valid Stage 1 layers or valid Stage 3-Decouple bulk-equivalent layers when snow is present, replacing the CoE melt mass path with energy-balance melt, changing public snow/WAT schemas, producing layer temperatures above freezing, publishing untyped or unbounded meltwater temperature, silently discarding refrozen liquid, leaving mass/liquid/energy residuals materially nonzero, or changing the bulk snow-density rubric under the Stage 3-Decouple arm. `[DIRECT][Static] + [INFERENCE][Static]`
