...
| `snow_evaluation_observability` | `internal tagged schema-v5 record` | Evaluation-only operator/tag IDs, paired-arm IDs, deterministic source/forcing/geometry/non-formulation fingerprints, component applicability and hourly/daily shortwave, longwave, sensible, latent, precipitation-advection, active/lower internal conduction, vapor exchange, cold-content export, available ice, total, residual, requested/evaluated support, and coverage. Disabled/default rows remain exact schema v4. | typed evaluation evidence writer | internal evaluation consumer only; production/public consumers forbidden |
...
| GAP-SNOWFREEZE-007 | v128 admits but does not itself prove the two typed evaluation surfaces. Until the shadow-observability package closes, the current Stage 3 diagnostic remains daily reinitialized and lacks complete operator/pairing tags, per-arm/per-term and cold-content export, typed error custody, and negative production-consumer proof. Even after bounded realization, cross-interval persistence, complete same-substep phase/liquid chronology, restart/reappearance, and terminal/receiving-surface behavior remain intentionally outside `INV-SNOWFREEZE-094/095`. | Blocks claims that runtime conforms to the admitted operators until `OBL-SNOWFREEZE-P-068/C-010` close; separately unadmitted authority continues to block cross-day/seasonal, physical-chronology, terminal, promotion, and cutover claims. | bounded operator implementation is promotable only as evaluation capability after the v128 producer/consumer obligations close; persistence and terminal authority require later amendments | `[DIRECT][Static] + [INFERENCE][Static]` |
