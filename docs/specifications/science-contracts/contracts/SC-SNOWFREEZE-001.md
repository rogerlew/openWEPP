...
| `snow_evaluation_authority_tag` | `record` | Mandatory non-authoritative custody tag containing operator ID, named authoritative source snapshot, interval/support, cadence, carrier identity, coverage, unresolved boundaries, and admitted claim class. | evaluation-request resolver | evaluation evidence consumer only |
...
| INV-SNOWFREEZE-094 | SNOW-STAGE3-EVALUATION-SHADOW-AUTHORITY: production retains exactly one snow-mass/state authority and the linked ledgers of `INV-SNOWFREEZE-091`; an evaluation shadow is admitted only when a typed, default-off request selects exactly one operator and a mandatory non-authoritative tag names its source snapshot, support/interval, cadence, carrier, coverage, unresolved boundaries, and claim class. `same_state_paired_carrier_v1` evaluates named carriers against one identical immutable authoritative snapshot and forcing support without state evolution; it admits carrier/component comparison only. `sequential_resolved_shadow_v1` clones one named snapshot and may evolve that clone only inside one declared continuous interval and the resolved Stage 3 domain; absent later authority, it terminates at that boundary and cannot seed another shadow or production interval. The evaluator has exclusive custody; the clone and its ledgers must not alias, mutate, replace, route into, publish as, calibrate, select, or otherwise reach production state, physics, defaults, fixtures, observations, public outputs, or downstream consumers. Untagged state, ambiguous operator/support, production consumer reachability, cross-boundary carry, or evidence that exceeds the admitted operator claim is a governance `HOLD`; runtime errors in an enabled future evaluator must fail the evaluation request without changing authoritative execution. Daily or independently reinitialized shadows cannot support accumulation-season, peak-SWE, peak-date, meltout, restart, reappearance, terminal/post-terminal, efficacy, promotion, or cutover claims. Persistence and terminal/receiving-surface authority require separate amendments; this invariant authorizes no runtime implementation or CoE ownership change. | evaluation hard-fail + governance hold | INV-SNOWFREEZE-091, INV-SNOWFREEZE-093, SC-SNOWENERGY-001#INV-SNOWENERGY-029, REF-SNOWFREEZE-STAGE3-EVALUATION-SHADOW-AUTHORITY, ADR-0042 | `[DIRECT][Static] + [INFERENCE][Ran]` |
...
| `INV-SNOWFREEZE-094` | test + governance; runtime only after a separate implementation package | typed evaluation request/tag, operator-specific custody, negative production-consumer proof, support/coverage metadata, and claim audit | Evaluation request hard-fails on invalid enabled metadata or state aliasing; governance `HOLD` on untagged state, production reachability, cross-boundary carry, unsupported claims, or any runtime/default/public mutation | SNOW-STAGE3-EVALUATION-SHADOW-AUTHORITY contract gate and future operator implementation/noninterference gates | `[DIRECT][Static] + [INFERENCE][Ran]` |
...
- OBL-SNOWFREEZE-P-067: A future evaluation-shadow implementation must resolve
  ...
  tests must prove identical input/support for the paired carrier, bounded
  interval/domain for the sequential shadow, negative production-consumer
  reachability, and rejection of cross-boundary persistence or overbroad
  claims. This obligation authorizes no implementation by itself.
  `[DIRECT][Static] + [INFERENCE][Static]`
...
| Stage 3 evaluation-shadow authority (`INV-SNOWFREEZE-094`) | typed request/tag resolution, clone custody, operator boundary, negative production-consumer scan, and evidence claim audit | Evaluation hard-fail on invalid enabled metadata or aliasing; governance `HOLD` on untagged/production-readable state, cross-boundary carry, unsupported operator, overbroad claims, or runtime/default/public mutation | evaluation-shadow contract and future implementation/noninterference gates; no cutover evidence | `[DIRECT][Static] + [INFERENCE][Ran]` |
...
|---|---|---|
| State source | one named authoritative snapshot shared exactly by all arms | one named authoritative snapshot cloned once |
...
| Admitted inference | same-support carrier and flux-component comparison | bounded within-interval resolved-state chronology |
...
allocation; two carriers on byte-identical snapshot/support; deliberate
same-state mismatch rejection; bounded sequential evolution; cross-boundary
carry rejection; untagged and incomplete-tag rejection; attempted production
...
| `SNOWFREEZE-STAGE3-EVALUATION-SHADOW` | SNOW-STAGE3-EVALUATION-SHADOW-AUTHORITY package | `active-authority / implementation-hold` | `maps-to-existing-INV` | `INV-SNOWFREEZE-094, OBL-SNOWFREEZE-P-067, OBL-SNOWFREEZE-C-009` | `dual-review` | Two tagged default-off evaluation operators are admitted with evaluator-only custody. Cross-interval persistence, seasonal/terminal claims, runtime implementation, production consumers, and cutover remain held. |
...
| GAP-SNOWFREEZE-007 | The admitted evaluation operators are not implemented as complete typed surfaces. The current Stage 3 diagnostic is daily reinitialized and lacks operator tags, per-term/cold-content export, negative production-consumer proof, persistent restart/reappearance custody, and terminal/receiving-surface authority. | Allows contract-first solver extraction and observability work but blocks claims that runtime conforms to `INV-SNOWFREEZE-094`; blocks cross-day/seasonal, terminal, promotion, and cutover claims. | operator implementation is promotable only as evaluation capability after `OBL-SNOWFREEZE-P-067/C-009`; persistence and terminal authority require later amendments | `[DIRECT][Ran] + [INFERENCE][Static]` |
