...
- OBL-SNOWFREEZE-P-037: Any SNOWDENSITY-09 diagnostic WAT-rerun producer must
  ...
  must not add parser/runfile/user CLI activation, output-schema changes,
  constants, tuning, or frost-physics edits.
  `[DIRECT][Static] + [INFERENCE][Static]`
...
4. Attribution boundary: SNOWDENSITY-09 may authorize resuming frost attribution
   only if the coupled opt-in snow-control gate passes and the SNOWDENSITY-08
   SNOTEL density gate remains cleared. Otherwise it must name the blocker and
   keep frost work held behind snow-control evidence.
