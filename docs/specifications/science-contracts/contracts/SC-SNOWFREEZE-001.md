...
| REF-SNOWFREEZE-STAGE3-EVOLVING-CARRIER-PLAUSIBILITY | `docs/work-packages/20260807-snow-stage3-evolving-state-carrier-plausibility-reconciliation-001/`, its result-blind four-site protocol, and `SC-SNOWENERGY-001` v9 | Characterization authority separating raw vapor/latent opportunity from actual bounded sequential transfer, freezing physical-envelope and geometry authority gaps, and preserving persistence hold without production correction. | `[DIRECT][Static] + [INFERENCE][Static]` |
| REF-SNOWFREEZE-STAGE3-WIND-SOURCE-CUSTODY | `docs/work-packages/20260807-snow-stage3-wind-source-custody-and-exposure-authority-001/`, its result-blind freeze and exact CLI identities, `SC-SNOWENERGY-001` v10, Google Earth Engine `IDAHO_EPSCOR/GRIDMET`, Abatzoglou (2013) DOI `10.1002/joc.3413`, and NASA GSFC NLDAS-2 metadata | Separates nominal `10 m` source-product wind, raw CLI/Stage 3 custody, PMET-local `2 m` adjustment, and virtual `5 m` snow transfer geometry; unresolved generator and exposure authority remain explicit and cannot be fitted. | `[DIRECT][Static] + [INFERENCE][Static]` |
| REF-SNOWFREEZE-STAGE3-PERSISTENT-ACCUMULATION-SHADOW | `docs/work-packages/20260807-snow-stage3-persistent-accumulation-shadow-001/`, `INV-SNOWFREEZE-094/096/098/099`, and `SC-SNOWENERGY-001#INV-SNOWENERGY-030` | Authorizes only isolated evaluation-state continuity, snowfall/liquid custody, dormancy/reappearance, and snapshot/restore mechanics while preserving physical efficacy and production/cutover holds. | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `snow_evaluation_shadow_state` | `typed snow mass/enthalpy/liquid clone` | Independently owned evaluation state cloned from the tagged authoritative snapshot. It cannot alias, mutate, replace, route into, publish as, or select production state. The bounded operators cannot outlive their call boundary; only `persistent_accumulation_shadow_v1` may commit a validated next state to its same-lane evaluator carry slot. | declared evaluation operator | evaluator only; production consumers forbidden |
| `snow_evaluation_persistent_carry` | `typed record` | Lane identity, monotonically increasing interval index, lifecycle status, snow layers with cold content, retained liquid, cumulative mass/energy operands, and deterministic snapshot version/fingerprint for `persistent_accumulation_shadow_v1`. Empty snow remains a valid dormant carry so later snowfall is reappearance, not authoritative-state reseeding. | persistent evaluation runner | same-lane evaluator and independent internal consumer only |
...
| INV-SNOWFREEZE-098 | SNOW-STAGE3-EVOLVING-STATE-CARRIER-PLAUSIBILITY characterization: before any persistent sequential shadow, a result-blind four-site consumer must prove exact GRIDMET/CLI wind custody while leaving unresolved exposure explicit; treat `5 m` as virtual geometry; reconstruct turbulent/radiative/advected terms, raw vapor opportunity, operator-order cold/melt, independently bounded deposition/sublimation, state endpoints, support, and censoring from schema-v6 primitives; and keep S/F actual transfer N/A. Valid active-ice capacity truncation is `VAPOR_OPPORTUNITY_TRANSFER_MISMATCH`; producer/independent transfer or melt disagreement, endpoint failure, and N/A aliasing are invalid evidence. Missing comparable magnitude envelopes, wind exposure, canopy applicability/equation authority, or stability-geometry equation authority is `NOT_EVALUABLE`/`AUTHORITY_MISSING`, never a fitted substitute. No fixed wind attenuation, surrogate equation, physics change, physically interpreted state persistence, production/public consumer, CoE retirement, or cutover is authorized. Physically interpreted persistence requires every canonical-site provenance, reconstruction, mass/energy, support, and physical-envelope gate to pass; any `NOT_EVALUABLE` gate retains that efficacy/validation hold. `INV-SNOWFREEZE-100` may separately admit isolated synthetic/internal continuity mechanics while preserving every physical claim limit. | evidence hard-fail + governance hold | INV-SNOWFREEZE-094/095/096/097, SC-SNOWENERGY-001#INV-SNOWENERGY-017/029/032, REF-SNOWFREEZE-STAGE3-EVOLVING-CARRIER-PLAUSIBILITY, TOL-SNOWFREEZE-018/019, ADR-0042 | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-SNOWFREEZE-099 | SNOW-STAGE3-WIND-SOURCE-CUSTODY-AND-EXPOSURE authority: retained CLI `w-vl` passes unchanged through raw `vwind`/`vwind_m_s` into Stage 3, while PMET's `10 m`-to-`2 m` `fwv_m_s` is local and cannot feed snow. GRIDMET metadata defines distributed daily `vs` as nominal `10 m` wind on an approximately `4 km` grid; Stage 3 `z_T=z_q=z_u=5 m` remains virtual transfer geometry relative to instantaneous snow, never measurement height. Surviving WEPPpy runs directly prove byte-identical CLI lineage, retained watershed centroids and GRIDMET-enabled flags, complete daily parquet wind, and exact parquet-to-CLI equality. The nearest pre-build source revision only statically reconstructs that its implementation would request GRIDMET `vs` at the watershed centroid, share one run-level series, and format it to one decimal; it does not prove the deployed historical request or code path. Exact deployed identity/request/response, product path/version/status, server-side pixel/sampling, day boundary, missing policy, source datum, forcing exposure, physical target canopy/snow class, and authoritative linkage are still required before `APPLICABLE` or `INAPPLICABLE`; otherwise emit `AUTHORITY_MISSING`. Modeled evergreen forest with `cancov=0.9` establishes target model intent only. Values, residuals, height conversion, and desired energy balance cannot infer exposure, fit attenuation, license canopy physics, or authorize production correction. The independent neutral diagnostic may bound geometry sensitivity only. | evidence hard-fail + governance hold | INV-SNOWFREEZE-098, SC-SNOWENERGY-001#INV-SNOWENERGY-031/033, REF-SNOWFREEZE-STAGE3-WIND-SOURCE-CUSTODY, ADR-0042 | `[DIRECT][Static] + [DIRECT][Ran] + [INFERENCE][Static]` |
| INV-SNOWFREEZE-100 | SNOW-STAGE3-PERSISTENT-ACCUMULATION-SHADOW: `persistent_accumulation_shadow_v1` is a third typed, absent-by-default, evaluation-only operator. It owns one private state per direct-runner lane with stable lane identity and strictly increasing interval chronology. The first interval clones its tagged pre-interval authoritative snapshot once; later intervals start exclusively from the prior committed shadow state. Each day is atomic: committed start state, complete ordered hourly/substep transaction, validated ledgers, then committed end state; an error commits nothing. Typed hourly snowfall is credited exactly once before vapor and melt availability. External liquid is credited exactly once to the liquid ledger and never to ice. Bounded deposition, sublimation, melt, refreeze/retention/routing, cold-content change/export, internal conduction, and terminal-unallocated energy remain distinct. Ice plus liquid mass and contract-defined energy identities must reconstruct independently from emitted operands. Complete disappearance commits a dormant empty state; later snowfall reappears from authorized input without stale temperature/cold content or authoritative reseeding. A versioned, fingerprint-bound snapshot must serialize every carried operand, reject unknown fields, duplicate serialized fields, nonfinite values, fingerprint mismatch, wrong lane, and caller-declared out-of-order restore, and produce exact uninterrupted-versus-restore equivalence. Enabled-only internal schema v7 records carry identity, start/end fingerprints and state, inputs/outputs, lifecycle, support, censoring, and closure; schema v4 disabled rows and historical v5/v6 meanings remain exact. Liquid or terminal energy without a resolved recipient is explicit censored diagnostic output and is never silently routed or discarded. The carry cannot reach production state, CoE, frost, routing, defaults, fixtures, observations, calibration, WAT/HBP/PASS, or public output. This authorizes a persistent state-continuity experiment and restart/reappearance mechanics only. Unresolved wind exposure, canopy applicability, stability geometry, physical magnitudes, snow-ground/receiving-surface energy, and terminal disposition remain claim limits: they do not block isolated mechanics but prohibit physical accumulation-season, peak-SWE/date, meltout-efficacy, validation, promotion, ownership, or cutover claims. | evaluation hard-fail + governance claim limit | INV-SNOWFREEZE-091, INV-SNOWFREEZE-094, INV-SNOWFREEZE-096, INV-SNOWFREEZE-098, INV-SNOWFREEZE-099, SC-SNOWENERGY-001#INV-SNOWENERGY-030, REF-SNOWFREEZE-STAGE3-PERSISTENT-ACCUMULATION-SHADOW, ADR-0042 | `[DIRECT][Static] + [INFERENCE][Static]` |
| INV-SNOWFREEZE-101 | SNOW-TERMINAL-ENTHALPY-EVENT-NUMERICS: a separately typed, absent-by-default terminal-event request may extend only `persistent_accumulation_shadow_v1` into the `0 < m_i <= 1 kg m^-2` one-volume domain defined by `SC-SNOWENERGY-001#INV-SNOWENERGY-034`. The request formulation/version is fingerprint-bound to persistent state and cannot change across restore. Each accepted transition and localized event preserves the complete post-chronology solid identity `m_i,end=m_i,start+m_refrozen+m_deposition-m_sublimation-m_melt`, the liquid identity `m_l,end=m_l,start+m_external+m_melt-m_refrozen-m_terminal_handoff`, the canonical positive cold-content sign, and the snow energy identity independently. Exact exhaustion commits an empty dormant snow state with no stale cold content or temperature. Enabled internal schema v8 publishes primitive start/end enthalpy state, adaptive error/bracket evidence, event/evaluated/unevaluated support, all phase/vapor operands, and censored terminal liquid/energy handoffs. Ordinary persistent evaluation remains exact schema v7; disabled and historical v4/v5/v6 meanings remain exact. The real rejecting consumer must reconstruct every identity without producer residuals and reject post-event snow flux, full-step aliases, omitted deposition/refreeze, request/state mismatch, and any named receiving surface. No authoritative state, routing, frost, WAT/HBP/PASS, public output, default, calibration, promotion, ownership, or cutover changes. Receiving-surface recipients and remaining-interval evolution remain explicit claim limits rather than blockers to the isolated snow event. | evaluation hard-fail + governance claim limit | INV-SNOWFREEZE-091, INV-SNOWFREEZE-094, INV-SNOWFREEZE-100, SC-SNOWENERGY-001#INV-SNOWENERGY-034, ADR-0042 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `INV-SNOWFREEZE-097` | result-blind two-source by two-forcing bridge protocol, exact source/input/binary/output custody, paired disabled controls, version-aware independent consumer, and conditional instrumented twins | exact forcing-hash join; same-forcing all-35-WY-plus-median aggregate reconstruction; schema-v4 aggregate-only limit; schema-v6 primitive closure; per-WY factorial effects; deterministic checkpoint lanes; behavior-neutrality and single-axis substitution closure | Evidence hard-fail on identity/hash/schema/window/closure/control/selector mismatch or unknown alias; governance `HOLD` on mismatched-forcing attribution, schema-v4 primitive claim, result-directed checkpointing, equifinal/forcing-dependent causal label, missing in-scope instrumentation, or any physics/production/cutover claim | forcing-matched endpoint matrix, independent aggregate/primitive reconstruction, checkpoint-manifest, protected-output, causal anti-evasion, and exact-claim gates | `[DIRECT][Static] + [INFERENCE][Static]` |
| `INV-SNOWFREEZE-098` | result-blind protocol, schema-v6 independent plausibility consumer, exact four-site custody, and persistence decision gate | wind/exposure custody; virtual geometry; tuple-level raw/bounded vapor and operator-order melt reconstruction; endpoint/support/censoring inventories; physical-envelope authority; no-fit/no-surrogate counterfactual budget; production nonreachability | Evidence hard-fail on malformed identity, producer disagreement, alias, nonclosure, laundering, or result-selected rule; governance `HOLD` on any `NOT_EVALUABLE` physical gate, missing authority, production reachability, persistence, or claims beyond characterization | protocol review, contract-derived adversarials, four-site reconstruction, real-consumer negative reachability, and terminal review/verification | `[DIRECT][Static] + [INFERENCE][Static]` |
| `INV-SNOWFREEZE-099` | result-blind custody freeze, exact CLI identities, WEPPpy provider recovery, source/consumer alias checks, neutral-height diagnostic, and exposure matrix | raw CLI-to-Stage-3 custody; PMET-local adjustment nonreachability; nominal source height versus virtual geometry; retained value fields versus statically reconstructed request/transform semantics; remaining deployed/server/exposure fields; two-sided applicability authority; no fitting/canopy/correction | Evidence hard-fail on identity or alias error; governance `HOLD`/`AUTHORITY_MISSING` on absent deployed/server custody or exposure linkage and on any inferred/fitted or production claim | contract-derived literal/source checks, provider/fixture equality, custody ledger, independent calculation, exposure review, and terminal verification | `[DIRECT][Static] + [DIRECT][Ran] + [INFERENCE][Static]` |
...
- OBL-SNOWFREEZE-P-070: The v130 predecessor-bridge evidence producer must
  ...
  build-input digest for every forcing lane where any paired water-year source
  effect or its paired-difference median fails `TOL-SNOWFREEZE-020`; no result-
  directed checkpoint selection is allowed. `[DIRECT][Static] +
  [INFERENCE][Static]`
- OBL-SNOWFREEZE-P-071: The v131 plausibility evidence producer must preserve
  the exact receipt-bound four-site schema-v6 traces and emit raw vapor/latent
  opportunity, bounded sequential deposition/sublimation, operator-order cold
  and melt operands, state endpoints, turbulent primitives, support, and
  applicability without changing production state, outputs, defaults, or
  schemas. S/F actual transfer remains N/A. `[DIRECT][Static] +
  [INFERENCE][Static]`
- OBL-SNOWFREEZE-P-072: The persistent evaluator must transact one ordered
  lane-day atomically from committed start state to committed end state, emit
  schema-v7 primitive snowfall, external-liquid, vapor, melt, retained/routed,
  cold-content, terminal-energy, lifecycle, support, fingerprint, and closure
  operands, and expose a complete versioned snapshot envelope. It must preserve
  exact disabled schema v4 and historical schema v5/v6 semantics and must not
  mutate any production state or output. `[INFERENCE][Static]`
- OBL-SNOWFREEZE-P-073: When and only when the fingerprint-bound terminal
  request is enabled, the persistent evaluator must emit schema-v8 primitive
  enthalpy, solid/liquid, vapor/phase, trial-error, bracket, event-support, and
  censored-handoff operands while preserving ordinary schema-v7 bytes and
  atomic state commit. `[INFERENCE][Static]`
...
- OBL-SNOWFREEZE-C-013: The v131 plausibility consumer must independently
  reconstruct raw vapor from turbulent primitives, bounded Q transfer from raw
  opportunity and pre-transfer active ice, and cold/melt in operator order
  before endpoint checks. It must reject producer disagreement, simultaneous
  or wrong-direction transfer, melt/liquid/vapor aliases, numeric-zero S/F N/A,
  malformed support, and median-of-medians. Valid capacity truncation remains
  a physical finding. Unknown exposure, geometry equation, canopy operator, or
  magnitude envelopes remain named `NOT_EVALUABLE`/`AUTHORITY_MISSING` and
  block physically interpreted persistence. No production/public consumer may
  read the evidence.
  `[INFERENCE][Static]`
- OBL-SNOWFREEZE-C-014: The independent schema-v7 consumer must reconstruct
  each lane-day and cumulative ice/liquid/energy identity from primitive
  operands, reject production/adjacent aliases and producer residuals, enforce
  ordered identity plus atomic start/end fingerprints, and prove exact
  uninterrupted-versus-snapshot/restore equivalence. It must keep dormant,
  disappeared, reappeared, terminal-unallocated, unresolved-recipient, and
  support-censored states explicit and reject unknown trace/snapshot versions.
  No production or public consumer may read persistent state or evidence as
  authority. `[INFERENCE][Static]`
- OBL-SNOWFREEZE-C-015: The independent schema-v8 consumer must reconstruct
  terminal endpoint solid, liquid, cold-content/enthalpy, energy, event time,
  evaluated/unevaluated support, and adaptive acceptance from primitives. It
  must reject producer residuals, full-step melt/sublimation, missing
  deposition/refreeze, post-event snow flux, request/state mismatch, and any
  receiving-surface label or downstream use. `[INFERENCE][Static]`
...
| Forcing-matched Stage 3 predecessor bridge (`INV-SNOWFREEZE-097`) | exact 2x2 endpoint matrix, source/input/binary/run custody, paired disabled controls, independent version-aware consumer, and conditional instrumented twins | Evidence hard-fail on custody, schema, window, aggregate/primitive closure, selector-equivalence, or protected-output failure; governance `HOLD` on mismatched-forcing attribution, schema-v4 primitive inference, adaptive checkpointing, equifinality, forcing-dependent causal axis, or claims beyond aggregate custody/operator mechanics | forcing-matched all-WY-plus-median bridge, factorial accounting, deterministic checkpoint, neutrality, single-axis substitution, and claim-limit gates; no physics, seasonal, terminal, production, ownership, or cutover evidence | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| TOL-SNOWFREEZE-019 | Operator-reconciliation sign and support classification (`INV-SNOWFREEZE-096`) | absolute zero/sign threshold `1e-6 J m^-2`; support materiality ratio threshold `0.05` | Frozen sign/support tolerances, not physics-fit thresholds. Exact-zero denominator makes the support ratio N/A; it is never coerced to zero. All predecessor-reproduction tolerances are superseded by forcing-matched `TOL-SNOWFREEZE-020`. |
| TOL-SNOWFREEZE-020 | Forcing-matched schema-v4/schema-v6 aggregate predecessor bridge (`INV-SNOWFREEZE-097`) | schema-v4 24-hour sum versus daily aggregate: `max(1e-6 J m^-2, 1e-12 * sum_abs_operands)`; each of 35 paired Snowbird water-year aggregate differences: the same scale-aware tolerance; Python `statistics.median` of the 35 paired differences: `1e-7 MJ m^-2` | The v4 aggregate closure plus both all-water-year and median gates are mandatory and use the exact same forcing SHA-256. This is evidence-custody tolerance only, not a physics-fit threshold; it cannot support primitive/schema-v4 or mismatched-forcing causal claims. |
| TOL-SNOWFREEZE-021 | Schema-v7 persistent shadow mass and energy reconstruction (`INV-SNOWFREEZE-100`) | per-substep/day/cumulative mass: `max(1e-12 kg m^-2, 1e-12 * sum_abs_operands)`; energy/cold: `max(1e-6 J m^-2, 1e-12 * sum_abs_operands)`; snapshot/restore identity: exact typed-field and fingerprint equality | Evaluation reconstruction only. It cannot authorize canonicalization, inferred recipients, dropped dormant intervals, physical efficacy, or production correction. |
| TOL-SNOWFREEZE-022 | Schema-v8 terminal event reconstruction (`INV-SNOWFREEZE-101`) | inherits `SC-SNOWENERGY-001#TOL-SNOWENERGY-001`; event mass root `max(1e-12 kg m^-2,1e-12*m_i,start)`, time bracket `1e-6 s`, scale-aware mass/energy closure as named there, and exact request/state fingerprint identity | Evaluation numerics only. LTE, root, and closure tolerances are distinct and authorize no clamp, inferred recipient, post-event flux, or production correction. |
...
This addendum binds `INV-SNOWFREEZE-098` and remains subordinate to
`SC-SNOWENERGY-001#INV-SNOWENERGY-017/029/032`. It authorizes one
result-blind characterization of the retained four-site schema-v6 cohort, not
new physics or state.

The exact runtime wind boundary is the daily CLI `w-vl` value repeated into
Stage 3 tuples. Surviving WEPPpy records directly bind the stored watershed
centroid, GRIDMET-enabled flag, daily parquet values, and exact CLI
serialization. Nearby pre-build source only statically reconstructs the
GRIDMET product/request and shared-series implementation; the deployed request
and code path are not retained. Exact product path/version, server pixel
support, day-boundary/fill policy, reference datum, or exposure remain absent.
Therefore open-site,
above-canopy, within-canopy, and sub-canopy
are not aliases. `z_T=z_q=z_u=5 m` remains virtual geometry, not a physical
sub-canopy instrument. Missing exposure blocks persistence and cannot be
replaced by scalar attenuation.

Schema-v6 `vapor_mass_exchange_kg_m2` is `m_v,raw`, a signed opportunity.
`S/F` actual transfer is N/A. For every Q tuple, independently reconstruct bounded
deposition and sublimation under `SC-SNOWENERGY-001` v9, then reconstruct cold
and melt in operator order and check total endpoints. Producer-versus-
independent disagreement is invalid evidence and package `HOLD`. Valid
availability truncation is `VAPOR_OPPORTUNITY_TRANSFER_MISMATCH`; it is not an
execution failure and cannot cancel across tuples or medians.

Canopy-aerodynamic absence is a hypothesis, not a correction. No canopy class
may be emitted until exposure and applicability establish that a distinct
operator is required. Current Rust and pinned libsnobal `z/L` parity is
provenance only and does not select `z/L` over `(z-d)/L`; without canonical
equation authority geometry is `NOT_EVALUABLE` and no counterfactual runs.
Frost `tmpadj`, fixed/fitted attenuation, and surrogate/proxy equations are
inadmissible.

Physical magnitude passage requires prospectively admitted comparable
envelopes for external energy, sensible/latent exchange, raw opportunity,
bounded transfer, state, and stability/exchange behavior. A missing envelope is
`NOT_EVALUABLE`, never an assumed range. Any such field blocks
`EVOLVING_CARRIER_PLAUSIBILITY_PASS` and cross-day persistence even when
mechanical reconstruction succeeds. Snowbird is full-report but non-decisive;
Mica Creek, Niwot, and Paradise govern the screen. All window totals precede
site medians, and separately reduced medians never form a ledger.
...
| `SNOWFREEZE-STAGE3-EVOLVING-CARRIER-PLAUSIBILITY` | SNOW-STAGE3-EVOLVING-STATE-CARRIER-PLAUSIBILITY-RECONCILIATION package and canonical addendum | `active` | `maps-to-existing-INV` | `INV-SNOWFREEZE-098, OBL-SNOWFREEZE-P-071, OBL-SNOWFREEZE-C-013` | `flagged-binding-addition` | Result-blind four-site characterization separates raw vapor/latent opportunity from bounded transfer and binds exposure/geometry/envelope authority gaps. No production correction, persistence, ownership, or cutover authority. |
...
| `2026-08-07` | `132` | `Codex` | SNOW-STAGE3-WIND-SOURCE-CUSTODY amendment: separated nominal source height, raw CLI/Stage 3 wind, PMET-local adjustment, and virtual transfer geometry; bound exact missing-authority fields and two-sided exposure predicates; prohibited fitting, canopy licensing, and production correction. |
| `2026-08-07` | `131` | `Codex` | SNOW-STAGE3-EVOLVING-STATE-CARRIER-PLAUSIBILITY amendment: added `REF-SNOWFREEZE-STAGE3-EVOLVING-CARRIER-PLAUSIBILITY`, `INV-SNOWFREEZE-098`, `OBL-SNOWFREEZE-P-071`, and `OBL-SNOWFREEZE-C-013`; bound result-blind four-site wind/geometry/canopy custody, raw-versus-bounded vapor and operator-order melt reconstruction, authority gaps, physical-envelope fail-closed posture, and persistence hold without production correction. |
