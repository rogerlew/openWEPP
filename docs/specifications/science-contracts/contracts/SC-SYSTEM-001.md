...
| GAP-SYSTEM-007 | WSHED10 adds parser export coverage for active impoundment branch payload families (drop/culvert/rockfill/emergency/filter/riser) and validates those exports in contract-derived parser vectors; however watershed runtime integration still lacks implemented active-structure coefficient projection into WS12 runtime coefficient families and remains fail-closed for active domains. | System integration now has active branch payload availability at parser boundaries, but full active-structure impoundment promotion remains blocked pending runtime active-coefficient projection/migration closure. | non-promotable | `[DIRECT][Static] + [Ran]` |
