...
| REF-SYSTEM-INFILE-CHANINP | `docs/specifications/science-contracts/contracts/SC-INFILE-CHANINP-001.md` §7a and `/workdir/wepp-forest_260430_baseline/src/wshinp.for:469-496` (`dac3c950d8b16cc73774bf5ce2e7e11f80baac70`) | WSHED-W10 authority for typed `chan.inp` absent/open-error compatibility defaults and parser-to-runtime handoff. | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| Hillslope contributor payloads | `hs{ID}_peakro`, `hs{ID}_watdur`, `hbp_latest_event_state`, `hbp_noevent_source_event_kind` |
...
5. M-F-REDO2 runoff-normalization: public `Q` is the cumulative-length depth
   (`runoff * efflen / totlen`), and public `QOFE == Q` is the canonical
   published convention (MOFEFID-B02 / `SC-RUNOFFPART-001#INV-RUNOFFPART-032`;
   the retained OFE-local `runoff * efflen / slplen` basis feeds only the
   byte-invariant `H.pass.runvol`/peak). The former `QOFE == Q where slplen !=
   totlen` rejection is superseded; conservation identities remain anchored to
   raw transfer/runoff operands, and per-OFE genuineness is proven by the
   distinctness/lineage evidence above.
