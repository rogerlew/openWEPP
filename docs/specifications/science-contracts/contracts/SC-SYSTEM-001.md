...
| GAP-SYSTEM-007 | WS12 parser-to-runtime coefficient projection no longer requires synthetic/manual seeding for inactive-structure conformance lanes after WSHED04 seam closure, and WSHED07 has migrated RK4/adaptive/regime-transition continuity behavior for those supported domains; however production integration still lacks full active-structure branch payload projection and remains fail-closed for those domains. | System integration manual-seeding dependency is removed and supported-domain continuity migration is active, but full all-structure impoundment promotion remains blocked pending active-structure projection expansion. | non-promotable | `[DIRECT][Static] + [INFERENCE][Static]` |
