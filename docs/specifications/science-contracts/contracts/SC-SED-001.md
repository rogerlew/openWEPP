...
| GAP-SED-006 | Watershed channel sediment process closure (`chnero/chnrt/detach`) remains open in companion watershed runtime work even though hillslope boundary payload authority (`INV-SED-010`) is explicit and enforced. WSHED06 closed WS11 channel sediment publication-family symbols (`ws10_channel_{id}_qsed`, `ws10_channel_{id}_tc`), WSHEDIMPL15 added WS15 channel control projection plus baseline conversion scaffold publication/guards (`crsh/depmid/depsid`), WSHEDIMPL16 added fail-closed contributor `particle_diameter_m` payload projection for transport-capacity lineage ingress, WSHEDIMPL17 added fail-closed WS17 segment/hydraulic scaffold projection/guard closure (`nslpts`, segment `x/slope/depa/depb/wida/widb`) required before literal `chnero/chnrt/detach` segment-loop migration, WSHEDIMPL18 migrated baseline `shield`/`trncap` transport-capacity authority into WS10 channel sediment publication so `tc` responds to class payloads instead of surrogate `tc=qsed` identity, WSHEDIMPL19 added fail-closed channel branch payload export plus upstream channel-dependency payload ingress continuity (`particle_class_count`, class fractions, class diameters) into WS10 aggregation, WSHEDIMPL20 added opt-in WS20 segment-loop `case12` routing scaffolding with unresolved-detachment diagnostics publication, WSHEDIMPL21 added WS21 case34/enddet diagnostics scaffolding with explicit unresolved detach-capacity diagnostics continuity, WSHEDIMPL22 replaced WS21 opt-in unresolved fallback behavior with baseline-lineage `dcap` + `case34/enddet` execution plus required fail-closed `crfrac` projection gating (`ws10_channel_{id}_crfrac_{class:04}`), WSHEDIMPL23 migrated baseline-authoritative `detach.for` iterative closure behavior for WS21 `case4` rows (`nt < cnpart`) so residual WS21 unresolved-detachment diagnostics are no longer required for that branch, and WSHEDIMPL24 migrated baseline-authoritative `case12.for` deposition-to-detachment transition continuation (`xdemax < x(i)` into `detach.for`) with explicit transition diagnostics publication (`ws24_case2_detach_segment_count`). Remaining baseline-authoritative full `chnero/chnrt` parity semantics are still unresolved. | End-to-end watershed sediment-routing closure remains blocked pending completion of remaining `chnero/chnrt/detach` segment-process migration families and validation closure. | non-promotable | `[DIRECT][Static] + [INFERENCE][Static]` |
