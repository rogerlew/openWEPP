...
| REF-SED-DC01-SHAPE | `SC-WATBAL-001#INV-WATBAL-102`..`104` + `direct_runtime/runoff.rs` | Default/off hourly-shape authority: while Lane D routing does not own the surface-water path, the source-complete closing hourly runoff depths from WB14 partition and WB19 saturation return supply the hourly-runoff shape consumed by the erosion solve, the serialized `V_h` surface, and downstream runon admission. Melt and runon enter WB14 hourly supply exactly once. Shape weights are a derivative consumer surface only; positive runoff without complete hourly custody fails closed, and no uniform or rainfall-window fallback is authorized. It cannot carry active-routed-water erosion acceptance. | `[DIRECT][Static]` |
...
| INV-SED-014 | Hydrograph-resolved closure invariant (ADR-0036 D4): each active hour's solve must satisfy the existing in-solve conservation gates (INV-SED-001/INV-SED-010 machinery per hour); the published daily aggregates must be the hour sums (`tdet/tdep/exported = Σ_h`) to f64-rounding; the serialized hourly surfaces must satisfy the integral closures `Σ_h V_h = runvol` and `Σ_h S_h =` the day's exported sediment mass. The internal erosion operand is the maximum hourly mean depth rate, `peakro_depth = max_h(V_h / Area / 3600 s) = max_h(q_hourly(h)/3600 s)` in `m/s`; erosion consumes that internal value, never the public volumetric value or a separate analytical estimator. Public PASS publication alone converts it as `peakro = peakro_depth · Area` in `m3/s`. No rescaling of the hourly profile toward another peak is permitted. | hard-fail | REF-SED-ADR0036, INV-SED-001, INV-SED-010, SC-WATBAL-001#INV-WATBAL-102, SC-WATBAL-001#INV-WATBAL-104 | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| `peakro_depth`, `watdur` | `DirectPeakRunoffDownstreamOperands::{peak_runoff_rate_m_s,runoff_duration_s}` -> erosion `peakro_m_s`, `effdrn_s`/`watdur_s` | Internal peak-runoff and rectangular-equivalent duration forcing surfaces for erosion branches | `m s^-1`, `s`; `watdur = Q/peakro_depth` | `[DIRECT][Static] + [INFERENCE][Static]` |
| public `peakro` | PASS/publication `peak_runoff_m3_s` | Volumetric publication only; never an erosion forcing operand | `m^3 s^-1`; exactly `peakro_depth · Area` | `[DIRECT][Static] + [INFERENCE][Static]` |
| `peak_hour_index`, `method_branch` | `DirectPeakRunoffDownstreamOperands::{peak_hour_index,method_branch}` | Maximum-hour custody and method provenance | zero-based hour index + method metadata | `[DIRECT][Static] + [INFERENCE][Static]` |
...
|---|---|---|---|---|---|
| `EROD-BND-001` | `Q`, `peakro_depth`, `watdur`, closing hourly runoff depths, `peak_hour_index`, `method_branch` | WB12 daily runoff depth plus `DirectPeakRunoffDownstreamOperands::{peak_runoff_rate_m_s,runoff_duration_s,peak_hour_index,method_branch}` | `SC-WATBAL-001` via WB14/WB19 closing-hour custody and WB16 derivation | `SC-SED-001` (`INV-SED-004`, `INV-SED-013`, `INV-SED-014`) | `[DIRECT][Static] + [INFERENCE][Static]` |
...
|---|---|---|---|---|
| Hydrology forcing intake (`Q`, `peakro_depth`, `watdur`, closing hourly runoff depths, peak provenance) | `SC-WATBAL-001` (`INV-WATBAL-102`..`104`) | `SC-SED-001` (`INV-SED-004`, `INV-SED-013`, `INV-SED-014`) | Canonical producer/consumer guard ownership is explicit: erosion receives the internal `m/s` maximum-hour operand and rectangular-equivalent duration; the public `m3/s` value is publication-only. | `[DIRECT][Static] + [INFERENCE][Static]` |
...
| Surface | Symbols |
|---|---|
| Peak-flow forcing inputs | internal `peakro_depth`, `watdur`, `Q`, closing hourly runoff depths |
| WB16 provenance payload | `peak_hour_index`, `method_branch` |

### WB16 Coupling Rules

1. Sediment-kernel readiness requires finite/non-negative `peakro_depth` and
   `watdur` for erosion forcing derived from Chapter-11 hydrologic inputs.
2. `peakro_depth = max_h(V_h / Area / 3600 s)` in `m/s`, and the
   rectangular-equivalent duration is `watdur = Q / peakro_depth` within
   `TOL-SED-009`. Public publication alone converts the peak to `m3/s` as
   `peakro = peakro_depth · Area`.
3. The closing hourly runoff depths, `peak_hour_index`, and `method_branch`
   retain source and derivation custody. Retired APPMTH scalar diagnostics are
   not required erosion inputs.
4. Missing or malformed required peak custody is a typed boundary failure.
   Sediment consumers must not synthesize a uniform, rainfall-window, or
   analytical fallback when the hourly peak operands are absent or invalid.
...
|---|---|---|
| Activation + hydrology forcing | `erod13_core_enabled`, `Q`, `peakro_depth`, `watdur`, `Ie`, `te` | Activation gate and Chapter-11 hydrologic forcing semantics (`INV-SED-004`); `peakro_depth` is the internal `m/s` maximum-hour operand. |
...
2. Validate hydrology-forcing continuity (`INV-SED-004`):
   - `Q > 0`, `peakro_depth > 0`, `watdur > 0`, `Ie >= 0`, `te > 0`,
   - `abs(watdur - (Q / peakro_depth)) <= TOL-SED-009`.
