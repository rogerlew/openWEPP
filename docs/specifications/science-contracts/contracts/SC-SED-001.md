...
| GAP-SED-006 | Watershed channel sediment process closure (`chnero/chnrt/detach`) remains open in companion watershed runtime work even though hillslope boundary payload authority (`INV-SED-010`) is explicit and enforced. WSHED06 closed WS11 channel sediment publication-family symbols (`ws10_channel_{id}_qsed`, `ws10_channel_{id}_tc`), WSHEDIMPL15 added WS15 channel control projection plus baseline conversion scaffold publication/guards (`crsh/depmid/depsid`), WSHEDIMPL16 added fail-closed contributor `particle_diameter_m` payload projection for transport-capacity lineage ingress, and WSHEDIMPL17 added fail-closed WS17 segment/hydraulic scaffold projection/guard closure (`nslpts`, segment `x/slope/depa/depb/wida/widb`) required before literal `chnero/chnrt/detach` segment-loop migration. | End-to-end watershed sediment-routing closure remains blocked pending full `chnero/chnrt/detach` process-family migration and validation closure. | non-promotable | `[DIRECT][Static] + [INFERENCE][Static]` |
