...
| `SC-HYDRAULICS-001` | Overland Hydraulics Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-HYDRAULICS-001.md` | `static` | `2026-05-23` |  | WS10 amendment added routing/impoundment consumer-coupling authority requiring preservation of WS10 typed payload/guard families (`WKERNEL-WS10-CHANNEL-E-001..003`, `WKERNEL-WS10-IMPOUNDMENT-E-001..003`) on top of WB16 peak-flow intake requirements; remains non-promotable while `GAP-HYD-003` is open. |
| `SC-IMPOUND-001` | Surface Impoundment Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-IMPOUND-001.md` | `static` | `2026-05-23` |  | WS10 amendment added production impoundment runtime alias/guard authority (`ws10_impoundment_*`, `WKERNEL-WS10-IMPOUNDMENT-E-001..003`) and contract-derived test-vector obligations; remains non-promotable while `GAP-IMPOUND-001`..`GAP-IMPOUND-003` are open (`GAP-IMPOUND-004` is promotable-with-risk). |
...
| `SC-ROUTE-001` | Watershed Routing and Channel Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-ROUTE-001.md` | `static` | `2026-05-23` |  | WS10 amendment added production channel runtime alias/dependency authority (`ws10_channel_*`, upstream node payloads), typed WS10 routing guard family (`WKERNEL-WS10-CHANNEL-E-001..003`), and contract-derived test vectors; remains non-promotable while `GAP-ROUTE-002`, `GAP-ROUTE-003`, and `GAP-ROUTE-005` are open. |
| `SC-RUNOFFPART-001` | Surface Runoff Partition Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-RUNOFFPART-001.md` | `static` | `2026-05-23` |  | WB16 amendment added closure-diagnostics peak-runoff authority (`peakro`, `watdur`) with deterministic `tstar` branch sequencing and typed WB16 guard posture, on top of WB15 interception and IRRIG10 forcing coupling; remains non-promotable while `GAP-RUNOFFPART-002`..`GAP-RUNOFFPART-004` are open. |
| `SC-SED-001` | Hillslope Erosion Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-SED-001.md` | `static` | `2026-05-23` |  | WB16 amendment added hydrologic peak/duration intake authority (`peakro`, `watdur`) with continuity/traceability and typed-guard requirements for erosion coupling readiness; remains non-promotable while `GAP-SED-002` and `GAP-SED-003` are open (`GAP-SED-001` and `GAP-SED-004` are promotable-with-risk). |
...
| `SC-SYSTEM-001` | System Integration Boundary and Watershed Assembly Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-SYSTEM-001.md` | `static` | `2026-05-23` |  | WS10 amendment added deterministic watershed production-kernel integration authority for WS10 runtime aliases and guard families, extending PL15 closeout governance (`INV-SYSTEM-013`) and PL14 replay staging guard posture (`INV-SYSTEM-012`). |
| `SC-WATBAL-001` | Water Balance Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-WATBAL-001.md` | `static` | `2026-05-23` |  | WB16 amendment added closure-diagnostics peak-runoff authority (`peakro`, `watdur`) with canonical branch rules and typed WB16 guard posture, extending prior WB15 interception and IRRIG10 storage-coupling terms. |
