...
| `SC-LANDSURFACEENERGY-001` | Land-Surface Energy-Balance Process Contract | `approved` | `active` | openWEPP maintainers + land-surface-energy/hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-LANDSURFACEENERGY-001.md` | `static+independent_oracle+contract_vectors` | `2026-08-29` |  | v12 retains exact centered interior differences and admits only the unique inward one-sided difference at an exact covered-coordinate closed bound for potential/final solves; domains, scaling authority, branches, ledgers, receipts, and rollback are unchanged. |
...
| `SC-SNOWENERGY-001` | Snow-Surface Energy and Sub-Canopy Longwave Contract | `approved` | `active` | openWEPP maintainers + snow-process reviewer | `docs/specifications/science-contracts/contracts/SC-SNOWENERGY-001.md` | `static+independent_oracle+contract_vectors` | `2026-08-29` |  | v26 retains bounded vapor/phase custody, the exact 60-second fallback, conditional exact-floor contraction, and authentic replay; accepted terminal composition now binds every contiguous child precipitation/heat receipt into the enclosing ledger while preserving distinct final-child boundary evidence and existing `TOL-SNOWENERGY-005`. |
...
| `SC-SURFACELIQUID-001` | Persistent Snow-Free Surface-Liquid Hydrology Custody Contract | `approved` | `active` | openWEPP maintainers + hydrology/land-surface-energy reviewer | `docs/specifications/science-contracts/contracts/SC-SURFACELIQUID-001.md` | `static+contract_vectors` | `2026-08-29` |  | v13 retains the exact 60-second adaptive floor and conservative routing, proves both OFE- and exact tile-basis debit sums, and separates exact per-trial WB14 receipt factorization lineage from cross-factorization physical comparison. |
