...
|---|---|---|---|---|---|---|---|---|---|
| `SC-CLIMATE-001` | Climate Forcing Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-CLIMATE-001.md` | `static` | `2026-05-23` |  | CLIM05/CLIM06 amendments added parsed snow/frost runtime coupling requirements (`snow.options.*`, `frost.options.*`), signed `S` publication semantics, explicit freeze/thaw branch authority, and active-coupling typed guard posture; remains non-promotable while `GAP-CLIMATE-003`..`GAP-CLIMATE-005` are open (`GAP-CLIMATE-002` is promotable-with-risk). |
...
| `SC-RUNOFFPART-001` | Surface Runoff Partition Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-RUNOFFPART-001.md` | `static` | `2026-05-23` |  | CLIM05/CLIM06 amendments added active snow and frost/frozen-soil runoff coupling authority using signed `S` and `frost.runtime_infcap_frz` with required `snow.options.*` and `frost.options.*`/`frost.runtime_*` surfaces while retaining WB14 typed runoff guard family; remains non-promotable while `GAP-RUNOFFPART-002`..`GAP-RUNOFFPART-004` are open. |
...
| `SC-WATBAL-001` | Water Balance Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-WATBAL-001.md` | `static` | `2026-05-23` |  | CLIM05/CLIM06 amendments added signed snow term (`S`) authority in WB12 storage reconciliation and frozen-soil infiltration-capacity coupling authority (`frost.runtime_infcap_frz`) in WB14 runoff reconciliation with active-coupling typed guard expectations, extending prior WB14 hyetograph coupling and WB13 replay-candidate authority. |
