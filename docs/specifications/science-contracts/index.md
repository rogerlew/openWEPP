...
|---|---|---|---|---|---|---|---|---|---|
| `SC-CLIMATE-001` | Climate Forcing Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-CLIMATE-001.md` | `static` | `2026-05-23` |  | WB14 amendment added explicit hyetograph-coupling forcing requirements (`timem_####`, `intsty_####`, count surfaces) and typed malformed-series consumer-failure posture; remains non-promotable while `GAP-CLIMATE-003`..`GAP-CLIMATE-005` are open (`GAP-CLIMATE-002` is promotable-with-risk). |
...
| `SC-PLANT-001` | Plant Growth Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-PLANT-001.md` | `static` | `2026-05-23` |  | PL16 amendment added equation-authoritative growth-runtime update semantics for active annual/perennial branches (GDD, stress-regulated biomass, canopy/LAI, root/depth, senescence decline) and required-symbol typed failure posture; promotion remains blocked by cross-contract gap `GAP-PLANT-004`. |
| `SC-RESIDUE-001` | Residue Management Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-RESIDUE-001.md` | `static` | `2026-05-23` |  | PL16 amendment aligned growth transition authority to reset-only (`planting/harvest/stop`) plus equation-driven non-reset payload behavior and explicit growth-equation guard posture, while retaining INT10 coupled lane-order closure; remains non-promotable while `GAP-RESIDUE-002` and `GAP-RESIDUE-003` are open. |
...
| `SC-RUNOFFPART-001` | Surface Runoff Partition Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-RUNOFFPART-001.md` | `static` | `2026-05-23` |  | WB14 amendment added production infiltration + subdaily hyetograph reconciliation authority with Green-Ampt lineage branch rules and typed WB14 runoff guards, superseding required externally seeded infiltration bookkeeping in acceptance paths; remains non-promotable while `GAP-RUNOFFPART-002`..`GAP-RUNOFFPART-004` are open. |
...
| `SC-WATBAL-001` | Water Balance Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-WATBAL-001.md` | `static` | `2026-05-23` |  | WB14 amendment added runoff-lane infiltration/hyetograph coupling authority, including deterministic computed-infiltration reconciliation surfaces and typed `HKERNEL-WB14-RUNOFF-E-00x` guard posture (in addition to prior WB13 replay-candidate authority). |
