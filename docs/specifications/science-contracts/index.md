...
|---|---|---|---|---|---|---|---|---|---|
| `SC-CLIMATE-001` | Climate Forcing Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-CLIMATE-001.md` | `static` | `2026-05-23` |  | CLIM05 amendment added parsed snow-control runtime coupling requirements (`snow.options.*`), signed `S` publication semantics, and active-coupling typed guard posture; remains non-promotable while `GAP-CLIMATE-003`..`GAP-CLIMATE-005` are open (`GAP-CLIMATE-002` is promotable-with-risk). |
...
| `SC-PLANT-001` | Plant Growth Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-PLANT-001.md` | `static` | `2026-05-23` |  | PL17 amendment added decomposition-kinetics parameter projection authority (`oratea`, `orater`) on decomposition runtime surfaces, with explicit typed hard-fail projection posture for missing/non-finite/non-positive rate parameters; promotion remains blocked by cross-contract gap `GAP-PLANT-004`. |
...
| `SC-SNOWFREEZE-001` | Snow and Freeze Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-SNOWFREEZE-001.md` | `static` | `2026-05-23` |  | CLIM05 amendment added parsed snow-control runtime coupling authority (`INV-SNOWFREEZE-010`) with explicit `S`/`snow.runtime_swe` closure and active-coupling guard posture; promotion remains `HOLD` while non-promotable gaps (`GAP-SNOWFREEZE-002/003/004`) remain open. |
...
| `SC-WATBAL-001` | Water Balance Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-WATBAL-001.md` | `static` | `2026-05-23` |  | CLIM05 amendment added signed snow term (`S`) authority in WB12 storage reconciliation equation and typed active-coupling guard expectations, extending prior WB14 runoff infiltration/hyetograph coupling and WB13 replay-candidate authority. |
