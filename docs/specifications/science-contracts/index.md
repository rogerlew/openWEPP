...
| `SC-SNOWENERGY-001` | Snow-Surface Energy and Sub-Canopy Longwave Contract | `approved` | `active` | openWEPP maintainers + snow-process reviewer | `docs/specifications/science-contracts/contracts/SC-SNOWENERGY-001.md` | `static+independent_oracle+contract_vectors` | `2026-08-30` |  | v27 retains bounded vapor/phase custody, the exact 60-second fallback, conditional exact-floor contraction, and authentic replay; only the persistent per-layer refreeze-history tracer is excluded from cross-factorization error estimation, while accepted composed owner bytes and independent physical refreeze/mass/energy closure remain exact. |
