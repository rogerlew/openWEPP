...
| `SC-EVAP-001` | Evapotranspiration Stress Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-EVAP-001.md` | `static` | `2026-05-23` |  | WB11 amendment promoted ET from routing scaffolding to production-kernel authority with deterministic ET/writeback guards; remains non-promotable while `GAP-EVAP-002` and `GAP-EVAP-003` are open. |
...
| `SC-PERC-001` | Percolation Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-PERC-001.md` | `static` | `2026-05-23` |  | WB11 amendment promoted percolation from routing scaffolding to production-kernel authority with deterministic `D`/`Pe` guard posture; remains non-promotable while `GAP-PERC-002` and `GAP-PERC-003` are open. |
| `SC-PLANT-001` | Plant Growth Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-PLANT-001.md` | `static` | `2026-05-23` |  | PL13A closed projected PL alias continuity for slot/crop/runtime families (including `conset/drset` drift to canonical `conseq/drseq`) and retained explicit exception posture for non-canonical structural scheduler symbols; promotion remains blocked by cross-contract gap `GAP-PLANT-004`. |
| `SC-RESIDUE-001` | Residue Management Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-RESIDUE-001.md` | `static` | `2026-05-23` |  | PL12+PL13 scheduler transition authority now covers decomposition payloads and growth-transition state/reset signaling with typed domain guards; remains non-promotable while `GAP-RESIDUE-002` and `GAP-RESIDUE-003` are open. |
...
| `SC-RUNOFFPART-001` | Surface Runoff Partition Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-RUNOFFPART-001.md` | `static` | `2026-05-23` |  | WB12 amendment added runoff reconciliation kernel authority with deterministic closure diagnostics and typed WB12 runoff guard posture; remains non-promotable while `GAP-RUNOFFPART-002`..`GAP-RUNOFFPART-004` are open. |
...
| `SC-SUBHYD-001` | Subsurface Hydrology and Drainage Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-SUBHYD-001.md` | `static` | `2026-05-23` |  | WB12 amendment added explicit storage-reconciliation coupling authority for `Qd` consumption while retaining WB11 lateral/drainage production posture; remains non-promotable while `GAP-SUBHYD-002` and `GAP-SUBHYD-003` are open (`GAP-SUBHYD-001` and `GAP-SUBHYD-004` are promotable-with-risk). |
...
| `SC-WATBAL-001` | Water Balance Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-WATBAL-001.md` | `static` | `2026-05-23` |  | WB12 amendment added runoff/storage reconciliation authority with deterministic closure diagnostics and typed WB12 guard posture on top of WB11 hydrology production kernels; remains non-promotable while `GAP-WATBAL-002` and `GAP-WATBAL-003` are open. |
