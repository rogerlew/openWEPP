...
| `SC-WATBAL-001` | Water Balance Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-WATBAL-001.md` | `static` | `2026-05-23` |  | WB13 amendment added canonical 25-column daily output-surface authority (`H5.wat.dat` equivalent), deterministic `(Y,J,OFE)` row-order rules, and typed WB13 output guards on top of WB11/WB12 production kernels; remains non-promotable while `GAP-WATBAL-002` and `GAP-WATBAL-003` are open. |
