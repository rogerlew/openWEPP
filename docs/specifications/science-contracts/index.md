...
| `SC-SYSTEM-001` | System Integration Boundary and Watershed Assembly Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-SYSTEM-001.md` | `static` | `2026-05-23` |  | INT10 amendment added cross-lane publication closure authority requiring successful `decomp -> growth -> watbal` completion before system-boundary publish; remains non-promotable while `GAP-SYSTEM-001` and `GAP-SYSTEM-002` are open (`GAP-SYSTEM-003` and `GAP-SYSTEM-004` are promotable-with-risk). |
| `SC-WATBAL-001` | Water Balance Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-WATBAL-001.md` | `static` | `2026-05-23` |  | INT10 amendment added watbal lane-entry coupled invariant/guard authority (`INV-WATBAL-011`) and replay obligations validating `decomp -> growth -> watbal` ordering with typed ordering-symbol failure posture. |
