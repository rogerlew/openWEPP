...
| `SC-HYDRAULICS-001` | Overland Hydraulics Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-HYDRAULICS-001.md` | `static` | `2026-05-23` |  | ARCH22 amendment added typed production-surface authority for covered hydraulics-coupled interfaces (`HillslopeProduction*Symbol`, `WatershedProduction*Symbol`) while preserving WB14/WB16/WS10 guard families; EROD11 closure amendments dispositioned alias-ownership ambiguity row `GAP-HYD-002` and prior promotable-with-risk rows (`GAP-HYD-001`, `GAP-HYD-004`) to `closed` via explicit governance risk acceptance. Contract remains non-promotable while `GAP-HYD-003` is open. |
...
| `SC-ROUTE-001` | Watershed Routing and Channel Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-ROUTE-001.md` | `static` | `2026-05-23` |  | ARCH22 amendment added typed production-surface authority for covered WS10 routing interfaces (`WatershedProduction*Symbol` + node/hillslope builders) while preserving `WKERNEL-WS10-CHANNEL-E-001..003`; EROD11 closure amendments dispositioned alias-ownership ambiguity row `GAP-ROUTE-002` and prior promotable-with-risk rows (`GAP-ROUTE-001`, `GAP-ROUTE-004`) to `closed` via explicit governance risk acceptance. Contract remains non-promotable while `GAP-ROUTE-003` and `GAP-ROUTE-005` are open. |
| `SC-RUNOFFPART-001` | Surface Runoff Partition Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-RUNOFFPART-001.md` | `static` | `2026-05-23` |  | WB20 amendment added forward-solver lane runoff-closure authority (`wb20_forward_solver_lane_enabled`) so WB12 runoff acceptance can be solver-residual-derived and exclude observed-target substitution; ARCH22/EROD11 closure posture remains unchanged. Contract remains non-promotable while `GAP-RUNOFFPART-003` and `GAP-RUNOFFPART-004` are open. |
| `SC-SED-001` | Hillslope Erosion Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-SED-001.md` | `static` | `2026-05-23` |  | WB16 amendment added hydrologic peak/duration intake authority (`peakro`, `watdur`) with continuity/traceability and typed-guard requirements for erosion coupling readiness; EROD11 closure amendments dispositioned alias-ownership ambiguity row `GAP-SED-002` and prior promotable-with-risk rows (`GAP-SED-001`, `GAP-SED-004`) to `closed` via explicit governance risk acceptance. Contract remains non-promotable while `GAP-SED-003` is open. |
...
| `SC-SYSTEM-001` | System Integration Boundary and Watershed Assembly Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-SYSTEM-001.md` | `static` | `2026-05-23` |  | WB20 amendment added forward-solver parity governance authority (`INV-SYSTEM-016`) requiring explicit lane-manifest and no-observed-target-substitution evidence for Tier-A parity lane disposition; prior ARCH22/PL14R/PL15R governance remains active. |
| `SC-WATBAL-001` | Water Balance Process Contract | `in_review` | `draft` | openWEPP maintainers + hydrology reviewer | `docs/specifications/science-contracts/contracts/SC-WATBAL-001.md` | `static` | `2026-05-23` |  | WB20 amendment added lane-scoped WB12 closure semantics with forward-solver selector authority (`wb20_forward_solver_lane_enabled`) so parity-lane acceptance is solver-residual-derived and excludes observed closure targets; WB19 hydrology authority and prior replay governance remain active. Contract remains non-promotable while `GAP-WATBAL-002` is open. |
