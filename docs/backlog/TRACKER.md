...
Last updated: 2026-07-04 (Claude Code — added hydrograph-resolved sediment/routing item).
...
| [Hydrograph-resolved sediment + routing (modeled hourly flow through the stack)](20260704-hydrograph-resolved-sediment-and-routing.md) | `concept` | ADR + SC-SED-001/HBP/SC-ROUTE-001 amendments before code | **Operator-directed 2026-07-04.** Structural resolution of the decreasing-flow/`qout<qin` erosion awkwardness: carry the modeled hourly flow (`wb14_hourly_excess_m`) through erosion → HBP EVENT surface → channel routing instead of the single-peak collapse + triangular-hydrograph reconstruction. Cross-cutting; the substrate for erosion Increment 2 (roadmap §E.2) and answers the [stream-water-temperature](20260627-stream-water-temperature-surface-energy-balance.md) HBP-hourly-serialization open question. |
