...
| [Forest lateral-flow absolute-magnitude authority](20260618-forest-lateral-flow-absolute-magnitude-authority.md) | `staged` | authority data landed; promotion via MOFEFID Lane C | The FARPOINT01 71% `runvol` `CONTRACT-GAP` (`CORRECT-BY-CONSTRUCTION`). **Unblocked 2026-07-01:** observed candidates landed in `tests/fixtures/forest_lateral_flow_authority/` (`08d6994e`); envelope/metric/applicability promotion is `MOFEFID-C01` in [planning/mofe-fidelity-campaign-strategy.md](../planning/mofe-fidelity-campaign-strategy.md). |
