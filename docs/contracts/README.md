...
| Management-file `lanuse` authority | [openwepp-management-lanuse-authority-contract.md](openwepp-management-lanuse-authority-contract.md) | Input authority for first-class landuse-physics operands: the management-file `lanuse` block (not the `.run`) is authoritative; typed-presence/fail-closed; no legacy cropland-field inference without a bridge contract; cropland-encoded fixtures are compatibility inputs. Provenance source for `SC-OFEROUTE-001`, `SC-SUBHYD-001`, and the growth–canopy contract when concrete operands are bound. Governed by [ADR-0034](../decisions/0034-management-file-lanuse-input-authority.md); concrete schema deferred to disturbed-forest campaign WS-1. |
